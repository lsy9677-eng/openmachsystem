...
import{render,teamText}from'./ui.js?v=3444';
...
if((state?.prelim?.linkedDraw?.userInitiated===true||state?.draw?.stage3441Explicit==='slot')&&linkedDrawNeedsRepair(state)&&!hasStartedMainMatches(state)){
  try{
...
/* Stage 34.4.4 · explicit main draw gate, visible prelim lock, shared queue elapsed-only */
(function stage3444MainGateAndOperatorClarity(){
  const byId=id=>document.getElementById(id);
  const isExplicitMainDraw=()=>Boolean(
    state?.draw?.stage3441Explicit==='slot'||
    state?.draw?.stage3441Explicit==='final'||
    state?.prelim?.linkedDraw?.userInitiated===true
  );
  const mainIds=()=>{
  ...
  function quarantineUnrequestedMain(){
    if(isExplicitMainDraw())return 0;
    const ids=mainIds();if(!ids.size)return 0;
    let removed=0;
    const cleanCourt=c=>{
      if(!c)return;
      if(ids.has(c.playing)){c.playing=null;removed++;}
      if(ids.has(c.wait1)){c.wait1=null;removed++;}
      if(Array.isArray(c.queue)){const before=c.queue.length;c.queue=c.queue.filter(id=>!ids.has(id));removed+=before-c.queue.length;}
    };
    ...
    try{allMatches(state.draw||{rounds:{}}).forEach(m=>{if(['playing','court_wait1','venue_shared_queue','shared_queue','queued'].includes(m.status)){m.status='ready';delete m.courtId;delete m.venueId;}});}catch(_e){}
    if(state.prelim?.linkedDraw?.active&&!state.prelim.linkedDraw.userInitiated){
      state.prelim.linkedDraw.active=false;
      state.prelim.linkedDraw.slots=[];
      state.prelim.linkedDraw.createdAt=null;
      state.prelim.linkedDraw.lastSyncedAt=null;
    }
    if(removed){try{saveState(state);}catch(_e){};console.info(`[230MATCH V3] unrequested main queue removed · ${removed}`);}
    return removed;
  }
  function visiblePrelimLock(){
    const details=byId('stage342PrelimDetails');
    const panel=byId('stage3443PrelimLockPanel');
    if(!details||!panel)return;
    details.open=true;
    const content=byId('stage342PrelimContent');
    if(content&&panel.parentElement!==content)content.prepend(panel);
    panel.hidden=false;
    panel.style.display='flex';
    const summary=details.querySelector('summary');
    if(summary)summary.insertAdjacentElement('afterend',panel);
  }
  function drawGateNotice(){
    const explicit=isExplicitMainDraw();
    let box=byId('stage3444DrawGateNotice');
    const mainPanel=byId('stage3441MainPrep')||byId('stage342MainDetails');
    if(!mainPanel)return;
    if(!box){box=document.createElement('div');box.id='stage3444DrawGateNotice';box.className='notice info';mainPanel.prepend(box);}
    box.className=`notice ${explicit?'success':'warning'}`;
    box.textContent=explicit?'본선 추첨 기록이 확인되었습니다. 확정된 경기만 공용대기·코트에 배정됩니다.':'본선 미추첨 상태입니다. 본선 카드는 공용대기와 코트에 생성되지 않습니다.';
  }
  function refresh3444(){
    quarantineUnrequestedMain();
    visiblePrelimLock();
    drawGateNotice();
    const legacy=byId('lockPrelimBtn');if(legacy)legacy.hidden=true;
  }
  const oldCommit3444=commit;
  commit=function(message){const r=oldCommit3444(message);setTimeout(refresh3444,0);return r;};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{refresh3444();setTimeout(refresh3444,350);setTimeout(refresh3444,1200);},{once:true});
  else{refresh3444();setTimeout(refresh3444,350);setTimeout(refresh3444,1200);}
  window.addEventListener('hashchange',()=>setTimeout(refresh3444,60));
  const style=document.createElement('style');style.textContent=`
    #stage342PrelimDetails>#stage3443PrelimLockPanel{margin:10px 14px 14px!important;display:flex!important}
    #stage3444DrawGateNotice{margin:0 0 12px}
  `;document.head.appendChild(style);
  const label=byId('buildStageLabel');if(label){label.textContent='230MATCH 34.4.4 · 본선 추첨 게이트·예선 확정 표시·공용대기 시간정책';label.title='Version 34.4.4';}
  document.documentElement.dataset.build='3444';
  console.info('[230MATCH V3] 34.4.4 ready · explicit main draw gate, visible prelim lock, elapsed-only shared queue');
})();
