...
// 4.4 workspace routing core:
// Tournament/division selection is resolved from URL at boot. Selection never mutates the live workspace in-place.
// A selection change reloads only the requested workspace and then normal sync resumes.
...
function applyClientDivisionSelection(source,tournamentId=''){
  ...
  let preferred='';try{const routeDivision=String(new URLSearchParams(location.search).get('division')||'');preferred=routeDivision||String(localStorage.getItem(activeDivisionKey(tid))||'');}catch{}
...
async function readInitialBundle(){
  ...
  let routeRequested='';try{routeRequested=String(new URLSearchParams(location.search).get('tournament')||'');}catch{}
  const requested=String(routeRequested||localRequested||room.activeTournamentId||'');const activeRow=rows.find(x=>x.id===requested)||rows[0];
...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();const remoteBundle=await readInitialBundle();let routeRequested='';try{routeRequested=String(new URLSearchParams(location.search).get('tournament')||'');}catch{}let requested='';try{requested=String(routeRequested||localStorage.getItem(ACTIVE_TOURNAMENT_KEY)||remoteBundle?.state?.multiTournament?.activeTournamentId||'');}catch{requested=String(routeRequested||remoteBundle?.state?.multiTournament?.activeTournamentId||'');}const cached=routeRequested?null:await readCachedState(requested),chosen=routeRequested?(remoteBundle?.state||null):chooseInitial(cached,remoteBundle?.state||null);if(chosen){applyState(chosen,cached&&chosen===cached?'cache':'firebase');if(cached&&chosen===cached&&canWriteFn())schedule();}unsubscribeRoom=api.onSnapshot(roomRef(),snap=>{void handleRoomSnapshot(snap);},e=>status('실시간 연결 오류','warning',e?.message||String(e)));status('실시간 연결','success','대회·부서 선택은 URL 작업공간 기준으로 고정되고 현재 대회 데이터만 실시간 반영됩니다.');return true;}
