...
const BUILD_LABEL='STAGE 32.6.2 · ENTRY DASHBOARD & SETTINGS NAVIGATION HOTFIX';
...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  renderPortalViews();
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='entry')setTimeout(renderApplicationPortal,30);if(target==='messages')setTimeout(()=>{renderPortalViews();renderSmsAcceptance(lastSmsAcceptance);},30);if(target==='print')setTimeout(renderPrintPreview,30);if(target==='acceptance')setTimeout(renderAcceptance,30);if(target==='rehearsal')setTimeout(()=>renderRehearsal(),30);if(target==='performance')setTimeout(renderPerformanceCenter,30);if(target==='diagnostics')setTimeout(renderDiagnostics,30);if(target==='manual')setTimeout(renderOperationsManual,30);
