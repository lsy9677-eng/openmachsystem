import{getAuthConfig,saveAuthConfig,startAuth,signInGoogle,signOutSocial,beginExternalLogin}from'./auth-engine.js?v=33156';

import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store.js?v=33156';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=33156';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=33156';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=33156';
import{submitResult}from'./result-engine.js?v=33156';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=33156';
import{downloadJson}from'./recovery.js?v=33156';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=33156';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=33156';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=33156';
import{render,teamText}from'./ui.js?v=33156';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=33156';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=33156';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=33156';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=33156';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=33156';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=33156';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=33156';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=33156';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=33156';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=33156';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=33156';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=33156';
import{verifyAndRepairMainFlow}from'./main-flow-integrity-engine.js?v=33156';
import{finalizeTournamentCompletion}from'./tournament-completion-engine.js?v=33156';
...
function authUserLabel(){return currentAuthUser?.displayName||currentAuthUser?.email||'로그인 사용자';}
function applyAuthenticatedRole(user,role='viewer'){
  currentAuthUser=user||null;currentRole=user?role:'viewer';sessionStorage.setItem(ROLE_KEY,currentRole);applyRoleUI();renderAuthStatus();
}
...
function loadAuthSettingsPanel(){const c=getAuthConfig();setValue('authFirebaseConfigJson',c.firebaseConfigText||getSyncSettings().firebaseConfigText||'');setValue('authAdminEmails',c.adminEmails||'');setValue('authOperatorEmails',c.operatorEmails||'');setValue('authKakaoLoginUrl',c.kakaoLoginUrl||'');setValue('authNaverLoginUrl',c.naverLoginUrl||'');}
function saveAuthSettingsPanel(){if(!requireAdmin('간편로그인 설정 저장'))return;saveAuthConfig({firebaseConfigText:getValue('authFirebaseConfigJson','').trim(),adminEmails:getValue('authAdminEmails','').trim(),operatorEmails:getValue('authOperatorEmails','').trim(),kakaoLoginUrl:getValue('authKakaoLoginUrl','').trim(),naverLoginUrl:getValue('authNaverLoginUrl','').trim()});notice('간편로그인과 역할 설정을 저장했습니다.','success');startAuth((u,r)=>applyAuthenticatedRole(u,r));}
...
const PORTAL_VIEW_TITLES={home:'홈', tournaments:'대회 목록', 'prelim-public':'대회 현황', 'my-match':'내 경기', entry:'참가 접수', guide:'대회 요강', board:'공지사항', participants:'참가자 기록', records:'시합 결과', print:'출력 센터', operation:'코트 현황', bracket:'대진표', settings:'대회 관리', roster:'참가자 기록', messages:'문자 센터', audit:'운영 점검', logs:'운영 로그'};
...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  renderPortalViews();
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='print')setTimeout(renderPrintPreview,30);
...
loadAuthSettingsPanel();renderAuthStatus();startAuth((user,role,error)=>{if(error)console.info('[230MATCH V3] social auth waiting for configuration');applyAuthenticatedRole(user,role);});

prepareRecoveryStorage().catch(error=>console.warn('로컬 복구점 저장소 준비 실패',error));
syncInputs();syncPrelimInputs();bind();bindPortal();bindPrintCenter();bindParticipantManager();bindEntryApplications();bindPublicParticipantRecords();bindResultArchive();bindTournamentLifecycleManager();bindBackupRecoveryManager();window.addEventListener('popstate',()=>navigatePortalView(location.hash.replace(/^#/, '')||'home',{focus:false}));initialPortalView();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});renderOperatorControls();updateSetupProgress();applyRoleUI();renderPortalViews();restartTimeTimer();updateClock();setInterval(updateClock,1000);installUnifiedMoveControlGuard();ensureUnifiedCourtMoveControls();
...
const BUILD_LABEL='STAGE 31.56 · SOCIAL LOGIN & ROLE SECURITY';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33156';
console.log('[230MATCH V3] stage31.56 social login role security loaded · viewer default · IndexedDB recovery enabled');
