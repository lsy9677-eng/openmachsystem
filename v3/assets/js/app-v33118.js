...
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js';
import{submitResult}from'./result-engine.js';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js';
...
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues}from'./unified-court-engine.js';
...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js';
...
function openCourtTransfer(sourceCourtId,sourceSlot){
  ...
  $('courtTransferTargetSelect').innerHTML=targets.map(c=>`<option value="${c.id}">${c.name} · 시합중 ${c.playing?'있음':'없음'} · 대기1 ${c.wait1?'있음':'없음'} · 수동대기 ${c.manualQueue?.length||0}경기</option>`).join('');
...
function confirmCourtTransfer(event){
  ...
  calculateTimeMetrics(state);commit(`경기 코트 이동 · ${result.court.name} · ${result.slot}`);
  $('courtTransferDialog').close();notice(result.slot==='manual'?'대상 코트의 관리자 지정 대기열에 등록했습니다.':'경기를 대상 코트로 이동했습니다.','success');
}
...
const BUILD_LABEL='STAGE 31.18 · EMPTY COURT FIRST & MANUAL CONTROL RESTORE';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33118';
console.log('[230MATCH V3] stage31.18 empty-court-first-manual-control-restore loaded · local fallback enabled');
