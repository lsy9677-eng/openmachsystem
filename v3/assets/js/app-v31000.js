...
let state=loadState();ensurePrelimState(state);ensureTimeState(state);ensureDrawMeta(state);ensureMessagingState(state);ensureContacts(state);ensureAuditState(state);
...
function commit(message){
  if(message)log(message);if(state.settings.autoTimeEnabled)calculateTimeMetrics(state);syncInputs();saveState(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});flashSaved();
}
function syncInputs(){
  ...
  setValue('byePriority',state.settings.byePriority||'group-first');setChecked('autoMessageEnabled',state.messaging.settings.autoMessageEnabled!==false);setValue('messageSenderName',state.messaging.settings.senderName||'230MATCH');setValue('messageDeliveryMode',state.messaging.settings.deliveryMode||'sms-uri');setChecked('messageOnCourtAssign',state.messaging.settings.onCourtAssign!==false);setChecked('messageOnQueueMove',state.messaging.settings.onQueueMove!==false);setChecked('smartMessageUpdate',state.messaging.settings.smartMessageUpdate!==false);setValue('messageRepeatPolicy',state.messaging.settings.repeatPolicy||'update-pending');setValue('templatePlaying',state.messaging.settings.templates.playing||'');setValue('templateWait1',state.messaging.settings.templates.wait1||'');setValue('templateShared',state.messaging.settings.templates.shared||'');
}
...
function refreshTimeEngine({save=false}={}){
  ...
  render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});
}
...
function runCurrentAudit(){
  const audit=runStateAudit(state);applyAuditResult(state,audit,null);
  commit(`운영 상태 점검 · 통과 ${audit.counts.pass} · 주의 ${audit.counts.warn} · 오류 ${audit.counts.fail}`);
  notice(`운영 점검 완료: 통과 ${audit.counts.pass}, 주의 ${audit.counts.warn}, 오류 ${audit.counts.fail}`,'success');
}
function runSimulationAudit(){
  const audit=runStateAudit(state),simulation=runFullSimulation(state);applyAuditResult(state,audit,simulation);
  commit(`복제 모의대회 · ${simulation.completedMatches}/${simulation.totalMatches}경기 · ${simulation.success?'완주':'실패'}`);
  notice(simulation.success?`복제 모의대회를 완주했습니다. 우승 ${simulation.winner?.name||'-'}`:'모의대회에서 오류를 발견했습니다.',simulation.success?'success':'error');
}
function runAllAudit(){
  const audit=runStateAudit(state),simulation=state.draw?.size?runFullSimulation(state):null;
...
function clearAudit(){
  state.audit={lastRunAt:null,overall:'not-run',results:[],simulation:null};commit('운영 점검 결과 삭제');
}
...
function bind(){
  ...
  if($('quickAuditBtn'))$('quickAuditBtn').onclick=()=>{try{runAllAudit();}catch(e){notice(e.message,'error');}};
  if($('runStateAuditBtn'))$('runStateAuditBtn').onclick=()=>{try{runCurrentAudit();}catch(e){notice(e.message,'error');}};
  if($('runFullSimulationBtn'))$('runFullSimulationBtn').onclick=()=>{try{runSimulationAudit();}catch(e){notice(e.message,'error');}};
  if($('runAllAuditBtn'))$('runAllAuditBtn').onclick=()=>{try{runAllAudit();}catch(e){notice(e.message,'error');}};
  if($('downloadAuditBtn'))$('downloadAuditBtn').onclick=()=>{try{downloadAudit();}catch(e){notice(e.message,'error');}};
  ...
  if($('messageStatusFilter'))$('messageStatusFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});

  if($('rosterSearch'))$('rosterSearch').oninput=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});
  if($('rosterPhoneFilter'))$('rosterPhoneFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});
...
syncInputs();syncPrelimInputs();bind();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory});restartTimeTimer();updateClock();setInterval(updateClock,1000);
console.log('[230MATCH V3] stage10 operation-audit loaded · no legacy code · no Firebase writes');
