...
export function ensureContacts(state){
  ...
  if(!state.contacts||typeof state.contacts!=='object'||Array.isArray(state.contacts))state.contacts={};
...
export function contactStats(state){
  ...
  const total=teams.length,withPhone=teams.filter(t=>getTeamContact(state,t).phone).length;
...
export function exportContactData(state){
  ...
  return{schemaVersion:'230match-v3-contacts-v1',tournament:state.tournament,exportedAt:new Date().toISOString(),
    contacts:(Array.isArray(state?.teams)?state.teams:[]).map(t=>({teamId:t.id,teamName:t.name,affiliation:t.affiliation||'',phone:getTeamContact(state,t).phone,manager:getTeamContact(state,t).manager}))};
}
export function importContactData(state,data){
  ensureContacts(state);
  const source=data&&typeof data==='object'?data:{};
  const list=Array.isArray(data)?data:(source.contacts||source.teams||[]);
