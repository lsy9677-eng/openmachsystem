...
const BUILD_LABEL='STAGE 32.2.5 · ARCHIVE ACTION BUTTON EVENT HOTFIX';
...
async function archiveLegacySummaryAndReset(){
  try{
    ...
    next.portal.legacyTournamentSummaries=[archive,...previousSummaries.filter(x=>x.id!==archive.id)];
    ...
    console.error('[Stage32.2.5] archive reset failed',error);
    notice(`기록 보관 처리 실패: ${error?.message||String(error)}`,'error');
  }
}
...
// Stage32.2.5: 설정 화면 재렌더링 후에도 보관 버튼 클릭을 확실히 처리한다.
document.addEventListener('click',event=>{
  const button=event.target.closest?.('#archiveLegacyAndResetBtn');
  if(!button)return;
  event.preventDefault();
  event.stopImmediatePropagation();
  archiveLegacySummaryAndReset();
},true);
