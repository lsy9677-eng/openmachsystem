...
const WORKSPACE_ROUTE_BUILD='7100';
function workspaceRouteUrl(tournamentId='',divisionId='',view='home'){
  const url=new URL(window.location.href);
  url.searchParams.set('v',WORKSPACE_ROUTE_BUILD);
  if(tournamentId)url.searchParams.set('tournament',String(tournamentId));else url.searchParams.delete('tournament');
  if(divisionId)url.searchParams.set('division',String(divisionId));else url.searchParams.delete('division');
  url.hash='#'+String(view||'home').replace(/^#/,'');
  return url.pathname+url.search+url.hash;
}
function routeTournamentId(){try{return String(new URLSearchParams(location.search).get('tournament')||'')}catch(_e){return''}}
function routeDivisionId(){try{return String(new URLSearchParams(location.search).get('division')||'')}catch(_e){return''}}
let workspaceSwitchBusy=false;
async function goWorkspaceRoute(tournamentId='',divisionId='',view='home'){
  const targetTournamentId=String(tournamentId||'').trim();
  const targetDivisionId=String(divisionId||'').trim();
  const targetView=String(view||'home').replace(/^#/,'')||'home';
  const currentTournamentId=String(state?.multiTournament?.activeTournamentId||state?.tournament?.id||'').trim();
  const currentDivisionId=String(state?.multiDivision?.activeDivisionId||'').trim();

  if(workspaceSwitchBusy)return false;

  // 같은 대회 안의 부서 전환은 네트워크/페이지 재시작 없이 메모리에서 즉시 교체합니다.
  if(targetTournamentId&&targetTournamentId===currentTournamentId&&targetDivisionId&&targetDivisionId!==currentDivisionId){
    try{
      workspaceSwitchBusy=true;
      document.body.classList.add('workspace-switching');
      ensureMultiDivisionRuntime();
      const record=(state.multiDivision?.divisions||[]).find(d=>String(d.id)===targetDivisionId);
      if(!record)throw new Error('선택한 부서 정보를 찾을 수 없습니다.');
      // 현재 부서를 한 번만 보존한 뒤 선택 부서를 적용합니다.
      syncCurrentDivisionRuntime();
      if(!applyDivisionSnapshot(record))throw new Error('부서 데이터를 적용하지 못했습니다.');
      try{localStorage.setItem(`230match-v7-division-${targetTournamentId}`,targetDivisionId);}catch(_e){}
      const href=workspaceRouteUrl(targetTournamentId,targetDivisionId,targetView);
      history.pushState({tournamentId:targetTournamentId,divisionId:targetDivisionId,portalView:targetView},'',href);
      syncInputs();syncPrelimInputs();
      safePersistState('부서 전환');
      if(registrationCloudReady)mirrorRegistrationRowsToCurrentDivision();
      renderCommittedState6400();
      navigatePortalView(targetView,{focus:false});
      return true;
    }catch(error){
      console.error('[230MATCH] 부서 즉시 전환 실패',error);
      notice(error?.message||'부서 전환에 실패했습니다.','error');
      return false;
    }finally{
  ...
  // 같은 작업공간이면 페이지를 다시 읽지 않습니다.
  if((!targetTournamentId||targetTournamentId===currentTournamentId)&&(!targetDivisionId||targetDivisionId===currentDivisionId)){
    navigatePortalView(targetView,{pushHistory:true});
    return true;
  }

  // 다른 대회는 해당 Firestore 문서 1건만 읽어서 현재 메모리에 교체합니다.
  if(targetTournamentId&&targetTournamentId!==currentTournamentId){
    try{
      workspaceSwitchBusy=true;
      document.body.classList.add('workspace-switching');
      try{syncCurrentDivisionRuntime();syncCurrentTournamentRuntime();safePersistState('대회 전환 전');}catch(_e){}
      const registry=state?.multiTournament?.tournaments||[];
      const bundle=await loadTournamentNow(targetTournamentId,registry);
      if(!bundle?.state)throw new Error('선택한 대회 데이터를 불러오지 못했습니다.');
      const href=workspaceRouteUrl(targetTournamentId,targetDivisionId,targetView);
      history.pushState({tournamentId:targetTournamentId,divisionId:targetDivisionId,portalView:targetView},'',href);
      try{localStorage.setItem('230match-v7-active-tournament',targetTournamentId);}catch(_e){}
      applySynchronizedState(bundle.state,'대회 전환');
      navigatePortalView(targetView,{focus:false});
      return true;
    }catch(error){
      console.error('[230MATCH] 대회 즉시 전환 실패',error);
      notice(error?.message||'대회 전환에 실패했습니다.','error');
      return false;
    }finally{
      workspaceSwitchBusy=false;
      document.body.classList.remove('workspace-switching');
...
// 70.0.2 · tournament links stay inside the running app.
document.addEventListener('click',event=>{
  const link=event.target.closest?.('a[href*="tournament="]');
  if(!link)return;
  try{
    const url=new URL(link.href,location.href);
    if(url.origin!==location.origin)return;
    const tid=String(url.searchParams.get('tournament')||'');
    if(!tid)return;
    const did=String(url.searchParams.get('division')||'');
    const view=String(url.hash||'#home').replace(/^#/,'')||'home';
    event.preventDefault();
    event.stopPropagation();
    goWorkspaceRoute(tid,did,view);
  }catch(_e){}
},true);
...
function globalPosts(){
  if(globalNoticeReady)return globalNoticeState.posts;
  return Array.isArray(state.portal?.posts)?state.portal.posts:[];
}
function mirrorGlobalPostsToState(){
  ensurePortalStateBaseOnly();
  state.portal.posts=structuredClone(globalNoticeState.posts||[]);
  state.portal.globalTicker=structuredClone(globalNoticeState.ticker||{enabled:false,text:'',startAt:'',endAt:'',updatedAt:''});
}
function ensurePortalStateBaseOnly(){
  ...
  globalNoticeLoading=(async()=>{
    try{
      ...
      if(room.globalNoticeBoard||room.globalTicker){
        ...
        const cloudPosts=Array.isArray(room.globalNoticeBoard)?room.globalNoticeBoard:[];
        const cached=readGlobalNoticeCache();
        const currentLegacy=Array.isArray(state.portal?.posts)?state.portal.posts:[];
        const safePosts=cloudPosts.length?cloudPosts:(cached.posts.length?cached.posts:currentLegacy);
        applyGlobalNoticeState({posts:safePosts,ticker:room.globalTicker||{},updatedAt:room.globalNoticeUpdatedAt||''});
      }else{
...
let syncAccessRefreshTimer=null;
...
function applyAuthenticatedRole(user,role='viewer',profile=null){
  ...
  if(document.body?.dataset.currentView==='entry')setTimeout(()=>{renderEntryLoginGate();renderApplicationPortal();},120);
...
function setRole(role){
  ...
  applyRoleUI();
  if(before!==currentRole)refreshSyncAccessMode();
  const label=target==='admin'?'관리자 모드':target==='operator'?'진행자 모드':'일반 회원 보기';
...
function applyRoleUI(){
  ...
  document.body.dataset.dataMode=canOperate()?'operation':'member-lite';
  const badge=document.getElementById('currentRoleBadge');if(badge)badge.textContent=isAdmin()?'관리자':isOperator()?'진행자':'일반 회원';
  ...
  const memberHiddenViews=new Set(['notifications']);
  document.querySelectorAll('[data-portal-go],[data-view],[data-mobile-view]').forEach(el=>{
    const view=el.dataset.portalGo||el.dataset.view||el.dataset.mobileView||'';
    if(memberHiddenViews.has(view))el.hidden=!canOperate();
  });
  // 코트·예선·본선·요강·공지·참가신청·내경기는 회원 핵심 정보라 항상 공개한다.
  document.querySelectorAll('[data-portal-go="operation"],[data-view="operation"],[data-settings-view="operation"],[data-portal-go="prelim-public"],[data-portal-go="bracket"],[data-portal-go="guide"],[data-portal-go="board"],[data-portal-go="entry"],[data-portal-go="my-match"]').forEach(el=>{
...
function validateRankAwareMainDraw(draw,{strictOneVsTwo=false}={}){
  ...
  for(const m of first){
    ...
    if(a&&b&&!a.placeholder&&!b.placeholder){
      ...
      if(strictOneVsTwo){
        if(!((ra===1&&rb===2)||(ra===2&&rb===1)))sameRank++;
      }else if(ra&&rb&&ra===rb)sameRank++;
...
function createExactMainDrawFromSlots(arranged,size){
  const drawSize=Number(size)||64;
  if(![32,64,128].includes(drawSize))throw new Error('지원 대진 규모는 32·64·128강입니다.');
  const exact=[...(arranged||[])].slice(0,drawSize);
  while(exact.length<drawSize)exact.push(null);

  const rounds={};
  for(let roundSize=drawSize;roundSize>=2;roundSize/=2){
    const count=roundSize/2;
    rounds[roundSize]=Array.from({length:count},(_,i)=>({
      id:`r${roundSize}_m${i+1}`,
      roundSize,matchNo:i+1,
      teamA:null,teamB:null,winner:null,scoreA:null,scoreB:null,
      status:'waiting_slots',court:null,
      nextMatchId:roundSize>2?`r${roundSize/2}_m${Math.floor(i/2)+1}`:null,
      nextSlot:roundSize>2?(i%2===0?1:2):null
    }));
  }

  rounds[drawSize].forEach((m,i)=>{
    m.teamA=exact[i*2]||null;
    m.teamB=exact[i*2+1]||null;
  ...
  // linked draw의 placeholder는 그대로 유지하고, 실제 BYE만 다음 라운드로 전파한다.
  for(let roundSize=drawSize;roundSize>2;roundSize/=2){
    for(const m of rounds[roundSize]){
  ...
  return {size:drawSize,rounds};
}

function createRankAwareDrawFromSlots(slots,size){
  const arranged=buildRankAwareMainSlots(slots,size);
  // 중요: generateDraw()는 전달받은 슬롯을 다시 순위 정렬/시드 배치하므로
  // 이미 만든 1위-vs-2위 페어를 깨뜨릴 수 있다. 여기서는 정확한 슬롯 순서를 그대로 사용한다.
  const draw=createExactMainDrawFromSlots(arranged,size);
...
function applySynchronizedState(nextState,source='동기화'){
  ...
  if(routedDivision){
    try{
      ensureMultiDivisionRuntime();
      const targetDivision=state.multiDivision?.divisions?.find(d=>String(d.id)===routedDivision);
      if(targetDivision&&String(targetDivision.id)!==String(state.multiDivision?.activeDivisionId)){
        syncCurrentDivisionRuntime();
        applyDivisionSnapshot(targetDivision);
      }
    }catch(error){console.warn('[230MATCH] URL 부서 적용 실패',error);}
  }
  ensurePrelimState(state);ensureTimeState(state);ensureDrawMeta(state);ensureMessagingState(state);ensureContacts(state);ensureAuditState(state);ensureEarlyMainSettings(state);ensureVenueSettings(state);ensureVenueQueues(state);ensureCourtStatuses(state);ensureCourtManualQueues(state);ensurePrelimCourtStatuses(state);ensureOperatorState();
  if(state.settings.autoTimeEnabled)calculateTimeMetrics(state);
  syncInputs();syncPrelimInputs();safePersistState(`${source} 상태`);
  render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});
  updateSetupProgress();renderOperatorControls();applyRoleUI();renderPortalViewFast(document.body?.dataset.currentView||'home');renderDivisionWorkspaceBar();flashSaved();
...
function restartTimeTimer(){
  // 4.5.0: 전체 대회 시간 계산을 주기적으로 반복하지 않는다.
  // 시간값은 실제 경기/코트 상태가 바뀌는 commit 경로에서만 계산한다.
  // 이 변경으로 60초 단위 전체 경기 순회로 인한 주기적 멈춤을 제거한다.
...
function contactEditPlayers(team){
  const current=Array.isArray(team?.players)?team.players.slice(0,2):[];
  const names=String(team?.name||'').split(/\s*\/\s*/);
  const phones=Array.isArray(team?.playerPhones)?team.playerPhones:[];
  return [0,1].map(i=>({
    name:String(current[i]?.name||names[i]||'').trim(),
    phone:String(current[i]?.phone||phones[i]||'').replace(/\D/g,'')
  }));
}
function openContactEdit(teamId){
  const team=state.teams.find(t=>t.id===teamId);if(!team)return;
  const c=getTeamContact(state,team)||{},players=contactEditPlayers(team);
  let representativeIndex=Number(team.representativeIndex||0);if(representativeIndex!==1)representativeIndex=0;
  if(c.phone){
    const saved=String(c.phone).replace(/\D/g,'');
    const idx=players.findIndex(p=>p.phone===saved);if(idx>=0)representativeIndex=idx;
  }
  $('contactEditTeamId').value=teamId;$('contactEditTeamName').textContent=`${team.name}${team.affiliation?` · ${team.affiliation}`:''}`;
  $('contactEditPlayer1Name').value=players[0].name;$('contactEditPlayer1Phone').value=players[0].phone;
  $('contactEditPlayer2Name').value=players[1].name;$('contactEditPlayer2Phone').value=players[1].phone;
  const rep=representativeIndex===1?$('contactEditRepresentative2'):$('contactEditRepresentative1');if(rep)rep.checked=true;
  const mode=team.smsTargetMode==='representative'?'representative':'both';
  const modeEl=mode==='representative'?$('contactEditSmsRepresentative'):$('contactEditSmsBoth');if(modeEl)modeEl.checked=true;
  validateContactInput();$('contactEditDialog').showModal();
}
function validateContactInput(){
  const phones=[$('contactEditPlayer1Phone')?.value||'',$('contactEditPlayer2Phone')?.value||''];
  const results=phones.map(validatePhone),box=$('contactPhoneValidation');
  const ok=results.every(r=>r.ok);
  if(box){box.className=`notice ${ok?'success':'error'}`;box.textContent=ok?'두 선수 연락처가 모두 정상입니다. 문자 수신 대상을 선택해 저장하세요.':results.find(r=>!r.ok)?.message||'두 선수 연락처를 확인하세요.';}
  if($('saveContactBtn'))$('saveContactBtn').disabled=!ok;return{ok,results};
}
function saveContact(event){
  event.preventDefault();const result=validateContactInput();if(!result.ok)return;
  const team=state.teams.find(t=>t.id===$('contactEditTeamId').value);if(!team)return;
  const players=[
    {name:String($('contactEditPlayer1Name').value||'').trim(),phone:String($('contactEditPlayer1Phone').value||'').replace(/\D/g,'')},
    {name:String($('contactEditPlayer2Name').value||'').trim(),phone:String($('contactEditPlayer2Phone').value||'').replace(/\D/g,'')}
  ];
  const repIndex=$('contactEditRepresentative2')?.checked?1:0;
  const smsTargetMode=$('contactEditSmsRepresentative')?.checked?'representative':'both';
  const oldPlayers=Array.isArray(team.players)?team.players:[];
  team.players=players.map((p,i)=>({...oldPlayers[i],name:p.name||oldPlayers[i]?.name||'',phone:p.phone}));
  team.playerPhones=players.map(p=>p.phone);
  team.representativeIndex=repIndex;team.smsTargetMode=smsTargetMode;
  const rep=players[repIndex];
  setTeamContact(state,team,{phone:rep.phone,manager:rep.name});
  // 연결된 참가신청 기록도 함께 동기화하여 이후 승인/입금 문자 대상이 달라지지 않게 합니다.
  const app=(state.portal?.applications||[]).find(a=>myMatchNormalize(a.teamName)===myMatchNormalize(team.name));
  if(app){
    const appPlayers=entryApplicationPlayers(app);
    app.players=players.map((p,i)=>({...appPlayers[i],name:p.name||appPlayers[i]?.name||'',phone:p.phone}));
    app.representativeIndex=repIndex;app.representativeName=rep.name;app.phone=rep.phone;app.smsTargetMode=smsTargetMode;app.updatedAt=new Date().toISOString();
  }
  const refreshed=refreshMessageContacts(state);commit(`팀 연락처 저장 · ${smsTargetMode==='both'?'두 선수 모두':'대표만'} 문자 · 발송대기 전환 ${refreshed.converted}건`);
  $('contactEditDialog').close();notice(`연락처를 저장했습니다. 문자 수신: ${smsTargetMode==='both'?'두 선수 모두':'대표 연락처만'}.`,'success');
}
...
function exportPrelimPilotReport(){if(!prelimPilotReport){notice('먼저 예선 실전 점검을 실행하세요.','error');return;}downloadJson(`230MATCH_예선실전점검_${Date.now()}.json`,prelimPilotReport);notice('예선 실전 점검 보고서를 저장했습니다.','success');}
  ...
  ['contactEditPlayer1Phone','contactEditPlayer2Phone'].forEach(id=>{if($(id))$(id).oninput=validateContactInput;});
...
function liveMatchPlacement(match){
  ...
  for(const court of courts){
    ...
    if(String(court.playing||'')===id)return{label:`${courtName} · 시합중`,kind:'playing',courtId:String(court.id||''),courtName,position:0};
    if(String(court.wait1||'')===id)return{label:`${courtName} · 대기1`,kind:'wait1',courtId:String(court.id||''),courtName,position:1};
    const q=Array.isArray(court.queue)?court.queue:[];
    const qi=q.findIndex(x=>String(x)===id);
    if(qi>=0)return{label:`${courtName} · 대기${qi+2}`,kind:'waiting',courtId:String(court.id||''),courtName,position:qi+2};
    const mq=Array.isArray(court.manualQueue)?court.manualQueue:[];
    const mi=mq.findIndex(x=>String(x)===id);
    if(mi>=0)return{label:`${courtName} · 관리자 대기${mi+2}`,kind:'waiting',courtId:String(court.id||''),courtName,position:mi+2,manual:true};
  }
  for(const [venueId,queue] of Object.entries(state.venueQueues||{})){
    ...
    if(idx>=0){
      ...
      return{label:`${venue?.name||venueId} · 공용대기 ${idx+1}번`,kind:'shared',venueId:String(venueId),position:idx+1};
    }
  }
...
function entryApplicationPlayersFromForm(){
  ...
  const smsTargetMode=document.querySelector('input[name="entrySmsTargetMode"]:checked')?.value==='representative'?'representative':'both';
  return {players,representativeIndex,representative:players[representativeIndex],smsTargetMode};
}
...
const REGISTRATION_COLLECTION='matchRegistrationsV1';

const PUBLIC_REGISTRATION_COLLECTION='matchRegistrationPublicV1';
let publicRegistrationRows=[];
let publicRegistrationReady=false;
let publicRegistrationUnsubscribe=null;
let publicRegistrationKey='';

function publicRegistrationPayload(item){
  return {
    id:String(item?.id||''),
    tournamentId:String(item?.tournamentId||registrationContext().tournamentId||''),
    divisionId:String(item?.divisionId||registrationContext().divisionId||''),
    divisionName:String(item?.tournamentDivision||item?.divisionName||registrationContext().divisionName||''),
    teamName:String(item?.teamName||''),
    affiliation:String(item?.affiliation||''),
    status:String(item?.status||'approved'),
    paid:Boolean(item?.paid),
    paymentStatus:String(item?.paymentStatus||(item?.paid?'paid':'unpaid')),
    createdAt:String(item?.createdAt||new Date().toISOString()),
    updatedAt:String(item?.updatedAt||item?.createdAt||new Date().toISOString())
  };
}
async function savePublicRegistrationMirror(item){
  if(!item?.id)return;
  const rt=await registrationRuntime();
  if(!rt?.user)return;
  const payload=publicRegistrationPayload(item);
  await rt.api.setDoc(rt.api.doc(rt.db,PUBLIC_REGISTRATION_COLLECTION,payload.id),payload,{merge:true});
}
function publicRegistrationBelongsToCurrentDivision(row){
  const ctx=registrationContext();
  if(String(row?.tournamentId||'')!==String(ctx.tournamentId||''))return false;
  const did=String(row?.divisionId||''),dname=String(row?.divisionName||'').trim();
  if(did&&did===String(ctx.divisionId||''))return true;
  if(dname&&dname===String(ctx.divisionName||'').trim())return true;
...
function registrationPublicSummary(){
  ...
  return {
    ...
    reserve:rows.filter(a=>a.status==='reserve').length,
    paid:rows.filter(a=>a.status==='approved'&&Boolean(a.paid)).length,
    unpaid:rows.filter(a=>a.status==='approved'&&!a.paid).length,
    capacity:simpleRegistrationCapacity()
  };
}
function renderRegistrationSummaryEverywhere(){
  const s=registrationPublicSummary(),text=`${s.active}/${s.capacity}`;
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=String(v);};
  set('homeRegistrationStatus',`${s.active} / ${s.capacity}`);
  set('homeRegistrationSub',s.reserve?`후보 ${s.reserve}팀 · 현황 보기`:'접수 현황 보기');
  set('homeHeroRegistrationCount',`${s.active} / ${s.capacity}`);
  set('homeHeroUnpaidCount',s.unpaid);set('homeHeroPaidCount',s.paid);set('homeHeroReserveCount',s.reserve);
  const progress=document.getElementById('homeHeroRegistrationProgress');if(progress)progress.style.width=`${Math.max(0,Math.min(100,s.active/Math.max(1,s.capacity)*100))}%`;
  const hub=document.querySelector('#stage3212MatchdayHub [data-portal-go="entry"] b');if(hub)hub.textContent=`참가 신청 (${text})`;
  set('stage3212EntryCount',s.reserve?`후보 ${s.reserve}팀 · 접수 현황`:`접수 ${text}`);
...
async function startPublicRegistrationSync({force=false}={}){
  try{
    ...
    const key=String(ctx.tournamentId);
...
async function backfillPublicRegistrationMirrors(){
  ...
  const ctx=registrationContext();
  if(canOperate()){
    const seeded=registrationCloudReady?registrationCloudRows:(state.portal?.applications||[]);
    for(const row of seeded){
      if(String(row?.tournamentId||ctx.tournamentId)!==String(ctx.tournamentId))continue;
      try{await savePublicRegistrationMirror({...row,tournamentId:row.tournamentId||ctx.tournamentId,divisionId:row.divisionId||ctx.divisionId,tournamentDivision:row.tournamentDivision||ctx.divisionName});}
      catch(error){console.warn('[230MATCH] 공개 참가현황 초기 동기화 실패',row?.id,error);}
    }
    return;
  }
  if(!registrationCloudReady)return;
  const rows=canOperate()?registrationCloudRows:registrationCloudRows.filter(r=>String(r.ownerUid||'')===String(currentAuthUser.uid||''));
  for(const row of rows){try{await savePublicRegistrationMirror(row);}catch(error){console.warn('[230MATCH] 공개 참가현황 복구 실패',row?.id,error);}}
}
...
function registrationContext(){
  ...
  return {
    ...
    divisionName:String(state.multiDivision?.divisions?.find(d=>String(d.id)===String(state.multiDivision?.activeDivisionId))?.name||state.tournament?.division||'')
  };
}
function registrationNormalize(row={}){
  return {...row,id:String(row.id||''),ownerUid:String(row.ownerUid||''),tournamentId:String(row.tournamentId||''),divisionId:String(row.divisionId||''),status:String(row.status||'pending'),createdAt:String(row.createdAt||''),updatedAt:String(row.updatedAt||row.createdAt||'')};
}
function registrationBelongsToCurrentDivision(row){
  const ctx=registrationContext();
  if(String(row?.tournamentId||'')!==String(ctx.tournamentId||''))return false;
  const rowDivisionId=String(row?.divisionId||'');
  const rowDivisionName=String(row?.tournamentDivision||row?.divisionName||'').trim();
  const currentName=String(ctx.divisionName||'').trim();
  // 최신 문서는 divisionId로 일치.
  if(rowDivisionId&&ctx.divisionId&&rowDivisionId===String(ctx.divisionId))return true;
  // 구버전 문서는 divisionId가 없거나 바뀐 경우가 있어 부서명으로 복구.
  if(rowDivisionName&&currentName&&rowDivisionName===currentName)return true;
  // 단일 부서 대회에서 예전 문서가 divisionId 없이 저장된 경우도 현재 부서로 취급.
  const divisions=state.multiDivision?.divisions||[];
  if(!rowDivisionId&&divisions.length<=1)return true;
  return false;
}
function registrationRowsForCurrentDivision(){
  const cloud=registrationCloudRows.filter(registrationBelongsToCurrentDivision);
  // 업그레이드 전 로컬/부서 snapshot에만 남은 신청도 화면에서 사라지지 않게 합칩니다.
  const local=Array.isArray(state.portal?.applications)?state.portal.applications:[];
  const merged=[...cloud];
  const seen=new Set(cloud.map(r=>String(r.id||'')));
  for(const row of local){
    if(!row?.id||seen.has(String(row.id)))continue;
    const compatible=String(row.tournamentId||state.tournament?.id||'')===String(registrationContext().tournamentId||'');
    if(!compatible)continue;
    merged.push(registrationNormalize({...row,tournamentId:row.tournamentId||registrationContext().tournamentId,divisionId:row.divisionId||registrationContext().divisionId,tournamentDivision:row.tournamentDivision||registrationContext().divisionName}));
    seen.add(String(row.id));
  }
  return merged;
}
function mirrorRegistrationRowsToCurrentDivision(){
  if(!registrationCloudReady)return;
  ensurePortalState();
  const rows=registrationRowsForCurrentDivision();
  // 빈 클라우드 조회가 기존 로컬 신청을 지워버리는 상황을 방지합니다.
  if(rows.length||!(state.portal?.applications||[]).length){
    state.portal.applications=structuredClone(rows);
    try{syncCurrentDivisionRuntime?.();}catch(_e){}
  }
}
function registrationAdminRowsByDivision(){
  const ctx=registrationContext();
  const rows=registrationCloudRows.filter(r=>String(r.tournamentId||'')===String(ctx.tournamentId||''));
  const seen=new Set(rows.map(r=>String(r.id||'')));
  for(const row of (state.portal?.applications||[])){
    if(!row?.id||seen.has(String(row.id)))continue;
    if(String(row.tournamentId||ctx.tournamentId)!==String(ctx.tournamentId))continue;
    rows.push(registrationNormalize({...row,tournamentId:row.tournamentId||ctx.tournamentId,divisionId:row.divisionId||ctx.divisionId,tournamentDivision:row.tournamentDivision||ctx.divisionName}));
    seen.add(String(row.id));
  }
  return rows;
}
async function registrationRuntime(){
  const rt=await getAuthRuntime();
  if(!rt?.db||!rt?.api)throw new Error('참가신청 Firebase 연결에 실패했습니다.');
  return rt;
}
async function saveRegistrationCloud(item){
  if(!currentAuthUser)throw new Error('로그인이 필요합니다.');
  const rt=await registrationRuntime();
  const ctx=registrationContext();
  const row=registrationNormalize({
    ...item,
    id:item.id||crypto.randomUUID(),
    tournamentId:item.tournamentId||ctx.tournamentId,
    divisionId:item.divisionId||ctx.divisionId,
    tournamentDivision:item.tournamentDivision||ctx.divisionName,
    ownerUid:item.ownerUid||currentAuthUser.uid,
    updatedAt:new Date().toISOString()
  });
...
function registrationProfilePhone(){
  ...
  return String(p.phone||d.phone||'').replace(/\D/g,'');
}
async function migrateLocalOwnRegistrations(){
  if(!currentAuthUser||canOperate())return;
  const phone=registrationProfilePhone();if(phone.length<10)return;
  const local=Array.isArray(state.portal?.applications)?state.portal.applications:[];
  const candidates=local.filter(row=>{
    const phones=entryApplicationPlayers(row).map(p=>String(p.phone||'').replace(/\D/g,''));
    return (!row.ownerUid||row.ownerUid===currentAuthUser.uid)&&phones.includes(phone);
  });
  for(const row of candidates){
    if(registrationCloudRows.some(x=>x.id===row.id))continue;
    try{await saveRegistrationCloud({...row,ownerUid:currentAuthUser.uid});}catch(error){console.warn('[230MATCH] 기존 로컬 참가신청 전환 실패',error);}
  }
}
async function startRegistrationCloudSync({force=false}={}){
  if(!currentAuthUser){
    registrationCloudUnsubscribe?.();registrationCloudUnsubscribe=null;registrationCloudRows=[];registrationCloudReady=false;registrationCloudKey='';
    return;
  }
  const rt=await registrationRuntime();
  const ctx=registrationContext();
  ...
  registrationCloudUnsubscribe?.();registrationCloudUnsubscribe=null;registrationCloudKey=key;
  const collectionRef=rt.api.collection(rt.db,REGISTRATION_COLLECTION);
  const q=canOperate()
  ...
  registrationCloudUnsubscribe=rt.api.onSnapshot(q,snap=>{
    registrationCloudRows=snap.docs.map(doc=>registrationNormalize({id:doc.id,...doc.data()}));
    registrationCloudReady=true;
    const ctx=registrationContext();
  ...
  setTimeout(()=>void migrateLocalOwnRegistrations(),300);
}

function requireEntryLogin(){
  if(currentAuthUser)return true;
  notice('참가 신청은 로그인 후 이용할 수 있습니다. 먼저 로그인해 주세요.','warning');
  openSocialLogin();
  return false;
}
function renderEntryLoginGate(){
  const submit=document.getElementById('entryApplicationSubmitBtn');
  if(!submit)return;
  let gate=document.getElementById('entryLoginGate');
  if(!gate){
    gate=document.createElement('div');
    gate.id='entryLoginGate';
    submit.parentElement?.insertBefore(gate,submit);
  }
  if(currentAuthUser){
    gate.className='entry-login-gate success';
    gate.textContent=`✓ ${authUserLabel()}님 로그인 상태 · 참가 신청 가능`;
    if(!entryEditingId)submit.textContent='참가 신청 접수';
  }else{
    gate.className='entry-login-gate';
    gate.textContent='🔒 참가 신청은 로그인 후 가능합니다. 상단 로그인 버튼 또는 참가 신청 버튼을 눌러 로그인하세요.';
    if(!entryEditingId)submit.textContent='로그인 후 참가 신청';
  }
}
function entryDivisionApplicationRows(){
  try{ensureMultiDivisionRuntime();}catch(_e){}
  const activeId=String(state.multiDivision?.activeDivisionId||'');
  const ctx=registrationContext();
  const cloud=registrationCloudReady?registrationAdminRowsByDivision():null;
  return (state.multiDivision?.divisions||[]).map((division,index)=>{
    const applications=cloud
      ? cloud.filter(a=>String(a.divisionId||'')===String(division.id))
      : (String(division.id)===activeId?[...(state.portal?.applications||[])]:[...(division.snapshot?.portal?.applications||[])]);
    return {
...
function renderEntryDivisionAdminOverview(){
  const admin=document.getElementById('entryAdminList');
  if(!admin||!isAdmin())return;
  const toolbar=document.querySelector('#view-entry .entry-admin-toolbar');
  if(!toolbar)return;
  let root=document.getElementById('entryDivisionAdminOverview');
  if(!root){
    root=document.createElement('section');
    root.id='entryDivisionAdminOverview';
    root.className='entry-admin-division-overview';
    toolbar.insertAdjacentElement('afterend',root);
  }
  const rows=entryDivisionApplicationRows();
  const totalPending=rows.reduce((n,r)=>n+r.pending,0);
  const current=rows.find(r=>r.current);
  root.innerHTML=`<div class="entry-admin-division-overview-head">
    <div><strong>전체 부서 참가신청 현황</strong><span>현재 부서와 다른 부서의 승인 대기도 함께 확인합니다.</span></div>
    <span>전체 미입금 <b>${totalPending}건</b></span>
  </div>
  <div class="entry-admin-division-cards">${rows.map(r=>`<article class="entry-admin-division-card ${r.current?'current':''} ${r.pending?'pending':''}">
    <div><strong>${portalEscape(r.name)}${r.current?' · 현재':''}</strong><small>미입금 ${r.pending} · 참가 ${r.approved} · 후보 ${r.reserve}</small></div>
  ...
  if(badge){
    const currentPending=current?.pending||0;
    badge.textContent=totalPending===currentPending?`미입금 ${currentPending}건`:`현재 미입금 ${currentPending}건 · 전체부서 ${totalPending}건`;
  }
}
...
async function submitPublicApplication(){
  ...
  const entryCtx=registrationContext();const common={tournamentId:selectedTournament.id,tournamentName:selectedTournament.name,tournamentDivision:entryCtx.divisionName||selectedTournament.division||'',divisionId:entryCtx.divisionId,teamName:name,affiliation,phone,memo,players,representativeIndex,representativeName:representative.name,ownerUid:currentAuthUser?.uid||'',smsTargetMode:smsTargetMode||'both',updatedAt:new Date().toISOString()};
  if(entryEditingId){
...
function entrySelectedPaymentGuide(){
  const selectedId=String(document.getElementById('entryApplicationTournament')?.value||'');
  const currentId=String(state.tournament?.id||state.multiTournament?.activeTournamentId||'');
  if(!selectedId||selectedId===currentId)return state.portal?.guide||{};
  try{
    ensureMultiTournamentRuntime();
    const record=(state.multiTournament?.tournaments||[]).find(r=>String(r.id||'')===selectedId);
    return record?.snapshot?.portal?.guide||record?.guide||state.portal?.guide||{};
  }catch(_e){return state.portal?.guide||{};}
}
function renderEntryPaymentInfo(){
  const guide=entrySelectedPaymentGuide();
  const bank=String(guide?.bank||'').trim(),account=String(guide?.account||'').trim(),fee=String(guide?.fee||'').trim();
  const accountText=document.getElementById('entryPaymentAccountText');
  const feeText=document.getElementById('entryPaymentFeeText');
  const noteText=document.getElementById('entryPaymentNoteText');
  const copy=document.getElementById('entryPaymentAccountCopyBtn');
  if(accountText)accountText.textContent=[bank,account].filter(Boolean).join(' ')||'입금계좌 미설정';
  if(feeText)feeText.textContent=`참가비 ${fee||'미설정'}`;
  if(noteText)noteText.textContent=guide?.paymentNote||'';
  if(copy){copy.disabled=!account;copy.dataset.copyAccount=account;}
}
async function copyEntryPaymentAccount(){
  const btn=document.getElementById('entryPaymentAccountCopyBtn');
  const account=String(btn?.dataset.copyAccount||entrySelectedPaymentGuide()?.account||'').trim();
  if(!account)return notice('복사할 계좌번호가 설정되지 않았습니다.','warning');
  try{
    if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(account);
    else{
      const area=document.createElement('textarea');area.value=account;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();document.execCommand('copy');area.remove();
    }
...
function renderEntrySelfManager(){
  ...
  if(!currentAuthUser){
    ...
    root.innerHTML='<div class="entry-self-empty">로그인하면 본인의 신청이 자동 표시됩니다.</div>';
    return;
  }
  const rows=entryOwnedRows();
    ...
    return `<article class="entry-self-card">
      <div class="entry-self-card-main">
        ...
        <div class="entry-self-card-meta">
          ...
          ${cancelRequested?'<span class="badge badge-warning">취소 승인 대기</span>':''}
        </div>
      </div>
  ...
  }).join('')||'<div class="entry-self-empty">현재 계정으로 신청한 내역이 없습니다.</div>';
}
...
function lookupPublicApplication(){
  ...
  const rows=(registrationCloudReady?registrationCloudRows:(state.portal?.applications||[])).filter(a=>a.phone===phone||entryApplicationPlayers(a).some(p=>p.phone===phone)).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
  root.innerHTML=rows.map(a=>{
    const reserveNo=simpleRegistrationReserveOrder(a);
...
function simpleRegistrationRows(){
  return registrationCloudReady?registrationRowsForCurrentDivision():(state.portal?.applications||[]);
}
function simpleRegistrationCapacity(){
  return Math.max(1,Number(state.tournament?.capacity||state.prelim?.settings?.activeTeamCount||96));
}
function simpleRegistrationReserveOrder(item){
  if(item?.status!=='reserve')return 0;
  const rows=simpleRegistrationRows().filter(a=>a.status==='reserve').sort((a,b)=>String(a.createdAt||'').localeCompare(String(b.createdAt||'')));
  return Math.max(0,rows.findIndex(a=>a.id===item.id)+1);
}
function simpleRegistrationAutoStatus(excludeId=''){
  const cap=simpleRegistrationCapacity();
  const active=simpleRegistrationRows().filter(a=>String(a.id)!==String(excludeId)&&a.status==='approved').length;
  return active<cap?'approved':'reserve';
}
...
async function simplePromoteNextReserve(){
  const rows=simpleRegistrationRows();
  if(rows.filter(a=>a.status==='approved').length>=simpleRegistrationCapacity())return null;
  const next=rows.filter(a=>a.status==='reserve').sort((a,b)=>String(a.createdAt||'').localeCompare(String(b.createdAt||'')))[0];
  if(!next)return null;
  next.status='approved';next.approvedAt=new Date().toISOString();next.updatedAt=next.approvedAt;next.adminMemo='정원 발생으로 후보에서 자동 참가 전환되었습니다.';
  await saveRegistrationCloud(next);await simpleSyncTeam(next);return next;
}
async function toggleEntryPayment(id){
  if(!requireAdmin('입금 확인'))return;
  const item=simpleRegistrationRows().find(a=>a.id===id);if(!item)return notice('참가 신청을 찾을 수 없습니다.','error');
  if(!['approved','reserve'].includes(item.status))return notice('취소된 신청은 입금 처리할 수 없습니다.','warning');
  const wasPaid=item.paymentStatus==='paid'||item.paid===true;
  if(wasPaid){if(!confirm(`${item.teamName}의 입금확인을 취소할까요?`))return;item.paid=false;item.paymentStatus='unpaid';item.paidAt='';item.paymentConfirmedByUid='';item.paymentConfirmedByName='';item.updatedAt=new Date().toISOString();}
  else{item.paid=true;item.paymentStatus='paid';item.paidAt=new Date().toISOString();item.paymentConfirmedByUid=currentAuthUser?.uid||'';item.paymentConfirmedByName=authUserLabel();item.updatedAt=item.paidAt;}
  try{await saveRegistrationCloud(item);await simpleSyncTeam(item);try{safePersistState(wasPaid?'입금확인 취소':'입금확인');}catch(_e){}renderApplicationPortal();lookupPublicApplication();renderParticipantManager?.();notice(wasPaid?'입금확인을 취소했습니다.':`입금확인 완료 · ${entryDateTime(item.paidAt)}`,'success');if(!wasPaid)openEntrySmsDialog('payment',item);}catch(error){notice(`입금확인 저장 실패: ${error?.message||error}`,'error');}
}

async function persistRegistrationAdminMutation(item,label='참가신청 변경'){
  try{await saveRegistrationCloud(item);return true;}
  catch(error){notice(`${label} 저장 실패: ${error?.message||error}`,'error');return false;}
}
...
function renderRegistrationAdminNotifySetting(){
  const input=document.getElementById('entryAdminNotifyPhone');if(!input)return;
  if(document.activeElement!==input)input.value=registrationAdminNotifyPhone();
}
function saveRegistrationAdminNotifySetting(){
  if(!requireAdmin('관리자 알림번호 설정'))return;
  const input=document.getElementById('entryAdminNotifyPhone');
  const phone=String(input?.value||'').replace(/\D/g,'');
  if(!validatePhone(phone))return notice('관리자 알림 휴대전화 번호를 정확히 입력하세요.','error');
  state.portal=state.portal||{};
  state.portal.registrationSmsSettings={...(state.portal.registrationSmsSettings||{}),adminPhone:phone};
  commit('참가접수 관리자 알림번호 저장');
  notice('참가신청·취소요청 관리자 알림번호를 저장했습니다.','success');
}
function registrationAdminNoticeBody(kind,item){
...
async function openAdminNoticeSms(kind,item,{ask=true}={}){
  ...
  try{
    ...
    notice(`PC에서는 문자앱을 직접 열 수 없어 관리자 번호와 ${label} 내용을 복사했습니다.`,'info');
    return true;
  }catch(_e){
    notice(`관리자 번호 ${adminPhone}\n${body}`,'info');
    return false;
  }
}
...
function renderApplicationPortal(){
  ...
  const publicRows=(rows,type)=>{
    ...
    const cards=visible.map((a,index)=>`<article class="entry-public-team-row"><span class="entry-public-order">${index+1}</span><div><strong>${portalEscape(a.teamName||'팀명 미등록')}</strong><small>${portalEscape(a.affiliation||'소속 미등록')} · ${a.paid?'입금완료':'미입금'}</small></div>${type==='reserve'?`<span class="badge badge-warning">후보 ${index+1}</span>`:'<span class="badge badge-safe">참가</span>'}</article>`).join('');
  ...
  if(admin){
    ...
    const cancelRequestedRows=applications.filter(a=>a.cancelRequestStatus==='requested');
    const quickMatch=a=>entryAdminQuickFilter==='all'||
      (entryAdminQuickFilter==='unpaid'&&['approved','reserve'].includes(a.status)&&!a.paid)||
    ...
    const badge=document.getElementById('entryPendingCount');if(badge)badge.textContent=`미입금 ${unpaid.length}건`;
    ...
    setQuick('entryQuickUnpaid',unpaid.length);
    setQuick('entryQuickPaid',paidRows.length);
    setQuick('entryQuickReserve',reserve.length);
    setQuick('entryQuickCancel',cancelRequestedRows.length);
    document.querySelectorAll('[data-entry-quick-filter]').forEach(btn=>btn.classList.toggle('active',btn.dataset.entryQuickFilter===entryAdminQuickFilter));
    renderRegistrationAdminNotifySetting();
    const toolbar=document.querySelector('#view-entry .entry-admin-toolbar');if(toolbar)toolbar.dataset.summary=`전체 ${applications.length}건 · 미입금 ${unpaid.length}건 · 입금완료 ${paidRows.length}건 · 후보 ${reserve.length}건 · 취소요청 ${cancelRequestedRows.length}건`;
...
async function processEntryApplication(id,action){
  if(!requireAdmin('참가 상태 조정'))return;
  const item=simpleRegistrationRows().find(a=>a.id===id);if(!item)return;
  if(!['reserve','promote'].includes(action))return;
  item.status=action==='reserve'?'reserve':'approved';item.updatedAt=new Date().toISOString();if(item.status==='approved')item.approvedAt=item.approvedAt||item.updatedAt;
  try{await saveRegistrationCloud(item);await simpleSyncTeam(item);safePersistState('참가 상태 조정');renderApplicationPortal();}catch(error){notice(`상태 저장 실패: ${error?.message||error}`,'error');}
}
...
function editEntryApplication(id){
  ...
  const d=document.getElementById('entrySelfEditDialog');
  if(!d)return notice('수정창을 열 수 없습니다. 새로고침 후 다시 시도해 주세요.','error');
  const players=entryApplicationPlayers(item);
  const p1=players[0]||{},p2=players[1]||{};
  document.getElementById('entrySelfEditId').value=item.id;
  document.getElementById('entrySelfEditP1Name').value=p1.name||'';
  document.getElementById('entrySelfEditP1Club').value=p1.club||'';
  document.getElementById('entrySelfEditP1Phone').value=p1.phone||'';
  document.getElementById('entrySelfEditP2Name').value=p2.name||'';
  document.getElementById('entrySelfEditP2Club').value=p2.club||'';
  document.getElementById('entrySelfEditP2Phone').value=p2.phone||'';
  (Number(item.representativeIndex||0)===1?document.getElementById('entrySelfRep2'):document.getElementById('entrySelfRep1')).checked=true;
  (item.smsTargetMode==='representative'?document.getElementById('entrySelfSmsRepresentative'):document.getElementById('entrySelfSmsBoth')).checked=true;
  document.getElementById('entrySelfEditMemo').value=item.memo||'';
  const fb=document.getElementById('entrySelfEditFeedback');if(fb){fb.hidden=true;fb.textContent='';}
  if(!d.open)d.showModal();
  setTimeout(()=>document.getElementById('entrySelfEditP1Name')?.focus(),40);
}
async function saveEntrySelfEdit(){
  ...
  if(item.cancelRequestStatus==='requested')return feedback('취소 승인 대기 중에는 신청 내용을 수정할 수 없습니다.');
  const cleanPhone=v=>String(v||'').replace(/\D/g,'');
  const players=[
    {name:String(document.getElementById('entrySelfEditP1Name')?.value||'').trim(),club:String(document.getElementById('entrySelfEditP1Club')?.value||'').trim(),phone:cleanPhone(document.getElementById('entrySelfEditP1Phone')?.value)},
    {name:String(document.getElementById('entrySelfEditP2Name')?.value||'').trim(),club:String(document.getElementById('entrySelfEditP2Club')?.value||'').trim(),phone:cleanPhone(document.getElementById('entrySelfEditP2Phone')?.value)}
  ];
  if(players.some(p=>!p.name||!p.club||p.phone.length<10))return feedback('두 선수의 이름·소속 클럽·휴대전화를 모두 정확히 입력하세요.');
  const representativeIndex=Number(document.querySelector('input[name="entrySelfRepresentative"]:checked')?.value||0);
  const representative=players[representativeIndex]||players[0];
  const smsTargetMode=String(document.querySelector('input[name="entrySelfSmsMode"]:checked')?.value||'both');
  item.players=players;
  item.teamName=players.map(p=>p.name).join(' / ');
  item.affiliation=players.map(p=>p.club).join(' / ');
  item.phone=representative.phone;
  item.representativeIndex=representativeIndex;
  ...
  try{
    ...
    try{await simpleSyncTeam(item);safePersistState('본인 참가 신청 수정');}catch(_e){}
    document.getElementById('entrySelfEditDialog')?.close();
    renderApplicationPortal();lookupPublicApplication();renderParticipantManager?.();
    notice('참가 신청 내용을 수정했습니다.','success');
  }catch(error){
    feedback(`수정 저장 실패: ${error?.message||error}`);
  }finally{
    if(saveBtn){saveBtn.disabled=false;saveBtn.textContent='수정 저장';}
  }
}

async function cancelEntryApplication(id){
  const item=(registrationCloudReady?registrationCloudRows:(state.portal?.applications||[])).find(a=>a.id===id);
  if(!item||!['approved','reserve'].includes(item.status))return;
  const mine=Boolean(currentAuthUser&&String(item.ownerUid||'')===String(currentAuthUser.uid||''));
  if(!mine)return notice('본인의 참가 신청만 취소 요청할 수 있습니다.','error');
  if(item.cancelRequestStatus==='requested')return notice('이미 취소 승인을 기다리고 있습니다.','info');
  const d=document.getElementById('entryCancelRequestDialog');
  if(!d)return notice('취소 요청창을 열 수 없습니다. 새로고침 후 다시 시도해 주세요.','error');
  document.getElementById('entryCancelRequestId').value=item.id;
  document.getElementById('entryCancelBank').value=item.refundBank||'';
  document.getElementById('entryCancelAccount').value=item.refundAccount||'';
  document.getElementById('entryCancelHolder').value=item.refundAccountHolder||'';
  document.getElementById('entryCancelReason').value=item.cancelReason||'';
  const fb=document.getElementById('entryCancelRequestFeedback');if(fb){fb.hidden=true;fb.textContent='';}
  if(!d.open)d.showModal();
  setTimeout(()=>document.getElementById('entryCancelBank')?.focus(),40);
}
async function submitEntryCancelRequest(){
  const id=String(document.getElementById('entryCancelRequestId')?.value||'');
  const item=(registrationCloudReady?registrationCloudRows:(state.portal?.applications||[])).find(a=>a.id===id);
  const mine=Boolean(currentAuthUser&&String(item?.ownerUid||'')===String(currentAuthUser.uid||''));
  const fb=document.getElementById('entryCancelRequestFeedback');
  const feedback=(msg,type='error')=>{if(fb){fb.hidden=false;fb.className=`notice ${type} entry-modal-feedback`;fb.textContent=msg;}};
  if(!item||!mine)return feedback('본인의 참가 신청을 찾을 수 없습니다.');
  const bank=String(document.getElementById('entryCancelBank')?.value||'').trim();
  const account=String(document.getElementById('entryCancelAccount')?.value||'').trim();
  const holder=String(document.getElementById('entryCancelHolder')?.value||'').trim();
  const reason=String(document.getElementById('entryCancelReason')?.value||'').trim()||'개인 사정';
  if(!bank)return feedback('환불 은행을 입력하세요.');
  if(account.replace(/\D/g,'').length<6)return feedback('환불 계좌번호를 정확히 입력하세요.');
  if(!holder)return feedback('예금주를 입력하세요.');
  if(!confirm(`${item.teamName} 참가 취소를 요청할까요?\n관리자 승인 전까지 참가 상태는 유지됩니다.`))return;
  item.cancelRequestStatus='requested';item.cancelReason=reason;item.refundBank=bank;item.refundAccount=account;item.refundAccountHolder=holder;item.cancelRequestedAt=new Date().toISOString();item.updatedAt=item.cancelRequestedAt;
  const btn=document.getElementById('entryCancelRequestSubmitBtn');
  ...
  try{
    ...
    if(btn){btn.disabled=false;btn.textContent='취소 요청 보내기';}
  }
}
...
async function rejectRegistrationCancellation(id){
  if(!requireAdmin('참가 취소 반려'))return;
  const item=simpleRegistrationRows().find(a=>a.id===id);if(!item||item.cancelRequestStatus!=='requested')return;
  const reason=prompt('취소 반려 사유를 입력하세요.','취소 조건을 확인해 주세요.');if(reason===null)return;
  item.cancelRequestStatus='rejected';item.cancelRejectReason=reason.trim();item.cancelRejectedAt=new Date().toISOString();item.updatedAt=item.cancelRejectedAt;
...
function bindEntryApplications(){
  if(!entryEditCaptureBound){
    entryEditCaptureBound=true;
    document.addEventListener('click',e=>{
      const edit=e.target.closest?.('[data-entry-edit]');
      if(!edit)return;
      e.preventDefault();
      e.stopPropagation();
      editEntryApplication(String(edit.dataset.entryEdit||''));
    },true);
  }
  const entrySubmit=document.getElementById('entryApplicationSubmitBtn');if(entrySubmit&&!entrySubmit.dataset.directBound){entrySubmit.dataset.directBound='1';entrySubmit.addEventListener('click',submitPublicApplication);}
  if(!entryApplicationDelegatedBound){entryApplicationDelegatedBound=true;
  document.addEventListener('click',e=>{
  ...
  document.getElementById('entryPaymentAccountCopyBtn')?.addEventListener('click',copyEntryPaymentAccount);
  document.getElementById('entryApplicationTournament')?.addEventListener('change',renderEntryPaymentInfo);
  ...
  bindOnce('entryCancelRequestCloseBtn','click',()=>document.getElementById('entryCancelRequestDialog')?.close());
  bindOnce('entryCancelRequestCancelBtn','click',()=>document.getElementById('entryCancelRequestDialog')?.close());
  bindOnce('entryCancelRequestSubmitBtn','click',()=>void submitEntryCancelRequest());
}
...
function tournamentDateValue(value){
  const raw=String(value||'').trim();
  if(!raw)return Number.POSITIVE_INFINITY;
  const t=Date.parse(/^\d{4}-\d{2}-\d{2}$/.test(raw)?`${raw}T12:00:00`:raw);
  return Number.isFinite(t)?t:Number.POSITIVE_INFINITY;
}
function sortUpcomingTournamentRecords(records){
  return [...(records||[])].sort((a,b)=>{
    const ad=String(a?.snapshot?.portal?.guide?.date||a?.guide?.date||a?.date||'');
    const bd=String(b?.snapshot?.portal?.guide?.date||b?.guide?.date||b?.date||'');
    const av=tournamentDateValue(ad),bv=tournamentDateValue(bd);
    if(av!==bv)return av-bv;
    return String(a?.name||a?.snapshot?.tournament?.name||'').localeCompare(String(b?.name||b?.snapshot?.tournament?.name||''),'ko');
  });
}
function renderHomeTournamentCards(){
  ...
  if(!root)return;
  ensureMultiTournamentRuntime();
  const allRecords=(state.multiTournament?.tournaments||[]).filter(r=>isRealTournamentName(r?.name||r?.snapshot?.tournament?.name));
  const records=sortUpcomingTournamentRecords(allRecords.filter(r=>{
    const id=String(r?.id||'');
    const snap=id===String(state.multiTournament?.activeTournamentId||'')?state:(r?.snapshot||{});
    return !(snap?.completion?.completedAt||snap?.tournament?.completedAt||r?.completedAt||r?.status==='completed');
  }));
  if(!records.length){
    root.innerHTML='<div class="home-tournament-empty">현재 운영·예정 대회가 없습니다. 종료된 대회는 대회 목록에서 확인할 수 있습니다.</div>';
    return;
  }
...
function ensureNoticeImageViewer(){
  let overlay=document.getElementById('noticeImageViewer');
  if(overlay)return overlay;
  const style=document.createElement('style');
  style.id='noticeImageViewerStyle';
  style.textContent=`
    .portal-board-image{cursor:zoom-in}
    .portal-board-image-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}
    #noticeImageViewer{position:fixed;inset:0;z-index:100000;background:rgba(0,0,0,.92);display:none;align-items:center;justify-content:center;padding:18px}
    #noticeImageViewer.open{display:flex}
    #noticeImageViewer .notice-viewer-shell{position:relative;width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px}
    #noticeImageViewer .notice-viewer-image-wrap{width:100%;height:calc(100% - 62px);overflow:auto;display:flex;align-items:center;justify-content:center;-webkit-overflow-scrolling:touch}
    #noticeImageViewer img{display:block;max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;touch-action:pinch-zoom}
    #noticeImageViewer .notice-viewer-tools{display:flex;gap:8px;justify-content:center;flex-wrap:wrap}
    #noticeImageViewer .notice-viewer-btn{border:1px solid rgba(255,255,255,.35);background:#fff;color:#10213f;border-radius:12px;padding:10px 16px;font-weight:800;cursor:pointer}
    #noticeImageViewer .notice-viewer-close{position:absolute;right:4px;top:4px;width:44px;height:44px;border-radius:50%;border:0;background:rgba(255,255,255,.96);font-size:24px;font-weight:900;cursor:pointer;z-index:2}
    @media (max-width:720px){#noticeImageViewer{padding:8px}#noticeImageViewer .notice-viewer-image-wrap{height:calc(100% - 56px)}#noticeImageViewer .notice-viewer-btn{padding:9px 12px}}
  `;
  document.head.appendChild(style);
  overlay=document.createElement('div');
  overlay.id='noticeImageViewer';
  overlay.setAttribute('role','dialog');
  overlay.setAttribute('aria-modal','true');
  overlay.setAttribute('aria-label','공지 이미지 확대 보기');
  overlay.innerHTML=`<div class="notice-viewer-shell"><button type="button" class="notice-viewer-close" aria-label="닫기">×</button><div class="notice-viewer-image-wrap"><img alt="공지 이미지 확대"></div><div class="notice-viewer-tools"><button type="button" class="notice-viewer-btn" data-notice-viewer-download>이미지 다운로드</button><button type="button" class="notice-viewer-btn" data-notice-viewer-close>닫기</button></div></div>`;
  document.body.appendChild(overlay);
  const close=()=>closeNoticeImageViewer();
  overlay.querySelector('.notice-viewer-close')?.addEventListener('click',close);
  overlay.querySelector('[data-notice-viewer-close]')?.addEventListener('click',close);
  overlay.addEventListener('click',e=>{if(e.target===overlay)close();});
  overlay.querySelector('[data-notice-viewer-download]')?.addEventListener('click',()=>downloadNoticeImageById(overlay.dataset.postId||''));
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&overlay.classList.contains('open'))close();});
  return overlay;
}
function openNoticeImageViewer(postId){
  const post=(state.portal?.posts||[]).find(p=>String(p.id)===String(postId));
  const src=post?stage6109ImageSrc(post):'';
  if(!src)return notice('확대할 공지 이미지를 찾지 못했습니다.','error');
  const overlay=ensureNoticeImageViewer(),img=overlay.querySelector('img');
  overlay.dataset.postId=String(post.id||'');
  if(img){img.src=src;img.alt=`${post.title||'공지'} 이미지 확대`;}
  overlay.classList.add('open');
  document.body.dataset.noticeViewerOverflow=document.body.style.overflow||'';
  document.body.style.overflow='hidden';
}
function closeNoticeImageViewer(){
  const overlay=document.getElementById('noticeImageViewer');if(!overlay)return;
  overlay.classList.remove('open');
  const img=overlay.querySelector('img');if(img)img.removeAttribute('src');
...
async function stage3610OpenTournamentGuide(id=''){
  ...
  if(targetId&&targetId!==currentId){
    goWorkspaceRoute(targetId,'','guide');
    return true;
  }
  renderTournamentGuide();
  ...
  if(!opened||document.body.dataset.currentView!=='guide'||!document.getElementById('view-guide')?.classList.contains('active')){
    console.error('[230MATCH] guide route activation failed');
    notice('대회 요강 화면을 열지 못했습니다.','error');
    return false;
  }
...
function visibleBoardPosts({admin=false}={}){return [...globalPosts()].filter(p=>admin||boardPostStatus(p)==='active').sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)||(b.important?1:0)-(a.important?1:0)||String(b.createdAt).localeCompare(String(a.createdAt)));}
...
async function saveBoardPost(){
  if(!requireAdmin('전체 공지 저장'))return;
  ...
  const postId=editId||crypto.randomUUID();
  const current=posts.find(p=>p.id===editId);
  let imageUrl=stage4108PendingNoticeImage||'',imageStoragePath=stage4108PendingNoticeStoragePath||'',imageDataUrl='';
  ...
  try{
    if(String(stage4108PendingNoticeImage||'').startsWith('data:image/')){
      ...
      const uploaded=await uploadManagedImage({
        folder:'noticePosts',ownerId:'230match-global',itemId:postId,
        dataUrl:stage4108PendingNoticeImage,fileName:stage4108PendingNoticeImageName,
        contentType:stage4108PendingNoticeImageType,previousPath:current?.imageStoragePath||''
      });
      imageUrl=uploaded.url;imageStoragePath=uploaded.path;
      stage4108PendingNoticeImage=uploaded.url;stage4108PendingNoticeStoragePath=uploaded.path;
    }else if(!stage4108PendingNoticeImage&&current?.imageStoragePath){
      await deleteManagedImage(current.imageStoragePath);imageStoragePath='';
    }else if(String(stage4108PendingNoticeImage||'').startsWith('data:')){
      imageDataUrl=stage4108PendingNoticeImage;
    }
  }catch(error){
    notice(`공지 이미지 업로드 실패: ${error?.message||error}`,'error');return;
  }

  const now=new Date().toISOString();
  const payload={
    title,body,
    pinned:Boolean(document.getElementById('boardPostPinned')?.checked),
    important:Boolean(document.getElementById('boardPostImportant')?.checked),
    popup:Boolean(document.getElementById('boardPostPopup')?.checked),
    startAt,endAt,popupStartAt,popupEndAt,
    imageUrl:imageUrl&&!String(imageUrl).startsWith('data:')?imageUrl:'',
    imageStoragePath,imageDataUrl,
    imageName:stage4108PendingNoticeImageName||'',
    imageType:stage4108PendingNoticeImageType||'',
    updatedAt:now
  };

  if(current)Object.assign(current,payload);
  else posts.unshift({id:postId,...payload,createdAt:now});

  // 전역 공지는 대회 state/commit 경로를 절대 거치지 않습니다.
  globalNoticeState.posts=posts;
  globalNoticeState.updatedAt=now;
  globalNoticeReady=true;
  mirrorGlobalPostsToState();
  cacheGlobalNoticeState();

  try{
    await saveGlobalNoticeCloud(current?'전체 공지 수정':'전체 공지 등록');
  }catch(error){
    // 로컬 캐시는 유지하되 저장 실패를 명확히 알립니다.
    renderBoardFast();renderPopupManager();
    notice(`공지 저장 실패: ${error?.message||error}`,'error');
    return;
  }

  const open=document.getElementById('homeNoticePopup');
  if(open?.open&&open.dataset.postId===editId&&!payload.popup)open.close();
  clearBoardPostForm();
  renderBoardFast();renderPopupManager();renderHomeFast();showEligibleHomePopup();
  notice(current?'공지를 수정했습니다.':'공지를 등록했습니다.','success');
}
...
function savePopupManagerItem(article){const id=article?.dataset.popupManagerId,post=globalPosts().find(p=>p.id===id);if(!post)return;const enabled=Boolean(article.querySelector('[data-popup-enabled]')?.checked),start=String(article.querySelector('[data-popup-start]')?.value||''),end=String(article.querySelector('[data-popup-end]')?.value||'');if(start&&end&&new Date(start)>=new Date(end)){notice('팝업 종료는 시작보다 뒤여야 합니다.','error');return;}post.popup=enabled;post.popupStartAt=start;post.popupEndAt=end;post.updatedAt=new Date().toISOString();globalNoticeState.posts=[...globalPosts()];mirrorGlobalPostsToState();cacheGlobalNoticeState();saveGlobalNoticeCloud('홈 팝업 설정 저장').catch(error=>notice(`팝업 설정 저장 실패: ${error?.message||error}`,'error'));renderPopupManager();renderBoardFast();if(!enabled){const d=document.getElementById('homeNoticePopup');if(d?.open&&d.dataset.postId===id)d.close();}showEligibleHomePopup();notice(enabled?'홈 팝업 설정을 저장했습니다.':'홈 팝업 표시를 해제했습니다.','success');}
...
function renderHomeFast(){
  ...
  renderRegistrationSummaryEverywhere();
  try{window.__updateTodayTournamentDashboard?.();}catch(_e){}
  void startPublicRegistrationSync();
  renderHomeTournamentCards();
  renderStage331OperationDashboard();
  ...
  setText('homeTournamentDivision',state.tournament?.division||'종목 미설정');
  setText('homePrelimStatus',`${prelim.filter(x=>x.status==='completed').length} / ${prelim.length}`);
  setText('homeMainStatus',`${main.filter(x=>x.status==='completed').length} / ${main.length}`);
...
function renderPortalViewFast(target){
  ...
  try{
    ...
    if(target==='board'){renderBoardFast();return;}
...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  if(target==='home'){
    ...
    if(typeof window.__update230MatchHomeFeedback==='function')setTimeout(window.__update230MatchHomeFeedback,80);
  }
...
function v3252Recipients(item){
  const ps=entryApplicationPlayers(item).map((p,index)=>({...p,index,phone:String(p.phone||'').replace(/\D/g,'')})).filter(p=>/^01[016789]\d{7,8}$/.test(p.phone));
  const mode=item?.smsTargetMode==='representative'?'representative':'both';
  const list=mode==='representative'?[ps.find(p=>p.index===Number(item?.representativeIndex||0))||ps[0]].filter(Boolean):ps;
  const seen=new Set();return list.filter(p=>{const k=p.phone;if(!k||seen.has(k))return false;seen.add(k);return true;});
}
function v3252CanEdit(item,lookup=''){
  if(!item||!['approved','reserve'].includes(item.status)||item.cancelRequestStatus==='requested')return false;
  if(currentAuthUser&&String(item.ownerUid||'')===String(currentAuthUser.uid||''))return true;
  return Boolean(lookup)&&entryApplicationPlayers(item).some(p=>String(p.phone||'').replace(/\D/g,'')===String(lookup).replace(/\D/g,''));
}
function v3252DeleteRequest(id){const item=simpleRegistrationRows().find(a=>a.id===id);const lookup=String(document.getElementById('entryLookupPhone')?.value||'').replace(/\D/g,'');if(!v3252CanEdit(item,lookup))return notice('로그인한 본인의 승인 대기 신청만 삭제 요청할 수 있습니다.','error');if(!confirm(`${item.teamName} 참가 신청의 삭제를 관리자에게 요청할까요?`))return;item.status='delete_requested';item.deleteRequestedAt=new Date().toISOString();item.updatedAt=item.deleteRequestedAt;commit(`참가 신청 삭제 요청 · ${item.teamName}`);lookupPublicApplication();renderApplicationPortal();notice('삭제 요청을 접수했습니다. 관리자가 확인 후 삭제합니다.','success');}
...
editEntryApplication=function(id){
  const item=(registrationCloudReady?registrationCloudRows:simpleRegistrationRows()).find(a=>String(a.id)===String(id));
  const lookup=String(document.getElementById('entryLookupPhone')?.value||'').replace(/\D/g,'');
  if(!v3252CanEdit(item,lookup)){
    notice('로그인한 본인의 참가 신청만 수정할 수 있습니다. 취소 승인 대기 중에는 수정할 수 없습니다.','error');
    return;
  }
  return v3252OriginalEdit(id);
};
function entrySmsDialogPlayers(item){
  const players=entryApplicationPlayers(item).map((p,index)=>({index,name:String(p.name||`선수 ${index+1}`),phone:String(p.phone||'').replace(/\D/g,'')}));
...
function renderEntrySmsRecipientEditor(item,mode){
  const root=document.getElementById('entrySmsRecipientEditor');if(!root)return;
  const players=entrySmsDialogPlayers(item);const actualMode=mode==='representative'?'representative':'both';
...
function currentEntrySmsRecipients(){
  ...
  root.querySelectorAll('[data-entry-sms-recipient]').forEach(check=>{
    ...
    if(validatePhone(phone)?.ok&&!list.some(x=>x.phone===phone))list.push({name,phone,index:Number(index)});
  });
  return list;
}
function syncEntrySmsTargetText(){
  const item=entrySmsItem?.item,recipients=currentEntrySmsRecipients(),target=document.getElementById('entrySmsTarget');if(!target)return;
  target.textContent=recipients.length?`${recipients.length}명 · ${recipients.map(p=>`${p.name} ${p.phone}`).join(' / ')}`:'수신번호를 선택하세요.';
}
function selectEntrySmsRecipients(mode){
  const root=document.getElementById('entrySmsRecipientEditor');if(!root||!entrySmsItem)return;
  const repIndex=Number(entrySmsItem.item?.representativeIndex||0)===1?1:0;
  root.querySelectorAll('[data-entry-sms-recipient]').forEach(check=>{check.checked=mode==='both'||Number(check.dataset.entrySmsRecipient)===repIndex;});
  syncEntrySmsTargetText();
}
openEntrySmsDialog=function(kind,item){
  entrySmsItem={kind,item};
  const d=document.getElementById('entrySmsDialog');if(!d)return;
  document.getElementById('entrySmsTitle').textContent=({approve:'참가 승인 문자',reserve:'후보 등록 문자',promote:'일반 참가 승격 문자',payment:'입금 완료 문자',reject:'신청 반려 문자',refund:'환불 완료 문자'})[kind]||'참가 신청 문자 확인';
  document.getElementById('entrySmsBody').value=entrySmsMessage(kind,item);
  renderEntrySmsRecipientEditor(item,item?.smsTargetMode==='representative'?'representative':'both');
  document.getElementById('entrySmsSelectBothBtn').onclick=()=>selectEntrySmsRecipients('both');
  document.getElementById('entrySmsSelectRepresentativeBtn').onclick=()=>selectEntrySmsRecipients('representative');
  document.getElementById('entrySmsRecipientEditor').oninput=syncEntrySmsTargetText;
  document.getElementById('entrySmsRecipientEditor').onchange=syncEntrySmsTargetText;
  d.showModal();
};
sendEntrySmsAligo=async function(){
  if(!entrySmsItem)return;const body=document.getElementById('entrySmsBody')?.value?.trim()||'',recipients=currentEntrySmsRecipients();
  if(!recipients.length)return notice('문자 받을 선수를 한 명 이상 선택하세요.','error');
  try{
    await sendAligoSmsV3(recipients,body,{source:'registration',kind:entrySmsItem.kind,title:'230MATCH 참가 안내'});
    entrySmsItem.item.smsHistory=entrySmsItem.item.smsHistory||[];entrySmsItem.item.smsHistory.unshift({kind:entrySmsItem.kind,channel:'aligo',sentAt:new Date().toISOString(),body,recipients});
    commit(`참가 안내 문자 발송 · ${entrySmsItem.item.teamName} · ${recipients.length}명`);notice(`알리고 문자를 ${recipients.length}명에게 발송했습니다.`,'success');closeEntrySmsDialog();renderApplicationPortal();
  }catch(e){notice(`문자 발송 실패: ${e.message||e}`,'error')}
};
sendEntrySmsPhone=function(){
  if(!entrySmsItem)return;const body=document.getElementById('entrySmsBody')?.value?.trim()||'',recipients=currentEntrySmsRecipients();
  if(!recipients.length)return notice('문자 받을 선수를 한 명 이상 선택하세요.','error');
  const phones=recipients.map(x=>x.phone);
  if(/Android|iPhone|iPad|iPod/i.test(navigator.userAgent||''))location.href=`sms:${phones.join(',')}?body=${encodeURIComponent(body)}`;else navigator.clipboard?.writeText(`${phones.join('\n')}\n\n${body}`);
  entrySmsItem.item.smsHistory=entrySmsItem.item.smsHistory||[];entrySmsItem.item.smsHistory.unshift({kind:entrySmsItem.kind,channel:'phone',sentAt:new Date().toISOString(),body,recipients});
  commit(`참가 안내 문자앱 열기 · ${entrySmsItem.item.teamName} · ${recipients.length}명`);notice(`문자앱 수신자 ${recipients.length}명을 준비했습니다.`,'success');closeEntrySmsDialog();
};
copyEntrySms=async function(){
  if(!entrySmsItem)return;const body=document.getElementById('entrySmsBody')?.value?.trim()||'',recipients=currentEntrySmsRecipients();
  await navigator.clipboard.writeText(`${recipients.map(x=>`${x.name} ${x.phone}`).join('\n')}\n\n${body}`);notice('선택한 수신자와 문구를 복사했습니다.','success');
};
const v3252RenderApplications=renderApplicationPortal;renderApplicationPortal=function(){v3252RenderApplications.apply(this,arguments);document.querySelectorAll('.entry-admin-row').forEach(row=>{const sms=row.querySelector('[data-entry-sms]');if(!sms)return;const id=sms.dataset.entrySms;const item=simpleRegistrationRows().find(a=>a.id===id);if(!item)return;if(!row.querySelector('[data-entry-sms-mode]')){const sel=document.createElement('select');sel.className='entry-sms-target-select';sel.dataset.entrySmsMode=id;sel.innerHTML=`<option value="both">두 선수 모두</option><option value="representative">대표전화만</option>`;sel.value=item.smsTargetMode==='representative'?'representative':'both';sms.before(sel)}if(item.status==='delete_requested'&&!row.querySelector('[data-entry-admin-delete]')){const b=document.createElement('button');b.type='button';b.className='btn btn-danger-outline btn-small';b.dataset.entryAdminDelete=id;b.textContent='삭제 승인';row.querySelector('.entry-actions')?.appendChild(b)}});setTimeout(v3252AutofillEntry,0)};
document.addEventListener('change',e=>{const sel=e.target.closest?.('[data-entry-sms-mode]');if(!sel)return;const item=(state.portal?.applications||[]).find(a=>a.id===sel.dataset.entrySmsMode);if(item){item.smsTargetMode=sel.value==='representative'?'representative':'both';commit(`문자 수신대상 설정 · ${item.teamName}`)}});
document.addEventListener('click',e=>{const del=e.target.closest?.('[data-entry-admin-delete]');if(del){e.preventDefault();e.stopImmediatePropagation();v3252AdminDelete(del.dataset.entryAdminDelete)}} ,true);
...
async function switchDivisionWorkspace(id,{view='home'}={}){
  ensureMultiDivisionRuntime();
  id=String(id||'').trim();
  if(!id)return false;
  const record=(state.multiDivision?.divisions||[]).find(d=>String(d.id)===id);
  if(!record){notice('선택한 부서 정보를 찾을 수 없습니다.','error');return false;}
  const tournamentId=String(state.multiTournament?.activeTournamentId||state.tournament?.id||routeTournamentId()||'');
  return goWorkspaceRoute(tournamentId,id,view);
}
function renderDivisionWorkspaceBarLegacy(){
  ...
  bar.querySelector('#activeDivisionSelect')?.addEventListener('change',e=>switchDivisionWorkspace(String(e.target.value||''),{view:'home'}));bar.querySelector('#openDivisionManagerBtn')?.addEventListener('click',openDivisionManager);applyRoleUI();
}
...
(function stage3560PlayerSelfResult(){
    ...
    [[sections[0],prelim,true],[sections[1],main,false]].forEach(([section,matches,isPrelim])=>{
      ...
      [...section.querySelectorAll('.my-match-game')].forEach((card,index)=>{
        ...
        if(eligible&&!card.querySelector('[data-player-result-open]')){
          ...
          if(match.status==='completed'){
            const safety=playerResultCorrectionSafety(match,isPrelim);
            button.title=safety.ok?(safety.warning||'본인 경기 결과를 수정할 수 있습니다.'):(safety.message||'현재 수정할 수 없습니다.');
  ...
  function prelimGroupForMatch(match){
    const groups=state.prelim?.groups||[];
    return groups.find(g=>
      String(g.id||'')===String(match?.groupId||'') ||
      Number(g.groupNo||0)===Number(match?.groupNo||0)
    )||null;
  }
  function mainMatchHasStarted(match){
    if(!match)return false;
    if(['playing','completed'].includes(String(match.status||'')))return true;
    if(match.winner)return true;
    const id=String(match.id||'');
    const court=[...(state.prelim?.courts||[]),...(state.courts||[])].find(c=>
      String(c.playing||'')===id
    );
    return Boolean(court);
  }
  function nextMainMatchFor(match){
    const nextId=String(match?.nextMatchId||'');
    if(!nextId)return null;
    try{return findMatch(state.draw,nextId)||null;}catch(_e){return null;}
  }
  function groupQualifierMainStarted(group){
    if(!group)return false;
    const groupNo=Number(group.groupNo||String(group.name||'').match(/\d+/)?.[0]||0);
    if(!groupNo)return false;
    const all=portalMainMatches();
    return all.some(m=>{
      if(!mainMatchHasStarted(m))return false;
      const teams=[m.teamA,m.teamB].filter(Boolean);
      return teams.some(team=>{
        try{return Number(mainDrawGroupNo(team)||0)===groupNo;}catch(_e){}
        return String(team?.name||team?.label||'').includes(`${groupNo}조`);
      });
    });
  }
  function playerResultCorrectionSafety(match,isPrelim){
    if(!match||match.status!=='completed')return{ok:true,warning:''};

    if(isPrelim){
      const group=prelimGroupForMatch(match);
      const teamCount=(group?.teams||[]).filter(Boolean).length;
      if(teamCount===3){
        if(!canOperate()&&groupQualifierMainStarted(group)){
          return{
            ok:false,
            message:'이 조의 본선 진출팀이 이미 본선 경기를 시작했습니다. 예선 결과 변경은 관리자에게 알려주세요.'
          };
        }
        return{
          ok:true,
          warning:'3팀조 예선 결과를 수정하면 게임득실과 조 순위, 본선 진출팀이 다시 계산될 수 있습니다.'
        };
      }
      return{ok:true,warning:''};
    }

    const next=nextMainMatchFor(match);
    if(next&&mainMatchHasStarted(next)&&!canOperate()){
      return{
        ok:false,
        message:`다음 라운드(${myMatchRoundLabel(next,false)})가 이미 시작되었습니다. 이전 경기 승자를 바꾸면 대진이 꼬일 수 있어 관리자만 수정할 수 있습니다.`
      };
    }
    if(next&&next.teamA&&next.teamB){
      return{
        ok:true,
        warning:`다음 라운드 대진이 이미 만들어져 있습니다. 결과를 바꾸면 다음 경기 진출팀도 함께 다시 반영됩니다.`
      };
    }
    return{ok:true,warning:''};
  }
...
function syncCurrentTournamentRuntime(){
  // 4.5.0: 현재 대회의 전체 snapshot을 매 입력마다 복제하지 않는다.
  // active tournament/division은 루트 state가 원본이며, 스냅샷은 실제 전환 시에만 만든다.
  ensureMultiTournamentRuntime();
  ...
  const guide=state.portal?.guide||{},prelim=state.prelim?.matches||[];let main=[];try{main=Object.values(state.draw?.rounds||{}).flat().filter(Boolean);}catch(_e){}
  record.name=state.tournament.name||'대회 준비 중';record.division=state.tournament.division||'';record.updatedAt=new Date().toISOString();record.date=guide.date||record.date||'';record.venue=guide.venue||record.venue||'';record.guide={date:record.date,venue:record.venue};
  record.status=(state.completion?.completedAt||state.tournament?.completedAt)?'completed':(prelim.some(x=>x?.status&&x.status!=='waiting')||main.some(x=>x?.status&&x.status!=='waiting'))?'ongoing':'recruiting';record.completedAt=state.completion?.completedAt||state.tournament?.completedAt||'';
  record.active=(state.teams||[]).filter(x=>x?.status!=='reserve').length;record.reserve=(state.teams||[]).filter(x=>x?.status==='reserve').length;record.prelimCompleted=prelim.filter(x=>x?.status==='completed').length;record.prelimTotal=prelim.length;record.mainCompleted=main.filter(x=>x?.status==='completed').length;record.mainTotal=main.length;
  // 과거 버전에서 남은 현재대회 snapshot은 더 이상 갱신하지 않는다. 루트 state가 유일한 실시간 원본이다.
  return record;
}

async function switchTournamentWorkspace(id,{view='home'}={}){
  ensureMultiTournamentRuntime();
  id=String(id||'').trim();
  if(!id)return false;
  const record=(state.multiTournament?.tournaments||[]).find(r=>String(r.id)===id);
  if(!record){notice('선택한 대회 정보를 찾을 수 없습니다.','error');return false;}
  return goWorkspaceRoute(id,'',view);
}
...
function renderDivisionWorkspaceBar(){
  ...
  bar.querySelector('#activeTournamentSelect')?.addEventListener('change',e=>switchTournamentWorkspace(String(e.target.value||''),{view:'home'}));
  bar.querySelector('#activeDivisionSelect')?.addEventListener('change',e=>switchDivisionWorkspace(String(e.target.value||''),{view:'home'}));
...
function stage5008TournamentRows(){
  ...
  const activeRows=registry.map((record,index)=>{
    ...
    return {
      ...
      active:workspace&&Object.keys(workspace).length?activeCount:Number(record?.active||0),reserve:workspace&&Object.keys(workspace).length?teams.filter(x=>x?.status==='reserve').length:Number(record?.reserve||0),
      ...
      status:(workspace?.completion?.completedAt||t.completedAt||record?.completedAt||record?.status==='completed')?'completed':((workspace&&Object.keys(workspace).length)&&(prelim.some(x=>x?.status&&x.status!=='waiting')||main.some(x=>x?.status&&x.status!=='waiting')))||record?.status==='ongoing'?'ongoing':'recruiting',
...
function stage6301SortTournamentRows(rows){
  const upcoming=[],completed=[];
  (rows||[]).forEach(x=>(x?.status==='completed'?completed:upcoming).push(x));
  upcoming.sort((a,b)=>{
    const av=tournamentDateValue(a?.date),bv=tournamentDateValue(b?.date);
    if(av!==bv)return av-bv;
  ...
  return {upcoming,completed,all:[...upcoming,...completed]};
}
function stage5008RenderTournamentList(){
  ...
  const allRaw=stage5008TournamentRows();
  const sortedAll=stage6301SortTournamentRows(allRaw).all;
  const visible=sortedAll.filter(x=>(st==='all'||x.status===st)&&(!q||`${x.name} ${x.division} ${x.venue}`.toLowerCase().includes(q)));
  const grouped=stage6301SortTournamentRows(visible);
  const counts={all:allRaw.length,recruiting:allRaw.filter(x=>x.status==='recruiting').length,ongoing:allRaw.filter(x=>x.status==='ongoing').length,completed:allRaw.filter(x=>x.status==='completed').length};
  ...
  root.classList.add('stage5008-compact-list','stage6301-grouped-list');
  const cardHtml=x=>{
  ...
  const section=(title,subtitle,items,kind)=>items.length?`<div class="stage6301-list-heading ${kind}"><div><strong>${title}</strong><span>${subtitle}</span></div><em>${items.length}개</em></div>${items.map(cardHtml).join('')}`:'';
  const html=[
    section('운영·예정 대회','개최일이 가까운 대회부터 표시됩니다.',grouped.upcoming,'upcoming'),
    section('종료된 대회','최근에 종료된 대회부터 따로 보관·표시됩니다.',grouped.completed,'completed')
  ].join('');
  root.innerHTML=html||'<div class="panel portal-empty">조건에 맞는 대회가 없습니다.</div>';
  applyRoleUI?.();
}
...
(function(){
  ...
  function renderVisibleDivisionBar(){
    ...
    bar.querySelectorAll('[data-v6002-division]').forEach(card=>{
      const activate=()=>{
        const id=String(card.dataset.v6002Division||'');
        if(!id||id===String(state.multiDivision?.activeDivisionId))return;
        const tid=String(state.multiTournament?.activeTournamentId||state.tournament?.id||'');
        switchDivisionWorkspace(id,{view:'home'});
      };
...
/* 230MATCH 5.2.4 · bracket ↔ court canonical placement consistency */
(()=>{
  const $=id=>document.getElementById(id);
  const mainMatches=()=>{try{return portalMainMatches();}catch(_e){return Object.values(state.draw?.rounds||{}).flat();}};
  const expectedStatus=p=>p.kind==='playing'?'playing':p.kind==='wait1'?'court_wait1':p.kind==='shared'?'venue_shared_queue':p.kind==='waiting'&&p.courtId?'court_manual_queue':null;
  function audit(){
    const rows=mainMatches().filter(m=>m&&!m.bye&&m.status!=='completed');
    let ok=0,waiting=0,mismatch=0;
    const details=[];
    for(const match of rows){
      const p=liveMatchPlacement(match);
      const canonical=['playing','wait1','waiting','shared'].includes(p.kind);
      if(!canonical){waiting++;continue;}
      const expected=expectedStatus(p);
      const statusOk=!expected||String(match.status||'')===expected||
        (p.kind==='shared'&&['venue_shared_queue','shared_queue'].includes(String(match.status||'')))||
        (p.kind==='waiting'&&['court_manual_queue','ready','waiting'].includes(String(match.status||'')));
      const courtOk=!p.courtName||!match.court||String(match.court)===String(p.courtName)||String(match.court)===String(p.courtId||'');
      if(statusOk&&courtOk)ok++;else{mismatch++;details.push({match,p,statusOk,courtOk});}
    }
    const set=(id,v)=>{const el=$(id);if(el)el.textContent=String(v);};
    set('bracketCourtSyncOk',`일치 ${ok}`);
    set('bracketCourtSyncWaiting',`미배정 ${waiting}`);
    set('bracketCourtSyncMismatch',`불일치 ${mismatch}`);
    const bad=$('bracketCourtSyncMismatch');if(bad)bad.className=`sync-chip ${mismatch?'bad':'ok'}`;
    const op=$('operationBracketSyncState');
    if(op){
      op.className=`operation-bracket-sync-state ${mismatch?'warn':'ok'}`;
      op.textContent=mismatch?`⚠ 대진표 위치정보 ${mismatch}경기 보정 필요 · 실제 코트 위치를 기준으로 화면은 표시 중입니다.`:`✓ 본선 대진표와 실제 코트 위치가 일치합니다.`;
    }
    return{ok,waiting,mismatch,details};
  }
  function repair(){
    if(!canOperate())return notice('관리자/운영자만 위치정보를 맞출 수 있습니다.','error');
    const report=audit();
    if(!report.mismatch)return notice('현재 본선 대진표와 코트 위치가 이미 일치합니다.','success');
    let changed=0;
    for(const {match,p} of report.details){
      const status=expectedStatus(p);
      if(status&&String(match.status||'')!==status){match.status=status;changed++;}
      if(p.courtName&&String(match.court||'')!==String(p.courtName)){match.court=p.courtName;changed++;}
      if(p.kind==='shared'&&match.court){match.court='';changed++;}
    }
    try{safePersistState('본선 대진표-코트 위치정보 동기화');}catch(_e){try{commit('본선 대진표-코트 위치정보 동기화');}catch(__e){}}
    decorateBracketLivePlacements();
    audit();
    renderPortalViewFast(document.body?.dataset.currentView||'operation');
    notice(`실제 코트 배정을 기준으로 본선 위치정보를 맞췄습니다. (${changed}개 항목)`,'success');
  }
  function courtCardById(courtId){
    const grid=$('operationUnifiedCourtGrid');
    return [...(grid?.querySelectorAll('.prelim-court-card')||[])].find(card=>{
      const transfer=card.querySelector('[data-unified-transfer]')?.dataset.unifiedTransfer;
      const status=card.querySelector('[data-prelim-court-status]')?.dataset.prelimCourtStatus;
      const sms=card.querySelector('[data-court-id]')?.dataset.courtId;
      return [transfer,status,sms].some(v=>String(v||'')===String(courtId||''));
    });
  }
  function highlightMatchLocation(matchId,courtId,kind){
    navigatePortalView('operation',{focus:false});
    setTimeout(()=>{
      if(kind==='shared'){
        const q=$('operationSharedQueue')||document.querySelector('.operation-shared-queue-section');
        q?.scrollIntoView({behavior:'smooth',block:'center'});
        q?.classList.add('court-sync-highlight');setTimeout(()=>q?.classList.remove('court-sync-highlight'),2400);
        return;
      }
      const card=courtCardById(courtId);
      if(!card)return;
      const slot=kind==='playing'?card.querySelector('.prelim-court-slot.playing'):kind==='wait1'?card.querySelector('.prelim-court-slot.wait1'):card;
      (slot||card).scrollIntoView({behavior:'smooth',block:'center'});
      (slot||card).classList.add('court-sync-highlight');
      setTimeout(()=>(slot||card).classList.remove('court-sync-highlight'),2400);
    },180);
  }
  document.addEventListener('click',e=>{
    ...
    if(e.target.closest?.('#bracketCourtSyncRepairBtn')){repair();return;}
  });
...
/* 230MATCH 5.2.5 · compact admin "today tournament" dashboard */
(()=>{
  const safe=v=>Array.isArray(v)?v:[];
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=String(v);};
  const done=list=>safe(list).filter(x=>x&&(['completed','done','finished'].includes(String(x.status||'').toLowerCase())||x.winner||x.winnerId)).length;
  function sharedQueueCount(){
    try{
      ensureVenueQueues(state);
      const venue=Object.values(state.operation?.venueQueues||{}).reduce((n,q)=>n+(Array.isArray(q)?q.length:0),0);
      const shared=Array.isArray(state.operation?.sharedQueue)?state.operation.sharedQueue.length:0;
      return venue+shared;
    }catch(_e){
      return (state.sharedQueue?.length||0)+Object.values(state.venueQueues||{}).reduce((n,q)=>n+(q?.length||0),0);
    }
  }
  function playingCourtCount(){
    try{
      const rows=typeof portalCourtRows==='function'?portalCourtRows():safe(state.courts);
      return rows.filter(c=>c?.playing).length;
    }catch(_e){return 0;}
  }
  function currentCancelRequests(){
    try{
      const rows=typeof simpleRegistrationRows==='function'?simpleRegistrationRows():safe(state.portal?.applications);
      return rows.filter(a=>a?.cancelRequestStatus==='requested').length;
    }catch(_e){return 0;}
  }
  function updateTodayDashboard(){
    const root=document.getElementById('stage7115TodayDashboard');
    if(!root)return;
    const summary=typeof registrationPublicSummary==='function'
      ? registrationPublicSummary()
      : {active:safe(state.teams).length,capacity:Number(state.tournament?.capacity||96),unpaid:0,reserve:0};
    const prelim=safe(state.prelim?.matches);
    const main=typeof portalMainMatches==='function'?safe(portalMainMatches()):safe(state.draw?.matches);
    set('todayEntryCount',`${summary.active}/${summary.capacity}`);
    set('todayUnpaidCount',summary.unpaid||0);
    set('todayReserveCount',summary.reserve||0);
    set('todayCancelCount',currentCancelRequests());
    set('todayPrelimCount',`${done(prelim)}/${prelim.length}`);
    set('todayPlayingCount',playingCourtCount());
    set('todaySharedCount',sharedQueueCount());
    set('todayMainCount',`${done(main)}/${main.length}`);
    const status=document.getElementById('stage7115TodayStatus');
    if(status){
      const live=playingCourtCount()>0;
      status.textContent=live?'경기 진행 중 · 숫자를 누르면 바로 이동':'접수·대회 준비 · 숫자를 누르면 바로 이동';
    }
  }
  function openEntryFilter(filter){
    try{entryAdminQuickFilter=filter||'all';}catch(_e){}
    navigatePortalView('entry',{pushHistory:true});
    setTimeout(()=>{
      try{renderApplicationPortal();}catch(_e){}
      const target=document.getElementById('entryAdminQuickSummary')||document.querySelector('.entry-admin-quick-summary');
      target?.scrollIntoView({behavior:'smooth',block:'start'});
    },100);
  }
  document.addEventListener('click',e=>{
    const entry=e.target.closest?.('[data-today-entry-filter]');
    if(entry){
      e.preventDefault();
      openEntryFilter(String(entry.dataset.todayEntryFilter||'all'));
      return;
    }
    const go=e.target.closest?.('[data-today-go]');
    if(go){
      e.preventDefault();
      const target=String(go.dataset.todayGo||'home');
      navigatePortalView(target,{pushHistory:true});
      const kind=String(go.dataset.todayCourtKind||'');
      if(target==='operation'&&kind){
        setTimeout(()=>{
          const btn=document.querySelector(`[data-court-summary-go="${kind}"]`);
          btn?.click();
        },180);
      }
    }
  ...
  const update=()=>{try{updateTodayDashboard();}catch(error){console.warn('[230MATCH] 오늘 대회 대시보드 갱신 실패',error);}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(update,350),{once:true});
  else setTimeout(update,350);
  document.addEventListener('click',()=>setTimeout(update,120),true);
  window.addEventListener('pageshow',update);
...
/* 230MATCH 5.3.1 · result → ranking/advance/court flow validator */
(()=>{
  const safe=v=>Array.isArray(v)?v:[];
  const setCard=(id,status,text)=>{
    const el=document.getElementById(id);if(!el)return;
    el.className=`stage7121-flow-card ${status}`;
    const span=el.querySelector('span');if(span)span.textContent=text;
  };
  const prelimMatches=()=>safe(state.prelim?.matches);
  const groups=()=>safe(state.prelim?.groups);
  const mainMatches=()=>{try{return portalMainMatches();}catch(_e){return Object.values(state.draw?.rounds||{}).flat();}};
  const courts=()=>{
    try{const r=portalCourtRows();if(r?.length)return r;}catch(_e){}
    return safe(state.prelim?.courts).length?safe(state.prelim?.courts):safe(state.courts);
  };
  const teamId=x=>String(x?.id||x?.teamId||'');
  const winnerId=m=>String(m?.winner?.id||m?.winnerId||m?.winner||'');
  const completed=m=>Boolean(m&&(['completed','done','finished'].includes(String(m.status||'').toLowerCase())||winnerId(m)));
  function rankRows(group){
    try{
      if(typeof calculateGroupStandings==='function'){
        const r=calculateGroupStandings(group,state.prelim?.matches||[]);
        if(Array.isArray(r)&&r.length)return r;
      }
    }catch(_e){}
    return safe(group?.standings||group?.ranking||group?.rankings);
  }
  function inspectPrelimRanking(){
    const gs=groups(),ms=prelimMatches();
    let checked=0,bad=0,waiting=0;
    const details=[];
    for(const g of gs){
      const gids=ms.filter(m=>String(m.groupId||'')===String(g.id||'') || Number(m.groupNo||0)===Number(g.groupNo||0));
      if(!gids.length)continue;
      const allDone=gids.every(completed);
      const ranking=rankRows(g);
      if(allDone){
        checked++;
        if(!ranking.length){
          bad++;details.push(`${g.groupNo||g.name||g.id}: 경기 완료인데 순위 없음`);
        }else{
          const ids=ranking.map(r=>teamId(r.team||r)).filter(Boolean);
          if(new Set(ids).size!==ids.length){bad++;details.push(`${g.groupNo||g.name||g.id}: 순위 중복팀`);}
        }
      }else waiting++;
    }
    return bad?{status:'bad',text:`오류 ${bad}`,detail:`완료 조 ${checked}개 중 순위 오류 ${bad}개${details.length?` · ${details.slice(0,5).join(' / ')}`:''}`}:
      {status:'ok',text:checked?`${checked}조 정상`:'진행 전',detail:`완료 조 ${checked} · 진행/대기 조 ${waiting} · 순위 중복 없음`};
  }
  function inspectMainQualifier(){
    const q=Math.max(1,Number(state.prelim?.settings?.qualifiersPerGroup||2));
    const gs=groups();
    let eligible=0,missing=0,dupe=0;
    const qualifierIds=[];
    for(const g of gs){
      const ranking=rankRows(g);
      if(ranking.length>=q){
        eligible++;
        for(const r of ranking.slice(0,q)){
          const id=teamId(r.team||r);if(id)qualifierIds.push(id);
        }
      }
    }
    const dupIds=qualifierIds.filter((id,i,a)=>a.indexOf(id)!==i);dupe=new Set(dupIds).size;
    const slots=safe(state.prelim?.linkedDraw?.slots);
    if(slots.length){
      const real=slots.filter(x=>x&&!x.placeholder);
      for(const id of qualifierIds){
        if(!real.some(x=>teamId(x)===id))missing++;
      }
    }
    if(dupe||missing)return{status:'bad',text:`오류 ${dupe+missing}`,detail:`본선 진출 중복 ${dupe} · 슬롯 미반영 ${missing}`};
    return{status:eligible?'ok':'warn',text:eligible?`${qualifierIds.length}팀 연결`:'예선 진행 전',detail:`순위 확정 조 ${eligible} · 본선 진출 후보 ${qualifierIds.length}팀 · 중복 없음`};
  }
  function inspectMainAdvance(){
    const ms=mainMatches();
    let checked=0,bad=0;
    const details=[];
    for(const m of ms){
      if(!completed(m)||!m.nextMatchId||!winnerId(m))continue;
      checked++;
      let next=null;try{next=findMatch(state.draw,m.nextMatchId);}catch(_e){}
      if(!next){bad++;details.push(`${m.id}: 다음 경기 없음`);continue;}
      const wid=winnerId(m);
      const inNext=[teamId(next.teamA),teamId(next.teamB)].includes(wid);
      if(!inNext){bad++;details.push(`${m.id}→${m.nextMatchId}: 승자 미반영`);}
    }
    return bad?{status:'bad',text:`오류 ${bad}`,detail:`완료 후 다음라운드 연결 ${checked}경기 중 오류 ${bad} · ${details.slice(0,5).join(' / ')}`}:
      {status:'ok',text:checked?`${checked}경기 정상`:'진행 전',detail:`완료 본선 경기 중 다음라운드 연결 ${checked}건 모두 정상`};
  }
  function inspectCourtAdvance(){
    const cs=courts();
    let stale=0,dup=0;
    const seen=new Map(),details=[];
    for(const c of cs){
      const playing=String(c?.playing||'');
      if(playing){
        let m=null;
        try{const u=findUnifiedMatch(state,playing);m=u?.match||null;}catch(_e){}
        if(!m){try{m=findPrelimMatch(state,playing)||findMatch(state.draw,playing);}catch(_e){}}
        if(m&&completed(m)){stale++;details.push(`${c.name||c.id}: 완료경기가 시합중에 남음`);}
      }
      for(const id of [c?.playing,c?.wait1,...safe(c?.queue),...safe(c?.manualQueue)].filter(Boolean)){
        const k=String(id),cid=String(c.id||'');
        if(seen.has(k)&&seen.get(k)!==cid)dup++; else seen.set(k,cid);
      }
    }
    if(stale||dup)return{status:'bad',text:`오류 ${stale+dup}`,detail:`완료경기 잔존 ${stale} · 코트 중복 ${dup}${details.length?` · ${details.slice(0,4).join(' / ')}`:''}`};
    return{status:'ok',text:`${cs.length}코트 정상`,detail:`완료경기 잔존 없음 · 다른 코트 중복배정 없음`};
  }
  function inspectDuplicateOrphans(){
    const ms=[...prelimMatches(),...mainMatches()];
    const ids=ms.map(m=>String(m?.id||'')).filter(Boolean);
    const dup=[...new Set(ids.filter((id,i,a)=>a.indexOf(id)!==i))];
    let orphan=0;
    ...
    if(dup.length||orphan)return{status:'bad',text:`오류 ${dup.length+orphan}`,detail:`중복 경기ID ${dup.length} · 존재하지 않는 nextMatchId ${orphan}`};
    return{status:'ok',text:'정상',detail:`전체 경기ID 중복 없음 · nextMatch 연결 고아 없음`};
  }
  function runResultFlowCheck(){
    const items=[
      ['flowPrelimRanking',inspectPrelimRanking()],
      ['flowMainQualifier',inspectMainQualifier()],
      ['flowMainAdvance',inspectMainAdvance()],
      ['flowCourtAdvance',inspectCourtAdvance()],
      ['flowDuplicate',inspectDuplicateOrphans()]
    ];
    let bad=0,warn=0;
    items.forEach(([id,r])=>{setCard(id,r.status,r.text);if(r.status==='bad')bad++;if(r.status==='warn')warn++;});
    const overall=document.getElementById('stage7121FlowOverall');
    if(overall){
      overall.className=`badge ${bad?'badge-danger':warn?'badge-warning':'badge-safe'}`;
      overall.textContent=bad?`문제 ${bad}`:warn?`확인 ${warn}`:'흐름 정상';
    }
    const d=document.getElementById('stage7121FlowDetail');
    if(d)d.innerHTML=items.map(([,r],i)=>`${i+1}. ${r.detail}`).join('<br>');
    window.__stage7121LastResultFlow=items;
    return{bad,warn,items};
  }
  document.addEventListener('click',e=>{
    if(e.target.closest?.('#stage7121RunFlowCheck')){runResultFlowCheck();return;}
    if(e.target.closest?.('#stage7121ToggleFlowDetail')){
      const d=document.getElementById('stage7121FlowDetail');if(!d)return;
      if(!window.__stage7121LastResultFlow)runResultFlowCheck();
      d.hidden=!d.hidden;return;
    }
  });
...
/* 230MATCH 5.3.5 · home admin feedback SMS + receipt history */
(()=>{
  const COLLECTION='adminRegistrationNotifications';
  const LOCAL_KEY='230match-home-feedback-history-v1';
  const KIND_LABELS={match:'시합 문의',improve:'개선사항',app:'앱 문의',etc:'기타'};
  let inboxUnsub=null,inboxStarted=false;

  const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const localRows=()=>{
    try{const rows=JSON.parse(localStorage.getItem(LOCAL_KEY)||'[]');return Array.isArray(rows)?rows:[];}catch(_e){return[];}
  };
  function saveLocalReceipt(row){
    const rows=localRows();
    rows.unshift(row);
    try{localStorage.setItem(LOCAL_KEY,JSON.stringify(rows.slice(0,8)));}catch(_e){}
    renderMyReceipt();
  }
  function renderMyReceipt(){
    const root=document.getElementById('stage7125MyFeedbackHistory');if(!root)return;
    const row=localRows()[0];
    if(!row){root.innerHTML='';return;}
    const when=row.sentAt?new Date(row.sentAt).toLocaleString('ko-KR'):'';
    const label=row.status==='copied'?'문자 내용이 복사되었습니다.':'관리자에게 보낼 문의 문자를 문자앱에 준비했습니다.';
    root.innerHTML=`<div class="stage7125-feedback-sent"><span>✓ ${esc(label)} <b>${esc(KIND_LABELS[row.kind]||'문의')}</b></span><span>${esc(when)}</span></div>`;
  }
  function feedbackContext(){
    let ctx={};try{ctx=registrationContext()||{};}catch(_e){}
    return {
      tournamentId:String(ctx.tournamentId||state?.multiTournament?.activeTournamentId||state?.tournament?.id||''),
      tournamentName:String(ctx.tournamentName||state?.tournament?.name||'현재 대회'),
      divisionId:String(ctx.divisionId||state?.multiDivision?.activeDivisionId||''),
      divisionName:String(ctx.divisionName||state?.tournament?.division||'')
    };
  }
  function senderLabel(){
    return String(currentAuthUser?.appProfile?.name||currentAuthUser?.displayName||currentAuthUser?.email||'로그인 사용자');
  }
  function senderPhone(){
    try{
      const p=v3252ProfileDefaults?.()||{};
      return String(p.phone||currentAuthUser?.appProfile?.phone||'').replace(/\D/g,'');
    }catch(_e){
      return String(currentAuthUser?.appProfile?.phone||'').replace(/\D/g,'');
    }
  }
  function senderTeamLabel(){
    try{
      const uid=String(currentAuthUser?.uid||'');
      const mine=(registrationCloudRows||[]).find(r=>String(r?.ownerUid||'')===uid&&registrationBelongsToCurrentDivision(r));
      return String(mine?.teamName||mine?.affiliation||'').trim();
    }catch(_e){return'';}
  }
  function feedbackSmsBody(kind,body){
    const c=feedbackContext(),label=KIND_LABELS[kind]||'문의';
    const team=senderTeamLabel(),phone=senderPhone();
    return `[230MATCH ${label}]
보낸사람: ${senderLabel()}${team?` / ${team}`:''}
...
※ 230MATCH 홈 화면에서 작성한 문의입니다.`;
  ...
  async function saveFeedbackCloud(row){
    const rt=await getAuthRuntime();
    if(!rt?.db||!rt?.api||!rt?.user)throw new Error('로그인이 필요합니다.');
    const id=`feedback_${Date.now()}_${String(rt.user.uid||'user').slice(0,10)}`;
  ...
  function buildFeedbackRow(kind,body,adminPhone){
  ...
  function prepareFeedbackNativeLink({showWarning=false}={}){
    const link=document.getElementById('stage7125SendFeedbackBtn');if(!link)return false;
    ...
    if(!validatePhone(adminPhone)){
      link.setAttribute('href','sms:');
      link.dataset.ready='0';
      if(showWarning)notice('관리자 문자 수신번호가 설정되지 않았습니다. 관리자 번호를 먼저 저장하세요.','error');
      return false;
    }
    if(body.length<2){
      link.setAttribute('href','sms:');
      link.dataset.ready='0';
      if(showWarning)notice('문의 내용을 입력하세요.','error');
      return false;
    }
  ...
  function renderAdminInbox(rows){
    ...
    if(!wrap||!root)return;
    wrap.hidden=!canOperate();
    if(!canOperate())return;
    const filtered=(rows||[]).filter(x=>x?.type==='home_feedback').sort((a,b)=>String(b.sentAt||'').localeCompare(String(a.sentAt||''))).slice(0,8);
    root.innerHTML=filtered.length?filtered.map(r=>{
      const when=r.sentAt?new Date(r.sentAt).toLocaleString('ko-KR'):'';
      return `<div class="stage7125-admin-feedback-row"><span class="kind">${esc(KIND_LABELS[r.kind]||'문의')}</span><div class="body"><b>${esc(r.senderName||'사용자')}</b>${r.senderPhone?` · ${esc(r.senderPhone)}`:''}<br>${esc(r.body||'')}</div><span class="meta">${esc(r.tournamentName||'')}${r.divisionName?` · ${esc(r.divisionName)}`:''}<br>문자앱 열기 · ${esc(when)}</span></div>`;
    }).join(''):'<div class="portal-empty">아직 홈 문의가 없습니다.</div>';
  }
  async function startAdminInbox(force=false){
    if(!canOperate())return;
    if(inboxStarted&&!force)return;
    try{
      const rt=await getAuthRuntime();if(!rt?.db||!rt?.api||!rt?.user)return;
      inboxUnsub?.();inboxStarted=true;
      const ref=rt.api.collection(rt.db,COLLECTION);
      inboxUnsub=rt.api.onSnapshot(ref,snap=>{
        const rows=snap.docs.map(d=>({id:d.id,...d.data()}));
        renderAdminInbox(rows);
      },e=>console.warn('[230MATCH] 홈 문의함 실시간 연결 실패',e));
    }catch(e){console.warn('[230MATCH] 홈 문의함 시작 실패',e);}
  }
  function updateFeedbackHome(){
    flushPendingFeedback();
    bindFeedbackNativeLink();
    renderMyReceipt();
    const guide=document.getElementById('stage7125FeedbackGuide');
    if(guide){
      const adminPhone=registrationAdminNotifyPhone();
      guide.textContent=validatePhone(adminPhone)
        ?'문자에는 현재 대회·부서와 로그인 사용자 정보가 자동으로 포함됩니다. 문자앱에서 마지막 전송 버튼만 눌러주세요.'
        :'관리자 문자 수신번호가 설정되면 사용할 수 있습니다.';
    }
    if(canOperate())startAdminInbox();
  }
  // 문의 버튼은 일반 click 라우팅이 아니라 실제 sms: 링크 자체로 동작한다.
  // 동적 렌더 후에도 직접 이벤트가 붙도록 홈 갱신 때 다시 확인한다.
...
(()=>{document.documentElement.dataset.build='6113';console.info('[230MATCH] 63.0.0 ready · root sync rewrite + nonblocking Storage + image-first guide');})();

console.info('[230MATCH] 63.0.0 core · single Firestore listener, lightweight save events, Storage-decoupled editor');

console.info('[230MATCH] 63.0.0 ready · chunked cloud workspace + bounded retry + idle cache');


/* 230MATCH 4.4.1 · tournament date ordering and completed section */
(function stage6301TournamentOrdering(){
  if(!document.getElementById('stage6301TournamentOrderCss')){
    const style=document.createElement('style');style.id='stage6301TournamentOrderCss';style.textContent=`
      #tournamentCardList.stage6301-grouped-list .stage6301-list-heading{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;margin-top:4px;border-radius:14px;background:#f4f7fb;border:1px solid #dfe7f1}
      #tournamentCardList.stage6301-grouped-list .stage6301-list-heading.completed{margin-top:12px;background:#f7f7f8}
      .stage6301-list-heading>div{display:flex;flex-direction:column;gap:3px}.stage6301-list-heading strong{font-size:15px;color:#172b4d}.stage6301-list-heading span{font-size:12px;color:#6b7b91}.stage6301-list-heading em{font-style:normal;font-size:12px;font-weight:800;color:#415a77;background:#fff;border:1px solid #d9e2ec;border-radius:999px;padding:5px 9px}
      @media(max-width:760px){#tournamentCardList.stage6301-grouped-list .stage6301-list-heading{padding:12px}.stage6301-list-heading span{font-size:11px}}
    `;document.head.appendChild(style);
  }
  document.documentElement.dataset.build='6301';
  console.info('[230MATCH] 64.0.0 ready · lazy tournament loading + visible-view rendering + no periodic full-state timer');
})();
