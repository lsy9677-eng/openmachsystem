...
import{listExistingTournaments,loadExistingTournament,convertExistingTournament}from'./legacy-firestore-bridge.js?v=332022';


const BUILD_LABEL='STAGE 32.2.2 · MODERN CUP SUMMARY ARCHIVE & CLEAN START';
...
function legacyArchivePhotos(bundle){
  const out=[];
  (bundle?.notices||[]).forEach(n=>{
    const urls=[...(Array.isArray(n.imageUrls)?n.imageUrls:[]),n.imageUrl].filter(Boolean);
    urls.forEach(url=>{if(!out.some(x=>x.url===url))out.push({url,title:String(n.title||'시합 결과'),body:String(n.body||''),noticeId:n.id||''});});
  });
  return out;
}
function parseLegacyThirds(){return String(document.getElementById('legacyArchiveThirds')?.value||'').split(/[\n,;/]+/).map(v=>v.trim()).filter(Boolean);}
function legacySummaryPayload(){
  if(!legacyBundlePreview)throw new Error('먼저 기존 대회를 선택하고 점검하세요.');
  return {
    id:`legacy-summary-${Date.now()}`,
...
async function archiveLegacySummaryAndReset(){
  ...
  if(!archive.champion||!archive.runnerUp||!archive.thirds.length){notice('우승·준우승·공동 3위 결과를 먼저 입력하세요.','error');return;}
  if(!confirm(`${archive.name}에서 참가팀 ${archive.teamNames.length}팀, 입상 결과, 결과사진 ${archive.resultPhotos.length}장만 보관하고 새 대회를 시작할까요?\n\n잘못 복원된 예선·본선·코트 데이터는 V3에서 초기화됩니다. 기존 Firebase 원본은 삭제하지 않습니다.`))return;
  if(!requireTypedConfirmation('기록 보관 후 새 대회 시작','새시작'))return;
  const recovery=saveRecovery(state,`${archive.name} · 요약 보관 전 전체상태`);try{await recovery.ready;}catch(_e){}
  const next=initialState();
  next.portal=next.portal||{};
  const previous=Array.isArray(state.portal?.resultArchives)?state.portal.resultArchives:[];
  next.portal.resultArchives=[archive,...previous.filter(x=>x.id!==archive.id)];
  next.portal.legacyTournamentSummaries=[archive,...(Array.isArray(state.portal?.legacyTournamentSummaries)?state.portal.legacyTournamentSummaries:[])];
  next.updatedAt=new Date().toISOString();
  state=next;ensurePortalState();ensureOperatorState();ensureContacts(state);commit(`${archive.name} 요약 보관 후 새 대회 시작`);
  disconnectCloudSync();
  notice('모던배의 참가팀·입상 결과·결과사진만 보관하고 새 대회 상태로 초기화했습니다.','success');
  navigatePortalView('home',{pushHistory:true});
}

function importLegacyTournament(){
