...
import{useUnifiedCourts,prelimPriorityActive,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=3438';
...
/* Stage 34.3.8 · prelim-first court priority + reliable auto SMS baseline */
(function(){
  function showPriorityState(){
    const host=document.querySelector('#view-operation .operation-unified-head, #view-operation .section-head');
    if(!host)return;
    let badge=document.getElementById('stage3438PriorityBadge');
    if(!badge){badge=document.createElement('span');badge.id='stage3438PriorityBadge';badge.className='badge';host.appendChild(badge);}
    const active=typeof prelimPriorityActive==='function'&&prelimPriorityActive(state);
    badge.textContent=active?'예선 우선배정 중':'본선 배정 가능';
    badge.className=`badge ${active?'badge-warning':'badge-safe'}`;
  }
  const oldCommit=commit;
  commit=function(message){const result=oldCommit(message);setTimeout(showPriorityState,0);return result;};
  setTimeout(()=>{if(typeof prelimPriorityActive==='function'&&prelimPriorityActive(state)&&useUnifiedCourts(state)){enqueueReadyMainToUnifiedCourts(state);safePersistState('예선 우선배정 자동정리');render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});}showPriorityState();autoSmsSnapshot=buildAutoSmsSnapshot();},0);
  const label=document.getElementById('buildStageLabel');if(label){label.textContent='230MATCH 34.3.8 · 예선 우선배정·자동문자 안정화';label.title='Version 34.3.8';}
  document.documentElement.dataset.build='3438';
  console.info('[230MATCH V3] 34.3.8 ready · prelim priority and auto SMS baseline active');
})();
