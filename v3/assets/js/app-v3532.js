...
import{useUnifiedCourts,prelimPriorityActive,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible,reconcilePrelimCourtReservations}from'./unified-court-engine.js?v=3511';
...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection}from'./sync-engine.js?v=3510';
...
import{startRecordVault}from'./record-vault-engine.js?v=3533';


const BUILD_LABEL='230MATCH 35.3.3 · 향후 대회 기록 자동 보존';
...
function exportPrelimPilotReport(){if(!prelimPilotReport){notice('먼저 예선 실전 점검을 실행하세요.','error');return;}downloadJson(`230MATCH_예선실전점검_${Date.now()}.json`,prelimPilotReport);notice('예선 실전 점검 보고서를 저장했습니다.','success');}
  ...
  if($('resetSmsShortTemplatesBtn'))$('resetSmsShortTemplatesBtn').onclick=()=>{
    setValue('templatePlaying','[{sender}] {team}팀 {court} 경기 시작. 바로 입장 바랍니다.');
    setValue('templateWait1','[{sender}] {team}팀 {court} 대기1. 예상 {wait}분, {start} 시작.');
    setValue('templateShared','[{sender}] {team}팀 본선 공용대기 {queueNo}번. 코트 배정 전까지 대기 바랍니다.');
...
function archiveCurrentTournament(){
  ...
  const payload={...snap,id:crypto.randomUUID(),tournamentId:state.tournament?.id||null,divisionId:state.multiDivision?.activeDivisionId||null,current:false,archivedAt:new Date().toISOString()};
  ...
  commit(`대회 목록 보관 · ${payload.name}`);window.MatchRecordVault?.archiveNow(state,{reason:'관리자 대회 보관'}).catch(error=>console.warn('기록 금고 확정 실패',error));renderTournamentList();notice(same>=0?'기존 대회 기록을 최신 상태로 갱신했습니다.':'현재 대회를 목록에 보관했습니다.','success');
}
...
function printPrelimAssignmentHtml(){
  ...
  const cards=groups.map((g,idx)=>{
    ...
    const court=g.courtName||gm[0]?.courtName||gm[0]?.assignedCourtName||'코트 미정';
...
(function stage3443AdminActionCenter(){
  ...
  function ensureCenter(){
    ...
    byId('aacMin3443').addEventListener('click',()=>{root.classList.toggle('minimized');byId('aacMin3443').textContent=root.classList.contains('minimized')?'+':'−';});
  ...
  const style=document.createElement('style');style.textContent=`
    .admin-action-center-3443{position:fixed;right:18px;bottom:18px;z-index:100000;width:min(420px,calc(100vw - 36px));border:1px solid #b8c9e2;border-radius:16px;background:#fff;box-shadow:0 14px 38px rgba(15,42,79,.22);overflow:hidden}.aac-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 13px;background:#102d57;color:#fff}.aac-head>div:first-child{display:grid;gap:2px}.aac-head small{opacity:.8}.aac-head-actions{display:flex;gap:6px}.aac-head-actions button{width:30px;height:30px;border:0;border-radius:8px;background:rgba(255,255,255,.16);color:#fff;font-size:18px;cursor:pointer}.aac-current{display:grid;grid-template-columns:1fr auto;gap:4px 10px;padding:12px 14px;border-left:5px solid #55759e}.aac-current strong,.aac-current span{grid-column:1}.aac-current small{grid-column:2;grid-row:1/3;color:#68758a}.aac-current.success{border-color:#1f9d62;background:#effbf5}.aac-current.error{border-color:#df3f4f;background:#fff1f2}.aac-current.warning{border-color:#e0a000;background:#fff8df}.aac-current.processing{border-color:#6c45dc;background:#f5f1ff}.aac-history{max-height:210px;overflow:auto;border-top:1px solid #e2e8f2}.aac-log{display:grid;grid-template-columns:70px 110px 1fr;gap:8px;padding:8px 12px;border-bottom:1px solid #edf1f7;font-size:12px}.aac-log time{color:#6d7788}.aac-log.error b{color:#c72f40}.aac-log.success b{color:#168553}.admin-action-center-3443.minimized .aac-current,.admin-action-center-3443.minimized .aac-history{display:none}.stage3443-prelim-lock-panel{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 16px;margin:0 0 14px;border:1px solid #b8c9e2;border-radius:14px;background:#f7faff}.p-lock-copy{display:grid;gap:4px}.p-lock-copy span{color:#5b6c82}.p-lock-actions{display:flex;gap:8px;flex-wrap:wrap}.p-lock-actions .btn{min-height:44px}@media(max-width:700px){.admin-action-center-3443{right:8px;bottom:76px;width:calc(100vw - 16px)}.stage3443-prelim-lock-panel{align-items:stretch;flex-direction:column}.p-lock-actions .btn{flex:1}.aac-log{grid-template-columns:64px 90px 1fr}}
  `;document.head.appendChild(style);
...
/* Stage 35.2.8~35.3.1 retired: Modern Cup remains only as previously stored archive data. */

/* Stage 35.3.2 · print the existing live main bracket exactly as displayed */
(function stage3532UseExistingBracketForPrint(){
  ...
  function activate(){
    ...
    const label=document.getElementById('buildStageLabel');
    if(label){label.textContent='230MATCH 35.3.2 · 기존 본선 대진표 그대로 출력';label.title='Version 35.3.2';}
  }
  ...
  console.info('[230MATCH V3] 35.3.2 ready · existing live bracket is reused for print and PNG');
})();


/* Stage 35.3.3 · future tournament record preservation */
(function stage3533RecordPreservation(){
 const apply=()=>{const label=document.getElementById('buildStageLabel');if(label){label.textContent='230MATCH 35.3.3 · 대회 기록 자동 보존';label.title='Version 35.3.3';}document.querySelectorAll('[id*="restoreModern"],[id*="ModernCup"]').forEach(el=>el.remove());};
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
 window.addEventListener('230match:record-vault-status',event=>{const d=event.detail||{};const badge=document.getElementById('saveStateBadge');if(!badge)return;if(d.state==='error'){badge.textContent='기록 저장 오류';badge.className='badge badge-danger';badge.title=d.error||d.message;}else if(d.state==='saving'){badge.textContent='기록 저장 중';badge.className='badge badge-muted';}else if(d.state==='success'){badge.textContent='기록 보존 정상';badge.className='badge badge-safe';badge.title=d.message||'';}});
 console.info('[230MATCH V3] 35.3.3 ready · automatic future tournament record vault active');
})();
