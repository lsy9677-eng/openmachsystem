...
const BUILD_LABEL='230MATCH 35.2.1 · 문자 설정 단순화·알리고 단문 V3 테스트본';
...
(function stage3523PrintPolish(){
  function stripClubText(value){return String(value||'').replace(/\([^)]*\)/g,'').replace(/\s+/g,' ').trim();}
  function shortNamePart(part,maxLen){const clean=stripClubText(part);return clean.length>maxLen?`${clean.slice(0,maxLen)}…`:clean||'미정';}
  function compactTeam(value,maxLen=3){
    const raw=(typeof printTeam==='function'?printTeam(value):String(value||'미정'))||'미정';
    return raw.split('/').map(part=>shortNamePart(part,maxLen)).join(' / ');
  }
  ...
  function assignmentCardsData(){
    ...
    return groups.map((g,idx)=>{
      ...
      return {
        ...
        orders:gm.map((m,i)=>`${i+1}경기 ${compactTeam(m.teamA,3)} vs ${compactTeam(m.teamB,3)}`)
      };
    });
  }
  ...
  printBracketHtml = window.printBracketHtml = function(){
    ...
    sizes.forEach((size,ri)=>{
      ...
      round.forEach((m,i)=>{
        const y=padY+(i+.5)*step-nodeH/2;positions.set(m.id,{x,y,cx:x+nodeW,cy:y+nodeH/2});
        const a=printEscape(compactTeam(m.teamA,4)),b=printEscape(compactTeam(m.teamB,4));
        const score=m.status==='completed'?`${m.scoreA??''}:${m.scoreB??''}`:(m.status==='playing'?'시합중':m.courtName||m.court||'');
        parts.push(`<g class="bp-match ${m.status||''}"><rect x="${x}" y="${y}" width="${nodeW}" height="${nodeH}" rx="10"/><line x1="${x+10}" y1="${y+nodeH/2}" x2="${x+nodeW-10}" y2="${y+nodeH/2}"/><text x="${x+10}" y="${y+17}">${a}</text><text x="${x+10}" y="${y+38}">${b}</text>${score?`<text class="bp-score" x="${x+nodeW-10}" y="${y+28}" text-anchor="end">${printEscape(score)}</text>`:''}</g>`);
      });
    });
    sizes.forEach((size,ri)=>{if(ri===sizes.length-1)return;(draw.rounds[size]||[]).forEach(m=>{const from=positions.get(m.id),to=positions.get(m.nextMatchId);if(!from||!to)return;const mid=from.cx+colGap/2;parts.push(`<path class="bp-link" d="M ${from.cx} ${from.cy} H ${mid} V ${to.cy} H ${to.x}"/>`);});});
    return printHeader('본선 가지형 대진표')+`<div class="bracket-print-note">본선 전체 가지형 대진표입니다. 팀명은 소속 없이 간단히 표시됩니다.</div><div class="bracket-print-wrap"><svg class="bracket-print-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="본선 전체 가지형 대진표">${parts.join('')}</svg></div>`;
  };
  ...
  style.textContent=`
    ...
    .bp-match rect{fill:#ffffff;stroke:#90a4bf;stroke-width:1.2}
    .bp-match line{stroke:#c3cfdd;stroke-width:1}
    .bp-match text{font-size:9.5px;font-weight:800;fill:#111827}
    .bp-match.completed rect{fill:#ecf9f0;stroke:#2f8a61}
    .bp-match.playing rect{fill:#fff1eb;stroke:#d24d3f}
    .bp-score{font-size:8px!important;fill:#4c617a!important}
    .bp-link{fill:none;stroke:#7f95b3;stroke-width:1.2}
  ...
  `;
  document.head.appendChild(style);
  const run=()=>{interceptSaveButton();const label=document.getElementById('buildStageLabel');if(label){label.textContent='230MATCH 35.2.3 · 출력센터 정리·PNG 저장 개선';label.title='Version 35.2.3';}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(run,0),{once:true});else setTimeout(run,0);
  window.addEventListener('load',()=>setTimeout(run,400));
  console.info('[230MATCH V3] 35.2.3 ready · print center export and layout polish active');
})();
