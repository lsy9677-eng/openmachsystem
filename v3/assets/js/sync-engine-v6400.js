...
const STORAGE_MODE='chunked-workspace-v2-worker';
...
function ensureEncodeWorker(){
  ...
  const code=`
    function hashString(text){let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619);}return(h>>>0).toString(36)}
    function bytesToBase64(bytes){let binary='';const chunk=0x8000;for(let i=0;i<bytes.length;i+=chunk)binary+=String.fromCharCode(...bytes.subarray(i,i+chunk));return btoa(binary)}
    self.onmessage=async e=>{const {id,workspace}=e.data||{};try{const digestObj={...workspace};delete digestObj.updatedAt;const digest=hashString(JSON.stringify(digestObj));const json=JSON.stringify(workspace);let data=json,encoding='json-v1';if(typeof CompressionStream==='function'){const stream=new Blob([json]).stream().pipeThrough(new CompressionStream('gzip'));const bytes=new Uint8Array(await new Response(stream).arrayBuffer());const base64=bytesToBase64(bytes);if(base64.length<json.length){data=base64;encoding='gzip-base64-v1';}}self.postMessage({id,ok:true,data,encoding,digest,originalBytes:new Blob([json]).size,storedBytes:new Blob([data]).size});}catch(error){self.postMessage({id,ok:false,error:error?.message||String(error)});}};`;
...
async function writeCurrentTournament(source){
  ...
  const encoded=await encodeWorkspace(workspace),d=encoded.digest||'';if(d&&d===lastSavedDigest)return true;
...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();const remoteBundle=await readInitialBundle(),requested=String(remoteBundle?.state?.multiTournament?.activeTournamentId||localStorage.getItem('230match-v7-active-tournament')||''),cached=await readCachedState(requested);let chosen=chooseInitial(cached,remoteBundle?.state||null);if(cached&&chosen===cached&&remoteBundle?.state)chosen=mergeRemoteRegistry(chosen,remoteBundle.state);if(chosen){applyState(chosen,cached&&chosen===cached?'cache':'firebase');if(cached&&chosen===cached&&canWriteFn())schedule();}unsubscribeRoom=api.onSnapshot(roomRef(),snap=>{void handleRoomSnapshot(snap);},e=>status('실시간 연결 오류','warning',e?.message||String(e)));status('실시간 연결','success','현재 대회만 즉시 불러오고 다른 대회는 선택할 때 가져오는 지연 로딩 방식입니다.');return true;}
