...
function refreshTimeEngine({save=false}={}){
  ...
  render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1});
}
...
function openManualAssign(venueId,matchId){
  ...
  if(!slots.length){notice('이 구장에는 빈 코트나 빈 대기1 자리가 없습니다.','error');return;}
  $('manualAssignVenueId').value=venueId;$('manualAssignMatchId').value=matchId;
  $('manualAssignMatchLabel').textContent=`${teamText(match.teamA)} vs ${teamText(match.teamB)}`;
  $('manualAssignCourtSelect').innerHTML=slots.map(x=>`<option value="${x.courtId}|${x.slot}">${x.label}</option>`).join('');
...
function bind(){
  ...
  if($('messageStatusFilter'))$('messageStatusFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1});

  if($('rosterSearch'))$('rosterSearch').oninput=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1});
  if($('rosterPhoneFilter'))$('rosterPhoneFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1});
...
syncInputs();syncPrelimInputs();bind();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1});restartTimeTimer();updateClock();setInterval(updateClock,1000);
console.log('[230MATCH V3] stage14 manual-court-assign loaded · no legacy code · no Firebase writes');
