import{getAuthConfig,saveAuthConfig,startAuth,signInGoogle,signOutSocial,beginExternalLogin,getExistingLoginEndpoints}from'./auth-engine.js?v=331651';
import{getAuthRuntime}from'./auth-engine.js?v=331651';
import{notificationSupport,getStoredVapidKey,saveStoredVapidKey,enableMyPush,disableMyPush,queuePush,listPushJobs,listPushTokens}from'./notification-engine.js?v=331651';

import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store.js?v=331651';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=331651';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=331651';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=331651';
import{submitResult}from'./result-engine.js?v=331651';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=331651';
import{downloadJson}from'./recovery.js?v=331651';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=331651';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=331651';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=331651';
import{render,teamText}from'./ui.js?v=331651';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=331651';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=331651';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=331651';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=331651';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=331651';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=331651';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=331651';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=331651';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=331651';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=331651';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=331651';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=331651';
import{verifyAndRepairMainFlow}from'./main-flow-integrity-engine.js?v=331651';
import{finalizeTournamentCompletion}from'./tournament-completion-engine.js?v=331651';


const BUILD_LABEL='STAGE 31.65.1 · SMS CENTER NAVIGATION HOTFIX';
...
function renderRehearsal(report=rehearsalReport){
  ...
  const summary=document.getElementById('rehearsalSummary');const stages=document.getElementById('rehearsalStages');
  if(!report){overall.className='rehearsal-overall';overall.textContent='리허설 대기 중';if(summary)summary.innerHTML='<div class="portal-empty">팀 수를 선택하고 리허설을 실행하세요.</div>';if(stages)stages.innerHTML='';return;}
...
function queueAutoSmsEvent(kind,match,placement){
  const recipients=smsMatchRecipients(match);if(!recipients.length)return;ensureMessagingState(state);if(!Array.isArray(state.messaging.smsApprovalHistory))state.messaging.smsApprovalHistory=[];
  const key=autoSmsEventKey(kind,match.id,placement);if(state.messaging.smsApprovalHistory.some(x=>x.key===key))return;
...
function detectAutoSmsEvents(){
  const current=buildAutoSmsSnapshot();if(!autoSmsSnapshot){autoSmsSnapshot=current;return;}const s=state.messaging?.settings||{};
  if(s.autoSmsApprovalEnabled===true&&canOperate()){
    for(const [id,p] of Object.entries(current.placements)){const before=autoSmsSnapshot.placements[id];const m=findAnyMatchById(id);if(!m)continue;
      if(p.slot==='playing'&&before?.slot!=='playing'&&s.autoSmsMatchStart!==false)queueAutoSmsEvent('start',m,p);
      else if(!before&&p.slot!=='playing'&&s.autoSmsCourtWaiting!==false)queueAutoSmsEvent('waiting',m,p);
      else if(before&&(before.court!==p.court||before.slot!==p.slot||before.position!==p.position)&&p.slot!=='playing'&&s.autoSmsCourtChanged!==false)queueAutoSmsEvent('changed',m,{...p,slotLabel:p.slot==='wait1'?'대기 1번':`대기 ${p.position}번`});
    }
    if(s.autoSmsMatchComplete===true)for(const [id,done] of Object.entries(current.completed)){if(done&&!autoSmsSnapshot.completed[id]){const m=findAnyMatchById(id);if(m)queueAutoSmsEvent('complete',m,current.placements[id]||{});}}
  }
...
function showNextAutoSmsDialog(){
  ...
  document.getElementById('autoSmsApprovalTitle').textContent=({start:'🎾 시합 시작 문자 확인',waiting:'⏳ 코트 대기 문자 확인',changed:'🔄 코트·순서 변경 문자 확인',complete:'✅ 경기 완료 문자 확인'})[item.kind]||'문자 확인';
  document.getElementById('autoSmsApprovalTarget').textContent=`${item.recipients.length}명 · ${item.recipients.map(x=>`${x.name} ${x.phone}`).join(' / ')}`;
  document.getElementById('autoSmsApprovalBody').value=item.body;d.__smsItem=item;d.showModal();
}
...
async function sendAligoSmsV3(recipients,msg,meta={}){
  ...
  const payload={receivers,receiver:receivers[0],recipients:list,targets:list,phones:receivers,to:receivers,msg:body,body,message:body,content:body,type,title:String(meta.title||'230MATCH 문자').slice(0,40),meta:{app:'230MATCH V3',version:'stage31.65',...meta}};
...
async function approveAutoSmsAligo(){const d=document.getElementById('autoSmsApprovalDialog'),item=d?.__smsItem;if(!item)return;const body=document.getElementById('autoSmsApprovalBody').value.trim();try{notice('알리고 문자 발송 중...','info');await sendAligoSmsV3(item.recipients,body,{source:'court_auto_approval',kind:item.kind,matchId:item.matchId,title:'230MATCH 경기 안내'});markAutoSmsHistory(item.key,'sent-aligo');notice(`알리고 문자 ${item.recipients.length}명 발송 완료`,'success');closeAutoSmsDialog('sent-aligo');}catch(e){notice(`알리고 발송 실패: ${e.message||e}`,'error');}}
function approveAutoSmsPhone(){const d=document.getElementById('autoSmsApprovalDialog'),item=d?.__smsItem;if(!item)return;const body=document.getElementById('autoSmsApprovalBody').value.trim(),phones=item.recipients.map(x=>smsDigits(x.phone)).filter(Boolean);if(/Android|iPhone|iPad|iPod/i.test(navigator.userAgent||''))location.href=`sms:${phones.join(',')}?body=${encodeURIComponent(body)}`;else navigator.clipboard?.writeText(`${phones.join('\n')}\n\n${body}`);markAutoSmsHistory(item.key,'opened-phone');notice('문자앱을 열거나 번호와 내용을 복사했습니다.','success');closeAutoSmsDialog('opened-phone');}
async function copyAutoSms(){const d=document.getElementById('autoSmsApprovalDialog'),item=d?.__smsItem;if(!item)return;const body=document.getElementById('autoSmsApprovalBody').value.trim();await navigator.clipboard.writeText(`${item.recipients.map(x=>`${x.name} ${x.phone}`).join('\n')}\n\n${body}`);notice('수신자와 문자 내용을 복사했습니다.','success');}
...
function commit(message){
  if(message)log(message);if(state.settings.autoTimeEnabled)calculateTimeMetrics(state);syncInputs();safePersistState(message||'현재 상태');render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});updateSetupProgress();renderOperatorControls();applyRoleUI();renderPortalViews();flashSaved();setTimeout(detectAutoSmsEvents,0);
}
...
function applySynchronizedState(nextState,source='동기화'){
  ...
  updateSetupProgress();renderOperatorControls();applyRoleUI();renderPortalViews();flashSaved();
...
function confirmPrelimResult(event){
  ...
  const newlyResolvedPlayIns=Object.values(state.draw?.rounds||{}).flat().filter(x=>
    x.isPlayIn&&!beforeResolvedPlayIns.has(x.id)&&x.teamA&&!x.teamA.placeholder&&x.teamB&&!x.teamB.placeholder
  );
  ...
  const autoResult=useUnifiedCourts(state)
    ?enqueueReadyMainToUnifiedCourts(state,{priorityMatchIds:newlyResolvedPlayIns.map(x=>x.id)})
    :autoAssignResolvedMain(state,{findMatch,queueReadyMatches,refillCourt});
  let directAssigned=false;
  if(useUnifiedCourts(state)&&sourceCourtId&&directPlayIn){
    try{
      moveUnifiedCourtMatchFlexible(state,{matchId:directPlayIn.id,targetCourtId:sourceCourtId,mode:'insert-playing'});
      directAssigned=true;
    }catch(error){console.warn('[230MATCH V3] direct play-in placement skipped',error);}
  }
  if((autoResult.assigned===true||Number(autoResult.assigned)>0||directAssigned)&&state.messaging.settings.autoMessageEnabled){
  ...
  prelimNotice(directAssigned
    ?'똥통 예비전이 확정되어 방금 예선이 끝난 코트의 시합중 자리에 최우선 배치했습니다. 기존 카드는 한 단계씩 뒤로 이동했습니다.'
    :autoResult.assigned
      ?'확정된 똥통은 우선 배정하고, 가능한 다른 본선 경기도 빈 코트 없이 계속 배정했습니다.'
      :autoResult.reason==='no-courts'
        ?'본선 팀은 확정됐습니다. 최초 본선 코트배정을 실행하면 운영이 시작됩니다.'
        :'예선 순위와 진출팀을 다시 계산했습니다. 미확정 똥통은 확정되는 즉시 최우선 배정됩니다.','success');
}
...
function bind(){
  ...
  $('exportJsonBtn').onclick=exportFullBackup;
...
function applicationStatusLabel(status){return status==='approved'?'참가 승인':status==='reserve'?'후보 승인':status==='rejected'?'반려':status==='cancelled'?'신청 취소':'승인 대기';}
function applicationStatusClass(status){return ['approved','reserve'].includes(status)?'safe':['rejected','cancelled'].includes(status)?'danger':'pending';}
...
function clearEntryApplicationForm(){entryEditingId=null;['entryApplicationTeam','entryApplicationAffiliation','entryApplicationPhone','entryApplicationMemo'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});renderEntryTournamentSelect();const tournament=document.getElementById('entryApplicationTournament');if(tournament&&[...tournament.options].some(o=>o.value==='current'&&!o.disabled))tournament.value='current';const submit=document.getElementById('entryApplicationSubmitBtn');if(submit)submit.textContent='참가 신청 접수';const cancel=document.getElementById('entryApplicationEditCancelBtn');if(cancel)cancel.hidden=true;}
function submitPublicApplication(){
  ...
  const name=String(document.getElementById('entryApplicationTeam')?.value||'').trim();
  const affiliation=String(document.getElementById('entryApplicationAffiliation')?.value||'').trim();
  const phone=String(document.getElementById('entryApplicationPhone')?.value||'').replace(/[^0-9]/g,'');
  const memo=String(document.getElementById('entryApplicationMemo')?.value||'').trim();
  ...
  if(!name){notice('팀명 또는 선수 이름을 입력하세요.','error');return;}
  if(!validatePhone(phone)){notice('연락처를 01012345678 형식으로 입력하세요.','error');return;}
  if(entryEditingId){
    ...
    const duplicate=state.portal.applications.find(a=>a.id!==item.id&&a.status==='pending'&&a.phone===phone&&myMatchNormalize(a.teamName)===myMatchNormalize(name));
    if(duplicate){notice(`같은 팀의 승인 대기 신청이 이미 있습니다. 신청번호 ${duplicate.code}`,'error');return;}
    Object.assign(item,{tournamentId:selectedTournament.id,tournamentName:selectedTournament.name,tournamentDivision:selectedTournament.division||'',teamName:name,affiliation,phone,memo,updatedAt:new Date().toISOString()});commit(`참가 신청 수정 · ${name}`);clearEntryApplicationForm();lookupPublicApplication();notice('참가 신청을 수정했습니다.','success');return;
  }
  const duplicate=state.portal.applications.find(a=>a.status==='pending'&&a.phone===phone&&myMatchNormalize(a.teamName)===myMatchNormalize(name));
  if(duplicate){notice(`이미 승인 대기 중인 신청입니다. 신청번호 ${duplicate.code}`,'info');renderApplicationPortal();return;}
  const item={id:crypto.randomUUID(),code:applicationCode(),tournamentId:selectedTournament.id,tournamentName:selectedTournament.name,tournamentDivision:selectedTournament.division||'',teamName:name,affiliation,phone,memo,status:'pending',paid:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  state.portal.applications.unshift(item);commit(`참가 신청 접수 · ${name}`);clearEntryApplicationForm();
  const receipt=document.getElementById('entryApplicationReceipt');if(receipt){receipt.hidden=false;receipt.innerHTML=`<strong>신청이 접수되었습니다.</strong><span>신청번호 ${portalEscape(item.code)} · 승인 상태는 연락처로 조회할 수 있습니다.</span>`;}
...
function renderApplicationPortal(){
  ...
  if(admin){
    ...
    admin.innerHTML=rows.map(a=>{const reserveNo=a.status==='reserve'?reserveApplicationOrder(a):0;return `<article class="entry-admin-row"><div class="entry-main"><strong>${portalEscape(a.teamName)}</strong><span>${portalEscape(a.tournamentName||state.tournament?.name||'현재 대회')}${a.tournamentDivision?` (${portalEscape(a.tournamentDivision)})`:''} · ${portalEscape(a.affiliation||'소속 없음')} · ${portalEscape(a.phone)} · ${portalEscape(a.code)}</span><small>${new Date(a.createdAt).toLocaleString('ko-KR')}${a.memo?` · ${portalEscape(a.memo)}`:''}${reserveNo?` · 후보 ${reserveNo}번`:''}</small></div><span class="entry-status ${applicationStatusClass(a.status)}">${applicationStatusLabel(a.status)}</span><span class="entry-payment ${a.paid?'paid':'unpaid'}">${a.paid?'입금 확인':'입금 미확인'}</span><div class="entry-actions">${a.status==='pending'?`<button class="btn btn-primary btn-small" data-entry-action="approve" data-entry-id="${a.id}">참가 승인</button><button class="btn btn-secondary btn-small" data-entry-action="reserve" data-entry-id="${a.id}">후보 승인</button><button class="btn btn-danger-outline btn-small" data-entry-action="reject" data-entry-id="${a.id}">반려</button>`:`<button class="btn btn-light btn-small" data-entry-action="pending" data-entry-id="${a.id}">대기로 되돌림</button>`}<button class="btn btn-light btn-small" data-entry-payment="${a.id}">${a.paid?'입금 취소':'입금 확인'}</button></div></article>`;}).join('')||'<div class="portal-empty">조건에 맞는 참가 신청이 없습니다.</div>';
  }
}
function processEntryApplication(id,action){
  ...
  if(action==='approve'||action==='reserve'){
    ...
    if(!existing){const team=participantNormalizedTeam({id:crypto.randomUUID(),name:item.teamName,affiliation:item.affiliation},state.teams.length);participantReorderByStatus(team,action==='approve'?'active':'reserve');setTeamContact(state,team,{phone:item.phone});}
    item.status=action==='approve'?'approved':'reserve';item.approvedAt=new Date().toISOString();item.updatedAt=item.approvedAt;item.adminMemo=action==='approve'?'참가팀 명단에 등록되었습니다.':'후보팀 명단에 등록되었습니다.';
...
function editEntryApplication(id){const phone=String(document.getElementById('entryLookupPhone')?.value||'').replace(/[^0-9]/g,'');const item=(state.portal?.applications||[]).find(a=>a.id===id);if(!item||item.phone!==phone||item.status!=='pending')return;entryEditingId=item.id;renderEntryTournamentSelect();const tournament=document.getElementById('entryApplicationTournament');if(tournament)tournament.value=item.tournamentId||'current';document.getElementById('entryApplicationTeam').value=item.teamName||'';document.getElementById('entryApplicationAffiliation').value=item.affiliation||'';document.getElementById('entryApplicationPhone').value=item.phone||'';document.getElementById('entryApplicationMemo').value=item.memo||'';document.getElementById('entryApplicationSubmitBtn').textContent='신청 수정 저장';document.getElementById('entryApplicationEditCancelBtn').hidden=false;document.getElementById('entryApplicationTeam').focus();window.scrollTo({top:0,behavior:'smooth'});}
function toggleEntryPayment(id){if(!requireAdmin('참가비 입금 확인'))return;const item=(state.portal?.applications||[]).find(a=>a.id===id);if(!item)return;item.paid=!item.paid;item.paymentUpdatedAt=new Date().toISOString();commit(`참가비 ${item.paid?'입금 확인':'입금 확인 취소'} · ${item.teamName}`);renderApplicationPortal();lookupPublicApplication();}
...
function publicParticipantRows(){
  return (state.teams||[]).flatMap((team,index)=>{
    const status=participantRecordStatus(team,index);const contact=getTeamContact(state,team);
    return participantRecordPlayers(team).map((player,playerIndex)=>({team,index,player,playerIndex,status,contact}));
  });
}
function renderPublicParticipantRecords(){
  ...
  const activeTeams=(state.teams||[]).filter((t,i)=>participantRecordStatus(t,i)==='active').length;
  const reserveTeams=Math.max(0,(state.teams||[]).length-activeTeams);
  ...
  const guide=document.getElementById('publicParticipantGuide');if(guide)guide.textContent=query?`“${query}” 검색 결과 ${visible.length}명입니다.`:`등록된 참가자 ${rows.length}명을 표시합니다.`;
  root.innerHTML=visible.map((row,idx)=>`<article class="public-participant-card"><div class="participant-record-number">${idx+1}</div><div class="participant-record-main"><strong>${portalEscape(row.player)}</strong><span>${portalEscape(row.team?.affiliation||'소속 미등록')}</span><small>${portalEscape(row.team?.name||'')}</small>${canOperate()&&row.contact?.phone?`<small class="participant-record-phone">${portalEscape(row.contact.phone)}</small>`:''}</div><span class="participant-record-state ${row.status}">${row.status==='active'?'참가':'후보'}</span></article>`).join('')||'<div class="portal-empty">조건에 맞는 참가자가 없습니다.</div>';
}
...
function openTournamentGuideEditor(){
  ...
  const guide=state.portal.guide||{};
  const map={guideDateInput:guide.date||'',guideVenueInput:guide.venue||'',guideFeeInput:guide.fee||'',guideBankInput:guide.bank||'',guideAccountInput:guide.account||'',guidePaymentNoteInput:guide.paymentNote||'',guideDetailInput:guide.detail||''};
...
function saveTournamentGuide(){
  ...
  state.portal.guide={date:val('guideDateInput'),venue:val('guideVenueInput'),fee:val('guideFeeInput'),bank:val('guideBankInput'),account:val('guideAccountInput'),paymentNote:val('guidePaymentNoteInput'),detail:val('guideDetailInput')};
...
function bindTournamentList(){
  ...
  document.getElementById('tournamentCardList')?.addEventListener('click',e=>{const open=e.target.closest('[data-tournament-open]');if(open){const item=tournamentArchiveRows().find(x=>x.id===open.dataset.tournamentOpen);if(item)renderTournamentDetail(item);return;}const del=e.target.closest('[data-tournament-delete]');if(del&&requireAdmin('대회 기록 삭제')){if(!confirm('이 대회 기록을 삭제할까요?'))return;state.portal.tournamentArchives=state.portal.tournamentArchives.filter(x=>x.id!==del.dataset.tournamentDelete);commit('대회 목록 기록 삭제');renderTournamentList();}});
...
async function createNewTournamentFromManager(){
  if(!requireAdmin('새 대회 생성'))return;
  const name=String(document.getElementById('newTournamentName')?.value||'').trim();
  const division=String(document.getElementById('newTournamentDivision')?.value||'').trim();
  if(!name){notice('새 대회명을 입력하세요.','error');return;}
  const typed=prompt(`새 대회 “${name}”로 전환합니다. 확인을 위해 새 대회명을 그대로 입력하세요.`,'');
  if(typed!==name){notice('대회명이 일치하지 않아 취소했습니다.','warning');return;}
  ...
  const result=await backup.ready;notice(`새 대회를 생성했습니다.${result?.saved?' 이전 상태는 자동 복구점에 저장했습니다.':''}`,'success');
}
...
function bindTournamentLifecycleManager(){
  ...
  document.getElementById('createNewTournamentBtn')?.addEventListener('click',createNewTournamentFromManager);
...
const PORTAL_VIEW_TITLES={home:'홈', tournaments:'대회 목록', 'prelim-public':'대회 현황', 'my-match':'내 경기', entry:'참가 접수', guide:'대회 요강', board:'공지사항', participants:'참가자 기록', records:'시합 결과', print:'출력 센터', operation:'코트 현황', bracket:'대진표', settings:'대회 관리', roster:'참가자 기록', messages:'문자 센터', audit:'운영 점검', logs:'운영 로그', readiness:'대회 당일 운영 준비', acceptance:'실전 운영 검수', rehearsal:'리허설·시뮬레이션', performance:'대용량 성능 테스트', diagnostics:'오류 기록·진단'};
const INTERNAL_ADMIN_PORTAL_VIEWS=new Set(['acceptance','rehearsal','performance']);
const INTERNAL_OPERATOR_PORTAL_VIEWS=new Set(['messages','audit','logs','readiness','diagnostics']);
function portalViewAllowed(name){
  const view=document.getElementById(`view-${name}`);
  if(!view)return false;
  if(INTERNAL_ADMIN_PORTAL_VIEWS.has(name))return isAdmin();
  if(INTERNAL_OPERATOR_PORTAL_VIEWS.has(name))return isOperator();
  const tab=document.querySelector(`.tab[data-view="${name}"]`);
  if(!tab||tab.hidden||tab.style.display==='none')return false;
  if(tab.hasAttribute('data-admin-only')&&!isAdmin())return false;
  if(tab.hasAttribute('data-operator-only')&&!isOperator())return false;
  return true;
}
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  renderPortalViews();
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='messages')setTimeout(()=>renderPortalViews(),30);if(target==='print')setTimeout(renderPrintPreview,30);if(target==='acceptance')setTimeout(renderAcceptance,30);if(target==='rehearsal')setTimeout(()=>renderRehearsal(),30);if(target==='performance')setTimeout(renderPerformanceCenter,30);if(target==='diagnostics')setTimeout(renderDiagnostics,30);
...
function bindAdminSettingsHub(){
  ...
  document.getElementById('adminSettingsHub')?.addEventListener('click',event=>{
    ...
    if(action){
      const map={social:'openSocialLoginBtn',viewer:'roleViewerBtn',operator:'roleOperatorBtn',admin:'roleAdminBtn',pin:'changeAdminPinBtn','save-recovery':'saveRecoveryBtn','open-recovery':'openRecoveryBtn'};
      closeAdminSettingsHub();
      if(map[action])setTimeout(()=>triggerSettingsSource(map[action]),80);
      return;
    }
    if(view){closeAdminSettingsHub();setTimeout(()=>navigatePortalView(view,{pushHistory:true}),80);}
  });
...
document.documentElement.dataset.build='33164';
console.log('[230MATCH V3] stage31.64 mobile usability final polish loaded');
...
(function initFinalDeploymentStability(){
  ...
  setTimeout(()=>{
    ...
    setNetworkBadge();
    saveLastKnownGood();
    if(ok)console.info('[230MATCH V3] final deployment self-check passed');
  },0);
})();
