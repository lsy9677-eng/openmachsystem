...
const BUILD_LABEL='STAGE 32.2.4 · MODERN CUP QUARTERFINAL & RESULT PHOTO MIGRATION';
...
function parseLegacyQuarterfinals(){
  const values=String(document.getElementById('legacyArchiveQuarterfinals')?.value||'').split(/[\n,;/]+/).map(v=>v.trim()).filter(Boolean);
...
function legacySummaryPayload(){
  ...
  return {
    ...
    quarterfinals:parseLegacyQuarterfinals(),
...
async function archiveLegacySummaryAndReset(){
  ...
  if(!archive.champion||!archive.runnerUp||archive.thirds.length<2){notice('우승·준우승·공동 3위 2팀을 먼저 입력하세요.','error');return;}
  if(archive.quarterfinals.length!==8){notice(`8강 진출팀은 정확히 8팀을 입력해야 합니다. 현재 ${archive.quarterfinals.length}팀입니다.`,'error');return;}
  if(!archive.resultPhotos.length){notice('보관할 결과사진을 1장 이상 선택하세요.','error');return;}
  if(!confirm(`${archive.name}에서 참가팀 ${archive.teamNames.length}팀, 8강 ${archive.quarterfinals.length}팀, 입상 결과, 결과사진 ${archive.resultPhotos.length}장만 보관하고 새 대회를 시작할까요?\n\n잘못 복원된 예선·본선·코트 데이터는 V3에서 초기화됩니다. 기존 Firebase 원본은 삭제하지 않습니다.`))return;
  ...
  disconnectCloudSync();
  notice('모던배의 참가팀·8강·입상 결과·선택한 결과사진을 보관하고 새 대회 상태로 초기화했습니다.','success');
