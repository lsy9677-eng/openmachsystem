...
function ensureDivisionSettingsModal(){
  ...
  modal.id='divisionSettingsModal';modal.className='modal';modal.hidden=true;
  modal.innerHTML=`<div class="modal-card division-settings-card"><div class="modal-head"><div><small>DIVISION SETTINGS</small><h2>부서별 운영 설정</h2><p>참가 정원·예선·본선·구장을 이 부서만 별도로 설정합니다.</p></div><button type="button" class="modal-close" data-division-settings-close>×</button></div><form id="divisionSettingsForm"><input type="hidden" id="divisionSettingsId"><div class="division-settings-grid"><label>부서명<input id="divisionSettingsName" required></label><label>참가 정원<input id="divisionSettingsCapacity" type="number" min="1" required></label><label>본선 규모<select id="divisionSettingsDrawSize"><option>32</option><option>64</option><option>128</option></select></label><label>조당 본선 진출팀<input id="divisionSettingsQualifiers" type="number" min="1" max="3"></label><label>2팀조 수<input id="divisionSettingsTwoGroups" type="number" min="0"></label><label>경기시간(분)<input id="divisionSettingsMinutes" type="number" min="10"></label></div><label class="division-venue-label">구장 설정 <small>한 줄에 구장명|코트수|예선|본선 형식으로 입력합니다.</small><textarea id="divisionSettingsVenues" rows="6" placeholder="국제|8|예선|본선\n원도심|4|예선|본선"></textarea></label><div class="division-settings-actions"><button type="button" class="btn btn-light" data-division-settings-close>취소</button><button type="submit" class="btn btn-primary">이 부서 설정 저장</button></div><p id="divisionSettingsMessage" class="division-settings-message"></p></form></div>`;
  document.body.appendChild(modal);
  modal.addEventListener('click',e=>{if(e.target===modal||e.target.closest('[data-division-settings-close]'))modal.hidden=true;});
...
function openDivisionSettings(id){
  ...
  modal.querySelector('#divisionSettingsTwoGroups').value=Number(ps.twoTeamGroups||0);
  modal.querySelector('#divisionSettingsMinutes').value=Number(settings.matchMinutes||40);
  modal.querySelector('#divisionSettingsVenues').value=venueLinesFromSnapshot(record);
  const msg=modal.querySelector('#divisionSettingsMessage');if(msg)msg.textContent='';
  modal.hidden=false;
}
function saveDivisionSettingsFromModal(e){
  e.preventDefault();
  const id=document.getElementById('divisionSettingsId')?.value;
  const record=state.multiDivision?.divisions?.find(d=>d.id===id);if(!record)return;
  const name=String(document.getElementById('divisionSettingsName')?.value||'').trim();
  const capacity=Math.max(1,Number(document.getElementById('divisionSettingsCapacity')?.value||96));
  const drawSize=Math.max(2,Number(document.getElementById('divisionSettingsDrawSize')?.value||64));
  const qualifiers=Math.max(1,Number(document.getElementById('divisionSettingsQualifiers')?.value||2));
  const twoGroups=Math.max(0,Number(document.getElementById('divisionSettingsTwoGroups')?.value||0));
  const matchMinutes=Math.max(10,Number(document.getElementById('divisionSettingsMinutes')?.value||40));
  const venues=parseDivisionVenueLines(document.getElementById('divisionSettingsVenues')?.value||'');
  const msg=document.getElementById('divisionSettingsMessage');
  if(!name){if(msg)msg.textContent='부서명을 입력하세요.';return;}
  if(state.multiDivision.divisions.some(d=>d.id!==id&&d.name===name)){if(msg)msg.textContent='같은 이름의 부서가 이미 있습니다.';return;}
  if(!venues.length){if(msg)msg.textContent='구장을 한 곳 이상 입력하세요.';return;}
  if(!venues.some(v=>v.usePrelim)){if(msg)msg.textContent='예선 사용 구장을 한 곳 이상 지정하세요.';return;}
  if(!venues.some(v=>v.useMain)){if(msg)msg.textContent='본선 사용 구장을 한 곳 이상 지정하세요.';return;}
  const snap=divisionSnapshotSettings(record);
  record.name=name;record.updatedAt=new Date().toISOString();
  snap.divisionConfig={...(snap.divisionConfig||{}),capacity};
  snap.settings={...(snap.settings||{}),drawSize,matchMinutes,venues};
  snap.prelim=snap.prelim||{};snap.prelim.settings={...(snap.prelim.settings||{}),activeTeamCount:capacity,qualifiersPerGroup:qualifiers,twoTeamGroups:twoGroups};
  ...
  if(id===state.multiDivision.activeDivisionId){
    state.tournament.division=name;state.tournament.capacity=capacity;
    state.settings={...(state.settings||{}),drawSize,matchMinutes,venues:divisionClone(venues)};
    state.prelim.settings={...(state.prelim?.settings||{}),activeTeamCount:capacity,qualifiersPerGroup:qualifiers,twoTeamGroups:twoGroups};
    ensureVenueSettings(state);ensureVenueQueues(state);
  }
  saveState(state);renderDivisionManagerList();renderDivisionWorkspaceBar();
  document.getElementById('divisionSettingsModal').hidden=true;
...
(function(){
  ...
  async function createTournamentFromWizard3433(){
    ...
    try{
      ...
      for(const draft of wizardDivisionDrafts){
        ...
        snap.settings={...(snap.settings||{}),drawSize:draft.drawSize,matchMinutes:draft.matchMinutes,venues:divisionClone(parseDivisionVenueLines(draft.venues))};
        record.snapshot=snap;
      }
  ...
  const label=document.getElementById('buildStageLabel');if(label){label.textContent='230MATCH 34.3.4 · 다부서 실행 중단 오류 수정본';label.title='Version 34.3.4';}
  document.documentElement.dataset.build='3434';
  console.info('[230MATCH V3] 34.3.4 ready · multi-division UI active');
})();
