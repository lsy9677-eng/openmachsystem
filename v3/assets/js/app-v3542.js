...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection}from'./sync-engine.js?v=3554';
...
const BUILD_LABEL='230MATCH 35.5.5 · 일반 사용자 메뉴 정리·내 경기 자동 표시';
...
function applyRoleUI(){
  ...
  document.querySelectorAll('[data-portal-go="operation"],[data-view="operation"],[data-settings-view="operation"]').forEach(el=>{el.hidden=!canOperate();});
  if(!canOperate()&&document.body?.dataset.currentView==='operation')navigatePortalView('my-match',{replaceHistory:true,focus:false});
}
...
function tournamentArchiveRows(){const modern=(state.portal?.archives||[]).map(archiveListItem);const legacy=(state.portal?.tournamentArchives||[]).filter(x=>!modern.some(m=>m.id===x.id));return [currentTournamentSnapshot(),...modern,...legacy];}
...
function portalViewAllowed(name){
  if(name==='operation'&&!canOperate())return false;
...
(function stage3530ModernCupExactRestore(){
  ...
  function patchProviders(){
    ...
    const oldArchived=archivedParticipantRows;
