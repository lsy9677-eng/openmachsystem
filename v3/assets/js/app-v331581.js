import{getAuthConfig,saveAuthConfig,startAuth,signInGoogle,signOutSocial,beginExternalLogin,getExistingLoginEndpoints}from'./auth-engine.js?v=3315722';
import{getAuthRuntime}from'./auth-engine.js?v=3315722';
import{notificationSupport,getStoredVapidKey,saveStoredVapidKey,enableMyPush,disableMyPush,queuePush,listPushJobs,listPushTokens}from'./notification-engine.js?v=3315722';

import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store.js?v=3315722';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=3315722';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=3315722';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=3315722';
import{submitResult}from'./result-engine.js?v=3315722';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=3315722';
import{downloadJson}from'./recovery.js?v=3315722';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=3315722';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=3315722';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=3315722';
import{render,teamText}from'./ui.js?v=3315722';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=3315722';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=3315722';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=3315722';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=3315722';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=3315722';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=3315722';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=3315722';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=3315722';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=3315722';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=3315722';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=3315722';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=3315722';
import{verifyAndRepairMainFlow}from'./main-flow-integrity-engine.js?v=3315722';
import{finalizeTournamentCompletion}from'./tournament-completion-engine.js?v=3315722';


const BUILD_LABEL='STAGE 31.58.1 · ENTRY TOURNAMENT SELECTOR';
...
const PORTAL_VIEW_TITLES={home:'홈', tournaments:'대회 목록', 'prelim-public':'대회 현황', 'my-match':'내 경기', entry:'참가 접수', guide:'대회 요강', board:'공지사항', participants:'참가자 기록', records:'시합 결과', print:'출력 센터', operation:'코트 현황', bracket:'대진표', settings:'대회 관리', roster:'참가자 기록', messages:'문자 센터', audit:'운영 점검', logs:'운영 로그', readiness:'대회 당일 운영 준비'};
...
syncInputs();syncPrelimInputs();bind();bindPortal();bindPrintCenter();bindParticipantManager();bindEntryApplications();bindPublicParticipantRecords();bindResultArchive();bindTournamentLifecycleManager();bindBackupRecoveryManager();bindNotificationCenter();bindTournamentReadiness();window.addEventListener('popstate',()=>navigatePortalView(location.hash.replace(/^#/, '')||'home',{focus:false}));initialPortalView();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});renderOperatorControls();updateSetupProgress();applyRoleUI();renderPortalViews();restartTimeTimer();updateClock();setInterval(updateClock,1000);installUnifiedMoveControlGuard();ensureUnifiedCourtMoveControls();
...
document.documentElement.dataset.build='3315722';
console.log('[230MATCH V3] stage31.57.2.3 settings hub and role login hotfix loaded');
