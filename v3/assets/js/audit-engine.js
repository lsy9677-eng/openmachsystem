...
export function runFullSimulation(state){
  ...
  // 예선 슬롯이 남아 있으면 복제 예선을 먼저 완주하고 순위를 본선 슬롯에 반영한다.
  ...
  if(unresolvedBefore>0){
    ...
    prelimSimulation={
      ...
      rankedGroups:pr.rankedGroups,totalGroups:pr.sim.prelim.groups.length,reason:pr.reason||null
    };
    sim=pr.sim;
    if(!pr.success){
      return{
        at:new Date().toISOString(),success:false,iterations:0,totalMatches:0,completedMatches:0,
        autoByeCount:0,winner:null,queueDuplicates:[],prelimSimulation,
        unresolvedSlots:unresolvedBefore,reason:pr.reason||'PRELIM_SIMULATION_FAILED',unfinished:[]
      };
    }
  ...
  if(unresolvedAfter>0){
    return{
      at:new Date().toISOString(),success:false,iterations:0,totalMatches:0,completedMatches:0,
      autoByeCount:0,winner:null,queueDuplicates:[],prelimSimulation,
      unresolvedSlots:unresolvedAfter,reason:'UNRESOLVED_LINKED_SLOTS',
      unfinished:allMatches(sim.draw).filter(m=>m.teamA?.placeholder||m.teamB?.placeholder)
        .map(m=>({id:m.id,status:'예선 결과 대기',teamA:m.teamA?.name||null,teamB:m.teamB?.name||null}))
    };
  }

  // 복제 모의대회는 코트 큐가 아니라 대진 연결 그래프 자체를 검증한다.
  // 큰 라운드부터 결승까지 순서대로 처리하면서 BYE는 자동 진출,
  // 양 팀이 있는 경기는 실제 모의경기로 계산한다.
  const matchesByRound=Object.keys(sim.draw.rounds||{}).map(Number).sort((a,b)=>b-a);
  let played=0,byes=0,iterations=0;
  const propagate=(match,winner)=>{
    match.winner=winner;match.status='completed';match.completedAt=new Date().toISOString();
    if(match.nextMatchId){
      const next=findMatch(sim.draw,match.nextMatchId);
      if(next){
        if(match.nextSlot===1)next.teamA=clone(winner);else next.teamB=clone(winner);
        if(next.teamA&&!next.teamA.placeholder&&next.teamB&&!next.teamB.placeholder)next.status='ready';
      }
    }
  };

  for(const roundSize of matchesByRound){
    const roundMatches=sim.draw.rounds[roundSize]||[];
    for(const match of roundMatches){
      iterations++;
      if(match.status==='completed'&&match.winner){
        // 기존 완료 결과도 다음 라운드 연결을 다시 확인한다.
        propagate(match,match.winner);
        continue;
      }
      const a=match.teamA&&!match.teamA.placeholder?match.teamA:null;
      const b=match.teamB&&!match.teamB.placeholder?match.teamB:null;
      if(a&&b){
        match.bye=false;played++;propagate(match,a);
      }else if(a||b){
        match.bye=true;byes++;propagate(match,a||b);
      }else{
        // 앞 라운드 처리 후에도 양쪽이 모두 비어 있으면 실제 경기가 아닌 구조상 빈 슬롯이다.
        // 참가팀 수보다 큰 대진표에서 발생하는 빈 가지를 실패로 계산하지 않는다.
        match.status='void';match.winner=null;match.bye=false;match.void=true;
      }
    }
  }

  const matches=allMatches(sim.draw);
  const unfinished=matches.filter(m=>m.status!=='void'&&(m.status!=='completed'||!m.winner));
  const final=matches.find(m=>Number(m.round)===1)||matches.find(m=>!m.nextMatchId)||null;
  const queueDupes=[];
  const success=unfinished.length===0&&Boolean(final?.winner);
  return{
    at:new Date().toISOString(),success,iterations,
    totalMatches:played,completedMatches:played,
    autoByeCount:byes,winner:final?.winner||null,
    queueDuplicates:queueDupes,prelimSimulation,unresolvedSlots:0,
