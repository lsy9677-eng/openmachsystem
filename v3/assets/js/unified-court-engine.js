...
export function enqueueReadyMainToUnifiedCourts(state,{priorityMatchIds=[]}={}){
  ...
  state.sharedQueue=[];
  return{assigned,prioritizedPlayIns:ready.filter(m=>m.isPlayIn).length,pendingPlayIns:incompletePlayIns.filter(m=>!ready.some(r=>r.id===m.id)).length,repair,reason:assigned?'assigned':ready.length?'no-active-courts':'no-ready'};
}
