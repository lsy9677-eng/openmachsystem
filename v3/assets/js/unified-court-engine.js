...
function pushMainBackToShared(state,court,id,{front=true}={}){
  if(!hasAuthorizedMainDraw(state))return false;
  if(!id)return false;
...
function enforcePrelimBeforeMain(state,court){
  court.queue=Array.isArray(court.queue)?court.queue:[];
  let corrected=0;
  const playingItem=court.playing?findUnifiedMatch(state,court.playing):null;
  const wait1Item=court.wait1?findUnifiedMatch(state,court.wait1):null;

  // 절대 규칙: 시합중이 본선이고 대기1이 예선이면 즉시 두 자리를 바로잡습니다.
  // 예선 결과 저장 직후 비동기 자동배정이 먼저 실행되더라도 이 규칙이 최종 방어선입니다.
  if(playingItem?.type==='main'&&wait1Item?.type==='prelim'&&wait1Item.match.status!=='completed'){
    const mainId=court.playing;
    const prelimId=court.wait1;
    court.playing=prelimId;
    court.wait1=null;
    setUnifiedStatus(state,prelimId,'playing',court);
    pushMainBackToShared(state,court,mainId,{front:true});
    corrected++;
  }

  // 시합중 본선 앞에 실행 가능한 예선 예약카드가 추가대기에 있으면 본선을 되돌리고 예선을 승격합니다.
  const currentPlaying=court.playing?findUnifiedMatch(state,court.playing):null;
  if(currentPlaying?.type==='main'){
...
function promoteLocalCourt(state,court){
  ...
  enforcePrelimBeforeMain(state,court);
...
export function advanceUnifiedCourt(state,courtId,completedId){
  ...
  // 결과 저장 직후에는 다른 자동배정 로직보다 먼저 기존 대기1 예선을 원자적으로 시합중으로 올립니다.
  // 이 단계가 끝나기 전에는 본선 공용대기 카드를 시합중에 넣을 수 없습니다.
  if(!court.playing&&court.wait1&&isPendingPrelimId(state,court.wait1)){
...
export function moveUnifiedCourtMatchFlexible(state,{matchId,targetCourtId,mode='auto'}){
  ...
  court.queue=Array.isArray(court.queue)?court.queue:[];
  if(item.type==='main'&&mode==='insert-playing'&&courtHasReservedPrelimAhead(state,court)){
    throw new Error('이 코트에는 먼저 진행할 예선 경기가 남아 있어 본선을 시합중으로 올릴 수 없습니다.');
  }
