...
function rebalanceUnifiedMainSlots(state,venueId=null){
  ...
  // 코트별 예선 예약열이 비어 있는 코트만 본선을 받습니다.
  // 다른 코트에 예선이 남아 있어도 해당 코트의 예선 일정이 끝났다면 본선 진행이 가능합니다.
  const mainEligible=courts.filter(c=>!courtHasPendingPrelim(state,c));

  // 1순위: 예선 일정이 끝난 빈 코트의 시합중 자리를 먼저 채웁니다.
  mainEligible.filter(c=>!c.playing).forEach(empty=>{
    const donor=mainEligible.find(c=>c.id!==empty.id&&c.wait1&&findUnifiedMatch(state,c.wait1)?.type==='main');
    if(donor){
  ...
  // 2순위: 해당 코트에 예선 예약 카드가 전혀 없을 때만 본선 대기1을 채웁니다.
  mainEligible.filter(c=>c.playing&&!c.wait1).forEach(c=>{c.wait1=takeSharedMain(state,c);});
  return courts;
}
