...
export async function loadExistingTournament(tid){
  ...
  const [teams,matches,draws,registrations,notices]=await Promise.all(['teams','matches','draws','registrations','noticePosts'].map(n=>queryByTournament(rt.db,n,tid)));
  return{tournament,teams,matches,draws,registrations,notices,loadedAt:new Date().toISOString(),sourceProject:'open-match-manager'};
}
