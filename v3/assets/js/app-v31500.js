...
import{transferTargets,transferCourtMatch}from'./court-transfer-engine.js';

let state=loadState();ensurePrelimState(state);ensureTimeState(state);ensureDrawMeta(state);ensureMessagingState(state);ensureContacts(state);ensureAuditState(state);ensureVenueSettings(state);ensureVenueQueues(state);
...
function refreshTimeEngine({save=false}={}){
  ...
  render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer});
}
...
function openCourtTransfer(sourceCourtId,sourceSlot){
  ...
  const matchId=sourceSlot==='playing'?source?.playing:source?.wait1;
  const match=findMatch(state.draw,matchId);
  if(!source||!match){notice('이동할 경기를 찾지 못했습니다.','error');return;}
  const targets=transferTargets(state,{sourceCourtId,sourceSlot});
  if(!targets.length){notice('같은 구장에 이동할 다른 자리가 없습니다.','error');return;}
  $('courtTransferSourceCourtId').value=sourceCourtId;
  $('courtTransferSourceSlot').value=sourceSlot;
  $('courtTransferMatchLabel').textContent=`${source.name} ${sourceSlot==='playing'?'시합중':'대기1'} · ${teamText(match.teamA)} vs ${teamText(match.teamB)}`;
  $('courtTransferTargetSelect').innerHTML=targets.map(x=>`<option value="${x.courtId}|${x.slot}">${x.label}</option>`).join('');
  $('courtTransferInfo').className='notice info';
  $('courtTransferInfo').textContent='빈 자리로는 이동하고, 경기 있는 자리는 교환 옵션이 켜져 있을 때 서로 교환합니다.';
  $('courtTransferDialog').showModal();
}
function confirmCourtTransfer(event){
  event.preventDefault();
  const [targetCourtId,targetSlot]=$('courtTransferTargetSelect').value.split('|');
  const result=transferCourtMatch(state,{
    sourceCourtId:$('courtTransferSourceCourtId').value,
    sourceSlot:$('courtTransferSourceSlot').value,
    targetCourtId,targetSlot,
    allowSwap:$('courtTransferAllowSwap').checked
  },id=>findMatch(state.draw,id));
  calculateTimeMetrics(state);
  commit(result.swapped?'코트 경기 교환':'코트 경기 이동');
  $('courtTransferDialog').close();
  notice(result.swapped?'두 경기의 코트 자리를 교환했습니다.':'경기를 다른 코트 자리로 이동했습니다.','success');
}
...
function bind(){
  ...
  if($('messageStatusFilter'))$('messageStatusFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer});

  if($('rosterSearch'))$('rosterSearch').oninput=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer});
  if($('rosterPhoneFilter'))$('rosterPhoneFilter').onchange=()=>render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer});
...
syncInputs();syncPrelimInputs();bind();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer});restartTimeTimer();updateClock();setInterval(updateClock,1000);
console.log('[230MATCH V3] stage15 court-transfer loaded · no legacy code · no Firebase writes');
