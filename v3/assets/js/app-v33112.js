...
function commit(message){
  if(message)log(message);if(state.settings.autoTimeEnabled)calculateTimeMetrics(state);syncInputs();saveState(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus});updateSetupProgress();flashSaved();
}
function syncInputs(){
...
function assign(){
  ...
  if(useUnifiedCourts(state)){
    ...
    if(!result.assigned){
      commit(`확정 본선 코트배정 확인 · 신규 0경기 · 운영중/대기1 ${active}경기 · 공용대기 ${queued}경기`);
      notice(result.reason==='play-in-gate'
        ?`똥통 예비전이 남아 있어 64강 배정을 보류했습니다. 현재 코트·대기1 ${active}경기, 공용대기 ${queued}경기입니다.`
        :`새로 배정할 경기가 없습니다. 확정 경기는 이미 자동 배정되어 있습니다. 현재 코트·대기1 ${active}경기, 공용대기 ${queued}경기입니다.`,'success');
      return;
    }
    commit(`예선·본선 통합 코트배정 · 신규 본선 ${result.assigned}경기`);
    notice(`확정 본선 ${result.assigned}경기를 배정했습니다.${result.playInOnly?' 똥통 예비전 완료 전까지 64강은 대기합니다.':''}`,'success');
    return;
  }
...
syncInputs();syncPrelimInputs();bind();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus});updateSetupProgress();restartTimeTimer();updateClock();setInterval(updateClock,1000);
const BUILD_LABEL='STAGE 31.12 · AUTO REFLECTION STATUS · PLAY-IN FIRST';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33111';
console.log('[230MATCH V3] stage31.12 auto-reflection-status-play-in-first loaded · linked draw strict · no legacy code · no Firebase writes');
