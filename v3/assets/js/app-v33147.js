...
import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store.js?v=33147';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=33147';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=33147';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=33147';
import{submitResult}from'./result-engine.js?v=33147';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=33147';
import{downloadJson}from'./recovery.js?v=33147';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=33147';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=33147';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=33147';
import{render,teamText}from'./ui.js?v=33147';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=33147';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=33147';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=33147';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=33147';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=33147';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=33147';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=33147';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=33147';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=33147';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=33147';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=33147';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=33147';
import{verifyAndRepairMainFlow}from'./main-flow-integrity-engine.js?v=33147';
import{finalizeTournamentCompletion}from'./tournament-completion-engine.js?v=33147';

let state=loadState();
function ensurePortalState(){
  if(!state.portal||typeof state.portal!=='object')state.portal={};
  if(!Array.isArray(state.portal.posts))state.portal.posts=[{id:crypto.randomUUID(),title:'230MATCH 대회 안내',body:'대회 일정과 경기 진행 상황은 홈 화면과 경기 현황에서 확인해 주세요.',pinned:true,createdAt:new Date().toISOString()}];
...
function commit(message){
  if(message)log(message);if(state.settings.autoTimeEnabled)calculateTimeMetrics(state);syncInputs();safePersistState(message||'현재 상태');render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});updateSetupProgress();renderOperatorControls();applyRoleUI();renderPortalViews();flashSaved();
}
...
function submitPublicApplication(){
  ...
  if(entryEditingId){
    ...
    Object.assign(item,{teamName:name,affiliation,phone,memo,updatedAt:new Date().toISOString()});commit(`참가 신청 수정 · ${name}`);clearEntryApplicationForm();lookupPublicApplication();notice('참가 신청을 수정했습니다.','success');return;
  }
  ...
  const item={id:crypto.randomUUID(),code:applicationCode(),teamName:name,affiliation,phone,memo,status:'pending',paid:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
...
function renderPortalViews(){
  ...
  const posts=[...state.portal.posts].sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)||String(b.createdAt).localeCompare(String(a.createdAt)));
  const home=document.getElementById('homeNoticeList');if(home)home.innerHTML=posts.slice(0,4).map(p=>`<article class="portal-list-item"><strong>${p.pinned?'📌 ':''}${portalEscape(p.title)}</strong><div class="portal-meta">${new Date(p.createdAt).toLocaleDateString('ko-KR')}</div></article>`).join('')||'<div class="portal-empty">등록된 공지가 없습니다.</div>';
  const board=document.getElementById('boardPostList');if(board)board.innerHTML=posts.map(p=>`<article class="portal-board-item"><div class="portal-meta">${p.pinned?'상단 고정 · ':''}${new Date(p.createdAt).toLocaleString('ko-KR')}</div><h3>${portalEscape(p.title)}</h3><div class="portal-board-body">${portalEscape(p.body)}</div>${isAdmin()?`<div class="portal-board-actions"><button type="button" class="btn btn-danger-outline" data-board-delete="${p.id}">삭제</button></div>`:''}</article>`).join('')||'<div class="portal-empty">등록된 게시물이 없습니다.</div>';
  ...
  const champion=state.operation?.champion;const current=document.getElementById('currentResultSummary');if(current)current.innerHTML=`<strong>${portalEscape(state.tournament?.name||'현재 대회')}</strong><div class="portal-meta">${portalEscape(state.tournament?.division||'')}</div><div class="portal-result-metrics"><div>예선 완료<br><b>${pDone}/${prelim.length}</b></div><div>본선 완료<br><b>${mDone}/${main.length}</b></div><div>우승팀<br><b>${portalEscape(champion?portalTeam(champion):'진행 중')}</b></div><div>최종 갱신<br><b>${state.updatedAt?new Date(state.updatedAt).toLocaleString('ko-KR'):'-'}</b></div></div>`;
  const archives=document.getElementById('resultArchiveList');if(archives)archives.innerHTML=[...state.portal.resultArchives].sort((a,b)=>String(b.archivedAt).localeCompare(String(a.archivedAt))).map(r=>`<article class="portal-record-item"><strong>${portalEscape(r.name)}</strong><div class="portal-meta">${portalEscape(r.division||'')} · ${new Date(r.archivedAt).toLocaleDateString('ko-KR')}</div><div>우승: <b>${portalEscape(r.champion||'미확정')}</b></div><div>예선 ${r.prelimCompleted}/${r.prelimTotal} · 본선 ${r.mainCompleted}/${r.mainTotal}</div></article>`).join('')||'<div class="portal-empty">보관된 대회 결과가 없습니다.</div>';
}
function saveBoardPost(){if(!requireAdmin('게시판 공지 저장'))return;const title=String(document.getElementById('boardPostTitle')?.value||'').trim();const body=String(document.getElementById('boardPostBody')?.value||'').trim();if(!title||!body){notice('제목과 내용을 입력하세요.','error');return;}state.portal.posts.unshift({id:crypto.randomUUID(),title,body,pinned:Boolean(document.getElementById('boardPostPinned')?.checked),createdAt:new Date().toISOString()});document.getElementById('boardPostForm').hidden=true;commit(`게시판 공지 등록 · ${title}`);}
function archiveCurrentResult(){if(!requireAdmin('대회 결과 보관'))return;const prelim=state.prelim?.matches||[],main=portalMainMatches();const item={id:crypto.randomUUID(),name:state.tournament?.name||'대회',division:state.tournament?.division||'',champion:state.operation?.champion?portalTeam(state.operation.champion):'',prelimCompleted:prelim.filter(x=>x.status==='completed').length,prelimTotal:prelim.length,mainCompleted:main.filter(x=>x.status==='completed').length,mainTotal:main.length,archivedAt:new Date().toISOString()};state.portal.resultArchives.unshift(item);commit(`대회 결과 기록 보관 · ${item.name}`);notice('현재 대회 결과를 기록에 보관했습니다.','success');}
const PUBLIC_PORTAL_VIEWS=new Set(['home','prelim-public','my-match','entry','board','participants','records','operation','bracket']);
const PORTAL_VIEW_TITLES={home:'홈', 'prelim-public':'대회 현황', 'my-match':'내 경기', entry:'참가 접수', board:'공지사항', participants:'참가자 기록', records:'시합 결과', operation:'코트 현황', bracket:'대진표', settings:'대회 관리', roster:'참가자 기록', messages:'문자 센터', audit:'운영 점검', logs:'운영 로그'};
...
function bindPortal(){
  ...
  document.getElementById('newBoardPostBtn')?.addEventListener('click',()=>{document.getElementById('boardPostForm').hidden=false;});
  document.getElementById('cancelBoardPostBtn')?.addEventListener('click',()=>{document.getElementById('boardPostForm').hidden=true;});
  ...
  document.addEventListener('click',e=>{const choice=e.target.closest?.('[data-my-match-index]');if(choice){const teams=document.getElementById('myMatchTeamChoices')?._teams||[];const team=teams[Number(choice.dataset.myMatchIndex)];if(team)renderMyMatchTeam(team);return;}const btn=e.target.closest?.('[data-board-delete]');if(!btn||!isAdmin())return;if(!confirm('이 게시물을 삭제할까요?'))return;state.portal.posts=state.portal.posts.filter(p=>p.id!==btn.dataset.boardDelete);commit('게시판 공지 삭제');});
}
...
syncInputs();syncPrelimInputs();bind();bindPortal();bindParticipantManager();bindEntryApplications();bindPublicParticipantRecords();window.addEventListener('popstate',()=>navigatePortalView(location.hash.replace(/^#/, '')||'home',{focus:false}));initialPortalView();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});renderOperatorControls();updateSetupProgress();applyRoleUI();renderPortalViews();restartTimeTimer();updateClock();setInterval(updateClock,1000);installUnifiedMoveControlGuard();ensureUnifiedCourtMoveControls();
...
const BUILD_LABEL='STAGE 31.47 · PARTICIPANT RECORDS & SEARCH';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33146';
console.log('[230MATCH V3] stage31.46 legacy mobile shell and bottom navigation loaded · viewer default · IndexedDB recovery enabled');
