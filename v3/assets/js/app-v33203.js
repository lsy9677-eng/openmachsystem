...
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=332012';
...
const BUILD_LABEL='STAGE 32.3 · NEW TOURNAMENT CREATION & INITIAL SETUP WIZARD';
...
function publicParticipantRows(){
  const current=(state.teams||[]).flatMap((team,index)=>{
    const status=participantRecordStatus(team,index);const contact=getTeamContact(state,team);
    return participantRecordPlayers(team).map((player,playerIndex)=>({team,index,player,playerIndex,status,contact,archiveName:''}));
  });
  if(current.length)return current;
  const archived=[...(state.portal?.participantArchives||[])].sort((a,b)=>String(b.archivedAt||'').localeCompare(String(a.archivedAt||'')))[0];
  if(!archived||!Array.isArray(archived.teamNames))return [];
  return archived.teamNames.flatMap((name,index)=>{
    const team={id:`archived-${index}`,name:String(name||''),affiliation:archived.name||'보관 대회'};
    return participantRecordPlayers(team).map((player,playerIndex)=>({team,index,player,playerIndex,status:'active',contact:null,archiveName:archived.name||''}));
  });
}
...
function openNewTournamentWizard(){if(!requireAdmin('새 대회 생성'))return;const modal=wizardEl('newTournamentWizard');if(!modal)return;newTournamentWizardStep=1;wizardEl('wizardTournamentName').value='';wizardEl('wizardTournamentDivision').value=state.tournament?.division||'';wizardEl('wizardTournamentDate').value='';wizardEl('wizardTournamentVenue').value=state.portal?.guide?.venue||'';wizardEl('wizardTournamentCapacity').value=state.prelim?.settings?.activeTeamCount||96;wizardEl('wizardCourtCount').value=state.settings?.courtCount||8;wizardEl('wizardCourtPrefix').value=state.settings?.courtPrefix||'국제';wizardEl('wizardConfirmChecked').checked=false;modal.hidden=false;modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';renderNewTournamentWizard();setTimeout(()=>wizardEl('wizardTournamentName')?.focus(),50);}
...
async function createTournamentFromWizard(){if(!wizardEl('wizardConfirmChecked')?.checked){setWizardMessage('최종 확인 항목에 체크하세요.','error');return;}const x=wizardEstimate();const map={newTournamentName:'wizardTournamentName',newTournamentDivision:'wizardTournamentDivision',newTournamentDate:'wizardTournamentDate',newTournamentVenue:'wizardTournamentVenue',newTournamentCapacity:'wizardTournamentCapacity',newTournamentTemplate:'wizardTemplate'};Object.entries(map).forEach(([dst,src])=>{const d=wizardEl(dst),s=wizardEl(src);if(d&&s)d.value=s.value;});if(wizardEl('copyTournamentGuide'))wizardEl('copyTournamentGuide').checked=wizardEl('wizardCopyGuide').checked;if(wizardEl('copyTournamentPosts'))wizardEl('copyTournamentPosts').checked=wizardEl('wizardCopyPosts').checked;if(wizardEl('copyTournamentTeams'))wizardEl('copyTournamentTeams').checked=wizardEl('wizardCopyTeams').checked;setWizardMessage('현재 상태를 복구점에 저장하고 새 대회를 생성하고 있습니다.');try{const ok=await createNewTournamentFromManager({skipPrompt:true,uploadCloud:wizardEl('wizardCloudUpload')?.checked});if(!ok)return;state.settings.drawSize=x.draw;state.settings.courtCount=Math.max(1,Number(wizardEl('wizardCourtCount')?.value||8));state.settings.courtPrefix=String(wizardEl('wizardCourtPrefix')?.value||'국제').trim();state.prelim.settings.qualifiersPerGroup=Number(wizardEl('wizardQualifiers')?.value||2);state.prelim.settings.twoTeamGroupCount=x.two;state.portal.guide.startTime=wizardEl('wizardTournamentStartTime')?.value||'09:00';saveState(state);if(wizardEl('wizardCloudUpload')?.checked){try{await pushStateNow(state);}catch(_e){}}setWizardMessage('새 대회 생성이 완료되었습니다.','success');setTimeout(()=>{closeNewTournamentWizard();navigatePortalView('tournaments',{pushHistory:true});},500);}catch(error){setWizardMessage(`생성 실패: ${error?.message||error}`,'error');}}
...
function bindTournamentLifecycleManager(){
  ...
  document.getElementById('createNewTournamentBtn')?.addEventListener('click',openNewTournamentWizard);
...
document.documentElement.dataset.build='332015';
console.log('[230MATCH V3] stage32.1.5 naver login token return hotfix loaded');
...
console.info('[230MATCH V3] stage32.1.6 loaded · app-v332016.js · mobile settings access ready');
