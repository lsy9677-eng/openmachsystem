...
import{useUnifiedCourts,prelimPriorityActive,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible,reconcilePrelimCourtReservations}from'./unified-court-engine.js?v=3503';
...
function confirmPrelimResult(event){
  ...
  // 본선 경기는 예선 결과 직후 해당 코트의 시합중 자리에 직접 삽입하지 않습니다.
  // 예선 대기1을 먼저 시합중으로 올린 뒤, 남는 자리만 통합 코트 엔진이 본선에 배정합니다.
  const directAssigned=false;
...
(function stage3500RebuiltMainLifecycle(){
  ...
  const label=$('buildStageLabel');if(label){label.textContent='230MATCH 35.0.3 · 슬롯 추첨 코트배정 연결 복구';label.title='Version 35.0.3';}
  document.documentElement.dataset.build='3500';
  console.info('[230MATCH V3] 35.0.3 ready · slot draw authorization persistence fixed');
})();
