...
const BUILD_LABEL='STAGE 32.4.1 · REGISTRATION ACTION BUTTON HOTFIX';
...
function toggleEntryPayment(id){
  if(!requireAdmin('참가비 입금 확인'))return;
  const item=(state.portal?.applications||[]).find(a=>a.id===id);
  if(!item)return;
  if(item.paid){
    if(!confirm(`${item.teamName}의 입금 확인을 취소할까요?`))return;
    item.paid=false;
    item.paymentStatus='unpaid';
    item.paymentCancelledAt=new Date().toISOString();
    item.paidAt='';
  }else{
    const when=prompt('입금 확인 시간을 입력하세요. 비워두면 현재 시간으로 저장됩니다.','');
    if(when===null)return;
    item.paid=true;
    item.paymentStatus='paid';
    const parsed=when.trim()?new Date(when):new Date();
    item.paidAt=Number.isNaN(parsed.getTime())?new Date().toISOString():parsed.toISOString();
    item.paymentAmount=state.portal?.guide?.fee||'';
  }
  item.paymentUpdatedAt=new Date().toISOString();
  commit(`참가비 ${item.paid?'입금 확인':'입금 확인 취소'} · ${item.teamName}`);
  renderApplicationPortal();
  lookupPublicApplication();
  if(item.paid)openEntrySmsDialog('payment',item);
}
