...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow}from'./sync-engine-v5002.js?v=5002';
...
(function stage3569RuntimeAndCourtRouteFix(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
  console.info('[230MATCH] 50.0.1 ready · runtime errors removed and viewer court route fixed');
})();
...
tournamentArchiveRows=function(){
  ...
  const active=state.multiTournament.tournaments.map(r=>tournamentSummaryFromWorkspace(r.id===state.multiTournament.activeTournamentId?state:r.snapshot,r.id,r.id===state.multiTournament.activeTournamentId)).filter(Boolean);
