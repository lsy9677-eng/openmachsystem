...
const BUILD_LABEL='STAGE 31.66 · SMS DELIVERY ACCEPTANCE & STABILITY';
...
async function sendAligoSmsV3(recipients,msg,meta={}){
  ...
  const payload={receivers,receiver:receivers[0],recipients:list,targets:list,phones:receivers,to:receivers,msg:body,body,message:body,content:body,type,title:String(meta.title||'230MATCH 문자').slice(0,40),meta:{app:'230MATCH V3',version:'stage31.66',...meta}};
...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  renderPortalViews();
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='messages')setTimeout(()=>{renderPortalViews();renderSmsAcceptance(lastSmsAcceptance);},30);if(target==='print')setTimeout(renderPrintPreview,30);if(target==='acceptance')setTimeout(renderAcceptance,30);if(target==='rehearsal')setTimeout(()=>renderRehearsal(),30);if(target==='performance')setTimeout(renderPerformanceCenter,30);if(target==='diagnostics')setTimeout(renderDiagnostics,30);
