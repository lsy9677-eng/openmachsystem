...
const STORAGE_MODE='chunked-workspace-v3-public-revision';
...
let dirtyGeneration=0,lastSavedDigest='',lastSavedPublicDigest='',lastAppliedDigest='',lastKnownRoomRevision=0,lastKnownPublicRevision=0,lastWriterUid='';
...
async function writeCurrentTournament(source){
  ...
  const roomUpdate={schemaVersion:8,roomId:ROOM_ID,activeTournamentId:id,revision:api.increment(1),lastWriterUid:rt.user.uid,lastWriterEmail:rt.user.email||'',serverUpdatedAt:api.serverTimestamp()};
  if(publicChanged)roomUpdate.publicRevision=api.increment(1);
  batch.set(roomRef(),roomUpdate,{merge:true});
  await batch.commit();knownChunkIds.set(id,newIds);lastSavedDigest=d;if(publicChanged){lastSavedPublicDigest=publicDigest;lastKnownPublicRevision++;}lastWriterUid=rt.user.uid;localStorage.setItem('230match-v7-active-tournament',id);scheduleCache();return true;
}
...
async function handleRoomSnapshot(snap){
  ...
  if(writer&&rt?.user?.uid&&writer===rt.user.uid&&!viewer)return;
  if(!targetId)return;
  try{
