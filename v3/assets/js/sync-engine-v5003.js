...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();if(canWriteFn()){unsubscribe=api.onSnapshot(manifestRef(),async snap=>{if(!snap.exists()){status('실시간 연결','success','새 저장구조에 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});return;}const m=snap.data();lastManifestRevision=Math.max(lastManifestRevision,Number(m.revision||0));const bundle=await readBundle();if(bundle)acceptRemote(bundle,'firebase');else status('자동 저장 정상','success','등록된 운영 대회가 없습니다. 새 대회를 만들어 시작하세요.',{roomId:DEFAULT_ROOM,schemaVersion:6});},e=>status('동기화 오류','error',e?.message||String(e)));status('실시간 연결','success','대회별 분리 저장 구조로 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});}else{await fetchRemote();startPolling();status('저부하 조회','success','대회별 문서를 45초 간격으로 조회합니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});}return true;}
