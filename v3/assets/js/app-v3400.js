...
const BUILD_LABEL='STAGE 33.3 · LOSER SCORE QUICK ENTRY';
...
function stage340ResetPanel(prefix){
  ...
  panel.dataset.loserSide='';panel.querySelectorAll('[data-loser-side]').forEach(b=>b.classList.remove('is-selected'));
...
function stage340BindScorePanels(){
  document.querySelectorAll('[data-score-panel]').forEach(panel=>{if(panel.dataset.bound==='1')return;panel.dataset.bound='1';const prefix=panel.dataset.scorePanel==='prelim'?'prelim':'';
    panel.querySelectorAll('[data-loser-side]').forEach(btn=>btn.addEventListener('click',()=>{panel.dataset.loserSide=btn.dataset.loserSide;panel.querySelectorAll('[data-loser-side]').forEach(x=>x.classList.toggle('is-selected',x===btn));}));
    panel.querySelectorAll('[data-score-value]').forEach(btn=>btn.addEventListener('click',()=>{const side=panel.dataset.loserSide;if(!side){notice('먼저 진 팀을 선택하세요.','error');return;}const input=$(prefix==='prelim'?(side==='A'?'prelimScoreA':'prelimScoreB'):(side==='A'?'scoreA':'scoreB'));input.value=btn.dataset.scoreValue;input.dispatchEvent(new Event('input',{bubbles:true}));stage340UpdatePreview(prefix);}));
...
function openPrelimResult(matchId){
  ...
  setTimeout(()=>$('prelimScoreA')?.focus(),0);
}
...
(function initFinalDeploymentStability(){
  ...
  setTimeout(()=>{
    ...
    setNetworkBadge();
    saveLastKnownGood();
    if(ok)console.info('[230MATCH V3] stage32.15 mobile field-operation readiness loaded');
  },0);
})();
...
(function stage3215FieldReadiness(){
  ...
  const auditRoutes=()=>{
    ...
    else console.info('[230MATCH V3] Stage32.15 route integrity PASS');
  };
...
(function stage330ProductionRelease(){
  ...
  function finalize(){
    ...
    try{
      ...
      else console.info('[230MATCH V3] Stage33.0 production release ready · route integrity PASS');
...
(function stage332Release(){
  const ready=()=>{
    ...
    window.__230MATCH_RELEASE__=Object.freeze({version:'34.0',channel:'production',label:'230MATCH V3 통합 현장 운영 안정본'});
    console.info('[230MATCH V3] Stage33.2 operation mistake prevention ready');
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ready,{once:true});else ready();
})();
