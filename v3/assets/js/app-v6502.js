...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow,loadTournamentNow}from'./sync-engine-v6502.js?v=6502';
...
document.documentElement.dataset.build='6502';
console.log('[230MATCH] 65.0.2 ready · self-contained member-lite core + public records/download support');
...
async function switchTournamentWorkspace(id){
  ...
  try{if(isAdmin()||isOperator())await pushStateNow(state);}catch(error){notice(`현재 대회 저장 대기: ${error?.message||error}`,'warning');return false;}
