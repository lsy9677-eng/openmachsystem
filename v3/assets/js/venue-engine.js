...
export function ensureVenueSettings(state){
  ...
  state.settings.venues=state.settings.venues.map((v,i)=>({
    ...
    courtMax:Math.max(1,Number(v.courtMax||v.totalCourts||v.courtCount)||1),
    courtNumbers:[...new Set((Array.isArray(v.courtNumbers)&&v.courtNumbers.length?v.courtNumbers:Array.from({length:Math.max(1,Number(v.courtCount)||1)},(_,n)=>n+1)).map(Number).filter(n=>Number.isInteger(n)&&n>=1&&n<=Math.max(1,Number(v.courtMax||v.totalCourts||v.courtCount)||1)))].sort((a,b)=>a-b),
    courtCount:0,
    courtPrefix:String(v.courtPrefix||v.name||`코트`).trim()||'코트',
    usePrelim:v.usePrelim!==false,
    useMain:v.useMain!==false
  })).map(v=>({...v,courtNumbers:v.courtNumbers.length?v.courtNumbers:[1],courtCount:v.courtNumbers.length||1}));
...
export function buildVenueCourts(venues){
  ...
  venues.forEach(venue=>{
    const numbers=Array.isArray(venue.courtNumbers)&&venue.courtNumbers.length?venue.courtNumbers:Array.from({length:venue.courtCount},(_,i)=>i+1);
    numbers.forEach(i=>{
      courts.push({
        id:`${venue.id}-court-${i}`,
