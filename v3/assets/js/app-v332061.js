...
const BUILD_LABEL='STAGE 32.6.1 · SETTINGS ENTRY & TEST APPLICATION HOTFIX';
    ...
    if(view){
      ...
      if(view==='entry-admin'){
        setTimeout(()=>{
          ...
          setOperationSection('setup');
          setTimeout(()=>{
            const section=document.querySelector('.entry-admin-manager');
            if(section){
              section.scrollIntoView({behavior:'smooth',block:'start'});
              section.classList.add('settings-target-flash');
              setTimeout(()=>section.classList.remove('settings-target-flash'),1800);
...
console.info('[230MATCH V3] stage32.6 loaded · main draw, SMS & multi-device pilot ready');
