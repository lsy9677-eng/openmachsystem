...
let state=loadState();ensurePrelimState(state);ensureTimeState(state);ensureDrawMeta(state);ensureMessagingState(state);ensureContacts(state);
...
console.log('[230MATCH V3] stage9.1 message-metrics-migration-fix loaded · no legacy code · no Firebase writes');
