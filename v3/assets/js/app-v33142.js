...
import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store.js?v=33142';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=33142';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=33142';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=33142';
import{submitResult}from'./result-engine.js?v=33142';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=33142';
import{downloadJson}from'./recovery.js?v=33142';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=33142';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=33142';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=33142';
import{render,teamText}from'./ui.js?v=33142';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=33142';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=33142';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=33142';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=33142';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=33142';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=33142';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=33142';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=33142';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=33142';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=33142';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=33142';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=33142';
import{verifyAndRepairMainFlow}from'./main-flow-integrity-engine.js?v=33142';
import{finalizeTournamentCompletion}from'./tournament-completion-engine.js?v=33142';
...
function bind(){
  ...
  document.querySelectorAll('.tab').forEach(tab=>tab.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.view').forEach(x=>x.classList.remove('active'));tab.classList.add('active');$(`view-${tab.dataset.view}`)?.classList.add('active');renderPortalViews();});
...
function submitPublicApplication(){
  ...
  const item={id:crypto.randomUUID(),code:applicationCode(),teamName:name,affiliation,phone,memo,status:'pending',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  state.portal.applications.unshift(item);commit(`참가 신청 접수 · ${name}`);
  ['entryApplicationTeam','entryApplicationAffiliation','entryApplicationPhone','entryApplicationMemo'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
...
function lookupPublicApplication(){
  ...
  root.innerHTML=rows.map(a=>`<article class="entry-status-card"><div><strong>${portalEscape(a.teamName)}</strong><span>${portalEscape(a.affiliation||'소속 없음')} · ${new Date(a.createdAt).toLocaleString('ko-KR')}</span></div><span class="entry-status ${applicationStatusClass(a.status)}">${applicationStatusLabel(a.status)}</span><div class="entry-code">${portalEscape(a.code)}</div>${a.adminMemo?`<p>${portalEscape(a.adminMemo)}</p>`:''}${a.status==='pending'?`<button type="button" class="btn btn-danger-outline btn-small" data-entry-cancel="${a.id}">신청 취소</button>`:''}</article>`).join('')||'<div class="portal-empty">해당 연락처의 참가 신청이 없습니다.</div>';
}
...
function processEntryApplication(id,action){
  ...
  if(action==='approve'||action==='reserve'){
    ...
    if(!existing){
      const team=participantNormalizedTeam({id:crypto.randomUUID(),name:item.teamName,affiliation:item.affiliation},state.teams.length);
      participantReorderByStatus(team,action==='approve'?'active':'reserve');setTeamContact(state,team,{phone:item.phone});
    }
    item.status=action==='approve'?'approved':'reserve';item.updatedAt=new Date().toISOString();item.adminMemo=action==='approve'?'참가팀 명단에 등록되었습니다.':'후보팀 명단에 등록되었습니다.';
  }else if(action==='reject'){
    const reason=prompt('반려 사유를 입력하세요.','참가 요건 또는 정원 확인이 필요합니다.');if(reason===null)return;item.status='rejected';item.adminMemo=reason.trim();item.updatedAt=new Date().toISOString();
  }else if(action==='pending'){item.status='pending';item.adminMemo='';item.updatedAt=new Date().toISOString();}
  commit(`참가 신청 처리 · ${item.teamName} · ${applicationStatusLabel(item.status)}`);renderApplicationPortal();renderParticipantManager();
}
function cancelEntryApplication(id){
  ...
  if(!confirm(`${item.teamName} 참가 신청을 취소할까요?`))return;item.status='cancelled';item.updatedAt=new Date().toISOString();commit(`참가 신청 취소 · ${item.teamName}`);lookupPublicApplication();
}
function bindEntryApplications(){
  ...
  document.addEventListener('click',e=>{const action=e.target.closest?.('[data-entry-action]');if(action){processEntryApplication(action.dataset.entryId,action.dataset.entryAction);return;}const cancel=e.target.closest?.('[data-entry-cancel]');if(cancel)cancelEntryApplication(cancel.dataset.entryCancel);});
}
...
function bindPortal(){
  document.querySelectorAll('[data-portal-go]').forEach(btn=>btn.addEventListener('click',()=>document.querySelector(`.tab[data-view="${btn.dataset.portalGo}"]`)?.click()));
...
syncInputs();syncPrelimInputs();bind();bindPortal();bindParticipantManager();bindEntryApplications();renderVenueSettingsEditor();calculateTimeMetrics(state);render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});renderOperatorControls();updateSetupProgress();applyRoleUI();renderPortalViews();restartTimeTimer();updateClock();setInterval(updateClock,1000);installUnifiedMoveControlGuard();ensureUnifiedCourtMoveControls();
...
const BUILD_LABEL='STAGE 31.42 · PUBLIC ENTRY APPLICATION & APPROVAL';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33142';
console.log('[230MATCH V3] stage31.42 public entry application and approval loaded · viewer default · IndexedDB recovery enabled');
