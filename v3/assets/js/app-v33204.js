...
const BUILD_LABEL='STAGE 32.4 · REGISTRATION PAYMENT & PROMOTION SMS';
...
function processEntryApplication(id,action){
  ...
  if(action==='approve'||action==='reserve'||action==='promote'){
    ...
    if(!existing){const team=participantNormalizedTeam({id:crypto.randomUUID(),name:item.teamName,affiliation:item.affiliation},state.teams.length);participantReorderByStatus(team,targetStatus==='approved'?'active':'reserve');setTeamContact(state,team,{phone:item.phone});}
...
function toggleEntryPayment(id){if(!requireAdmin('참가비 입금 확인'))return;const item=(state.portal?.applications||[]).find(a=>a.id===id);if(!item)return;if(item.paid){if(!confirm(`${item.teamName}의 입금 확인을 취소할까요?`))return;item.paid=false;item.paymentStatus='unpaid';item.paymentCancelledAt=new Date().toISOString();item.paidAt='';}else{const when=prompt('입금 확인 시간을 입력하세요. 비워두면 현재 시간으로 저장됩니다.','');if(when===null)return;item.paid=true;item.paymentStatus='paid';item.paidAt=when.trim()?new Date(when).toISOString():new Date().toISOString();item.paymentAmount=state.portal?.guide?.fee||'';}item.paymentUpdatedAt=new Date().toISOString();commit(`참가비 ${item.paid?'입금 확인':'입금 확인 취소'} · ${item.teamName}`);renderApplicationPortal();lookupPublicApplication();if(item.paid)openEntrySmsDialog('payment',item);} · ${item.teamName}`);renderApplicationPortal();lookupPublicApplication();}

function refundEntryPayment(id){if(!requireAdmin('참가비 환불 처리'))return;const item=(state.portal?.applications||[]).find(a=>a.id===id);if(!item||!item.paid)return;if(!confirm(`${item.teamName} 참가비 환불 완료로 처리할까요?`))return;item.paid=false;item.paymentStatus='refunded';item.refundedAt=new Date().toISOString();item.paymentUpdatedAt=item.refundedAt;commit(`참가비 환불 완료 · ${item.teamName}`);renderApplicationPortal();lookupPublicApplication();openEntrySmsDialog('refund',item);}
function manualEntrySms(id){const item=(state.portal?.applications||[]).find(a=>a.id===id);if(!item)return;const kinds=['approve','reserve','payment','promote','reject','refund'];const selected=prompt('문자 종류를 입력하세요.\napprove=참가승인, reserve=후보, payment=입금완료, promote=승격, reject=반려, refund=환불','payment');if(!selected||!kinds.includes(selected.trim()))return;openEntrySmsDialog(selected.trim(),item);}
