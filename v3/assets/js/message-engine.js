...
export function ensureMessagingState(state){
  ...
  state.messaging.settings={
    ...
    ...previous,
    templates:{
      playing:'[{sender}] {team}팀 {court} 경기 시작. 바로 입장 바랍니다.',
      wait1:'[{sender}] {team}팀 {court} 대기1. 예상 {wait}분, {start} 시작.',
      shared:'[{sender}] {team}팀 본선 공용대기 {queueNo}번. 코트 배정 전까지 대기 바랍니다.',
      ...(previous.templates||{})
    }
  };
  const legacy={playing:'[{sender}] {team}님, 현재 {court} 코트 경기입니다. 상대팀: {opponent}. 즉시 코트로 이동해 주세요.',wait1:'[{sender}] {team}님, {court} 코트 대기 1번입니다. 상대팀: {opponent}. 예상 대기 {wait}분, 예상 시작 {start}.',shared:'[{sender}] {team}님, 본선 공용대기 {queueNo}번입니다. 상대팀: {opponent}. 코트 배정 전까지 대기해 주세요.'};
  const short={playing:'[{sender}] {team}팀 {court} 경기 시작. 바로 입장 바랍니다.',wait1:'[{sender}] {team}팀 {court} 대기1. 예상 {wait}분, {start} 시작.',shared:'[{sender}] {team}팀 본선 공용대기 {queueNo}번. 코트 배정 전까지 대기 바랍니다.'};
  for(const key of Object.keys(short)){
    if(!state.messaging.settings.templates[key]||state.messaging.settings.templates[key]===legacy[key])state.messaging.settings.templates[key]=short[key];
  }
}
