...
import{loadState,saveState,clearState,saveRecovery,getRecoveries,deleteRecovery,initialState}from'./store.js?v=33122';
import{prepareTeams,generateDraw,allMatches,findMatch,generateLinkedDrawSlots,syncLinkedDrawQualifiers}from'./bracket-engine.js?v=33122';
import{ensureDrawMeta,canModifyDraw,createDrawWithMethod,lockDraw,unlockDrawForDevelopment,clearDrawHistory}from'./draw-method-engine.js?v=33122';
import{buildCourts,assignInitial,queueReadyMatches,refillCourt}from'./court-engine.js?v=33122';
import{submitResult}from'./result-engine.js?v=33122';
import{ensurePrelimState,generatePrelim,assignPrelimCourts,findPrelimMatch,submitPrelimResult,resetPrelim,autoFitPrelimGroups,swapActiveReserveTeam,isPrelimLocked,lockPrelim,unlockPrelim}from'./prelim-engine.js?v=33122';
import{downloadJson}from'./recovery.js?v=33122';
import{ensureTimeState,calculateTimeMetrics}from'./time-engine.js?v=33122';
import{ensureMessagingState,generatePlayingMessages,generateWait1Messages,generateCurrentCourtMessages,generateCurrentWaitMessages,generateAllTimeMessages,markMessageSent,deleteMessage,clearSentMessages,markAllSent,smsUri,refreshMessageContacts,mergePendingDuplicates,getMessageHistory}from'./message-engine.js?v=33122';
import{ensureContacts,getTeamContact,setTeamContact,validatePhone,exportContactData,importContactData}from'./contact-engine.js?v=33122';
import{render,teamText}from'./ui.js?v=33122';
import{ensureAuditState,runStateAudit,runPrelimSimulation,runFullSimulation,applyAuditResult}from'./audit-engine.js?v=33122';
import{earlyMainStats,markResolvedMainMatchesReady,canAssignEarlyMain,ensureEarlyMainSettings,autoAssignResolvedMain}from'./early-main-engine.js?v=33122';
import{useUnifiedCourts,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible}from'./unified-court-engine.js?v=33122';
import{shouldUseLinkedDraw,linkedDrawNeedsRepair,rebuildLinkedDraw,hasStartedMainMatches}from'./linked-draw-guard-engine.js?v=33122';
import{ensureVenueSettings,ensureVenueQueues,venuePreset,buildVenueCourts,prelimVenues,mainVenues}from'./venue-engine.js?v=33122';
import{moveQueueItem,reorderQueueItem}from'./queue-control-engine.js?v=33122';
import{availableCourtSlots,assignQueueMatchToCourt,returnWait1ToVenueQueue}from'./manual-court-engine.js?v=33122';

import{ensureCourtStatuses,pauseCourt,resumeCourt}from'./court-status-engine.js?v=33122';
import{ensureCourtManualQueues,assignToCourtManualQueue,moveCourtMatchFlexible,returnManualQueueItemToVenue,reorderCourtManualQueue}from'./court-manual-queue-engine.js?v=33122';
import{reorderPrelimQueue as reorderPrelimQueueItem,movePrelimQueuedMatch,returnPrelimWait1ToQueue}from'./prelim-queue-control-engine.js?v=33122';
import{ensurePrelimCourtStatuses,pausePrelimCourt,resumePrelimCourt}from'./prelim-court-status-engine.js?v=33122';
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow}from'./sync-engine.js?v=33122';
...
function ensureUnifiedCourtMoveControls(){
  ...
  cards.forEach((card,index)=>{
    ...
    if(court.playing&&playing&&!playing.querySelector('[data-force-unified-transfer="playing"]')){
      ...
      const btn=document.createElement('button');btn.type='button';btn.className='btn btn-light';btn.textContent='이동·관리';
    ...
    queueItems.forEach((item,qIndex)=>{
      if(!item.querySelector('[data-force-unified-transfer^="queue:"]')){
        const btn=document.createElement('button');btn.type='button';btn.className='btn btn-light prelim-extra-move-btn force-visible-move';btn.textContent='이동·관리';
        btn.dataset.forceUnifiedTransfer=`queue:${qIndex}`;btn.onclick=()=>openUnifiedCourtTransfer(court.id,`queue:${qIndex}`);item.appendChild(btn);
      }
    });
  });
}
...
const BUILD_LABEL='STAGE 31.22 · FORCED UNIFIED CARD MOVE CONTROLS';
const buildStageLabel=document.getElementById('buildStageLabel');
if(buildStageLabel)buildStageLabel.textContent=BUILD_LABEL;
document.documentElement.dataset.build='33122';
console.log('[230MATCH V3] stage31.22 forced-unified-card-move-controls loaded · local fallback enabled');
