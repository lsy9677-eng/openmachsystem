...
import{useUnifiedCourts,prelimPriorityActive,enqueueReadyMainToUnifiedCourts,advanceUnifiedCourt,reconcileUnifiedMainQueues,findUnifiedMatch,moveUnifiedCourtMatchFlexible,reconcilePrelimCourtReservations}from'./unified-court-engine.js?v=3502';
...
/* Stage 35.0.2 · strict per-court prelim reservation priority + operator queue layout */
(function stage3502StrictPrelimPriority(){
  let repairing=false;
  function violationCount(){
    return (state.prelim?.courts||[]).reduce((n,c)=>{
      const pending=[c.wait1,...(c.queue||[])].filter(Boolean).some(id=>{
        const x=findUnifiedMatch(state,id);return x?.type==='prelim'&&x.match?.status!=='completed';
      });
      const mainPlaying=c.playing&&findUnifiedMatch(state,c.playing)?.type==='main';
      return n+(pending&&mainPlaying?1:0);
    },0);
  }
  function repairNow(source='자동 점검'){
    if(repairing||!useUnifiedCourts(state))return;
    const before=violationCount();
    if(!before)return;
    repairing=true;
    try{
      const result=reconcilePrelimCourtReservations(state);
      if(result.returnedMain||result.added){
        saveState(state);
        render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});
        renderOperatorControls();
        window.stage3443ActionCenterPush?.('error','예선 우선순위 자동 복구',`${source}: 예선 예약 경기보다 앞선 본선 ${result.returnedMain||before}경기를 공용대기로 되돌렸습니다.`);
      }
    }catch(err){
      window.stage3443ActionCenterPush?.('error','예선 우선순위 복구 오류',String(err?.message||err));
    }finally{repairing=false;}
  }
  document.addEventListener('click',e=>{
    if(e.target.closest('[data-prelim-result],[data-main-result],[data-result],[data-op-queue-up],[data-op-queue-down]'))setTimeout(()=>repairNow('경기 처리 후 점검'),80);
  },true);
  setInterval(()=>repairNow('실시간 점검'),1200);
  window.stage3502RepairPrelimPriority=repairNow;
  console.info('[230MATCH V3] 35.0.2 ready · strict court prelim reservation priority active');
})();
