...
(function stage3523PrintPolish(){
  ...
  printBracketHtml = window.printBracketHtml = function(){
    ...
    sizes.forEach((size,ri)=>{
      ...
      round.forEach((m,i)=>{
        ...
        const score=m.status==='completed'?`${m.scoreA??''}:${m.scoreB??''}`:(m.status==='playing'?'시합중':m.courtName||m.court||'대진 대기');
        parts.push(`<g class="bp-match ${m.status||''}"><rect x="${x}" y="${y}" width="${nodeW}" height="${nodeH}" rx="10"/><text class="bp-match-no" x="${x+12}" y="${y+19}">${matchLabel}</text><text class="bp-match-state" x="${x+12}" y="${y+38}">${printEscape(score)}</text></g>`);
      });
    });
    sizes.forEach((size,ri)=>{if(ri===sizes.length-1)return;(draw.rounds[size]||[]).forEach(m=>{const from=positions.get(m.id),to=positions.get(m.nextMatchId);if(!from||!to)return;const mid=from.cx+colGap/2;parts.push(`<path class="bp-link" d="M ${from.cx} ${from.cy} H ${mid} V ${to.cy} H ${to.x}"/>`);});});
  ...
  style.textContent=`
    ...
    .bp-match rect{fill:#eef4ff;stroke:#173b70;stroke-width:2}
    ...
    .bp-match.completed rect{fill:#d9f5e4;stroke:#18794e;stroke-width:2.2}
    .bp-match.playing rect{fill:#ffe1d8;stroke:#c9362b;stroke-width:2.2}
    .bp-score{font-size:8px!important;fill:#4c617a!important}
    .bp-link{fill:none;stroke:#244d82;stroke-width:2}
  ...
  `;
  document.head.appendChild(style);
  const run=()=>{interceptSaveButton();const label=document.getElementById('buildStageLabel');if(label){label.textContent='230MATCH 35.2.4 · 팀명 누락 보정·진한 가지형 대진표';label.title='Version 35.2.4';}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(run,0),{once:true});else setTimeout(run,0);
  window.addEventListener('load',()=>setTimeout(run,400));
  console.info('[230MATCH V3] 35.2.4 ready · complete 3-character assignment names and dark bracket tree active');
})();
