...
function confirmResult(event){
  ...
  const m=submitResult(state,{matchId:id,winnerId,scoreA,scoreB});
...
function confirmPrelimResult(event){
  ...
  const m=submitPrelimResult(state,{matchId:$('prelimResultMatchId').value,winnerId,scoreA,scoreB});
...
(function stage332Release(){
  const ready=()=>{
    const label=document.getElementById('buildStageLabel');
    if(label){label.textContent='230MATCH V3 · 빠른 점수 입력';label.title='Version 33.3';}
    document.documentElement.dataset.release='33.2';
    window.__230MATCH_RELEASE__=Object.freeze({version:'33.3',channel:'production',label:'230MATCH V3 빠른 점수 입력'});
