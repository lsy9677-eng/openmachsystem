...
// V7 원칙: 대회별 문서만 저장한다. 전체 대회 배열은 서버에 절대 저장하지 않는다.
...
function applyBundle(bundle, source = 'firebase') {
  ...
  const d = digest(bundle.state);
  if (d && d === lastAppliedDigest) return;
  applyingRemote = true;
  try { applyRemoteFn(bundle.state); lastAppliedDigest = d; status(source === 'firebase' ? '다른 기기 반영' : '클라우드 불러오기', 'success', `운영 대회 ${bundle.count}개를 불러왔습니다.`); }
...
function onSaved(event) { if (applyingRemote) return; const state = event?.detail?.state || getStateFn(); if (state) schedule(state); }
...
export async function connectCloudSync() {
  disconnectCloudSync(false); await ensureDb();
  if (canWriteFn()) {
