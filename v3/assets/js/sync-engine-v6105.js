...
async function gunzipText(bytes) {
  if (typeof DecompressionStream !== 'function') throw new Error('이 브라우저는 압축된 대회 데이터 읽기를 지원하지 않습니다. 최신 브라우저로 열어 주세요.');
  const stream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'));
  return await new Response(stream).text();
}
async function encodeWorkspace(workspace) {
  const json = JSON.stringify(workspace);
  const compressed = await gzipText(json);
  if (compressed) {
    const base64 = bytesToBase64(compressed);
    if (base64.length < json.length) return { data: base64, encoding: 'gzip-base64-v1', originalBytes: new Blob([json]).size, storedBytes: new Blob([base64]).size };
  }
  return { data: json, encoding: 'json-v1', originalBytes: new Blob([json]).size, storedBytes: new Blob([json]).size };
}
async function decodeWorkspace(raw) {
  ...
  try {
    if (raw?.workspaceEncoding === 'gzip-base64-v1') return JSON.parse(await gunzipText(base64ToBytes(raw.workspaceJson)));
    return JSON.parse(raw.workspaceJson);
  } catch (error) {
    console.warn('[230MATCH] workspace decode failed', error);
    return null;
  }
}
async function readAllTournaments() {
  ...
  const docs = [];
  for (const snap of tournamentSnaps.docs) {
    const raw = snap.data();
    const decoded = await decodeWorkspace(raw);
    if (!decoded) continue;
    ...
    if (!isRealTournament(workspace)) continue;
...
async function writeCurrentTournament(state) {
  ...
  const workspaceJson = encoded.data;
  if (encoded.storedBytes > 900000) throw new Error(`클라우드 저장 데이터가 너무 큽니다(압축 후 ${Math.round(encoded.storedBytes / 1024)}KB). Firebase 문서 한도에 근접해 저장을 중단했습니다.`);
  ...
  const payload = {
    ...
    workspaceJson,
    workspaceEncoding: encoded.encoding,
    workspaceOriginalBytes: encoded.originalBytes,
    workspaceStoredBytes: encoded.storedBytes,
