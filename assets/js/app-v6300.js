...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow,loadTournamentNow}from'./sync-engine-v6300.js?v=6300';
...
const WORKSPACE_ROUTE_BUILD='6300';
...
async function stage3610OpenTournamentGuide(id=''){
  ...
  if(targetId&&targetId!==currentId){
    try{switchTournamentWorkspace(targetId,{view:'guide'});}catch(error){notice('대회 요강을 불러오지 못했습니다: '+(error?.message||error),'error');return false;}
  }
  renderTournamentGuide();
...
function bindTournamentList(){
  ...
  document.getElementById('tournamentCardList')?.addEventListener('click',async e=>{
    ...
    if(select){e.preventDefault();switchTournamentWorkspace(select.dataset.activeTournamentSwitch);return;}
...
function switchDivisionWorkspace(id,{view='home'}={}){
  ...
  const tournamentId=String(state.multiTournament?.activeTournamentId||state.tournament?.id||'').trim();
  if(!tournamentId){notice('현재 대회 정보를 찾을 수 없습니다.','error');return false;}
  try{localStorage.setItem(`230match-v7-active-division:${tournamentId}`,id);}catch(_e){}
...
document.addEventListener('click',e=>{
  ...
  if(id&&id!==current){try{switchTournamentWorkspace(id,{view:'guide'});}catch(error){notice('대회 요강을 불러오지 못했습니다: '+(error?.message||error),'error');return;}return;}
...
function renderDivisionWorkspaceBar(){
  ...
  bar.querySelector('#activeTournamentSelect')?.addEventListener('change',e=>switchTournamentWorkspace(e.target.value));
...
if(!window.__stage3572TournamentSelectBound){window.__stage3572TournamentSelectBound=true;document.addEventListener('click',e=>{const btn=e.target.closest('[data-active-tournament-switch]');if(btn){e.preventDefault();switchTournamentWorkspace(btn.dataset.activeTournamentSwitch);}});}
...
if(!window.__stage5006HomeTournamentBound){
  ...
  document.addEventListener('click',async e=>{
    ...
    if(select){e.preventDefault();switchTournamentWorkspace(select.dataset.homeTournamentSelect);return;}
...
(()=>{document.documentElement.dataset.build='6300';console.info('[230MATCH] 62.0.2 ready · compact mobile workspace + local selection core');})();
