/* v1031 DEV stable v1022 rollback no blank mode
   based on stable v1022 balanced court distribution engine
...
(function(){
  ...
  const VERSION = 'v1034-legacy-final-off-no-blank';
  ...
  try{console.log('[v1031] stable v1022 rollback no-blank draw loaded');}catch(e){}
})();
...
(function(){
  ...
  try{console.log('[v1031] balanced court distribution override installed');}catch(e){}
})();


/* v1031: safe stamp, no broad quarantine/hiding */
(function(){
  'use strict';
  if(window.__V1031_SAFE_NO_BLANK_STAMP__) return;
  window.__V1031_SAFE_NO_BLANK_STAMP__=true;
  try{ console.log('[v1031] safe no-blank stamp loaded'); }catch(e){}
})();


/* v1034: DEV legacy lower final/main sections OFF, no blank page. Keep new clean main panel and court board. */
(function(){
  'use strict';
  if(window.__V1034_LEGACY_FINAL_OFF_INSTALLED) return;
  window.__V1034_LEGACY_FINAL_OFF_INSTALLED=true;
  const VERSION='v1034-legacy-final-off-no-blank';
  const HIDE_CLASS='v1034-legacy-off-hidden';
  const PROTECT_SEL='#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat,#page-home,#page-tournament,#page-register,#page-players,#page-ranking,.app-header,.nav-tabs,.header-actions,#bracketContent,.page,#page-bracket,.main-content,main,body,html';

  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  function isProtected(el){try{return !el || el.matches(PROTECT_SEL) || !!el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat');}catch(e){return true;}}
  function hide(el){
    try{
  ...
  function directText(el){
    try{
      ...
      return S(out || el.textContent || '');
    }catch(e){return S(el&&el.textContent);}
  }
  function nearestSmallBlock(el){
    try{
      let n=el;
      while(n && n!==document.body){
        ...
        const txt=S(n.innerText||n.textContent||'');
        if(/sec-title|card-title|grp-card|card|main-tree-card|t-bracket|t-round|empty-state|home-panel-row|ranking|rank|result|final/i.test(cls+' '+id)) return n;
        if(n.parentElement && (n.parentElement.id==='bracketContent' || n.parentElement.classList.contains('page')) && txt.length<1200) return n;
        n=n.parentElement;
      }
    }catch(e){}
    return el;
  }
  function hideHeadingAndFollowing(titleEl){
    try{
      if(!titleEl || isProtected(titleEl)) return;
      const block=nearestSmallBlock(titleEl) || titleEl;
      hide(block);
      let n=block.nextElementSibling;
      let guard=0;
      while(n && guard++<8){
        if(isProtected(n)) break;
        const t=S(n.innerText||n.textContent||'');
        if(!t){ n=n.nextElementSibling; continue; }
        if(/예선\s*경기표|코트\s*사용\s*현황판|새\s*본선\s*운영|대회\s*$|부서\s*$/.test(t)) break;
        if(/최종\s*결과|우승|준우승|본선\s*경기\s*현황|본선\s*토너먼트\s*대진표|본선\s*진출팀|결승|TBD|이전\s*라운드\s*승자|가지형\s*대진표/.test(t)) hide(n);
        n=n.nextElementSibling;
      }
    }catch(e){}
  }
  function disableLegacyDataFunctions(){
    try{
      if(typeof window.renderRankingCard==='function' && !window.renderRankingCard.__v1034Off){
        const old=window.renderRankingCard;
        const fn=function(key,ranks,teams){
          try{
            // In bracket page, final-result card from legacy renderer must not be emitted.
            if(document.querySelector('#page-bracket.active') || document.getElementById('bracketContent')) return '';
          }catch(e){}
          return old.apply(this,arguments);
        };
        fn.__v1034Off=true; fn.__old=old; window.renderRankingCard=fn;
      }
    }catch(e){}
    try{
      if(typeof window.renderMainRoundShortcutChips==='function' && !window.renderMainRoundShortcutChips.__v1034Off){
        const old=window.renderMainRoundShortcutChips;
        const fn=function(){ return ''; };
        fn.__v1034Off=true; fn.__old=old; window.renderMainRoundShortcutChips=fn;
      }
    }catch(e){}
  }
  function cleanupLegacyLower(){
    try{
      disableLegacyDataFunctions();
      const root=document.getElementById('bracketContent') || document.getElementById('page-bracket') || document.body;
      if(!root) return;
      // Exact legacy ids/classes generated by old main renderer.
      root.querySelectorAll('[id^="mainStage_"],[id^="mainPreviewTitle_"],[id^="mainPreviewCard_"],[id^="mainToggle_"],[id^="mainStageTitle_"],#mainAdvTeams,#mainStep1,#mainStep2,#mMain,.t-bracket,.t-round,.main-tree-card,.v308-main-round-shortcuts').forEach(hide);
      // Headings: hide heading and immediate legacy content after it.
      root.querySelectorAll('h1,h2,h3,h4,.sec-title,.card-title,div,section,article').forEach(el=>{
        if(isProtected(el)) return;
        const dt=directText(el);
        const all=S(el.innerText||el.textContent||'');
        if(dt.length>90 && !/본선\s*경기\s*현황|최종\s*결과|본선\s*토너먼트\s*대진표|본선\s*진출팀/.test(dt)) return;
        if(/^(🏅\s*)?최종\s*결과$/.test(dt) || /본선\s*경기\s*현황/.test(dt) || /본선\s*토너먼트\s*대진표/.test(dt) || /본선\s*진출팀/.test(dt)) hideHeadingAndFollowing(el);
        // Final result card sometimes has no separate heading in the block.
        if(all.length<900 && /우승/.test(all) && /준우승/.test(all) && !/새\s*본선\s*운영|예선\s*경기표/.test(all)) hide(nearestSmallBlock(el)||el);
        if(all.length<900 && /결승/.test(all) && /우승|준우승|최종/.test(all) && !/새\s*본선/.test(all)) hide(nearestSmallBlock(el)||el);
      });
      // If old final result rows are direct children of a large container, hide each row/card.
      root.querySelectorAll('.gs-row,.grp-body,.grp-card,.card').forEach(el=>{
        if(isProtected(el)) return;
        const t=S(el.innerText||el.textContent||'');
        if(t.length<1200 && /우승/.test(t) && /준우승/.test(t) && !/예선|새\s*본선|코트\s*사용/.test(t)) hide(el);
      });
    }catch(e){ console.warn('[v1034] legacy cleanup failed', e); }
  }
  function forceNewPanelRoundLabels(){
    try{
      document.querySelectorAll('#v1003MainPanel .v1004-bmatch,#v1003MainPanel .v1003-card').forEach(card=>{
        const t=S(card.innerText||card.textContent||'');
        if(/시합중|대기|공용대기|배정/.test(t) && !/64강|32강|16강|8강|준결승|결승/.test(t)){
          const src=card.querySelector('.v1004-bsrc,.v1003-src');
          if(src && !/64강/.test(src.textContent||'')) src.textContent='64강 · '+S(src.textContent||'');
        }
      });
    }catch(e){}
  }
  const oldRender=window.renderBracket;
  if(typeof oldRender==='function' && !oldRender.__v1034LegacyOff){
    const wrapped=function(){
      const r=oldRender.apply(this,arguments);
      setTimeout(()=>{cleanupLegacyLower(); forceNewPanelRoundLabels();},0);
      setTimeout(()=>{cleanupLegacyLower(); forceNewPanelRoundLabels();},120);
      setTimeout(()=>{cleanupLegacyLower(); forceNewPanelRoundLabels();},500);
      return r;
    };
    wrapped.__v1034LegacyOff=true; wrapped.__old=oldRender; window.renderBracket=wrapped;
  }
  function tick(){ cleanupLegacyLower(); forceNewPanelRoundLabels(); }
  [0,120,400,900,1600,2800,5000].forEach(t=>setTimeout(tick,t));
  setInterval(tick,1200);
  try{
    const mo=new MutationObserver(()=>{ clearTimeout(window.__v1034LegacyCleanupTimer); window.__v1034LegacyCleanupTimer=setTimeout(tick,80); });
    mo.observe(document.body||document.documentElement,{childList:true,subtree:true});
  }catch(e){}
  try{
    const st=document.createElement('style'); st.id='v1034LegacyOffStyle';
    st.textContent='.'+HIDE_CLASS+'{display:none!important;visibility:hidden!important;pointer-events:none!important}';
    document.head.appendChild(st);
  }catch(e){}
  try{ console.log('[v1034] legacy final/main lower sections off loaded'); }catch(e){}
})();
