/* v1040 queue/status/balanced main draw engine
...
(function(){
  'use strict';
  if(window.__V1040_MAIN_DRAW_INSTALLED) return;
  window.__V1040_MAIN_DRAW_INSTALLED = true;
  window.__CLEAN_MAIN_ONLY_MODE__ = true;
  window.__MAIN_DRAW_AUTHORITY_VERSION__ = 'v1039-source-isolated-main';

  const VERSION = 'v1039-source-isolated-main';
  const PANEL_ID = 'v1040MainPanel';
  const BRACKET_ID = 'v1040MainBracket';
  ...
  function positionBalancedMatchOrder(matchCount){
    // 32경기 기준: 1,32,17,2,31,16,3,30,18 ...
    // 상단·하단·중앙을 번갈아 배정해 특정 구간이 오래 기다리지 않게 한다.
    const out=[]; const seen=new Set();
    let lo=0, hi=matchCount-1;
    let midHi=Math.floor(matchCount/2), midLo=midHi-1;
    let useHighMid=true;
    while(out.length<matchCount){
      for(const x of [lo,hi]){
        if(x>=0&&x<matchCount&&!seen.has(x)){seen.add(x);out.push(x);}
      }
      lo++; hi--;
      const m=useHighMid?midHi++:midLo--;
      useHighMid=!useHighMid;
      if(m>=0&&m<matchCount&&!seen.has(m)){seen.add(m);out.push(m);}
    }
  ...
  function currentRoundIndex(key){
    const size=Number(G.draws[key]?.mainDrawSize||firstRoundMatches(key)[0]?.bracketN||64);
    const maxRound=Math.log2(size)-1;
    for(let r=0;r<=maxRound;r++){
      const ms=mainMatches(key).filter(m=>Number(m.round||0)===r);
      const expected=Math.max(1,size/Math.pow(2,r+1));
      if(ms.length<expected || ms.some(m=>!matchDone(key,m))) return r;
    }
    return maxRound;
  }
  function renderCurrentRoundStatus(key){
    const size=Number(G.draws[key]?.mainDrawSize||firstRoundMatches(key)[0]?.bracketN||64);
    const r=currentRoundIndex(key);
    const label=roundLabel(size,r);
    const ms=mainMatches(key).filter(m=>Number(m.round||0)===r).sort((a,b)=>Number(a.slot||0)-Number(b.slot||0));
    if(!ms.length) return '';
    const cards=ms.map(m=>{
      const st=matchState(key,m); const a=sideText(key,m,1), b=sideText(key,m,2);
      const state=st.done?'완료':m.court?'시합중':m.manualCourtTarget?'대기1':(m.manualSharedHold||m.__queueStatus==='shared')?'공용대기':'배정대기';
      const court=S(m.court||m.manualCourtTarget||'');
      return `<div class="v1040-status-card"><div class="v1040-status-head"><b>${Number(m.slot||0)+1}경기</b><span>${esc(state)}</span></div><div class="v1040-status-team">${esc(a)} <em>vs</em> ${esc(b)}</div><div class="v1040-status-meta">${esc(label)}${court?' · '+esc(court):''}${st.done?' · '+esc(scoreText(key,m)):''}</div></div>`;
    }).join('');
    return `<section class="v1040-current"><div class="v1040-current-title">⚡ 본선 경기 현황 <span>${esc(label)}</span></div><div class="v1040-status-grid">${cards}</div></section>`;
  }
  ...
  async function assignInitial(key){
    ...
    const selectedVenues=Object.keys(byVenue);
    const primaryVenue=selectedVenues[0]||venueOfCourt(cs[0])||'국제';
    const byReady={};
    ready.forEach(m=>{
      let v=venueOfCourt(m.venue||m.__venue)||primaryVenue;
      // 현재 선택 코트에 없는 과거 구장값은 선택된 구장으로 정규화한다.
      if(!selectedVenues.includes(v)) v=primaryVenue;
      m.venue=v; m.venueLabel=v; m.__venue=v; m.mainBlock=v;
      (byReady[v]||(byReady[v]=[])).push(m);
    });
    let idx=0, assigned=0, wait=0, shared=0;
    for(const v of Object.keys(byReady).sort(venueSort)){
      const courts=(byVenue[v]&&byVenue[v].length?byVenue[v]:cs).slice().sort(courtSort);
      const q=byReady[v].slice().sort((a,b)=>Number(a.mainQueueSeq||9999)-Number(b.mainQueueSeq||9999));
      // 1차: 각 코트 시합중 1경기
  ...
  async function refreshQueue(key,manual){
    ...
    for(const c of cs){
      ...
      let active=ready.find(m=>S(m.court)===S(c) && !matchDone(key,m));
      let wait=ready.find(m=>!m.court && S(m.manualCourtTarget)===S(c) && !matchDone(key,m));
      const takeShared=()=>ready.filter(m=>m.manualSharedHold && !m.court && !m.manualCourtTarget && (venueOfCourt(m.__sharedCourtLabel||m.venue||m.__venue)||'기타')===v).sort((a,b)=>Number(a.mainQueueSeq||9999)-Number(b.mainQueueSeq||9999))[0];

      // 결과 입력으로 시합중이 비면 대기1을 즉시 시합중으로 승격한다.
      if(!active && wait){
        setActive(wait,c,0); active=wait; wait=null; changed=true;
      }
      // 승격 직후 같은 실행 안에서 공용대기 1순위를 대기1로 채운다.
      if(!wait){
        const shared=takeShared();
        if(shared){setWait1(shared,c,0); wait=shared; changed=true;}
      }
      // 코트가 완전히 비어 있어도 공용대기를 시합중으로 바로 보내지 않는다.
      // 반드시 대기1에 먼저 머문 뒤 다음 갱신에서 시합중으로 승격한다.
    }
  ...
  function ensurePanel(){
    ...
    if(!p){
      ...
      // Keep the authoritative panel OUTSIDE bracketContent.
      // renderBracket() replaces bracketContent.innerHTML, so inserting inside it makes controls disappear.
      const legacyContent=$('bracketContent');
      if(legacyContent && legacyContent.parentNode===page) page.insertBefore(p,legacyContent);
      else page.prepend(p);
    }
  ...
  function renderPanel(key){
    ...
    resolveAll(key,true); ensureProgression(key);
    box.innerHTML=renderBracket(key)+renderCurrentRoundStatus(key);
  }
  function hideLegacy(){
    try{
  ...
  const AUTO_KEY='V1040_MAIN_AUTO_ENABLED';
  ...
  function install(){
    window.MainDrawCleanV1040={version:VERSION,ensurePanel,renderPanel,generateDraw,assignCourts:assignInitial,refreshQueue,selectedKey,setAutoState,autoEnabled};
    window.MainDrawCleanV1037=window.MainDrawCleanV1040; window.MainDrawCleanV1036=window.MainDrawCleanV1040;
    ...
    if(observeTarget) pageObserver.observe(observeTarget,{childList:true,subtree:true});
    setInterval(()=>{
      try{
    ...
    try{console.log('[v1040] queue/status/balanced engine loaded');}catch(e){}
  }
  install();
})();
