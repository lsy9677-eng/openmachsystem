/* v1012 cache-proof clean main draw slot mode
...
(function(){
  'use strict';
  if(window.__V1012_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1012_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1012 uses its own guard and filename so cached v1008~v1011 scripts cannot block this patch.
  const VERSION = 'v1012-korean-round-nameonly-fix';
  ...
  function mainCourts(key){
    ...
    // 본선 코트는 예선 코트와 분리한다. 장유중/예선 전용 코트가 본선 공용대기에 섞이지 않게 mainAllowedCourts를 최우선 사용한다.
    try{ if(typeof window.getDivisionPhaseConfiguredCourts==='function'){const x=window.getDivisionPhaseConfiguredCourts(tid,div,'main'); if(x&&x.length) return uniq(x);} }catch(e){}
    try{ if(typeof window.getDivisionMainConfiguredCourts==='function'){const x=window.getDivisionMainConfiguredCourts(tid,div); if(x&&x.length) return uniq(x);} }catch(e){}
    try{
      const t=ar(G&&G.tournaments).find(x=>S(x.id)===tid)||null;
      const ds=t&&t.divSettings&&t.divSettings[div];
      if(ds && Array.isArray(ds.mainAllowedCourts) && ds.mainAllowedCourts.length) return uniq(ds.mainAllowedCourts);
    }catch(e){}
    try{ const d=G.draws[key]||{}; const x=[d.mainCourts,d.mainAllowedCourts,d.allowedMainCourts].flatMap(v=>Array.isArray(v)?v:[]); if(x.length) return uniq(x); }catch(e){}
    try{ if(window.OperationQueueV993 && typeof OperationQueueV993.getMainCourts==='function'){const x=OperationQueueV993.getMainCourts(tid,div); if(x&&x.length) return uniq(x);} }catch(e){}
    try{ if(window.OperationQueueV993 && typeof OperationQueueV993.getBoardCourtsForKey==='function'){const x=OperationQueueV993.getBoardCourtsForKey(key); if(x&&x.length) return uniq(x);} }catch(e){}
    try{ if(typeof window.getUsedCourtsForKey==='function'){const x=window.getUsedCourtsForKey(key); if(x&&x.length) return uniq(x);} }catch(e){}
    try{ const d=G.draws[key]||{}; return uniq([d.allowedCourts,d.courts].flatMap(x=>Array.isArray(x)?x:[])); }catch(e){return []}
  }
  ...
  function makeMatch(key, slot, e1, e2, venue, size){
    const id=`v1012_main_${slot}_${Date.now()}`;
    ...
    const m={id,phase:'main',round:0,slot,bracketN:size,localDrawSize:size,localRoundSize:size,localRoundLabel:mainRoundLabel(size,0),roundLabel:mainRoundLabel(size,0),winner:null,rubbers:[],court:'',courts:[],manualCourtTarget:'',
      ...
      venue:v,venueLabel:v,__venue:v,mainBlock:v,cleanMainDraw:true,v1012CleanMain:true,v1007CleanMain:true,v1003CleanMain:true,createdAt:now(),updatedAt:now()};
  ...
  async function assignCourts(key){
    ...
    const allCourts=Object.values(byVenue).flat(); if(!allCourts.length){toast('본선 사용 코트를 찾을 수 없습니다. 대회 부서의 본선 사용 코트를 확인하세요.','error');return false;}

    const occ={}; allCourts.forEach(c=>occ[c]={cur:0,wait:0});
    // 예선/이미 진행 중 경기 점유를 반영한다. 새 본선 경기는 전부 초기화 후 다시 배치한다.
    ...
    mains.forEach((m,i)=>{
      ...
      if(!placed){
        ...
        markShared(m,target,orderIso,i);
        sharedCnt++;
      }
      m.updatedAt=now();
    });
    try{ if(G.draws&&G.draws[key]){ G.draws[key].v1011MainCourtAssignedAt=now(); G.draws[key].v1011MainCourtAssignment='current_wait1_shared'; } }catch(e){}
    ...
    toast(`새 본선 코트배정 완료: 현재경기 ${currentCnt} · 코트대기1 ${waitCnt} · 공용대기 ${sharedCnt}${unresolved.length?' · 미확정 '+unresolved.length+'경기 보류':''}`,'success'); return true;
  }
  ...
  function installSharedQueueBridge(){
    try{
      if(window.__v1011SharedQueueBridgeInstalled) return;
      ...
      wrapped.__v1011SharedQueueBridge=true; wrapped.__old=old;
      window.getIndividualAutoSharedWaitingItems=wrapped;
      window.__v1011SharedQueueBridgeInstalled=true;
    }catch(e){ console.warn('[v1012] shared queue bridge failed', e); }
  }
  function installCourtBoardCleanPatch(){
    try{
      const old=window.describeCourtBoardMatch;
      if(typeof old==='function' && !old.__v1012CleanMainWrapped){
        const wrapped=function(key,m){
          ...
          try{
            ...
            if(base&&base.cleanMainDraw){
              ...
              r.label=base.localRoundLabel||mainRoundLabel(base.bracketN||64,base.round||0);
              r.title=title; r.rawTitle=title; r.autoTitle=title; r.detail='';
            }
          }catch(e){}
          return r;
        };
        wrapped.__v1012CleanMainWrapped=true; wrapped.__old=old;
      ...
      if(typeof oldRound==='function' && !oldRound.__v1012CleanMainWrapped){
        ...
        rw.__v1012CleanMainWrapped=true; rw.__old=oldRound;
        window.getMainRoundLabelByRoundIndex=rw; try{ getMainRoundLabelByRoundIndex=rw; }catch(e){}
      }
    }catch(e){ console.warn('[v1012] court board clean patch failed', e); }
  }
  window.MainDrawCleanV1012={version:VERSION,ensurePanel,generateDraw,assignCourts,hideLegacyMainUi,selectedKey,prelimComplete,resolveCleanMatches,isPrivileged:cleanMainPrivileged,normalizeCleanMatchDisplay,nameOnlyFromLabel,venueKo};
  window.MainDrawCleanV1010=window.MainDrawCleanV1012; window.MainDrawCleanV1007=window.MainDrawCleanV1012; window.MainDrawCleanV1004=window.MainDrawCleanV1012; window.MainDrawCleanV1003=window.MainDrawCleanV1012;
  install();
  function apply(){installSharedQueueBridge();installCourtBoardCleanPatch();ensurePanel();hideLegacyMainUi();forcePanelVisible();}
  ...
  try{console.log('[v1012] korean round/name-only main draw loaded');}catch(e){}
})();
...
(function(){
  ...
  var VERSION='v1012-button-korean-nameonly-fix';
  function $(id){return document.getElementById(id);}
  ...
  function api(){return window.MainDrawCleanV1010 || window.MainDrawCleanV1007 || window.MainDrawCleanV1004 || window.MainDrawCleanV1003 || window.MainDrawCleanV1002 || null;}
  ...
  try{console.log('[v1012] main draw buttons cache-proof fixed');}catch(e){}
})();
