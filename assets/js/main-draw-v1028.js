/* v1028 DEV clean version reset wrapper - latest file. */
/* v1028 DEV true clean main draw mode
...
(function(){
  'use strict';
  if(window.__V1028_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1028_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1028 uses its own guard and filename so cached older scripts cannot block this patch.
  const VERSION = 'v1028-dev-clean-version-reset-fix';
  ...
  try{console.log('[v1022] balanced court distribution draw loaded');}catch(e){}
})();
...
(function(){
  ...
  const VERSION='v1022-balanced-court-distribution-fix';
  ...
  try{console.log('[v1022] balanced queue watchdog installed');}catch(e){}
})();
...
(function(){
  ...
  try{console.log('[v1022] balanced court distribution override installed');}catch(e){}
})();


/* v1023: dev clean main view stabilizer
   - legacy main status/legacy tree/ranking sections are hidden in bracketContent
   - prelim table is kept stable by default after clean main draw exists
   - stable advancement summary is rendered inside the new clean main panel
*/
(function(){
  'use strict';
  if(window.__V1023_DEV_CLEAN_MAIN_VIEW_INSTALLED) return;
  window.__V1023_DEV_CLEAN_MAIN_VIEW_INSTALLED = true;
  const VERSION='v1028-dev-clean-version-reset-fix';
  const S=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const ar=v=>Array.isArray(v)?v:[];
  function $(id){return document.getElementById(id);}
  function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
  function api(){return window.MainDrawCleanV1023||window.MainDrawCleanV1020||window.MainDrawCleanV1019||window.MainDrawCleanV1018||window.MainDrawCleanV1017||window.MainDrawCleanV1016||window.MainDrawCleanV1015||window.MainDrawCleanV1014||window.MainDrawCleanV1010||null;}
  function selectedKey(){try{const a=api(); if(a&&typeof a.selectedKey==='function') return a.selectedKey()||'';}catch(e){} try{const tid=$('brTS')?.value||''; const div=[...(document.querySelectorAll('#brMultiDiv button.active,[data-div].active')||[])].map(b=>b.dataset?.div||S(b.textContent)).find(x=>x&&!/전체/.test(x)) || $('brDS')?.value || ''; return tid&&div&&div!=='__ALL__'?tid+'_'+div:Object.keys((G&&G.draws)||{}).find(k=>k.indexOf(tid+'_')===0)||'';}catch(e){return '';}}
  function splitKey(key){const i=String(key).lastIndexOf('_'); return {tid:String(key).slice(0,i), div:String(key).slice(i+1)};}
  function nameOnly(s){
    return S(s).replace(/\([^)]*\)/g,'').replace(/\s*[-–—]\s*[^/]+(?=\s*(?:\/|$))/g,'').replace(/\s*·\s*[^/]+(?=\s*(?:\/|$))/g,'').replace(/\s+\/\s+/g,' / ').trim();
  }
  function groupNo(g,gi){try{const vals=[g?.confirmedGroupNo,g?.finalGroupNo,g?.displayGroupNo,g?.manualGroupNo,g?.v728GroupNo,g?.v702GroupNo,g?.groupNo,g?.no]; for(const v of vals){const n=Number(v); if(Number.isFinite(n)&&n>0) return n;}}catch(e){} return Number(gi)+1;}
  function groupDone(key, gi){try{const ms=ar(G.matches&&G.matches[key]).filter(m=>m&&String(m.phase||'')==='group'&&Number(m.group)===Number(gi)); return ms.length>0 && ms.every(m=>{try{const st=typeof getMatchResultState==='function'?getMatchResultState(key,m):{}; return st.done||m.done||m.completed||m.winner!=null;}catch(e){return m.done||m.completed||m.winner!=null;}});}catch(e){return false;}}
  function standingIndexes(key, gi, g){
    try{ if(typeof calcGS==='function'){ const rows=ar(calcGS(key,gi,ar(g&&g.teams),ar(G.teams&&G.teams[key]))); const idxs=rows.map(r=>Number(r&&(r.ti??r.teamIdx??r.idx))).filter(Number.isFinite); if(idxs.length) return idxs; }}catch(e){}
    return ar(g&&g.teams).map(Number).filter(Number.isFinite);
  }
  function teamName(key, idx){
    try{ const t=ar(G.teams&&G.teams[key])[Number(idx)]||{}; if(t.pairLabel) return nameOnly(t.pairLabel); if(t.entryLabel) return nameOnly(t.entryLabel); if(Array.isArray(t.individualPlayers)&&t.individualPlayers.length) return nameOnly(t.individualPlayers.slice(0,2).map(p=>S(p&&p.name||p)).filter(Boolean).join(' / ')); if(Array.isArray(t.players)&&t.players.length) return nameOnly(t.players.slice(0,2).join(' / ')); return nameOnly(t.club||t.name||''); }catch(e){return '';}
  }
  function stableEntries(key){
    const draw=(G&&G.draws&&G.draws[key])||{}; const groups=ar(draw.groups); const out=[];
    groups.forEach((g,gi)=>{ const gn=groupNo(g,gi); const done=groupDone(key,gi); const idxs=done?standingIndexes(key,gi,g):[]; [1,2].forEach(rk=>{ const ti=Number(idxs[rk-1]); const ok=done&&Number.isFinite(ti); out.push({gn,rk,ok,name:ok?teamName(key,ti):`${gn}조 ${rk}위`}); }); });
    return out;
  }
  function renderStableAdvList(){
    try{
      const panel=$('v1003MainPanel'); const key=selectedKey(); if(!panel||!key||!(G&&G.draws&&G.draws[key])) return;
      let box=$('v1023StableAdvList');
      if(!box){ box=document.createElement('div'); box.id='v1023StableAdvList'; const ref=$('v1003CleanBracket'); if(ref&&ref.parentNode) ref.parentNode.insertBefore(box,ref); else panel.appendChild(box); }
      const entries=stableEntries(key); if(!entries.length){ box.innerHTML=''; return; }
      const fixed=entries.filter(x=>x.ok).length;
      const chips=entries.map(e=>`<span class="v1023-adv-chip ${e.ok?'ready':'slot'}"><b>${e.gn}조 ${e.rk}위</b> ${esc(e.name)}</span>`).join('');
      box.innerHTML=`<details class="v1023-adv-box"><summary>📌 본선 자리 현황 <b>${fixed}/${entries.length}</b> 확정 · 기존 진출팀 목록 대신 이 목록을 기준으로 고정 표시</summary><div class="v1023-adv-grid">${chips}</div></details>`;
    }catch(e){ console.warn('[v1023] stable adv list failed', e); }
  }
  function hide(el){try{ if(!el||el.closest('#v1003MainPanel')) return; el.classList.add('v1023-legacy-main-hidden'); el.style.setProperty('display','none','important'); }catch(e){}}
  function showPrelimStable(){
    try{
      const key=selectedKey(); if(key){ window.BRACKET_PRELIM_TOGGLE=window.BRACKET_PRELIM_TOGGLE||{}; if(window.__V1023_FORCE_PRELIM_VISIBLE!==false) window.BRACKET_PRELIM_TOGGLE[key]=false; }
      document.querySelectorAll('[id^="prelimSection_"]').forEach(el=>{ if(window.__V1023_FORCE_PRELIM_VISIBLE===false) return; el.classList.remove('v1023-legacy-main-hidden'); el.style.setProperty('display', el.classList.contains('indiv-prelim-grid')?'grid':'block','important'); el.style.setProperty('visibility','visible','important'); });
      document.querySelectorAll('.prelim-sticky-toggle-btn').forEach(btn=>{ if(/펼치기/.test(S(btn.textContent))) btn.textContent='📁 예선내용 접기'; });
    }catch(e){}
  }
  function hideLegacyMainBlocks(){
    try{
      const bc=$('bracketContent'); if(!bc) return;
      const kids=[...bc.children]; let inLegacy=false;
      kids.forEach(el=>{
        if(el.closest('#v1003MainPanel')) return;
        const t=S(el.innerText||el.textContent||'');
        const starts=/본선\s*토너먼트\s*대진표|본선\s*경기\s*현황|진출전\(똥통\)\s*경기\s*현황/.test(t);
        if(starts){ inLegacy=true; hide(el); return; }
        if(inLegacy){
          // legacy main status/ranking section follows the legacy tree. Keep prelim/court sections untouched because they are rendered before this range.
          hide(el); return;
        }
      });
      // Old advancement lists are intentionally replaced by the stable list inside the new panel.
      document.querySelectorAll('#bracketContent #mainAdvTeams,#bracketContent .v929-hidden-adv-list,#bracketContent [id*="mainAdvTeams"],#bracketContent .card,#bracketContent section,#bracketContent article').forEach(el=>{
        if(el.closest('#v1003MainPanel')) return;
        const t=S(el.innerText||el.textContent||'');
        if(/본선\s*진출팀/.test(t) || /우승|준우승|공동\s*3위|최종\s*순위|최종\s*결과/.test(t)&&/본선|결승|준결승/.test(t)) hide(el);
      });
    }catch(e){ console.warn('[v1023] hide legacy blocks failed', e); }
  }
  function installStyle(){
    if($('v1023CleanMainViewStyle')) return;
    const st=document.createElement('style'); st.id='v1023CleanMainViewStyle'; st.textContent=`
      .v1023-legacy-main-hidden{display:none!important;visibility:hidden!important;pointer-events:none!important}
      #v1023StableAdvList{margin:10px 0 12px}.v1023-adv-box{border:1.5px solid #bfdbfe;border-radius:14px;background:linear-gradient(135deg,#f8fbff,#fff);padding:10px 12px}.v1023-adv-box summary{cursor:pointer;font-size:.82rem;font-weight:1000;color:#0f1e3a;line-height:1.45}.v1023-adv-grid{display:flex;flex-wrap:wrap;gap:6px;margin-top:9px;max-height:220px;overflow:auto}.v1023-adv-chip{display:inline-flex;gap:5px;align-items:center;border:1px solid #cbd5e1;border-radius:999px;padding:4px 8px;font-size:.72rem;font-weight:800;background:#f8fafc;color:#475569}.v1023-adv-chip.ready{background:#ecfdf5;border-color:#86efac;color:#166534}.v1023-adv-chip.slot{background:#f8fafc;border-color:#cbd5e1;color:#64748b}
    `; document.head.appendChild(st);
  }
  function apply(){ installStyle(); showPrelimStable(); hideLegacyMainBlocks(); renderStableAdvList(); try{ const p=$('v1003MainPanel'); if(p){ const s=p.querySelector('.v1003-title small'); if(s) s.textContent=VERSION; }}catch(e){} }
  // Override toggle to keep prelim stable by default; users can set window.__V1023_FORCE_PRELIM_VISIBLE=false in console if they need the old toggle while debugging.
  try{ const oldToggle=window.togglePrelimSection; window.togglePrelimSection=function(key){ if(window.__V1023_FORCE_PRELIM_VISIBLE!==false){ window.BRACKET_PRELIM_TOGGLE=window.BRACKET_PRELIM_TOGGLE||{}; window.BRACKET_PRELIM_TOGGLE[key]=false; try{ if(typeof renderBracket==='function') renderBracket(); }catch(e){} setTimeout(apply,80); return false; } return oldToggle&&oldToggle.apply(this,arguments); }; }catch(e){}
  try{ const oldRender=window.renderBracket; if(typeof oldRender==='function'&&!oldRender.__v1023CleanWrapped){ const w=function(){ const key=selectedKey(); if(key&&window.__V1023_FORCE_PRELIM_VISIBLE!==false){ window.BRACKET_PRELIM_TOGGLE=window.BRACKET_PRELIM_TOGGLE||{}; window.BRACKET_PRELIM_TOGGLE[key]=false; } const r=oldRender.apply(this,arguments); [0,80,240,650].forEach(ms=>setTimeout(apply,ms)); return r; }; w.__v1023CleanWrapped=true; window.renderBracket=w; }}catch(e){}
  document.addEventListener('click',()=>setTimeout(apply,120),true); document.addEventListener('input',()=>setTimeout(apply,120),true);
  [0,250,800,1600,3200,6500].forEach(t=>setTimeout(apply,t)); setInterval(apply,1800);
  window.MainDrawCleanV1023=Object.assign({}, api()||{}, {version:VERSION, applyCleanView:apply, renderStableAdvList});
  try{console.log('[v1028] dev true isolated main view fix loaded');}catch(e){}
})();


/* v1024: active-court venue sync fix
   - If 운영 화면에서 본선 사용 코트를 국제만 선택했는데 기존 추첨 구간/카드에 능동·원도심이 남는 경우를 보정한다.
   - Display venue is derived from actual assigned/waiting/shared court first, then from the currently selected active court set.
   - When only one active venue is available, all clean main-draw venue badges/segments are forced to that venue.
*/
(function(){
  'use strict';
  if(window.__v1024ActiveCourtVenueSyncInstalled) return;
  window.__v1024ActiveCourtVenueSyncInstalled=true;
  const VERSION='v1024-active-court-venue-sync-fix';
  function S(v){return String(v==null?'':v).trim();}
  function ar(v){return Array.isArray(v)?v:[];}
  function venueKo(v){
    const s=S(v);
    if(!s) return '';
    if(/gukje|국제|장유국제/i.test(s)) return '국제';
    if(/neungdong|능동/i.test(s)) return '능동';
    if(/wondosim|원도심|원도|인조/i.test(s)) return '원도심';
    if(/jangyu|장유중|클레이/i.test(s)) return '장유중';
    if(/geum|금병/i.test(s)) return '금병';
    if(/samgye|삼계/i.test(s)) return '삼계';
    if(/dongbu|동부/i.test(s)) return '동부';
    return s;
  }
  function venueOfCourt(c){ return venueKo(c); }
  function api(){return window.MainDrawCleanV1023||window.MainDrawCleanV1022||window.MainDrawCleanV1021||window.MainDrawCleanV1020||window.MainDrawCleanV1019||null;}
  function selectedKey(){
    try{const a=api(); if(a&&typeof a.selectedKey==='function') return S(a.selectedKey());}catch(e){}
    try{const k=S(window.__CURRENT_RENDER_KEY||window.__v650CurrentGroupLabelKey||''); if(k) return k;}catch(e){}
    try{const tid=S((document.getElementById('brTS')||{}).value||''); const div=(window.BR_MULTI_DIVS&&window.BR_MULTI_DIVS[0]&&window.BR_MULTI_DIVS[0]!=='__ALL__')?window.BR_MULTI_DIVS[0]:''; if(tid&&div) return tid+'_'+div;}catch(e){}
    return '';
  }
  function uniq(list){return [...new Set(ar(list).map(S).filter(Boolean))];}
  function strictCourtList(key){
    // 현재 화면에서 사용자가 선택한 코트 필터가 있으면 가장 강한 제한으로 본다.
    try{ if(typeof window.getSelectedCourtFilters==='function'){ const a=uniq(window.getSelectedCourtFilters(key)); if(a.length) return a; } }catch(e){}
    // 코트 현황판에 실제 표시되는 코트가 있으면 다음 우선순위로 사용한다.
    try{ if(window.OperationQueueV993&&typeof OperationQueueV993.getBoardCourtsForKey==='function'){ const a=uniq(OperationQueueV993.getBoardCourtsForKey(key)); if(a.length) return a; } }catch(e){}
    try{ const a=api(); if(a&&typeof a.mainCourts==='function'){ const c=uniq(a.mainCourts(key)); if(c.length) return c; } }catch(e){}
    return [];
  }
  function activeVenues(key){
    const courts=strictCourtList(key);
    const venues=uniq(courts.map(venueOfCourt).filter(Boolean));
    return {courts, venues};
  }
  function assignedCourt(m){
    return S(m&&(
      m.court || m.currentCourt || m.manualCourtTarget ||
      (Array.isArray(m.courts)&&m.courts[0]) ||
      m.__sharedCourtLabel || m.sharedCourtLabel || ''
    ));
  }
  function normalizeOne(key,m,onlyVenue,allowedCourts,allowedVenues){
    if(!m||!m.cleanMainDraw) return false;
    let changed=false;
    let v='';
    const c=assignedCourt(m);
    const cv=venueOfCourt(c);
    if(cv && (!allowedVenues.length || allowedVenues.includes(cv))) v=cv;
    else if(onlyVenue) v=onlyVenue;
    else v=venueKo(m.venue||m.venueLabel||m.__venue||m.mainBlock||'국제')||'국제';

    // 제한된 코트 목록에 없는 과거 배정값은 화면 혼동을 만들기 때문에 공용대기 후보로 되돌린다.
    if(allowedCourts.length && c && !allowedCourts.includes(c)){
      const first=allowedCourts.find(x=>venueOfCourt(x)===v) || allowedCourts[0] || '';
      m.court=''; m.currentCourt=''; m.courts=first?[first]:[]; m.manualCourtTarget=''; m.__sharedCourtLabel=first;
      m.manualSharedHold=true; m.__manualSharedHold=true; m.v1011SharedWait=true; m.v1024RestrictedVenueReset=true;
      changed=true;
    }

    ['venue','venueLabel','__venue','mainBlock'].forEach(k=>{ if(m[k]!==v){m[k]=v; changed=true;} });
    if(changed) m.updatedAt=new Date().toISOString();
    return changed;
  }
  function syncActiveCourtVenue(key, opts){
    key=key||selectedKey(); if(!key||!window.G||!G.matches||!G.matches[key]) return {changed:false};
    const info=activeVenues(key);
    const onlyVenue=info.venues.length===1?info.venues[0]:'';
    let changed=false;
    const mains=ar(G.matches[key]).filter(m=>m&&m.cleanMainDraw);
    mains.forEach(m=>{ if(normalizeOne(key,m,onlyVenue,info.courts,info.venues)) changed=true; });
    try{
      const draw=G.draws&&G.draws[key];
      if(draw && onlyVenue && mains.length){
        const last=Math.max(0, ...mains.map((m,i)=>Number(m.slot??i)).filter(Number.isFinite));
        const next=[{venue:onlyVenue,start:0,end:last,count:last+1}];
        const before=JSON.stringify(draw.mainDrawVenueSegments||[]);
        if(before!==JSON.stringify(next)){ draw.mainDrawVenueSegments=next; changed=true; }
      }
    }catch(e){}
    if(changed && opts&&opts.persist){
      try{ if(typeof window.stM==='function') window.stM(key); }catch(e){}
      try{ if(typeof window.stD==='function') window.stD(key); }catch(e){}
    }
    return {changed, onlyVenue, venues:info.venues, courts:info.courts};
  }
  function patchApi(){
    const a=api(); if(!a||a.__v1024ActiveVenuePatched) return;
    a.__v1024ActiveVenuePatched=true;
    const wrap=function(name){
      const old=a[name];
      if(typeof old!=='function') return;
      a[name]=function(){
        const key=(name==='generateDraw')?arguments[0]:selectedKey();
        try{ syncActiveCourtVenue(key); }catch(e){}
        const r=old.apply(this,arguments);
        Promise.resolve(r).finally(()=>{try{ syncActiveCourtVenue(key||selectedKey(), {persist:false}); }catch(e){}});
        return r;
      };
    };
    ['ensurePanel','assignCourts','refreshLiveMainQueue','promoteSharedToOpenSlots','autoQueueResolvedAfterPrelim'].forEach(wrap);
    a.version=VERSION;
    a.syncActiveCourtVenue=syncActiveCourtVenue;
  }
  function updatePanelTitle(){
    try{ const p=document.getElementById('v1003MainPanel'); if(p){ const s=p.querySelector('.v1003-title small'); if(s) s.textContent=VERSION; } }catch(e){}
  }
  function apply(){
    patchApi();
    const key=selectedKey();
    const res=syncActiveCourtVenue(key);
    updatePanelTitle();
    if(res.changed){
      try{ const a=api(); if(a&&typeof a.ensurePanel==='function') setTimeout(()=>a.ensurePanel(),0); }catch(e){}
      try{ if(typeof window.renderBracket==='function') setTimeout(()=>window.renderBracket(),60); }catch(e){}
    }
  }
  window.MainDrawCleanV1024=Object.assign({}, api()||{}, {version:VERSION, syncActiveCourtVenue});
  document.addEventListener('click',()=>setTimeout(apply,90),true);
  document.addEventListener('change',()=>setTimeout(apply,90),true);
  [0,300,900,1800,3500].forEach(t=>setTimeout(apply,t));
  setInterval(()=>{try{syncActiveCourtVenue(selectedKey()); updatePanelTitle();}catch(e){}},2500);
  try{console.log('[v1024] active court venue sync fix loaded');}catch(e){}
})();


/* v1025 DEV: isolate clean main view from legacy main renderer
   - Do not force prelim section open; user collapse/expand must be respected.
   - Hide legacy main tournament tree/status/final-result sections outside the new clean main panel.
   - Keep the new clean main panel as the only trusted main draw UI.
*/
(function(){
  'use strict';
  if(window.__V1025_DEV_ISOLATED_MAIN_VIEW_INSTALLED) return;
  window.__V1025_DEV_ISOLATED_MAIN_VIEW_INSTALLED=true;
  const VERSION='v1028-dev-clean-version-reset-fix';
  function $(id){return document.getElementById(id);} 
  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  function isBracketPage(){try{return !!document.querySelector('#page-bracket.active') || /대진표/.test(S(document.querySelector('.nav-tab.active')?.textContent||''));}catch(e){return false;}}
  function insideClean(el){try{return !!(el && el.closest && el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat'));}catch(e){return false;}}
  function hardHide(el,why){
    if(!el || insideClean(el)) return;
    try{
      el.dataset.v1025LegacyMainHidden=why||'legacy-main';
      el.style.setProperty('display','none','important');
      el.style.setProperty('visibility','hidden','important');
      el.style.setProperty('pointer-events','none','important');
      el.classList.add('v1025-legacy-main-hidden');
    }catch(e){}
  }
  function setVersion(){
    try{
      const p=$('v1003MainPanel');
      if(p){
        const small=p.querySelector('.v1003-title small');
        if(small) small.textContent=VERSION;
      }
      const api=window.MainDrawCleanV1024||window.MainDrawCleanV1023||window.MainDrawCleanV1022||window.MainDrawCleanV1020||window.MainDrawCleanV1019;
      if(api){ api.version=VERSION; api.__v1025Isolated=true; }
      window.MainDrawCleanV1025=Object.assign({}, api||{}, {version:VERSION, isolated:true});
    }catch(e){}
  }
  function disablePrelimForce(){
    try{ window.__V1023_FORCE_PRELIM_VISIBLE=false; }catch(e){}
    try{
      // Remove any inline force left by previous v1023 loop only when user has chosen collapsed state.
      const map=window.BRACKET_PRELIM_TOGGLE||{};
      Object.keys(map).forEach(key=>{
        if(map[key]===true){
          const id='prelimSection_'+String(key).replace(/[^a-zA-Z0-9_-]/g,'_');
          const sec=$(id);
  ...
  function patchPrelimToggle(){
    try{
      if(typeof window.togglePrelimSection==='function' && !window.togglePrelimSection.__v1025RespectUser){
        const old=window.togglePrelimSection;
        window.togglePrelimSection=function(key){
          window.__V1023_FORCE_PRELIM_VISIBLE=false;
          const r=old.apply(this, arguments);
          setTimeout(()=>{disablePrelimForce(); hideLegacyMain();},60);
          setTimeout(()=>{disablePrelimForce(); hideLegacyMain();},220);
          return r;
        };
        window.togglePrelimSection.__v1025RespectUser=true;
      }
    }catch(e){}
  }
  function hideLegacyMain(){
    try{
      setVersion();
      disablePrelimForce();
      const root=$('page-bracket')||$('bracketContent')||document.body;
      if(!root) return;

      // 1) Hide deterministic legacy containers by id/class.
      root.querySelectorAll('[id^="mainStage_"],[id^="mainToggle_"],[id^="mainPreviewTitle_"],[id^="mainPreviewCard_"],.main-tree-card,.v308-main-round-shortcuts').forEach(el=>{
        if(insideClean(el)) return;
        const box=el.closest('.card,section,article') || el;
        hardHide(box,'legacy-main-container');
      });

      // 2) Hide title-led legacy main blocks from their heading until the next section heading.
      Array.from(root.querySelectorAll('.sec-title, h2, h3, div')).forEach(el=>{
        if(insideClean(el)) return;
        const t=S(el.innerText||el.textContent||'');
        if(!t || t.length>120) return;
        const legacy=/본선\s*토너먼트\s*대진표|본선\s*경기\s*현황|최종\s*결과|본선\s*진출팀/.test(t);
        const clean=/새\s*본선\s*운영|새\s*가지형\s*본선|본선\s*자리\s*현황/.test(t);
        if(!legacy || clean) return;
        let cur=el;
        let first=true;
        let guard=0;
        while(cur && guard++<30){
          if(!first && cur.classList && cur.classList.contains('sec-title')) break;
          if(!insideClean(cur)) hardHide(cur, 'legacy-main-section-'+t.slice(0,18));
          cur=cur.nextElementSibling;
          first=false;
        }
      });

      // 3) Hide old ranking/result cards that are not title-led but contain winner labels.
      root.querySelectorAll('.grp-card,.card,section,article').forEach(el=>{
        if(insideClean(el)) return;
        const t=S(el.innerText||'');
        if(t.length>1200) return;
        if(/최종\s*결과|우승|준우승/.test(t) && /본선|결승|최종/.test(t)) hardHide(el,'legacy-final-result-card');
      });
    }catch(e){console.warn('[v1025] hide legacy main failed', e);}
  }
  function installStyle(){
    if($('v1025IsolatedMainStyle')) return;
    const st=document.createElement('style');
    st.id='v1025IsolatedMainStyle';
    st.textContent=`
      .v1025-legacy-main-hidden{display:none!important;visibility:hidden!important;pointer-events:none!important;}
      #v1003MainPanel{display:block!important;visibility:visible!important;opacity:1!important;}
      #v1003MainPanel .v1004-round-title{background:#0f1e3a!important;color:#fff!important;}
      #v1003MainPanel .v1004-bmatch .v1025-state-chip{display:inline-flex;align-items:center;gap:4px;margin-left:4px;padding:2px 7px;border-radius:999px;background:#eef2ff;color:#1d4ed8;font-size:.68rem;font-weight:1000;border:1px solid #bfdbfe;}
    `;
    document.head.appendChild(st);
  }
  function patchApi(){
    try{
      const api=window.MainDrawCleanV1024||window.MainDrawCleanV1023||window.MainDrawCleanV1022||window.MainDrawCleanV1020||window.MainDrawCleanV1019;
      if(!api || api.__v1025Patched) return;
      api.__v1025Patched=true;
      ['ensurePanel','generateDraw','assignCourts','refreshLiveMainQueue','hideLegacyMainUi','hideLegacyTreeOnly'].forEach(name=>{
        if(typeof api[name]!=='function') return;
        const old=api[name];
        api[name]=function(){
          const r=old.apply(this, arguments);
          Promise.resolve(r).finally(()=>{setTimeout(()=>{hideLegacyMain(); setVersion();},60);});
          return r;
        };
      });
      api.version=VERSION;
  ...
  document.addEventListener('input',()=>setTimeout(apply,80),true);
  try{
    const mo=new MutationObserver(()=>{clearTimeout(window.__v1025MainCleanTimer); window.__v1025MainCleanTimer=setTimeout(apply,80);});
    mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});
  }catch(e){}
  [0,120,400,900,1600,2800,4500].forEach(t=>setTimeout(apply,t));
  setInterval(apply,1500);
  try{console.log('[v1025] isolated clean main view fix loaded');}catch(e){}
})();


/* v1026: final visible version stamp and legacy isolation reinforcement */
(function(){
  'use strict';
  const VERSION='v1028-dev-clean-version-reset-fix';
  function stamp(){
    try{
      document.querySelectorAll('#v1003MainPanel .v1003-title small').forEach(el=>{ el.textContent=VERSION; });
      const panel=document.getElementById('v1003MainPanel');
      if(panel) panel.dataset.cleanVersion=VERSION;
    }catch(e){}
  }
  function hideLegacy(){
    try{
      const re=/본선\s*(토너먼트|경기\s*현황|진출팀|최종\s*결과)|우승자|준우승|결승\s*\(|결승\s*$/;
      document.querySelectorAll('#bracketContent > div, #bracketContent section, #bracketContent .card').forEach(el=>{
        if(el.closest('#v1003MainPanel')) return;
        const txt=(el.innerText||'').replace(/\s+/g,' ').trim();
        if(txt && txt.length<2500 && re.test(txt)){
          el.style.setProperty('display','none','important');
          el.style.setProperty('visibility','hidden','important');
          el.classList.add('v1026-legacy-main-hidden');
        }
      });
    }catch(e){}
  }
  function apply(){stamp();hideLegacy();}
  [0,80,250,700,1500,3000].forEach(t=>setTimeout(apply,t));
  document.addEventListener('click',()=>setTimeout(apply,120),true);
  document.addEventListener('input',()=>setTimeout(apply,120),true);
  setInterval(apply,1200);
  try{console.log('[v1028] final stamp/isolation loaded');}catch(e){}
})();


/* v1027 DEV: legacy main quarantine + active round status reinforcement
   Purpose:
   - The clean main panel is the only trusted main draw view.
   - Legacy main status/final-result/winner sections are quarantined outside #v1003MainPanel.
   - User prelim collapse state is respected; old force-open prelim loop is disabled.
   - Clean match cards show the active round label such as 64강 in the status row.
*/
(function(){
  'use strict';
  if(window.__V1028_MAIN_DRAW_LEGACY_QUARANTINE_INSTALLED) return;
  window.__V1028_MAIN_DRAW_LEGACY_QUARANTINE_INSTALLED = true;
  const VERSION='v1028-dev-clean-version-reset-fix';
  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  function $(id){try{return document.getElementById(id);}catch(e){return null;}}
  function inClean(el){try{return !!(el && el.closest && el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat'));}catch(e){return false;}}
  function hardHide(el, why){
    try{
      if(!el || inClean(el) || el===document.body || el===document.documentElement) return;
      el.classList.add('v1028-legacy-main-quarantined');
      el.dataset.v1028Hidden = why || 'legacy-main';
      el.style.setProperty('display','none','important');
      el.style.setProperty('visibility','hidden','important');
      el.style.setProperty('pointer-events','none','important');
    }catch(e){}
  }
  function tooBig(el){try{return (el.innerText||'').length>9000 || el.id==='bracketContent' || el.classList.contains('main-content') || el.tagName==='MAIN';}catch(e){return false;}}
  function blockFor(el){
    try{
      let cur=el;
      for(let i=0;i<5 && cur && cur.parentElement;i++,cur=cur.parentElement){
        if(inClean(cur)) return null;
        if(cur.id==='bracketContent' || cur.tagName==='MAIN' || cur===document.body) break;
        const cls=String(cur.className||'');
        if(/card|section|panel|accordion|bracket|result|ranking|draw|round/i.test(cls) && !tooBig(cur)) return cur;
      }
      return tooBig(el.parentElement) ? el : (el.parentElement || el);
    }catch(e){return el;}
  }
  function hideRangeFromHeading(head, why){
    try{
      const b=blockFor(head);
      if(b && b!==head && !tooBig(b)){ hardHide(b, why); return; }
      hardHide(head, why+'-heading');
      let cur=head.nextElementSibling, count=0;
      while(cur && count<8){
        if(inClean(cur)) break;
        const t=S(cur.innerText||cur.textContent||'');
        if(count>0 && /^(예선|코트\s*사용|대회|팀\s*등록|참가자|시합\s*결과|공지|새\s*본선)/.test(t)) break;
        if(/본선|결승|준결승|우승|준우승|최종|TBD|이전\s*라운드/.test(t)) hardHide(cur, why+'-sibling');
  ...
  function hideLegacyMain(){
    try{
      window.__V1023_FORCE_PRELIM_VISIBLE = false;
      document.querySelectorAll('#mMain,[id^="mainStage_"],.t-bracket,.t-round,.main-status-board,.v1013-legacy-main-tree-hidden').forEach(el=>hardHide(el,'legacy-container'));
      document.querySelectorAll('h1,h2,h3,h4,h5,.sec-title,.card-title,.section-title,summary,button,div').forEach(el=>{
        if(!el || inClean(el)) return;
        const t=S(el.innerText||el.textContent||'');
        if(!t || t.length>220) return;
        if(/본선\s*경기\s*현황|최종\s*결과|본선\s*진출팀|본선\s*토너먼트\s*대진표|기존\s*본선/.test(t)) hideRangeFromHeading(el,'legacy-heading');
        else if(/우승|준우승/.test(t) && /최종|결과|순위/.test(S((el.parentElement&&el.parentElement.innerText)||t))) hideRangeFromHeading(el,'legacy-final');
      });
      // Some legacy final cards have no useful heading; hide compact rows that already declare winners while the clean main panel exists.
      if($('#v1003MainPanel')){
        document.querySelectorAll('#bracketContent .card, #bracketContent section, #bracketContent article, #bracketContent div').forEach(el=>{
          if(inClean(el) || tooBig(el)) return;
          const t=S(el.innerText||'');
          if(t.length<1800 && /최종\s*결과|우승|준우승/.test(t) && !/새\s*본선\s*운영/.test(t)) hardHide(el,'legacy-winner-card');
        });
      }
    }catch(e){console.warn('[v1028] legacy quarantine failed', e);}
  }
  function stamp(){
    try{
      const p=$('v1003MainPanel');
      if(!p) return;
      const sm=p.querySelector('.v1003-title small'); if(sm) sm.textContent=VERSION;
      p.querySelectorAll('*').forEach(el=>{ if(S(el.textContent).match(/^v10\d{2}/) && el.textContent.length<80) el.textContent=el.textContent.replace(/v10\d{2}[-\w가-힣]*/g, VERSION); });
    }catch(e){}
  }
  function reinforceRoundStatus(){
    try{
      document.querySelectorAll('#v1003MainPanel .v1004-round').forEach(round=>{
        const label=S(round.querySelector('.v1004-round-title')?.textContent||'');
        if(!label) return;
        round.querySelectorAll('.v1004-bmatch').forEach(card=>{
          const status=card.querySelector('.v1004-bstatus');
          if(!status) return;
          let t=S(status.textContent||'');
          if(!t) t='배정 대기';
          if(!t.includes(label)) status.textContent=label+' · '+t.replace(/^(64강|32강|16강|8강|준결승|결승)\s*·\s*/,'');
        });
      });
    }catch(e){console.warn('[v1028] round status failed', e);}
  }
  function keepCleanVisible(){
    try{
      const p=$('v1003MainPanel');
      if(p){
        p.classList.remove('v1028-legacy-main-quarantined','v1026-legacy-main-hidden','v1025-legacy-main-hidden','v1023-legacy-main-hidden','v1003-legacy-hidden');
  ...
  function apply(){keepCleanVisible(); stamp(); reinforceRoundStatus(); hideLegacyMain();}
  window.v1028ApplyMainDrawIsolation=apply;
  try{
    const oldRender=window.renderBracket;
    if(typeof oldRender==='function' && !oldRender.__v1028IsolationWrapped){
      const wrapped=function(){ const r=oldRender.apply(this,arguments); setTimeout(apply,30); setTimeout(apply,160); setTimeout(apply,600); return r; };
      wrapped.__v1028IsolationWrapped=true; window.renderBracket=wrapped;
    }
  }catch(e){}
  try{
    const mo=new MutationObserver(()=>{clearTimeout(window.__v1028IsoTimer); window.__v1028IsoTimer=setTimeout(apply,80);});
    mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});
  }catch(e){}
  if(!document.getElementById('v1028LegacyQuarantineStyle')){
    const st=document.createElement('style'); st.id='v1028LegacyQuarantineStyle';
    st.textContent='.v1028-legacy-main-quarantined{display:none!important;visibility:hidden!important;pointer-events:none!important} #v1003MainPanel{display:block!important;visibility:visible!important}';
    document.head.appendChild(st);
  }
  [0,120,350,800,1500,2800,5000].forEach(t=>setTimeout(apply,t));
  setInterval(apply,2500);
  try{console.log('[v1028] legacy quarantine and round status patch loaded');}catch(e){}
})();


/* v1028 DEV: final hard version stamp and legacy quarantine watchdog */
(function(){
  'use strict';
  const VERSION='v1028-dev-clean-version-reset-fix';
  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  function inClean(el){try{return !!(el&&el.closest&&el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat'));}catch(e){return false;}}
  function hide(el){try{if(!el||inClean(el)||el===document.body||el===document.documentElement)return; el.classList.add('v1028-legacy-main-quarantined'); el.style.setProperty('display','none','important'); el.style.setProperty('visibility','hidden','important'); el.style.setProperty('pointer-events','none','important');}catch(e){}}
  function stamp(){try{const p=document.getElementById('v1003MainPanel'); if(!p)return; p.dataset.cleanVersion=VERSION; const sm=p.querySelector('.v1003-title small'); if(sm)sm.textContent=VERSION; p.querySelectorAll('*').forEach(el=>{const t=S(el.textContent); if(/^v10\d{2}/.test(t)&&t.length<90) el.textContent=VERSION;});}catch(e){}}
  function quarantine(){try{
    const re=/본선\s*(토너먼트|경기\s*현황|진출팀)|최종\s*결과|우승|준우승|기존\s*본선/;
    document.querySelectorAll('#bracketContent .card,#bracketContent section,#bracketContent article,#bracketContent > div,h1,h2,h3,h4,.sec-title,.card-title').forEach(el=>{ if(inClean(el))return; const txt=S(el.innerText||el.textContent||''); if(txt&&txt.length<2600&&re.test(txt)) hide(el.closest('.card,section,article,div')||el); });
    document.querySelectorAll('#mMain,[id^="mainStage_"],.t-bracket,.t-round,.main-status-board').forEach(hide);
  }catch(e){}}
  function apply(){stamp();quarantine();}
  window.v1028ApplyMainDrawIsolation=apply;
  if(!document.getElementById('v1028FinalStyle')){const st=document.createElement('style');st.id='v1028FinalStyle';st.textContent='.v1028-legacy-main-quarantined{display:none!important;visibility:hidden!important;pointer-events:none!important} #v1003MainPanel{display:block!important;visibility:visible!important}';document.head.appendChild(st);}
  [0,80,200,600,1200,2500,5000].forEach(t=>setTimeout(apply,t));
  setInterval(apply,1800);
  try{console.log('[v1028] final clean version reset loaded');}catch(e){}
})();
