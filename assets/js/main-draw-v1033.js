...
(function(){
  ...
  const VERSION = 'v1031-stable-v1022-rollback-no-blank';
...
(function(){
  ...
  const VERSION='v1033-safe-legacy-cleanup-no-blank';
...
(function(){
  ...
  try{console.log('[v1033] balanced court distribution override installed');}catch(e){}
})();


/* v1033: safe legacy cleanup, no broad quarantine/hiding */
(function(){
  'use strict';
  if(window.__V1033_SAFE_LEGACY_CLEANUP__) return;
  window.__V1033_SAFE_LEGACY_CLEANUP__=true;
  try{ console.log('[v1033] safe legacy cleanup loaded'); }catch(e){}
})();


/* v1033: safe lower legacy cleanup only. Never hide page/bracketContent/main containers. */
(function(){
  'use strict';
  if(window.__V1033_SAFE_LEGACY_SECTION_CLEANUP__) return;
  window.__V1033_SAFE_LEGACY_SECTION_CLEANUP__=true;
  const VERSION='v1033-safe-legacy-cleanup-no-blank';
  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  function isProtected(el){
    try{
      if(!el) return true;
      if(el.id==='v1003MainPanel' || el.closest('#v1003MainPanel')) return true;
      if(el.id==='bracketContent' || el.id==='page-bracket' || el.classList.contains('page') || el.classList.contains('main-content') || el.tagName==='MAIN' || el.tagName==='BODY' || el.tagName==='HTML') return true;
      if(el.id==='courtBoardWrap' || el.closest('#courtBoardWrap,.court-status-board,.court-board,.indiv-prelim-grid,.grp-card')) return true;
    }catch(e){}
    return false;
  }
  function hide(el){
    try{
      ...
      el.classList.add('v1033-legacy-hidden');
  ...
  function safeHideTitleBlock(titleEl){
    try{
      if(!titleEl || titleEl.closest('#v1003MainPanel')) return;
      const t=S(titleEl.innerText||titleEl.textContent||'');
      if(!/최종\s*결과|본선\s*경기\s*현황|본선\s*토너먼트\s*대진표|본선\s*진출팀/.test(t)) return;
      let block=titleEl.closest('.card,section,article');
      if(block && !isProtected(block)) { hide(block); return; }
      // When the title is a standalone heading inside the page, hide only the title and the immediate result card/list after it.
      hide(titleEl);
      let n=titleEl.nextElementSibling;
      let cnt=0;
      while(n && cnt<4){
        const nt=S(n.innerText||n.textContent||'');
        if(n.id==='v1003MainPanel' || n.querySelector('#v1003MainPanel')) break;
        if(/예선\s*경기표|코트\s*사용|새\s*본선\s*운영|대회\s*$|부서\s*$/.test(nt)) break;
        if(/우승|준우승|결승|본선|TBD|대진표|최종/.test(nt) && !isProtected(n)) hide(n);
        n=n.nextElementSibling; cnt++;
      }
    }catch(e){}
  }
  function cleanup(){
    try{
      let root=document.getElementById('bracketContent') || document.getElementById('page-bracket') || document;
      // Exact old-main IDs only.
      root.querySelectorAll('[id^="mainStage_"],[id^="mainPreviewTitle_"],[id^="mainPreviewCard_"],[id^="mainToggle_"],[id^="mainStageTitle_"],#mainAdvTeams,#mainStep1,#mainStep2,#mMain').forEach(hide);
      // Old main buttons only; do not touch new panel/float buttons.
      root.querySelectorAll('button,a').forEach(el=>{
        if(el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat')) return;
        const t=S(el.innerText||el.textContent||'');
        const oc=S(el.getAttribute('onclick')||'');
        if(/가지형\s*대진표|본선\s*시드보호\s*재추첨|본선확정|본선잠금|128고정|수동128/.test(t+' '+oc)) hide(el);
      });
      root.querySelectorAll('.sec-title,.card-title,h1,h2,h3,h4').forEach(safeHideTitleBlock);
      // Cards that are clearly legacy final result only.
      root.querySelectorAll('.card,section,article').forEach(el=>{
        if(isProtected(el)) return;
        const t=S(el.innerText||el.textContent||'');
        if(t.length<30 || t.length>1400) return;
        if(/최종\s*결과/.test(t) && /우승|준우승/.test(t)) hide(el);
        if(/64강\s*진행중/.test(t) && /결승\s*결과|우승자/.test(t)) hide(el);
      });
      const panel=document.getElementById('v1003MainPanel');
      if(panel){
        panel.style.removeProperty('display');
        panel.style.setProperty('display','block','important');
        const sm=panel.querySelector('.v1003-title small');
        if(sm) sm.textContent=VERSION;
      }
    }catch(e){console.warn('[v1033] safe legacy cleanup failed',e);}
  }
  window.v1033SafeLegacyCleanup=cleanup;
  [0,200,700,1500,3000].forEach(t=>setTimeout(cleanup,t));
  ['click','input','change','keyup','pointerup','touchend'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(cleanup,120),true));
  try{ const mo=new MutationObserver(()=>{clearTimeout(window.__v1033CleanupTimer); window.__v1033CleanupTimer=setTimeout(cleanup,120);}); mo.observe(document.documentElement,{subtree:true,childList:true}); }catch(e){}
  setInterval(cleanup,2500);
  try{console.log('[v1033] safe legacy cleanup patch loaded');}catch(e){}
})();
