/* v1017 clean main draw live queue watchdog mode
...
(function(){
  'use strict';
  if(window.__V1017_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1017_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1017 uses its own guard and filename so cached v1008~v1014 scripts cannot block this patch.
  const VERSION = 'v1017-live-main-queue-watchdog-fix';
  ...
  function repairSegmentVenues(key, forceRecompute){
    ...
    }catch(e){ console.warn('[v1017] venue repair failed', e); return {changed:false, venues:[]}; }
  }
  ...
  function scheduleAutoQueueResolved(key){
    try{
      ...
      __autoQueueState[key]=setTimeout(()=>{ __autoQueueState[key]=null; autoQueueResolvedAfterPrelim(key).catch(e=>console.warn('[v1017] auto queue failed',e)); }, 350);
    }catch(e){}
  }
  function promoteSharedToOpenSlots(key){
    ...
    shared.forEach((m,i)=>{
      ...
      if(!placed){
        ...
        if(S(m.__sharedCourtLabel||m.v1011SharedCourt)!==target){ markShared(m,target,orderIso,Number(m.slot||i)); changed=true; }
      }
  ...
  async function refreshLiveMainQueue(key, reason){
    ...
    let changed=false;
    resolveCleanMatches(key,true);
    const rep=repairSegmentVenues(key,false); if(rep.changed) changed=true;
    ...
    if(targets.length){ res=placeCleanMatches(key,targets,{auto:true}); if(!res.noCourts) changed=true; }
    const promo=promoteSharedToOpenSlots(key); if(promo.changed) changed=true;
    if(!changed) return false;
    try{ if(G.draws&&G.draws[key]){ G.draws[key].v1016MainQueueRefreshedAt=now(); G.draws[key].v1016MainQueueReason=String(reason||'auto'); } }catch(e){}
    await persist(key);
    try{ if(typeof window.renderBracket==='function') window.renderBracket(); }catch(e){}
    ...
    if(promo.changed) msg.push(`공용대기 승격 현재 ${promo.currentCnt} · 대기1 ${promo.waitCnt}`);
    toast(`본선 대기열 자동 갱신: ${msg.join(' · ')||'정리 완료'}`,'success');
    return true;
  }
  ...
  function installResultSaveQueueHook(){
    try{
      ...
      const wrapped=async function(){
        ...
        try{ r=await oldSave.apply(this,arguments); }
          ...
          [120,650,1500,3000].forEach(ms=>setTimeout(()=>{ try{ refreshLiveMainQueue(key,'after_result_save').catch(e=>console.warn('[v1017] after save queue failed',e)); }catch(e){} },ms));
        }
        return r;
      };
    ...
    }catch(e){ console.warn('[v1017] result save hook failed', e); }
  }
  function install(){
  ...
  function installSharedQueueBridge(){
    ...
    }catch(e){ console.warn('[v1017] shared queue bridge failed', e); }
  }
  function installCourtBoardCleanPatch(){
    ...
    }catch(e){ console.warn('[v1017] court board clean patch failed', e); }
  }
  window.MainDrawCleanV1017={version:VERSION,ensurePanel,generateDraw,assignCourts,autoQueueResolvedAfterPrelim,hideLegacyMainUi,hideLegacyTreeOnly,selectedKey,prelimComplete,resolveCleanMatches,isPrivileged:cleanMainPrivileged,normalizeCleanMatchDisplay,nameOnlyFromLabel,venueKo,mainCourts,venueGroups,promoteSharedToOpenSlots,refreshLiveMainQueue};
  window.MainDrawCleanV1016=window.MainDrawCleanV1017; window.MainDrawCleanV1015=window.MainDrawCleanV1017; window.MainDrawCleanV1014=window.MainDrawCleanV1017; window.MainDrawCleanV1010=window.MainDrawCleanV1017; window.MainDrawCleanV1007=window.MainDrawCleanV1017; window.MainDrawCleanV1004=window.MainDrawCleanV1017; window.MainDrawCleanV1003=window.MainDrawCleanV1017;
  window.MainDrawCleanV1015=window.MainDrawCleanV1017; window.MainDrawCleanV1014=window.MainDrawCleanV1017;
  window.MainDrawCleanV1013=window.MainDrawCleanV1017;
  window.MainDrawCleanV1012=window.MainDrawCleanV1017; window.MainDrawCleanV1010=window.MainDrawCleanV1017; window.MainDrawCleanV1007=window.MainDrawCleanV1017; window.MainDrawCleanV1004=window.MainDrawCleanV1017; window.MainDrawCleanV1003=window.MainDrawCleanV1017;
  install();
  ...
  try{console.log('[v1017] live main queue/status draw loaded');}catch(e){}
})();
...
(function(){
  ...
  function api(){return window.MainDrawCleanV1017 || window.MainDrawCleanV1016 || window.MainDrawCleanV1015 || window.MainDrawCleanV1014 || window.MainDrawCleanV1010 || window.MainDrawCleanV1007 || window.MainDrawCleanV1004 || window.MainDrawCleanV1003 || window.MainDrawCleanV1002 || null;}
  ...
  try{console.log('[v1017] main draw buttons cache-proof fixed');}catch(e){}
})();


/* v1017: live main queue watchdog
...
(function(){
  'use strict';
  if(window.__V1017_MAIN_QUEUE_WATCHDOG_INSTALLED) return;
  window.__V1017_MAIN_QUEUE_WATCHDOG_INSTALLED = true;
  const VERSION='v1017-live-main-queue-watchdog-fix';
  const S=v=>String(v==null?'':v).trim();
  const ar=v=>Array.isArray(v)?v:[];
  function api(){return window.MainDrawCleanV1017||window.MainDrawCleanV1016||window.MainDrawCleanV1015||window.MainDrawCleanV1014||window.MainDrawCleanV1010||null;}
  function toast(msg,type){try{ if(typeof window.toast==='function') window.toast(msg,type||'info'); else console.log('[v1017]',msg); }catch(e){} }
  ...
  function cleanKeys(){
    try{
      ...
      Object.keys(matches).forEach(k=>{
        ...
        const assigned=!!(d.v1011MainCourtAssignment||d.v1014MainCourtAssignment||d.v1011MainCourtAssignedAt||d.v1014MainCourtAssignedAt||list.some(m=>m&&m.cleanMainDraw&&(S(m.court)||S(m.manualCourtTarget)||S(m.__sharedCourtLabel)||m.manualSharedHold||m.v1011SharedWait)));
  ...
  async function refreshAll(reason, quiet){
    ...
    try{
      for(const k of keys){
        ...
        catch(e){ console.warn('[v1017] queue refresh failed', k, e); }
      }
      if(changed && !quiet){ toast('본선 큐 자동 갱신 완료: '+changed+'개 부서','success'); }
      return changed>0;
    }finally{ running=false; }
  }
  window.v1017RefreshAllCleanMainQueues=function(){ return refreshAll('manual_force', false); };
  function ensureForceButton(){
    try{
      const panel=document.getElementById('v1003MainPanel');
      if(!panel||panel.querySelector('#v1017QueueRefreshBtn')) return;
      const row=panel.querySelector('.v1003-controls')||panel;
      const btn=document.createElement('button');
      btn.type='button';
      btn.id='v1017QueueRefreshBtn';
  ...
  function hookFunction(name){
    try{
      ...
      if(typeof fn!=='function'||fn.__v1017QueueWatchHook) return;
      const wrapped=async function(){
        let r;
      ...
      wrapped.__v1017QueueWatchHook=true; wrapped.__old=fn;
      window[name]=wrapped;
  ...
  try{console.log('[v1017] main queue watchdog installed');}catch(e){}
})();
