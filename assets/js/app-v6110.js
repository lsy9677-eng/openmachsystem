...
import{uploadManagedImage,deleteManagedImage,managedImageUrl}from'./storage-image-engine-v6109.js?v=6109';
...
import{loadState,saveState,clearState,saveRecovery,getRecoveries,getRecovery,deleteRecovery,prepareRecoveryStorage,initialState}from'./store-v5000.js?v=5000';
...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow}from'./sync-engine-v6105.js?v=6107';
...
function renderTournamentGuide(){
  ...
  const guideHero=document.querySelector('#view-guide .tournament-guide-hero');
  if(imageSection&&guideHero&&guideHero.nextElementSibling!==imageSection)guideHero.insertAdjacentElement('afterend',imageSection);
...
async function stage3610OpenTournamentGuide(id=''){
  ...
  if(target&&target!==String(state.multiTournament?.activeTournamentId||state.tournament?.id||'')){
    try{switchTournamentWorkspace(target);}catch(error){notice('대회 요강을 불러오지 못했습니다: '+(error?.message||error),'error');return;}
  }
...
async function saveTournamentGuide(){
  ...
  let imageUrl=stage328PendingGuideImage||'',imageStoragePath=stage328PendingGuideStoragePath||'',imageDataUrl='';
  try{
    if(String(stage328PendingGuideImage||'').startsWith('data:image/')){
      ...
      imageUrl=uploaded.url;imageStoragePath=uploaded.path;stage328PendingGuideImage=uploaded.url;stage328PendingGuideStoragePath=uploaded.path;
    }else if(!stage328PendingGuideImage&&previous.imageStoragePath){
      await deleteManagedImage(previous.imageStoragePath);imageStoragePath='';
    }else if(String(stage328PendingGuideImage||'').startsWith('data:')){imageDataUrl=stage328PendingGuideImage;}
  }catch(error){notice(`요강 이미지 업로드 실패: ${error?.message||error}`,'error');return;}
  state.portal.guide={...previous,date:val('guideDateInput'),venue:val('guideVenueInput'),fee:val('guideFeeInput'),bank:val('guideBankInput'),account:val('guideAccountInput'),organizer:val('guideOrganizerInput'),entryPeriod:val('guideEntryPeriodInput'),eligibility:val('guideEligibilityInput'),matchFormat:val('guideMatchFormatInput'),awards:val('guideAwardsInput'),refundPolicy:val('guideRefundPolicyInput'),contact:val('guideContactInput'),extra:val('guideExtraInput'),paymentNote:val('guidePaymentNoteInput'),detail:val('guideDetailInput'),imageUrl:imageUrl&&!String(imageUrl).startsWith('data:')?imageUrl:'',imageStoragePath,imageDataUrl,imageName:stage328PendingGuideImageName||'',imageType:stage328PendingGuideImageType||''};
  const editor=document.getElementById('tournamentGuideEditor');if(editor)editor.hidden=true;
  commit('대회 요강·참가비·입금 계좌 저장');renderTournamentGuide();notice(imageUrl?'대회 요강과 이미지를 Firebase Storage에 저장했습니다.':'대회 요강을 저장했습니다.','success');
}
...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  renderPortalViews();
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='my-match')setTimeout(v3252AutoMyMatch,60);if(target==='entry')setTimeout(renderApplicationPortal,30);if(target==='notifications')setTimeout(refreshNotificationManager,30);if(target==='messages')setTimeout(()=>{renderPortalViews();renderSmsAcceptance(lastSmsAcceptance);},30);if(target==='print')setTimeout(renderPrintPreview,30);if(target==='acceptance')setTimeout(renderAcceptance,30);if(target==='rehearsal')setTimeout(()=>renderRehearsal(),30);if(target==='performance')setTimeout(renderPerformanceCenter,30);if(target==='diagnostics')setTimeout(renderDiagnostics,30);if(target==='manual')setTimeout(renderOperationsManual,30);
...
document.addEventListener('click',e=>{const btn=e.target.closest?.('[data-portal-go="guide"],.mobile-guide-button');if(!btn)return;e.preventDefault();e.stopImmediatePropagation();navigatePortalView('guide',{pushHistory:true});},true);
...
(function stage6001UnifiedTournamentEditor(){
  ...
  async function saveAll(){
    ...
    saving=true;const saveBtn=d.querySelector('[data-s6001-save-all]'),cancel=d.querySelector('[data-s6001-cancel]');saveBtn.disabled=true;cancel.disabled=true;saveBtn.textContent='서버 저장 중…';setSaveText('변경사항을 서버에 저장하고 있습니다.','창을 닫지 마세요.','saving');
    try{
      ...
      const previousGuide=state.portal.guide||{};let guideImageUrl=previousGuide.imageUrl||'',guideImageStoragePath=previousGuide.imageStoragePath||'',guideImageName=previousGuide.imageName||'',guideImageType=previousGuide.imageType||'',guideImageDataUrl=previousGuide.imageDataUrl||'';
      const removeGuideImage=Boolean(d.querySelector('#s6001GuideRemoveImage')?.checked);const guideFile=d.querySelector('#s6001GuideImage')?.files?.[0];
      if(removeGuideImage){if(guideImageStoragePath)await deleteManagedImage(guideImageStoragePath);guideImageUrl='';guideImageStoragePath='';guideImageName='';guideImageType='';guideImageDataUrl='';}
      if(guideFile){const compressed=await stage328CompressGuideImage(guideFile);const tid=String(state.tournament?.id||state.multiTournament?.activeTournamentId||'current');const uploaded=await uploadManagedImage({folder:'tournamentGuides',ownerId:tid,dataUrl:compressed.dataUrl,fileName:compressed.name,contentType:compressed.type,previousPath:guideImageStoragePath});guideImageUrl=uploaded.url||'';guideImageStoragePath=uploaded.path||'';guideImageName=uploaded.name||compressed.name;guideImageType=uploaded.type||compressed.type;guideImageDataUrl='';}
      state.portal.guide={...previousGuide,date:d.querySelector('#s6001Date')?.value||'',startTime:d.querySelector('#s6001Time')?.value||'09:00',venue:d.querySelector('#s6001Venue')?.value||'',fee:String(d.querySelector('#s6001GuideFee')?.value||'').trim(),entryPeriod:String(d.querySelector('#s6001GuideEntryPeriod')?.value||'').trim(),eligibility:String(d.querySelector('#s6001GuideEligibility')?.value||'').trim(),matchFormat:String(d.querySelector('#s6001GuideMatchFormat')?.value||'').trim(),awards:String(d.querySelector('#s6001GuideAwards')?.value||'').trim(),refundPolicy:String(d.querySelector('#s6001GuideRefund')?.value||'').trim(),contact:String(d.querySelector('#s6001GuideContact')?.value||'').trim(),organizer:String(d.querySelector('#s6001GuideOrganizer')?.value||'').trim(),detail:String(d.querySelector('#s6001GuideDetail')?.value||'').trim(),imageUrl:guideImageUrl,imageStoragePath:guideImageStoragePath,imageName:guideImageName,imageType:guideImageType,imageDataUrl:guideImageDataUrl};
      ...
      state.updatedAt=now();saveState(state);await pushStateNow(state);
      dirty=false;setSaveText('저장이 완료되었습니다.','대회와 모든 부서 설정이 서버에 반영되었습니다.','success');notice('대회 편집 내용을 서버에 저장했습니다.','success');
      try{renderDivisionWorkspaceBar();renderTournamentList();renderHomeTournamentCards?.();renderPortalViews();}catch(_e){}
      setTimeout(()=>d.close(),450);
    }catch(error){setSaveText('서버 저장에 실패했습니다.',error?.message||String(error),'error');notice(`대회 편집 저장 실패: ${error?.message||error}`,'error');}
  ...
  function installHandlers(){
    document.addEventListener('click',e=>{
      ...
      if(e.target.closest?.('[data-s6001-open-guide]')){e.preventDefault();const dlg=document.getElementById('stage6001TournamentEditor');if(dlg?.open)dlg.close();navigatePortalView('guide',{pushHistory:true});return;}
...
(function stage3569RuntimeAndCourtRouteFix(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
  console.info('[230MATCH] 50.0.6 runtime loaded');
})();
...
(()=>{
  ...
  const start=()=>{installDialog();enhanceAll();observer.observe(document.body,{childList:true,subtree:true});};
