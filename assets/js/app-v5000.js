...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow}from'./sync-engine-v5000.js?v=5000';
...
const BUILD_LABEL='230MATCH 3.0.0 · 클린 다중대회 운영본';
...
(function stage3569RuntimeAndCourtRouteFix(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
  console.info('[230MATCH] 50.0.0 ready · runtime errors removed and viewer court route fixed');
})();
...
function ensureMultiTournamentRuntime(){
  ...
  const hasNamedTournament=Boolean(String(state.tournament?.name||'').trim());
