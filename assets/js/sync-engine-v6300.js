...
const STORAGE_MODE='chunked-workspace-v1';
const SAVE_DEBOUNCE=2500;
const CACHE_DEBOUNCE=12000;
const TRANSIENT_RETRY=12000;
...
function compactCloneWithoutRegistry(source){if(!source||typeof source!=='object')return null;const shallow={...source};delete shallow.multiTournament;return clone(shallow);}
...
async function readInitialBundle(){
  const [roomSnap,tournamentSnaps]=await Promise.all([api.getDoc(roomRef()),api.getDocs(tournamentsCollection())]);const room=roomSnap.exists()?roomSnap.data():{};const rows=parentRowsFromSnaps(tournamentSnaps);if(!rows.length)return null;
  ...
  const decodedRows=await Promise.all(rows.map(async row=>{if(row.id===activeRow.id)return{row,decoded:activeDecoded};const decoded=await decodeWorkspace(row.raw);if(decoded){decoded.tournament={...(decoded.tournament||{}),id:row.id};if(Array.isArray(row.raw.workspaceChunks))knownChunkIds.set(row.id,[...row.raw.workspaceChunks]);}return{row,decoded};}));
  const meta=decodedRows.map(({row,decoded})=>metaFromRaw(row.id,row.raw,decoded||null));
  const state=normalizeState(clone(activeDecoded));state.multiTournament={activeTournamentId:activeRow.id,tournaments:meta,noActiveTournament:false};localStorage.setItem('230match-v7-active-tournament',activeRow.id);lastKnownRoomRevision=Number(room.revision||0);lastWriterUid=String(room.lastWriterUid||'');lastSavedDigest=String(activeRow.raw.workspaceDigest||'');return{state,room,count:meta.length};
}
...
async function writeCurrentTournament(source){
  ...
  const rt=await runtime({requireUser:true}),id=safeId(workspace.tournament.id),d=digestWorkspace(workspace);if(d&&d===lastSavedDigest)return true;
  const encoded=await encodeWorkspace(workspace),parts=splitPayload(encoded.data),newIds=parts.map((_,i)=>chunkId(id,i)),oldIds=await previousChunksFor(id);
  ...
  batch.set(tournamentRef(id),{
    ...
    workspaceChunks:newIds,workspaceEncoding:encoded.encoding,workspaceOriginalBytes:encoded.originalBytes,workspaceStoredBytes:encoded.storedBytes,workspaceDigest:d,
    workspaceJson:api.deleteField(),workspace:api.deleteField(),lastWriterUid:rt.user.uid,lastWriterEmail:rt.user.email||'',serverUpdatedAt:api.serverTimestamp()
  },{merge:true});
  batch.set(roomRef(),{schemaVersion:8,roomId:ROOM_ID,activeTournamentId:id,revision:api.increment(1),lastWriterUid:rt.user.uid,lastWriterEmail:rt.user.email||'',serverUpdatedAt:api.serverTimestamp()},{merge:true});
  await batch.commit();knownChunkIds.set(id,newIds);lastSavedDigest=d;lastWriterUid=rt.user.uid;localStorage.setItem('230match-v7-active-tournament',id);scheduleCache();return true;
}
...
async function flush(){
  ...
  try{await ensureDb();const saved=await writeCurrentTournament(getStateFn());if(saved)status('클라우드 저장 완료','success','현재 대회를 여러 안전 조각으로 나누어 저장했습니다.');}
...
function schedule(){if(applyingRemote)return;const state=getStateFn();if(!activeIdOf(state)||!isRealTournament(state))return;dirtyGeneration++;queueFlush(SAVE_DEBOUNCE);status('저장 대기','info','입력이 끝난 뒤 유휴 시간에 현재 대회만 저장합니다.');}
function onSaved(){schedule();}
async function handleRoomSnapshot(snap){if(!snap.exists())return;const room=snap.data()||{},rt=await runtime(),writer=String(room.lastWriterUid||''),revision=Number(room.revision||0),targetId=String(room.activeTournamentId||'');if(writer&&rt?.user?.uid&&writer===rt.user.uid){lastKnownRoomRevision=Math.max(lastKnownRoomRevision,revision);return;}if(revision&&revision<=lastKnownRoomRevision)return;lastKnownRoomRevision=Math.max(lastKnownRoomRevision,revision);if(!targetId)return;try{const current=getStateFn(),registry=current?.multiTournament?.tournaments||[],bundle=await readOneTournament(targetId,registry);if(bundle)applyState(bundle.state,'remote');}catch(error){status('다른 기기 반영 실패','warning',error?.message||String(error));}}
...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();const remoteBundle=await readInitialBundle(),requested=String(remoteBundle?.state?.multiTournament?.activeTournamentId||localStorage.getItem('230match-v7-active-tournament')||''),cached=await readCachedState(requested);let chosen=chooseInitial(cached,remoteBundle?.state||null);if(cached&&chosen===cached&&remoteBundle?.state)chosen=mergeRemoteRegistry(chosen,remoteBundle.state);if(chosen){applyState(chosen,cached&&chosen===cached?'cache':'firebase');if(cached&&chosen===cached&&canWriteFn())schedule();}unsubscribeRoom=api.onSnapshot(roomRef(),snap=>{void handleRoomSnapshot(snap);},e=>status('실시간 연결 오류','warning',e?.message||String(e)));status('실시간 연결','success','저부하 단일 리스너 + 분할 저장으로 연결되었습니다.');return true;}
