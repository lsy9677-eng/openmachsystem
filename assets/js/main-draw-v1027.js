/* v1027 DEV legacy quarantine round status wrapper - latest file. */
/* v1026 DEV true isolated clean main draw mode
...
(function(){
  'use strict';
  if(window.__V1026_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1026_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1026 uses its own guard and filename so cached older scripts cannot block this patch.
  const VERSION = 'v1026-dev-true-isolated-main-view-fix';
...
(function(){
  ...
  try{console.log('[v1026] dev true isolated main view fix loaded');}catch(e){}
})();
...
(function(){
  ...
  try{console.log('[v1026] final stamp/isolation loaded');}catch(e){}
})();
...
(function(){
  'use strict';
  if(window.__V1027_MAIN_DRAW_LEGACY_QUARANTINE_INSTALLED) return;
  window.__V1027_MAIN_DRAW_LEGACY_QUARANTINE_INSTALLED = true;
  const VERSION='v1027-dev-legacy-quarantine-round-status-fix';
  function S(v){return String(v==null?'':v).replace(/\s+/g,' ').trim();}
  ...
  function hardHide(el, why){
    try{
      ...
      el.classList.add('v1027-legacy-main-quarantined');
      el.dataset.v1027Hidden = why || 'legacy-main';
      el.style.setProperty('display','none','important');
      el.style.setProperty('visibility','hidden','important');
      el.style.setProperty('pointer-events','none','important');
    }catch(e){}
  ...
  function hideLegacyMain(){
    ...
    }catch(e){console.warn('[v1027] legacy quarantine failed', e);}
  }
  function stamp(){
    try{
      const p=$('v1003MainPanel');
  ...
  function reinforceRoundStatus(){
    ...
    }catch(e){console.warn('[v1027] round status failed', e);}
  }
  function keepCleanVisible(){
    try{
      const p=$('v1003MainPanel');
      if(p){
        p.classList.remove('v1027-legacy-main-quarantined','v1026-legacy-main-hidden','v1025-legacy-main-hidden','v1023-legacy-main-hidden','v1003-legacy-hidden');
  ...
  window.v1027ApplyMainDrawIsolation=apply;
  try{
    const oldRender=window.renderBracket;
    if(typeof oldRender==='function' && !oldRender.__v1027IsolationWrapped){
      ...
      wrapped.__v1027IsolationWrapped=true; window.renderBracket=wrapped;
    }
  }catch(e){}
  try{
    const mo=new MutationObserver(()=>{clearTimeout(window.__v1027IsoTimer); window.__v1027IsoTimer=setTimeout(apply,80);});
    mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});
  }catch(e){}
  if(!document.getElementById('v1027LegacyQuarantineStyle')){
    const st=document.createElement('style'); st.id='v1027LegacyQuarantineStyle';
    st.textContent='.v1027-legacy-main-quarantined{display:none!important;visibility:hidden!important;pointer-events:none!important} #v1003MainPanel{display:block!important;visibility:visible!important}';
    document.head.appendChild(st);
  }
  [0,120,350,800,1500,2800,5000].forEach(t=>setTimeout(apply,t));
  setInterval(apply,2500);
  try{console.log('[v1027] legacy quarantine and round status patch loaded');}catch(e){}
})();
