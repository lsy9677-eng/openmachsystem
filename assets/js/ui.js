...
function renderOperationSharedQueue(state,handlers){
  ...
  root.innerHTML=[...venueMap.entries()].map(([venueId,venueName])=>{
    ...
    return`<section class="venue-queue-section operation-queue-section"><header><h3>📍 ${venueName} 본선 공용대기</h3><span>${queue.length}경기</span></header><div class="venue-queue-cards operation-queue-cards">${queue.length?queue.map((id,i)=>{const m=findMatch(state.draw,id);return`<article class="queue-card operation-queue-card" data-main-match-id="${id}"><span class="num">${i+1}</span><b>${m?`${operationalTeamHtml(m.teamA,state)} vs ${operationalTeamHtml(m.teamB,state)}`:publicMatchFallback(id)}</b><em>${m?`${roundLabel(m.roundSize)}${internalMatchIdHtml(id)}`:'경기 없음'}</em>${m?`<span class="time-badge">대기 ${Math.max(0,Math.floor((Date.now()-new Date(m.waitStartedAt||Date.now()).getTime())/60000))}분 경과 · 코트 배정 대기</span>`:''}<div class="queue-card-actions"><button class="btn btn-light" data-op-queue-up="${id}" data-venue-id="${venueId}" ${i===0?'disabled':''}>▲</button><button class="btn btn-light" data-op-queue-down="${id}" data-venue-id="${venueId}" ${i===queue.length-1?'disabled':''}>▼</button><button class="btn btn-primary" data-manual-assign="${id}" data-venue-id="${venueId}">${availableCourtSlots(state,venueId).length?'수동 코트배정':'배정 자리 없음'}</button><button class="btn btn-purple" data-admin-queue="${id}" data-venue-id="${venueId}">코트 대기 지정</button><button class="btn btn-secondary" data-op-queue-move="${id}" data-venue-id="${venueId}">구장 이동</button></div></article>`}).join(''):'<div class="empty-state"><p>대기 경기 없음</p></div>'}</div></section>`;
  }).join('');
...
console.info('[230MATCH] ui 5.9.25 · hold controls removed; queue ordering controls retained');
