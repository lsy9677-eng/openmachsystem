...
function renderQueue(state,handlers){
  ...
  root.innerHTML=venues.map(v=>{
    const queue=[...new Set((queues[v.id]||[]).map(id=>String(id)))];
    return`<section class="venue-queue-section"><header><h3>📍 ${v.name} 공용대기</h3><span>${queue.length}경기</span></header>
      <div class="venue-queue-cards">${queue.length?queue.map((id,i)=>{const m=findMatch(state.draw,id);const held=state.operation?.heldMatches?.find(x=>String(x.matchId)===String(id));const isHeld=Boolean(held||m?.status==='held');return`<article class="queue-card operation-queue-card ${isHeld?'stage5918-held-queue-card':''}" data-main-match-id="${id}"><span class="num">${i+1}</span>${isHeld?'<span class="stage5918-hold-badge">보류</span>':''}<b>${m?`${operationalTeamHtml(m.teamA,state)} vs ${operationalTeamHtml(m.teamB,state)}`:publicMatchFallback(id)}</b><em>${m?`${roundLabel(m.roundSize)}${internalMatchIdHtml(id)}`:'경기 없음'}</em>${m?(isHeld?`<span class="time-badge stage5918-held-time">보류 중 · 자동배정 제외</span>`:sharedQueueTimeBadgeHtml(m)):''}${isHeld?`<div class="stage5924-hold-reason"><strong>보류 사유</strong> ${held?.reason||m?.holdReason||'운영자 보류'}</div>`:''}<div class="queue-card-actions"><button class="btn btn-light" data-queue-up="${id}" data-venue-id="${v.id}" ${i===0?'disabled':''}>▲</button><button class="btn btn-light" data-queue-down="${id}" data-venue-id="${v.id}" ${i===queue.length-1?'disabled':''}>▼</button><button class="btn btn-primary" data-manual-assign="${id}" data-venue-id="${v.id}" ${isHeld?'disabled title="보류 해제 후 배정할 수 있습니다."':''}>${availableCourtSlots(state,v.id).length?'코트배정':'배정 자리 없음'}</button><button class="btn btn-purple" data-admin-queue="${id}" data-venue-id="${v.id}" ${isHeld?'disabled title="보류 해제 후 이동할 수 있습니다."':''}>코트 대기 지정</button><button class="btn btn-secondary" data-queue-move="${id}" data-venue-id="${v.id}" ${isHeld?'disabled title="보류 해제 후 이동할 수 있습니다."':''}>구장 이동</button><button class="btn ${isHeld?'btn-warning':'btn-danger-outline'}" data-hold-main="${id}">${isHeld?'보류 해제':'보류'}</button></div></article>`}).join(''):'<div class="empty-state"><p>대기 경기 없음</p></div>'}</div>
    </section>`;
  }).join('');
...
function renderOperationSharedQueue(state,handlers){
  ...
  root.innerHTML=[...venueMap.entries()].map(([venueId,venueName])=>{
    const queue=[...new Set((queues[venueId]||[]).map(id=>String(id)))];
    return`<section class="venue-queue-section operation-queue-section"><header><h3>📍 ${venueName} 본선 공용대기</h3><span>${queue.length}경기</span></header><div class="venue-queue-cards operation-queue-cards">${queue.length?queue.map((id,i)=>{const m=findMatch(state.draw,id);const held=state.operation?.heldMatches?.find(x=>String(x.matchId)===String(id));const isHeld=Boolean(held||m?.status==='held');return`<article class="queue-card operation-queue-card ${isHeld?'stage5918-held-queue-card':''}" data-main-match-id="${id}"><span class="num">${i+1}</span>${isHeld?'<span class="stage5918-hold-badge">보류</span>':''}<b>${m?`${operationalTeamHtml(m.teamA,state)} vs ${operationalTeamHtml(m.teamB,state)}`:publicMatchFallback(id)}</b><em>${m?`${roundLabel(m.roundSize)}${internalMatchIdHtml(id)}`:'경기 없음'}</em>${m?(isHeld?`<span class="time-badge stage5918-held-time">보류 중 · 자동배정 제외</span>`:`<span class="time-badge">대기 ${Math.max(0,Math.floor((Date.now()-new Date(m.waitStartedAt||Date.now()).getTime())/60000))}분 경과 · 코트 배정 대기</span>`):''}${isHeld?`<div class="stage5924-hold-reason"><strong>보류 사유</strong> ${held?.reason||m?.holdReason||'운영자 보류'}</div>`:''}<div class="queue-card-actions"><button class="btn btn-light" data-op-queue-up="${id}" data-venue-id="${venueId}" ${i===0?'disabled':''}>▲</button><button class="btn btn-light" data-op-queue-down="${id}" data-venue-id="${venueId}" ${i===queue.length-1?'disabled':''}>▼</button><button class="btn btn-primary" data-manual-assign="${id}" data-venue-id="${venueId}" ${isHeld?'disabled title="보류 해제 후 배정할 수 있습니다."':''}>${availableCourtSlots(state,venueId).length?'수동 코트배정':'배정 자리 없음'}</button><button class="btn btn-purple" data-admin-queue="${id}" data-venue-id="${venueId}" ${isHeld?'disabled title="보류 해제 후 이동할 수 있습니다."':''}>코트 대기 지정</button><button class="btn btn-secondary" data-op-queue-move="${id}" data-venue-id="${venueId}" ${isHeld?'disabled title="보류 해제 후 이동할 수 있습니다."':''}>구장 이동</button><button class="btn ${isHeld?'btn-warning':'btn-danger-outline'}" data-hold-main="${id}">${isHeld?'보류 해제':'보류'}</button></div></article>`}).join(''):'<div class="empty-state"><p>대기 경기 없음</p></div>'}</div></section>`;
  }).join('');
...
console.info('[230MATCH] ui 5.9.18 · held queue cards stay visible with hold badge');

console.info('[230MATCH] ui 5.9.24 · queue ids display-deduped + one hold reason block');
