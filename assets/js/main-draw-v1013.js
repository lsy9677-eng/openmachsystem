/* v1013 clean main draw display mode
...
(function(){
  'use strict';
  if(window.__V1013_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1013_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1013 uses its own guard and filename so cached v1008~v1012 scripts cannot block this patch.
  const VERSION = 'v1013-single-panel-round-fix';
  ...
  function makeMatch(key, slot, e1, e2, venue, size){
    ...
    const m={id,phase:'main',round:0,slot,bracketN:size,localDrawSize:size,localRoundSize:size,localRoundLabel:mainRoundLabel(size,0),roundLabel:mainRoundLabel(size,0),winner:null,rubbers:[],court:'',courts:[],manualCourtTarget:'',
      ...
      venue:v,venueLabel:v,__venue:v,mainBlock:v,cleanMainDraw:true,v1013CleanMain:true,v1012CleanMain:true,v1007CleanMain:true,v1003CleanMain:true,createdAt:now(),updatedAt:now()};
  ...
  function installSharedQueueBridge(){
    ...
    }catch(e){ console.warn('[v1013] shared queue bridge failed', e); }
  }
  function installCourtBoardCleanPatch(){
    ...
    }catch(e){ console.warn('[v1013] court board clean patch failed', e); }
  }
  window.MainDrawCleanV1013={version:VERSION,ensurePanel,generateDraw,assignCourts,hideLegacyMainUi,hideLegacyTreeOnly,selectedKey,prelimComplete,resolveCleanMatches,isPrivileged:cleanMainPrivileged,normalizeCleanMatchDisplay,nameOnlyFromLabel,venueKo};
  ...
  install();
  function apply(){installSharedQueueBridge();installCourtBoardCleanPatch();ensurePanel();hideLegacyMainUi();hideLegacyTreeOnly();forcePanelVisible();}
  ...
  try{console.log('[v1013] single-panel round/name-only main draw loaded');}catch(e){}
})();
...
(function(){
  ...
  var VERSION='v1013-button-single-panel-fix';
  function $(id){return document.getElementById(id);}
  ...
  try{console.log('[v1013] main draw buttons cache-proof fixed');}catch(e){}
})();
