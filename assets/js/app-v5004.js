...
/* Stage 50.0.4 · all active tournaments visible/selectable/editable */
if(!window.__stage5004TournamentCardBound){
  window.__stage5004TournamentCardBound=true;
  document.addEventListener('click',async e=>{
    const edit=e.target.closest?.('[data-edit-tournament-id]');
    if(edit){e.preventDefault();e.stopImmediatePropagation();const id=edit.dataset.editTournamentId;if(id!==state.multiTournament?.activeTournamentId)switchTournamentWorkspace(id);setTimeout(()=>{renderTournamentList();stage329OpenTournamentEdit();},0);return;}
    const del=e.target.closest?.('[data-delete-tournament-id]');
    if(del){e.preventDefault();e.stopImmediatePropagation();await deleteTournamentById(del.dataset.deleteTournamentId);}
  },true);
}
document.documentElement.dataset.build='5004';
console.info('[230MATCH] 50.0.4 ready · all tournaments shown with select/edit/delete');
