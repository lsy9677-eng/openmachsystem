...
function entry(x,i){return {id:t(x?.id||x?.teamId||(x?.ti!=null?`team-${x.ti}`:`slot-${i+1}`)),ti:x?.ti!=null?n(x.ti,null):null,name:t(x?.nm||x?.name||x?.teamName||x?.label||`본선 자리 ${i+1}`),group:x?.gn??x?.group??null,rank:n(x?.rk||x?.rank||2,2),placeholder:!!x?.placeholder||x?.ti==null,raw:clone(x)}}
function extractQualifiers(key=selectedKey()){
  ...
  return {key,tid,div,source,entries:rows.map(entry),draw:clone(draw)};
}
...
const API={version:VERSION,selectedKey,extractQualifiers,createShadowDraw,validatePlan,printPlan,compareWithLegacy,downloadPlan};window.V2MainEngineShadow=API;console.log(`[V2-MAIN-ENGINE-SHADOW] v${VERSION} ready · no writes · no UI hooks`);export default API;
