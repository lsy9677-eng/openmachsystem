...
(function(){
  ...
  const VERSION = 'v1036-authoritative-main-bracket-balanced-fix';
  ...
  try{console.log('[v1035] clean main only balanced engine loaded');}catch(e){}
})();
...
(function(){
  ...
  try{console.log('[v1035] balanced queue watchdog installed');}catch(e){}
})();
...
/* v1022: balanced court distribution override
   - Do not assign 64강 cards by visible slot order 1,2,3...
   - Use a prebuilt balanced queue order per venue segment: 1, last, middle, 2, last-1, middle-1...
   - Only ready cards (actual name vs actual name) enter current/wait1/shared queues.
*/
(function(){
  'use strict';
  if(window.__V1022_BALANCED_COURT_DISTRIBUTION_FIX__) return;
  window.__V1022_BALANCED_COURT_DISTRIBUTION_FIX__ = true;
  const VERSION='v1036-authoritative-main-bracket-balanced-fix';
  const VENUE_ORDER=['국제','능동','원도심','삼계','금병','동부','장유중','기타'];
  const S=v=>String(v==null?'':v).trim();
  const ar=v=>Array.isArray(v)?v:[];
  function api(){return window.MainDrawCleanV1020||window.MainDrawCleanV1019||window.MainDrawCleanV1018||window.MainDrawCleanV1017||window.MainDrawCleanV1016||null;}
  function toast(msg,type){try{ if(typeof window.toast==='function') window.toast(msg,type||'info'); else console.log('[v1022]',msg); }catch(e){} }
  function now(){return new Date().toISOString();}
  function venueKo(v){
    const x=S(v);
    if(!x) return '';
    if(/wondosim|원도심|원도|인조/i.test(x)) return '원도심';
    if(/gukje|국제|장유국제/i.test(x)) return '국제';
    if(/neungdong|능동/i.test(x)) return '능동';
    if(/jangyu|장유중|클레이/i.test(x)) return '장유중';
    if(/geum|금병/i.test(x)) return '금병';
    if(/samgye|삼계/i.test(x)) return '삼계';
    if(/dongbu|동부/i.test(x)) return '동부';
    return x;
  }
  function venueOfCourt(c){return venueKo(c)||'기타';}
  function selectedKey(){
    try{ const a=api(); if(a&&typeof a.selectedKey==='function') return a.selectedKey()||''; }catch(e){}
    try{ const tid=(document.getElementById('brTS')||{}).value||''; const div=(document.getElementById('brDS')||{}).value||''; if(tid&&div&&div!=='__ALL__') return tid+'_'+div; }catch(e){}
    try{return Object.keys((window.G&&G.draws)||{})[0]||'';}catch(e){return '';}
  }
  function splitKey(key){const i=S(key).lastIndexOf('_'); return {tid:S(key).slice(0,i), div:S(key).slice(i+1)};}
  function balancedIndexOrder(n){
    ...
    const upper=Math.ceil(n/2);
    let low=0, high=n-1, midA=upper, midB=upper-1;
    while(out.length<n){
      [low++, high--, midA++, midB--].forEach(x=>{ if(x>=0&&x<n&&!used.has(x)){used.add(x); out.push(x);} });
      if(low>n&&high<0&&midA>n&&midB<0) break;
    }
    for(let i=0;i<n;i++) if(!used.has(i)) out.push(i);
    return out;
  }
  function mainMains(key){return ar(window.G&&G.matches&&G.matches[key]).filter(m=>m&&String(m.phase||'')==='main'&&m.cleanMainDraw&&!m.winner);}
  function expandSegments(key,count){
    const d=(window.G&&G.draws&&G.draws[key])||{};
    let segs=ar(d.mainDrawVenueSegments).map(s=>({venue:venueKo(s.venue)||'기타',start:Number(s.start||0),end:Number(s.end==null?s.start:s.end)})).filter(s=>Number.isFinite(s.start)&&Number.isFinite(s.end));
    if(segs.length) return segs.sort((a,b)=>a.start-b.start);
    const mains=mainMains(key).slice().sort((a,b)=>Number(a.slot||0)-Number(b.slot||0));
    if(!mains.length) return [{venue:'국제',start:0,end:Math.max(0,count-1)}];
    const out=[];
    mains.forEach((m,i)=>{
      const v=venueKo(m.venue||m.venueLabel||m.__venue||'국제')||'국제';
      const last=out[out.length-1];
      if(!last||last.venue!==v) out.push({venue:v,start:Number(m.slot??i),end:Number(m.slot??i)});
      else last.end=Number(m.slot??i);
    });
    return out;
  }
  function rebuildBalancedQueueSeq(key){
    key=key||selectedKey();
    const mains=mainMains(key);
    if(!key||!mains.length) return {changed:false,total:0};
    const bySlot=new Map();
    mains.forEach((m,i)=>bySlot.set(Number(m.slot??i),m));
    const maxSlot=Math.max(...mains.map((m,i)=>Number(m.slot??i)));
    const segs=expandSegments(key, maxSlot+1);
    let seq=1, changed=false;
    segs.forEach(seg=>{
      const start=Number(seg.start||0), end=Number(seg.end==null?start:seg.end);
      const len=Math.max(0,end-start+1);
      balancedIndexOrder(len).forEach(localIdx=>{
        ...
        const m=bySlot.get(slot);
        if(!m) return;
        const v=venueKo(seg.venue||m.venue||m.__venue||'국제')||'국제';
        if(m.venue!==v||m.venueLabel!==v||m.__venue!==v||m.mainBlock!==v){changed=true;}
        if(Number(m.v1022QueueSeq)!==seq||Number(m.mainQueueSeq)!==seq||Number(m.__mainQueueOrder)!==seq){changed=true;}
        m.venue=v; m.venueLabel=v; m.__venue=v; m.mainBlock=v;
        m.v1022QueueSeq=seq;
        m.mainQueueSeq=seq;
        m.v1019QueueSeq=seq;
        m.v1011QueueOrder=seq;
        m.__mainQueueOrder=seq;
        m.mainQueueLabel=`${v} 균등대기 ${seq}`;
        seq++;
      });
    });
    try{ const d=G.draws[key]; if(d){d.v1022QueueSequenceBuiltAt=now(); d.v1022QueueSequenceMode='balanced_slot_order';} }catch(e){}
    return {changed,total:seq-1};
  }
  function mainQueueNumber(m){
    const n=Number(m?.v1022QueueSeq ?? m?.mainQueueSeq ?? m?.v1019QueueSeq ?? m?.__mainQueueOrder ?? m?.v1011QueueOrder);
    if(Number.isFinite(n)&&n>0) return n;
    const s=Number(m?.slot);
    return Number.isFinite(s)?10000+s:999999;
  }
  function cleanName(v){
    return S(v).replace(/\s*\([^)]*\)/g,'').replace(/\s*\/\s*/g,' / ').replace(/\s+/g,' ').trim();
  }
  function placeholder(v){
    const s=S(v);
    if(!s) return true;
    if(/^TBD$/i.test(s)) return true;
    if(/슬롯\s*대기|이전\s*라운드|대기/.test(s)) return true;
    if(/^\d+\s*조\s*[12]\s*위$/.test(s)) return true;
    if(/^\d+조\s*[12]위$/.test(s)) return true;
    return false;
  }
  function sideName(m,side){
    const n=side===1?S(m.display1||m.name1||m.team1Name||m.source1Name):S(m.display2||m.name2||m.team2Name||m.source2Name);
    if(n && !placeholder(n)) return cleanName(n);
    const src=side===1?S(m.source1Label):S(m.source2Label);
    if(src && !placeholder(src)) return cleanName(src);
    return n||src;
  }
  function readyForCourt(m){
    const a=sideName(m,1), b=sideName(m,2);
    return !!(a&&b&&!placeholder(a)&&!placeholder(b));
  }
  function clearCleanCourtFields(m){
    m.court=''; m.currentCourt=''; m.courts=[]; m.manualCourtTarget=''; m.__sharedCourtLabel='';
    m.manualSharedHold=false; m.__manualSharedHold=false; m.v1011SharedWait=false;
    delete m.waitingFirstAt; delete m.lastWaitingFirstAt; delete m.courtAssignedAt; delete m.manualCourtPinnedAt; delete m.courtQueueOrder;
  }
  function qIso(m,offset){return new Date(Date.UTC(2000,0,1,0,0,Math.max(1,mainQueueNumber(m))+(offset||0))).toISOString();}
  function markCurrent(m,c,iso){
    clearCleanCourtFields(m);
    m.court=c; m.currentCourt=c; m.courts=[c]; m.manualCourtTarget=c;
    m.courtAssignedAt=iso; m.manualCourtPinnedAt=iso; m.courtQueueOrder=qIso(m,0); m.v1022CourtState='current'; m.updatedAt=now();
  }
  function markWait1(m,c,iso){
    clearCleanCourtFields(m);
    m.court=''; m.currentCourt=''; m.courts=[c]; m.manualCourtTarget=c;
    m.waitingFirstAt=iso; m.courtAssignedAt=iso; m.manualCourtPinnedAt=iso; m.courtQueueOrder=qIso(m,1); m.v1022CourtState='wait1'; m.updatedAt=now();
  }
  function markShared(m,c,iso){
    clearCleanCourtFields(m);
    m.__sharedCourtLabel=c; m.v1011SharedCourt=c; m.manualSharedHold=true; m.__manualSharedHold=true; m.v1011SharedWait=true;
    m.waitingFirstAt=iso; m.courtAssignedAt=iso; m.manualCourtPinnedAt=iso; m.courtQueueOrder=qIso(m,2); m.v1022CourtState='shared'; m.updatedAt=now();
  }
  function isCleanAssigned(m){return !!(S(m.court)||S(m.manualCourtTarget)||ar(m.courts).length||S(m.__sharedCourtLabel)||m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait);}
  function venueGroups(key){
    try{ const a=api(); if(a&&typeof a.venueGroups==='function'){ const got=a.venueGroups(key); if(got&&got.length) return got.map(g=>({venue:venueKo(g.venue),courts:ar(g.courts).map(S).filter(Boolean)})); } }catch(e){}
    const map={};
    try{ ar((api()&&api().mainCourts&&api().mainCourts(key))||[]).forEach(c=>{const v=venueOfCourt(c); (map[v]||(map[v]=[])).push(c);}); }catch(e){}
    return Object.keys(map).map(v=>({venue:v,courts:map[v]})).sort((a,b)=>(VENUE_ORDER.indexOf(a.venue)<0?999:VENUE_ORDER.indexOf(a.venue))-(VENUE_ORDER.indexOf(b.venue)<0?999:VENUE_ORDER.indexOf(b.venue)));
  }
  function buildOcc(key,excludeIds){
    const vg=venueGroups(key); const byVenue={}; vg.forEach(v=>byVenue[v.venue]=ar(v.courts).slice());
    const all=Object.values(byVenue).flat(); const occ={}; all.forEach(c=>occ[c]={cur:0,wait:0});
    ar(G.matches&&G.matches[key]).filter(m=>m&&m.winner==null&&!excludeIds.has(S(m.id))).forEach(m=>{
      const c=S(m.court||m.currentCourt||''); if(c&&occ[c]) occ[c].cur++;
      const w=S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0])||''); if(w&&occ[w]) occ[w].wait++;
    });
    return {vg,byVenue,all,occ};
  }
  function rotate(list,start){list=ar(list); if(!list.length)return list; start=((Number(start)||0)%list.length+list.length)%list.length; return list.slice(start).concat(list.slice(0,start));}
  function placeBalanced(key,matches){
    const list=ar(matches).filter(readyForCourt).sort((a,b)=>mainQueueNumber(a)-mainQueueNumber(b)||Number(a.slot||0)-Number(b.slot||0));
    const excludeIds=new Set(list.map(m=>S(m.id)));
    const {vg,byVenue,all,occ}=buildOcc(key,excludeIds);
    if(!all.length) return {currentCnt:0,waitCnt:0,sharedCnt:0,noCourts:true};
    let currentCnt=0,waitCnt=0,sharedCnt=0;
    const base=Date.now();
    list.forEach((m,i)=>{
      const v=venueKo(m.venue||m.venueLabel||m.__venue)||vg[0]?.venue||'국제';
      const courts=byVenue[v]&&byVenue[v].length?byVenue[v]:all;
      const rotated=rotate(courts,mainQueueNumber(m)-1);
      const iso=new Date(base+i*1000).toISOString();
      let done=false;
      for(const c of rotated){ const o=occ[c]||(occ[c]={cur:0,wait:0}); if(o.cur===0){ markCurrent(m,c,iso); o.cur++; currentCnt++; done=true; break; } }
      if(!done){ for(const c of rotated){ const o=occ[c]||(occ[c]={cur:0,wait:0}); if(o.wait===0){ markWait1(m,c,iso); o.wait++; waitCnt++; done=true; break; } } }
      if(!done){ markShared(m,rotated[0]||all[0]||'',iso); sharedCnt++; }
    });
    return {currentCnt,waitCnt,sharedCnt,noCourts:false};
  }
  async function persist(key){
    try{ if(window.__FB_WRITE_CACHE){ if(__FB_WRITE_CACHE.draws) delete __FB_WRITE_CACHE.draws[key]; if(__FB_WRITE_CACHE.matches) delete __FB_WRITE_CACHE.matches[key]; } }catch(e){}
    try{ if(typeof window.stM==='function') await window.stM(key); }catch(e){console.warn('[v1022] stM failed',e);}
    try{ if(typeof window.stD==='function') await window.stD(key); }catch(e){console.warn('[v1022] stD failed',e);}
    try{ if(typeof window.waitForPendingWrites==='function'&&window.db) await window.waitForPendingWrites(window.db); }catch(e){}
  }
  async function renderAfter(key){
    try{ if(typeof window.renderBracket==='function') window.renderBracket(); }catch(e){}
    setTimeout(()=>{try{ const a=api(); if(a){a.ensurePanel&&a.ensurePanel(); a.hideLegacyMainUi&&a.hideLegacyMainUi();} }catch(e){}},120);
  }
  async function assignCourtsBalanced(key){
    key=key||selectedKey(); if(!key){toast('대회와 부서를 먼저 선택하세요','error');return false;}
    const a=api(); try{a&&a.resolveCleanMatches&&a.resolveCleanMatches(key,true);}catch(e){}
    rebuildBalancedQueueSeq(key);
    const mains=mainMains(key);
    if(!mains.length){toast('새 본선 대진이 없습니다. 먼저 새 본선 추첨을 실행하세요.','info');return false;}
    const ready=mains.filter(readyForCourt);
    mains.forEach(clearCleanCourtFields);
    try{ const d=G.draws[key]; if(d){d.v1022BalancedQueueEnabled=true; d.v1019MainQueueEnabled=true; d.v1022BalancedQueueAssignedAt=now(); d.v1022MainCourtAssignment='balanced_slot_order_current_wait_shared';} }catch(e){}
    if(!ready.length){ await persist(key); await renderAfter(key); toast('본선 자동배정 ON: 양쪽 팀명이 확정되는 카드부터 균등 순서로 자동 투입됩니다.','success'); return true; }
    const res=placeBalanced(key,ready);
    if(res.noCourts){toast('본선 사용 코트를 찾을 수 없습니다.','error');return false;}
    await persist(key); await renderAfter(key);
    toast(`본선 균등 코트배정 완료: 현재 ${res.currentCnt} · 대기1 ${res.waitCnt} · 공용대기 ${res.sharedCnt}`,'success');
    return true;
  }
  async function refreshBalanced(key,reason){
    key=key||selectedKey(); if(!key) return false;
    const d=(G.draws&&G.draws[key])||{};
    const manual=/manual|button|assign|force/i.test(String(reason||''));
    if(!manual&&!d.v1022BalancedQueueEnabled&&!d.v1019MainQueueEnabled&&!d.v1011MainCourtAssignment) return false;
    const a=api(); try{a&&a.resolveCleanMatches&&a.resolveCleanMatches(key,true);}catch(e){}
    rebuildBalancedQueueSeq(key);
    const unassigned=mainMains(key).filter(m=>readyForCourt(m)&&!isCleanAssigned(m));
    if(!unassigned.length){ if(manual) toast('본선 큐 갱신: 새로 배정할 확정 카드가 없습니다.','info'); return false; }
    try{ if(G.draws&&G.draws[key]){G.draws[key].v1022BalancedQueueEnabled=true; G.draws[key].v1019MainQueueEnabled=true; G.draws[key].v1022QueueRefreshedAt=now();} }catch(e){}
    const res=placeBalanced(key,unassigned);
    if(res.noCourts){ if(manual) toast('본선 사용 코트를 찾을 수 없습니다.','error'); return false; }
    await persist(key); await renderAfter(key);
    if(manual) toast(`본선 큐 갱신: 신규 ${unassigned.length}경기 · 현재 ${res.currentCnt} · 대기1 ${res.waitCnt} · 공용 ${res.sharedCnt}`,'success');
    return true;
  }
  async function refreshAll(reason,quiet){
    const keys=Object.keys((window.G&&G.matches)||{}).filter(k=>mainMains(k).length);
    let n=0;
    for(const k of keys){ try{ if(await refreshBalanced(k,reason||'watchdog')) n++; }catch(e){console.warn('[v1022] refresh failed',k,e);} }
    if(n&&!quiet) toast('본선 큐 균등 갱신 완료: '+n+'개 부서','success');
    return n>0;
  }
  function patchApi(){
    const a=api(); if(!a) return;
    a.version=VERSION;
    a.assignCourts=assignCourtsBalanced;
    a.refreshLiveMainQueue=refreshBalanced;
    a.mainQueueNumber=mainQueueNumber;
    a.rebuildBalancedQueueSeq=rebuildBalancedQueueSeq;
    a.balancedIndexOrder=balancedIndexOrder;
    window.MainDrawCleanV1022=a;
    window.MainDrawCleanV1021=a;
    window.v1022RefreshAllCleanMainQueues=function(){return refreshAll('manual_button',false);};
    window.v1018RefreshAllCleanMainQueues=window.v1022RefreshAllCleanMainQueues;
    window.v1017RefreshAllCleanMainQueues=window.v1022RefreshAllCleanMainQueues;
    window.v773RunMainAssign=function(){return assignCourtsBalanced(selectedKey());};
  }
  function bindButtons(){
    patchApi();
    try{ const p=document.getElementById('v1003MainPanel'); if(p){ const title=p.querySelector('.v1003-title small'); if(title) title.textContent=VERSION; } }catch(e){}
    ['v1003AssignBtn','v1005AssignBtn','v1005FloatAssign'].forEach(id=>{ const b=document.getElementById(id); if(b&&!b.__v1022Assign){ b.__v1022Assign=true; b.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault(); ev&&ev.stopPropagation&&ev.stopPropagation(); return assignCourtsBalanced(selectedKey());}; } });
    const q=document.getElementById('v1018QueueRefreshBtn');
    if(q&&!q.__v1022Refresh){ q.__v1022Refresh=true; q.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault(); ev&&ev.stopPropagation&&ev.stopPropagation(); return refreshAll('manual_button',false);}; }
  }
  [0,300,1000,2500,5000].forEach(t=>setTimeout(bindButtons,t));
  setInterval(bindButtons,2000);
  setInterval(()=>refreshAll('watchdog',true),3500);
  try{console.log('[v1035] balanced court distribution override installed');}catch(e){}
})();

/* v1035: clean-main-only source-safe patch
   - disables legacy lower main bracket/final result at source via dev.html flag
   - removes obsolete controls: 가지형 대진표 접기/펼치기, 조별 본선 연결구조 보기
   - first round clean bracket displays 64강 scores/status/result from cleanMainDraw matches
   - later round placeholder cards reflect previous-round winners visually
   - shared-wait cards show 공용대기, not a concrete court assignment
   - initial court assignment uses balanced slot order: 1,64,2,32,33,63...
*/
(function(){
  'use strict';
  if(window.__V1035_CLEAN_MAIN_ONLY_PATCH__) return;
  window.__V1035_CLEAN_MAIN_ONLY_PATCH__=true;
  window.__CLEAN_MAIN_ONLY_MODE__=true;
  const VERSION='v1036-authoritative-main-bracket-balanced-fix';
  const HIDE_CLASS='v1035-legacy-control-hidden';
  const S=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const ar=v=>Array.isArray(v)?v:[];
  const esc=s=>String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const now=()=>new Date().toISOString();
  function toast(msg,type){try{if(typeof window.toast==='function')window.toast(msg,type||'info');else console.log('[v1035]',msg);}catch(e){}}
  function api(){return window.MainDrawCleanV1022||window.MainDrawCleanV1021||window.MainDrawCleanV1020||window.MainDrawCleanV1019||null;}
  function key(){try{const a=api(); if(a&&typeof a.selectedKey==='function') return a.selectedKey()||'';}catch(e){} try{const tid=(document.getElementById('brTS')||{}).value||'';const div=(document.getElementById('brDS')||{}).value||''; if(tid&&div&&div!=='__ALL__') return tid+'_'+div;}catch(e){} try{return Object.keys((window.G&&G.matches)||{}).find(k=>ar(G.matches[k]).some(m=>m&&m.cleanMainDraw))||'';}catch(e){return '';}}
  function venueKo(v){try{const a=api(); if(a&&typeof a.venueKo==='function') return a.venueKo(v)||'';}catch(e){} const x=S(v); if(/wondosim|원도심|원도|인조/i.test(x))return'원도심'; if(/gukje|국제|장유국제/i.test(x))return'국제'; if(/neung|능동/i.test(x))return'능동'; if(/삼계/.test(x))return'삼계'; if(/금병/.test(x))return'금병'; if(/동부/.test(x))return'동부'; return x||'';}
  function roundLabel(size,round){const n=Math.max(2,Math.floor((Number(size)||64)/Math.pow(2,Number(round)||0))); if(n===2)return'결승'; if(n===4)return'준결승'; return n+'강';}
  function sizeOf(k,m){try{return Number((G.draws&&G.draws[k]&&G.draws[k].mainDrawSize)||m.bracketN||m.localDrawSize||64)||64;}catch(e){return Number(m&&m.bracketN)||64;}}
  function cleanName(v){return S(v).replace(/\s*\([^)]*\)/g,'').replace(/\s*\/\s*/g,' / ').replace(/\s+/g,' ').trim();}
  function placeholder(v){const s=S(v); if(!s)return true; if(/^TBD$/i.test(s))return true; if(/슬롯\s*대기|이전\s*라운드|대기/.test(s))return true; if(/^\d+\s*조\s*[12]\s*위$/.test(s)||/^\d+조\s*[12]위$/.test(s))return true; return false;}
  function sideName(m,side){
    const raw=side===1?S(m.display1||m.name1||m.team1Name||m.source1Name):S(m.display2||m.name2||m.team2Name||m.source2Name);
    if(raw&&!placeholder(raw)) return cleanName(raw);
    const src=side===1?S(m.source1Label):S(m.source2Label);
    if(src&&!placeholder(src)) return cleanName(src);
    return raw||src||'TBD';
  }
  function teamNameByIdx(k,idx){try{const t=ar(G&&G.teams&&G.teams[k])[Number(idx)]||{}; if(t.pairLabel)return cleanName(t.pairLabel); if(t.entryLabel)return cleanName(t.entryLabel); if(Array.isArray(t.individualPlayers)&&t.individualPlayers.length)return cleanName(t.individualPlayers.slice(0,2).map(p=>S(p&&p.name||p)).filter(Boolean).join(' / ')); if(Array.isArray(t.players)&&t.players.length)return cleanName(t.players.slice(0,2).map(S).filter(Boolean).join(' / ')); return cleanName(t.club||t.name||'');}catch(e){return'';}}
  function winnerName(k,m){try{const w=m&&m.winner; if(w==null)return''; if(String(w)===String(m.t1))return teamNameByIdx(k,m.t1)||sideName(m,1); if(String(w)===String(m.t2))return teamNameByIdx(k,m.t2)||sideName(m,2); const by=teamNameByIdx(k,w); return by||'';}catch(e){return'';}}
  function resultState(k,m){
    let sc1='',sc2='',done=false,wname='';
    try{ if(typeof window.getMatchResultState==='function'){const st=window.getMatchResultState(k,m)||{}; done=!!st.done; sc1=st.disp1??st.sc1??m.score1??m.s1??''; sc2=st.disp2??st.sc2??m.score2??m.s2??'';} }catch(e){}
    if(!done) done=m&&m.winner!=null;
    if(sc1===''||sc1==null) sc1=m.score1??m.s1??m.sc1??0;
    if(sc2===''||sc2==null) sc2=m.score2??m.s2??m.sc2??0;
    if(done) wname=winnerName(k,m)||'';
    return {done,sc1,sc2,wname};
  }
  function statusInfo(k,m){
    const size=sizeOf(k,m); const label=roundLabel(size,Number(m.round||0));
    const res=resultState(k,m);
    if(res.done) return {kind:'done', text:`${label} 완료${res.wname?' · 승자 '+res.wname:''}`};
    if(m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait) return {kind:'shared', text:`${label} · 공용대기 - ${venueKo(m.__sharedCourtLabel||m.v1011SharedCourt||m.venue||m.__venue)||'구장'}`};
    const c=S(m.court||m.currentCourt||''); if(c) return {kind:'current', text:`${label} · 시합중 - ${c}`};
    const w=S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0])||''); if(w) return {kind:'wait1', text:`${label} · 코트 대기1 - ${w}`};
    const a=sideName(m,1), b=sideName(m,2); if(!placeholder(a)&&!placeholder(b)) return {kind:'ready', text:`${label} · 배정 대기`};
    return {kind:'slot', text:'슬롯 대기'};
  }
  function mains(k){return ar(window.G&&G.matches&&G.matches[k]).filter(m=>m&&String(m.phase||'')==='main'&&m.cleanMainDraw).sort((a,b)=>Number(a.slot||0)-Number(b.slot||0));}
  function nodeWinner(k,node){
    if(!node) return '';
    if(node.match) return resultState(k,node.match).wname||'';
    const a=node.a||'', b=node.b||'';
    return node.winner||'';
  }
  function updateCleanBracketDom(k){
    try{
      k=k||key(); const panel=document.getElementById('v1003MainPanel'); if(!panel||!k)return;
      const list=mains(k); if(!list.length)return;
      const rounds=[...panel.querySelectorAll('.v1004-round')]; if(!rounds.length)return;
      const r0=[...rounds[0].querySelectorAll('.v1004-bmatch')];
      const firstNodes=[];
      r0.forEach((card,i)=>{
        const m=list[i]; if(!m)return;
        const a=sideName(m,1), b=sideName(m,2); const res=resultState(k,m); const st=statusInfo(k,m);
        const slots=card.querySelectorAll('.v1004-bslot');
        if(slots[0]) slots[0].innerHTML=`${esc(a)}<small>${esc(res.sc1)}</small>`;
        if(slots[1]) slots[1].innerHTML=`${esc(b)}<small>${esc(res.sc2)}</small>`;
        let sEl=card.querySelector('.v1004-bstatus'); if(!sEl){sEl=document.createElement('div');sEl.className='v1004-bstatus'; const src=card.querySelector('.v1004-bsrc'); if(src)card.insertBefore(sEl,src); else card.appendChild(sEl);}
        sEl.className='v1004-bstatus v1004-status-'+st.kind; sEl.textContent=st.text;
        card.className=card.className.replace(/\bv1004-(current|wait1|shared|ready|slot|done)\b/g,'').trim()+' v1004-'+st.kind;
        firstNodes.push({match:m,winner:res.wname||''});
      });
      let prev=firstNodes;
      for(let ri=1;ri<rounds.length;ri++){
        const cards=[...rounds[ri].querySelectorAll('.v1004-bmatch')];
        const next=[];
        cards.forEach((card,i)=>{
          const n1=prev[i*2], n2=prev[i*2+1];
          const a=nodeWinner(k,n1)||'TBD'; const b=nodeWinner(k,n2)||'TBD';
          const slots=card.querySelectorAll('.v1004-bslot');
          if(slots[0]) slots[0].innerHTML=`${esc(a)}<small>0</small>`;
          if(slots[1]) slots[1].innerHTML=`${esc(b)}<small>0</small>`;
          const src=card.querySelector('.v1004-bsrc'); if(src) src.textContent='이전 라운드 승자';
          next.push({a,b,winner:''});
        });
        prev=next;
      }
      const small=panel.querySelector('.v1003-title small'); if(small) small.textContent=VERSION;
    }catch(e){console.warn('[v1035] update bracket dom failed',e);}
  }
  function removeLegacyControls(){
    try{
      document.querySelectorAll('button,a').forEach(el=>{const t=S(el.textContent); if(/가지형\s*대진표\s*(펼치기|접기)|조별\s*본선\s*연결구조\s*(보기|접기)|조별\s*본선\s*연결구조\s*보기/.test(t)){el.classList.add(HIDE_CLASS); el.style.setProperty('display','none','important');}});
      // Hide exact old final result blocks only, never the page container or new panel.
      const root=document.getElementById('bracketContent')||document.getElementById('page-bracket')||document.body;
      root.querySelectorAll('.sec-title').forEach(el=>{const t=S(el.textContent); if(/^🏅?\s*최종\s*결과$/.test(t)||/^최종\s*결과$/.test(t)){el.classList.add(HIDE_CLASS); el.style.setProperty('display','none','important'); let n=el.nextElementSibling; let guard=0; while(n&&guard++<3){const tx=S(n.textContent); if(tx&&(/우승|준우승/.test(tx)||/grp-card|card/.test(String(n.className||'')))){n.classList.add(HIDE_CLASS); n.style.setProperty('display','none','important');} n=n.nextElementSibling;}}});
    }catch(e){}
  }
  // Balanced order requested by operator: 1,64,2,32,33,63,3,31,34,62...
  function balancedSlotOrder(n){
    n=Math.max(0,Number(n)||0); const out=[], used=new Set();
    const add=x=>{if(x>=0&&x<n&&!used.has(x)){used.add(x);out.push(x);}};
    const half=Math.floor(n/2);
    for(let i=0;out.length<n;i++){
      add(i);          // 1,2,3...
      add(n-1-i);      // 64,63,62...
      add(half-1-i);   // 32,31,30...
      add(half+i);     // 33,34,35...
    }
    return out;
  }
  function venueGroups(k){try{const a=api(); if(a&&typeof a.venueGroups==='function'){const got=a.venueGroups(k); if(got&&got.length)return got.map(g=>({venue:venueKo(g.venue),courts:ar(g.courts).map(S).filter(Boolean)}));}}catch(e){} return [{venue:'국제',courts:[]}];}
  function rebuildQueue(k){
    const ms=mains(k).filter(m=>m&&m.winner==null); if(!ms.length)return 0;
    const groups=venueGroups(k); const available=groups.map(g=>g.venue).filter(Boolean); const fallback=available[0]||'국제';
    const byVenue={}; ms.forEach(m=>{let v=venueKo(m.venue||m.venueLabel||m.__venue)||fallback; if(!available.includes(v)) v=fallback; (byVenue[v]||(byVenue[v]=[])).push(m); m.venue=m.venueLabel=m.__venue=m.mainBlock=v;});
    let seq=1;
    groups.forEach(g=>{const arr=ar(byVenue[g.venue]).sort((a,b)=>Number(a.slot||0)-Number(b.slot||0)); const order=balancedSlotOrder(arr.length); order.forEach(idx=>{const m=arr[idx]; if(!m)return; m.v1035QueueSeq=seq; m.v1022QueueSeq=seq; m.mainQueueSeq=seq; m.__mainQueueOrder=seq; m.v1019QueueSeq=seq; m.v1011QueueOrder=seq; seq++;});});
    Object.keys(byVenue).filter(v=>!groups.some(g=>g.venue===v)).forEach(v=>{ar(byVenue[v]).forEach(m=>{m.v1035QueueSeq=seq; m.v1022QueueSeq=seq; m.mainQueueSeq=seq; m.__mainQueueOrder=seq; seq++;});});
    try{const d=G.draws[k]; if(d){d.v1035QueueSequenceBuiltAt=now(); d.v1035QueueSequenceMode='balanced_slot_extreme_quarter_order'; d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}}catch(e){}
    return seq-1;
  }
  function qNum(m){const n=Number(m&& (m.v1035QueueSeq??m.v1022QueueSeq??m.mainQueueSeq??m.__mainQueueOrder)); return Number.isFinite(n)&&n>0?n:999999;}
  function ready(m){const a=sideName(m,1),b=sideName(m,2);return !!(a&&b&&!placeholder(a)&&!placeholder(b));}
  function clearFields(m){m.court='';m.currentCourt='';m.courts=[];m.manualCourtTarget='';m.__sharedCourtLabel='';m.manualSharedHold=false;m.__manualSharedHold=false;m.v1011SharedWait=false;delete m.waitingFirstAt;delete m.lastWaitingFirstAt;delete m.courtAssignedAt;delete m.manualCourtPinnedAt;delete m.courtQueueOrder;}
  function markCur(m,c,iso){clearFields(m);m.court=c;m.currentCourt=c;m.courts=[c];m.manualCourtTarget=c;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1035CourtState='current';m.updatedAt=now();}
  function markWait(m,c,iso){clearFields(m);m.courts=[c];m.manualCourtTarget=c;m.waitingFirstAt=iso;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1035CourtState='wait1';m.updatedAt=now();}
  function markShared(m,c,iso){clearFields(m);m.__sharedCourtLabel=c;m.v1011SharedCourt=c;m.manualSharedHold=true;m.__manualSharedHold=true;m.v1011SharedWait=true;m.waitingFirstAt=iso;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1035CourtState='shared';m.updatedAt=now();}
  function assigned(m){return !!(S(m.court)||S(m.manualCourtTarget)||ar(m.courts).length||S(m.__sharedCourtLabel)||m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait);}
  function rotate(list,start){list=ar(list); if(!list.length)return list; start=((Number(start)||0)%list.length+list.length)%list.length; return list.slice(start).concat(list.slice(0,start));}
  function occ(k,exclude){const groups=venueGroups(k);const by={};groups.forEach(g=>by[g.venue]=ar(g.courts).slice());const all=Object.values(by).flat();const o={};all.forEach(c=>o[c]={cur:0,wait:0}); ar(G.matches&&G.matches[k]).filter(m=>m&&m.winner==null&&!exclude.has(S(m.id))).forEach(m=>{const c=S(m.court||m.currentCourt);if(c&&o[c])o[c].cur++;const w=S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0]));if(w&&o[w])o[w].wait++;});return{groups,by,all,o};}
  async function persist(k){try{if(window.__FB_WRITE_CACHE){if(__FB_WRITE_CACHE.draws)delete __FB_WRITE_CACHE.draws[k];if(__FB_WRITE_CACHE.matches)delete __FB_WRITE_CACHE.matches[k];}}catch(e){} try{if(typeof window.stM==='function')await window.stM(k);}catch(e){console.warn('[v1035] stM failed',e);} try{if(typeof window.stD==='function')await window.stD(k);}catch(e){} }
  async function renderAfter(k){try{if(typeof window.renderBracket==='function')window.renderBracket();}catch(e){} setTimeout(()=>{try{api()&&api().ensurePanel&&api().ensurePanel(); updateCleanBracketDom(k); removeLegacyControls();}catch(e){}},150);}
  async function assignBalanced(k){
    k=k||key(); if(!k){toast('대회와 부서를 먼저 선택하세요','error');return false;}
    try{api()&&api().resolveCleanMatches&&api().resolveCleanMatches(k,true);}catch(e){}
    rebuildQueue(k);
    const list=mains(k).filter(m=>m.winner==null&&ready(m)).sort((a,b)=>qNum(a)-qNum(b)||Number(a.slot||0)-Number(b.slot||0));
    if(!list.length){toast('배정 가능한 본선 카드가 없습니다. 양쪽 팀명이 확정된 카드부터 배정됩니다.','info');return false;}
    list.forEach(clearFields);
    const ex=new Set(list.map(m=>S(m.id))); const state=occ(k,ex); if(!state.all.length){toast('본선 사용 코트를 찾을 수 없습니다','error');return false;}
    let cur=0,wait=0,shared=0; const base=Date.now();
    list.forEach((m,i)=>{let v=venueKo(m.venue||m.__venue)||state.groups[0].venue; if(!state.by[v]||!state.by[v].length)v=state.groups[0].venue; const courts=state.by[v]&&state.by[v].length?state.by[v]:state.all; const rotated=rotate(courts,qNum(m)-1); const iso=new Date(base+i*1000).toISOString(); let done=false; for(const c of rotated){const o=state.o[c]||(state.o[c]={cur:0,wait:0}); if(o.cur===0){markCur(m,c,iso);o.cur++;cur++;done=true;break;}} if(!done){for(const c of rotated){const o=state.o[c]||(state.o[c]={cur:0,wait:0}); if(o.wait===0){markWait(m,c,iso);o.wait++;wait++;done=true;break;}}} if(!done){markShared(m,rotated[0]||state.all[0],iso);shared++;}});
    try{const d=G.draws[k]; if(d){d.v1035MainQueueEnabled=true; d.v1035AssignedAt=now(); d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}}catch(e){}
    await persist(k); await renderAfter(k); toast(`본선 균등 코트배정 완료: 현재 ${cur} · 대기1 ${wait} · 공용 ${shared}`,'success'); return true;
  }
  async function refreshAppendOnly(k,manual){
    k=k||key(); if(!k)return false;
    try{api()&&api().resolveCleanMatches&&api().resolveCleanMatches(k,true);}catch(e){}
    rebuildQueue(k);
    const d=(G.draws&&G.draws[k])||{}; if(!manual&&!d.v1035MainQueueEnabled)return false;
    const add=mains(k).filter(m=>m.winner==null&&ready(m)&&!assigned(m)).sort((a,b)=>qNum(a)-qNum(b));
    if(!add.length){if(manual)toast('본선 큐 갱신: 새로 배정할 확정 카드가 없습니다.','info'); updateCleanBracketDom(k); return false;}
    const ex=new Set(add.map(m=>S(m.id))); const state=occ(k,ex); if(!state.all.length){if(manual)toast('본선 사용 코트를 찾을 수 없습니다','error');return false;}
    let wait=0,shared=0; const base=Date.now();
    add.forEach((m,i)=>{let v=venueKo(m.venue||m.__venue)||state.groups[0].venue; if(!state.by[v]||!state.by[v].length)v=state.groups[0].venue; const courts=state.by[v]&&state.by[v].length?state.by[v]:state.all; const rotated=rotate(courts,qNum(m)-1); const iso=new Date(base+i*1000).toISOString(); let done=false; // refresh never promotes new/shared card directly to current; it only fills wait1 or shared.
      for(const c of rotated){const o=state.o[c]||(state.o[c]={cur:0,wait:0}); if(o.cur>0&&o.wait===0){markWait(m,c,iso);o.wait++;wait++;done=true;break;}}
      if(!done){markShared(m,rotated[0]||state.all[0],iso);shared++;}
    });
    try{const dr=G.draws[k]; if(dr){dr.v1035MainQueueEnabled=true; dr.v1035RefreshedAt=now(); dr.v1022BalancedQueueEnabled=false; dr.v1019MainQueueEnabled=false; dr.v1011MainCourtAssignment='';}}catch(e){}
    await persist(k); await renderAfter(k); if(manual)toast(`본선 큐 갱신: 대기1 ${wait} · 공용 ${shared}`,'success'); return true;
  }
  function patchApi(){
    const a=api(); if(a){a.version=VERSION; a.assignCourts=assignBalanced; a.refreshLiveMainQueue=(k,r)=>refreshAppendOnly(k,/manual|button|force/i.test(String(r||''))); a.balancedIndexOrder=balancedSlotOrder; a.rebuildBalancedQueueSeq=rebuildQueue; window.MainDrawCleanV1035=a;}
    window.v1035AssignBalanced=assignBalanced; window.v1035RefreshQueue=(manual=true)=>refreshAppendOnly(key(),manual); window.v773RunMainAssign=()=>assignBalanced(key());
  }
  function bindButtons(){patchApi(); ['v1003AssignBtn','v1005AssignBtn','v1005FloatAssign'].forEach(id=>{const b=document.getElementById(id); if(b&&!b.__v1035Assign){b.__v1035Assign=true;b.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return assignBalanced(key());};}}); const q=document.getElementById('v1018QueueRefreshBtn'); if(q&&!q.__v1035Refresh){q.__v1035Refresh=true;q.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return refreshAppendOnly(key(),true);};}}
  // Stop old v1022 watchdog from reassigning by clearing its enable flags on active clean draws.
  function neutralizeOldWatchdogs(){try{Object.keys((G&&G.draws)||{}).forEach(k=>{const d=G.draws[k]; if(d&&ar(G.matches&&G.matches[k]).some(m=>m&&m.cleanMainDraw)){d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}});}catch(e){}}
  function tick(){neutralizeOldWatchdogs(); bindButtons(); removeLegacyControls(); updateCleanBracketDom(key());}
  const oldRender=window.renderBracket; if(typeof oldRender==='function'&&!oldRender.__v1035CleanOnly){const w=function(){const r=oldRender.apply(this,arguments); setTimeout(tick,0); setTimeout(tick,150); setTimeout(tick,600); return r;}; w.__v1035CleanOnly=true; w.__old=oldRender; window.renderBracket=w;}
  const oldCard=window.renderRankingCard; if(typeof oldCard==='function'&&!oldCard.__v1035OffInBracket){const w=function(){try{if(document.querySelector('#page-bracket.active')||document.getElementById('bracketContent'))return'';}catch(e){} return oldCard.apply(this,arguments);}; w.__v1035OffInBracket=true; w.__old=oldCard; window.renderRankingCard=w; try{renderRankingCard=w;}catch(e){}}
  [0,200,800,1600,3200,6000].forEach(t=>setTimeout(tick,t)); setInterval(tick,2000); setInterval(()=>refreshAppendOnly(key(),false),5000);
  try{const st=document.createElement('style');st.id='v1035CleanOnlyStyle';st.textContent='.'+HIDE_CLASS+'{display:none!important}.v1004-status-done{color:#166534;background:#dcfce7}.v1004-bmatch.v1004-done{border-color:#86efac}.v1004-status-shared{color:#0f766e!important;background:#ecfdf5!important}';document.head.appendChild(st);}catch(e){}
  try{console.log('[v1035] clean-main-only balanced patch loaded');}catch(e){}
})();

/* v1036: authoritative clean-main bracket + hard balanced assignment
   - 새 본선 패널을 유일한 본선 대진표로 사용
   - 64강 결과/점수/승자를 새 가지형 대진표에 직접 반영
   - 기존 하단 본선/최종결과/조별 연결구조 컨트롤은 안전하게 숨김
   - 코트 배정은 1,64,2,32,33,63 흐름의 극단/중앙 분산 순서를 강제 적용
*/
(function(){
  'use strict';
  if(window.__V1036_AUTHORITATIVE_MAIN_BRACKET_FIX__) return;
  window.__V1036_AUTHORITATIVE_MAIN_BRACKET_FIX__=true;
  window.__CLEAN_MAIN_ONLY_MODE__=true;
  const VERSION='v1036-authoritative-main-bracket-balanced-fix';
  const HIDE='v1036-legacy-hidden';
  const S=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const ar=v=>Array.isArray(v)?v:[];
  const esc=s=>String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const now=()=>new Date().toISOString();
  function ge(id){try{return document.getElementById(id)}catch(e){return null}}
  function toast(msg,type){try{if(typeof window.toast==='function')window.toast(msg,type||'info');else console.log('[v1036]',msg);}catch(e){}}
  function api(){return window.MainDrawCleanV1035||window.MainDrawCleanV1033||window.MainDrawCleanV1031||window.MainDrawCleanV1022||window.MainDrawCleanV1021||window.MainDrawCleanV1020||window.MainDrawCleanV1019||null;}
  function selectedKey(){
    try{const a=api(); if(a&&typeof a.selectedKey==='function'){const k=a.selectedKey(); if(k)return k;}}catch(e){}
    try{const k=S(window.__CURRENT_RENDER_KEY||window.__v650CurrentGroupLabelKey||''); if(k&&window.G&&G.draws&&G.draws[k])return k;}catch(e){}
    try{const tid=S((ge('brTS')&&ge('brTS').value)||(ge('regTS')&&ge('regTS').value)||(ge('rankTS')&&ge('rankTS').value)||''); if(tid){const ds=ge('brDS'); const div=S(ds&&ds.value); if(div&&div!=='__ALL__')return tid+'_'+div; const keys=Object.keys((window.G&&G.draws)||{}).filter(k=>k.indexOf(tid+'_')===0); if(keys.length)return keys[0];}}catch(e){}
    try{return Object.keys((window.G&&G.matches)||{}).find(k=>ar(G.matches[k]).some(m=>m&&m.cleanMainDraw))||'';}catch(e){return'';}
  }
  function venueKo(v){
    const x=S(v); if(!x)return'';
    if(/wondosim|원도심|원도|인조/i.test(x))return'원도심';
    if(/gukje|국제|장유국제/i.test(x))return'국제';
    if(/neung|능동/i.test(x))return'능동';
    if(/samgye|삼계/i.test(x))return'삼계';
    if(/geum|금병/i.test(x))return'금병';
    if(/dongbu|동부/i.test(x))return'동부';
    return x;
  }
  function courtVenue(c){return venueKo(c)||'';}
  function natural(a,b){return S(a).localeCompare(S(b),'ko',{numeric:true,sensitivity:'base'});}
  function placeholder(v){const s=S(v); if(!s)return true; if(/^TBD$/i.test(s))return true; if(/슬롯\s*대기|이전\s*라운드|부전승/.test(s))return true; if(/^\d+\s*조\s*[12]\s*위$/.test(s)||/^\d+조\s*[12]위$/.test(s))return true; return false;}
  function cleanName(v){return S(v).replace(/\s*\([^)]*\)/g,'').replace(/\s*\/\s*/g,' / ').trim();}
  function teamNameByIdx(k,idx){
    try{const t=ar(G&&G.teams&&G.teams[k])[Number(idx)]||{}; if(t.pairLabel)return cleanName(t.pairLabel); if(t.entryLabel)return cleanName(t.entryLabel); if(Array.isArray(t.individualPlayers)&&t.individualPlayers.length)return cleanName(t.individualPlayers.slice(0,2).map(p=>S(p&&p.name||p)).filter(Boolean).join(' / ')); if(Array.isArray(t.players)&&t.players.length)return cleanName(t.players.slice(0,2).map(S).filter(Boolean).join(' / ')); return cleanName(t.club||t.name||'');}catch(e){return'';}
  }
  function sideName(k,m,side){
    const ti=side===1?m.t1:m.t2;
    if(ti!=null){const by=teamNameByIdx(k,ti); if(by)return by;}
    const raw=side===1?S(m.display1||m.name1||m.team1Name||m.source1Name):S(m.display2||m.name2||m.team2Name||m.source2Name);
    if(raw&&!placeholder(raw))return cleanName(raw);
    const src=side===1?S(m.source1Label):S(m.source2Label);
    if(src&&!placeholder(src))return cleanName(src);
  ...
  function winnerName(k,m){
    try{const w=m&&m.winner; if(w==null||w==='')return''; if(String(w)===String(m.t1))return teamNameByIdx(k,m.t1)||sideName(k,m,1); if(String(w)===String(m.t2))return teamNameByIdx(k,m.t2)||sideName(k,m,2); const by=teamNameByIdx(k,w); return by||'';}catch(e){return'';}
  }
  function resultState(k,m){
    let sc1='',sc2='',done=false;
    try{if(typeof window.getMatchResultState==='function'){const st=window.getMatchResultState(k,m)||{}; done=!!st.done; sc1=st.disp1??st.sc1??st.score1??m.score1??m.s1??m.sc1??''; sc2=st.disp2??st.sc2??st.score2??m.score2??m.s2??m.sc2??'';}}catch(e){}
    if(sc1===''||sc1==null)sc1=m.score1??m.s1??m.sc1??m.disp1??0;
    if(sc2===''||sc2==null)sc2=m.score2??m.s2??m.sc2??m.disp2??0;
    if(!done)done=(m.winner!=null&&m.winner!=='' )||m.done===true||m.completed===true||m.status==='done';
    const w=done?winnerName(k,m):'';
    return {sc1,sc2,done,winner:w};
  }
  function mains(k){return ar(window.G&&G.matches&&G.matches[k]).filter(m=>m&&String(m.phase||'')==='main'&&m.cleanMainDraw).sort((a,b)=>Number(a.round||0)-Number(b.round||0)||Number(a.slot||0)-Number(b.slot||0));}
  function firstRound(k){return mains(k).filter(m=>Number(m.round||0)===0).sort((a,b)=>Number(a.slot||0)-Number(b.slot||0));}
  function sizeOf(k,ms){try{return Number((G.draws&&G.draws[k]&&G.draws[k].mainDrawSize)||ms[0]?.bracketN||ms[0]?.localDrawSize||ms.length*2||64)||64;}catch(e){return 64;}}
  function roundLabel(size,r){const n=Math.max(2,Math.floor((Number(size)||64)/Math.pow(2,Number(r)||0))); if(n===2)return'결승'; if(n===4)return'준결승'; return n+'강';}
  function statusInfo(k,m){
    const label=roundLabel(sizeOf(k,[m]),Number(m.round||0)); const rs=resultState(k,m);
    if(rs.done)return {kind:'done',text:`${label} 완료${rs.winner?' · 승자 '+rs.winner:''}`};
    if(m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait)return {kind:'shared',text:`${label} · 공용대기 - ${venueKo(m.__sharedCourtLabel||m.v1011SharedCourt||m.venue||m.__venue)||'구장'}`};
    const c=S(m.court||m.currentCourt); if(c)return {kind:'current',text:`${label} · 시합중 - ${c}`};
    const w=S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0])||''); if(w)return {kind:'wait1',text:`${label} · 코트 대기1 - ${w}`};
    const a=sideName(k,m,1),b=sideName(k,m,2); if(!placeholder(a)&&!placeholder(b))return {kind:'ready',text:`${label} · 배정 대기`};
    return {kind:'slot',text:'슬롯 대기'};
  }
  function nodeWinner(n){if(!n)return''; if(n.match)return n.result?.winner||''; return n.winner||'';}
  function buildRoundNodes(k,fr,size){
    const rounds=[];
    let prev=fr.map(m=>({match:m,result:resultState(k,m)}));
    rounds.push(prev);
    let count=Math.max(1,Math.floor(fr.length/2));
    while(count>=1){
      const next=[];
      for(let i=0;i<count;i++){
        const a=prev[i*2], b=prev[i*2+1];
        next.push({a,b,winner:'',label:'이전 라운드 승자'});
      }
      rounds.push(next);
      if(count===1)break;
      prev=next; count=Math.floor(count/2);
    }
    return rounds;
  }
  function cardHtml(k,node,i,ri,size){
    if(node.match){
      const m=node.match, rs=node.result||resultState(k,m), st=statusInfo(k,m);
      const v=venueKo(m.venue||m.venueLabel||m.__venue||m.__sharedCourtLabel)||'국제';
      const a=sideName(k,m,1), b=sideName(k,m,2);
      const win1=rs.winner&&rs.winner===a, win2=rs.winner&&rs.winner===b;
      return `<div class="v1036-bmatch v1036-${st.kind}" data-mid="${esc(m.id||'')}" data-round="${ri}" data-slot="${i}"><div class="v1036-top"><b>${i+1}경기</b><span class="v1036-venue">${esc(v)}</span></div><div class="v1036-team ${win1?'winner':''}">${esc(a)}<small>${esc(rs.sc1)}</small></div><div class="v1036-team ${win2?'winner':''}">${esc(b)}<small>${esc(rs.sc2)}</small></div><div class="v1036-status">${esc(st.text)}</div><div class="v1036-src">${esc(m.source1Label||'')} / ${esc(m.source2Label||'')}</div></div>`;
    }
    const a=nodeWinner(node.a)||'TBD', b=nodeWinner(node.b)||'TBD';
    return `<div class="v1036-bmatch v1036-placeholder" data-round="${ri}" data-slot="${i}"><div class="v1036-top"><b>${i+1}경기</b></div><div class="v1036-team">${esc(a)}<small>0</small></div><div class="v1036-team">${esc(b)}<small>0</small></div><div class="v1036-status">${a==='TBD'&&b==='TBD'?'이전 라운드 승자':'승자 대기'}</div></div>`;
  }
  function renderAuthoritative(k){
    k=k||selectedKey(); const box=ge('v1003CleanBracket'); if(!box||!k)return false;
    try{const a=api(); if(a&&typeof a.resolveCleanMatches==='function')a.resolveCleanMatches(k,true);}catch(e){}
    const fr=firstRound(k); if(!fr.length){box.innerHTML='<div class="v1036-empty">새 본선 대진이 없습니다. 새 본선 추첨을 실행하세요.</div>'; return false;}
    const size=sizeOf(k,fr); const rounds=buildRoundNodes(k,fr,size);
    const seg=(function(){try{const d=G.draws&&G.draws[k]||{}; return ar(d.mainDrawVenueSegments).map(s=>`<span class="v1036-seg">${esc(venueKo(s.venue))} ${Number(s.start)+1}~${Number(s.end)+1}경기</span>`).join('');}catch(e){return'';}})();
    box.innerHTML=`<div class="v1036-head"><b>새 본선 경기표 · ${roundLabel(size,0)}부터 시작</b><div>${seg}</div></div><div class="v1036-wrap"><div class="v1036-bracket">${rounds.map((items,ri)=>`<section class="v1036-round"><h4>${roundLabel(size,ri)}</h4><div class="v1036-round-body">${items.map((n,i)=>cardHtml(k,n,i,ri,size)).join('')}</div></section>`).join('')}</div></div>`;
    const panel=ge('v1003MainPanel'); if(panel){const sm=panel.querySelector('.v1003-title small'); if(sm)sm.textContent=VERSION;}
    return true;
  }
  function getCourts(k){
    let out=[];
    try{if(window.OperationQueueV993&&typeof OperationQueueV993.getBoardCourtsForKey==='function')out=OperationQueueV993.getBoardCourtsForKey(k)||[];}catch(e){}
    try{if(!out.length&&api()&&typeof api().mainCourts==='function')out=api().mainCourts(k)||[];}catch(e){}
    try{if(!out.length&&typeof window.getUsedCourtsForKey==='function')out=window.getUsedCourtsForKey(k)||[];}catch(e){}
    return [...new Set(ar(out).map(S).filter(Boolean))].sort(natural);
  }
  function venueGroups(k){const courts=getCourts(k); const map={}; courts.forEach(c=>{const v=venueKo(courtVenue(c))||'국제'; (map[v]||(map[v]=[])).push(c);}); let arr=Object.keys(map).map(v=>({venue:v,courts:map[v].sort(natural)})); if(!arr.length)arr=[{venue:'국제',courts:[]}]; return arr.sort((a,b)=>b.courts.length-a.courts.length||a.venue.localeCompare(b.venue,'ko'));}
  function balancedPositionOrder(matchCount){
    const posCount=matchCount*2; const positions=[]; const usedPos=new Set(); const addPos=p=>{if(p>=1&&p<=posCount&&!usedPos.has(p)){usedPos.add(p);positions.push(p);}};
    for(let i=0;positions.length<posCount&&i<posCount;i++){
      addPos(1+i); addPos(posCount-i); addPos(Math.floor(posCount/2)-i); addPos(Math.floor(posCount/2)+1+i);
    }
    const out=[], usedMatch=new Set();
    positions.forEach(p=>{const mi=Math.floor((p-1)/2); if(mi>=0&&mi<matchCount&&!usedMatch.has(mi)){usedMatch.add(mi);out.push(mi);}});
    for(let i=0;i<matchCount;i++)if(!usedMatch.has(i))out.push(i);
    return out;
  }
  function rebuildQueue(k){
    const ms=firstRound(k).filter(m=>!resultState(k,m).done); if(!ms.length)return 0;
    const groups=venueGroups(k); const avail=groups.map(g=>g.venue); const fallback=avail[0]||'국제';
    const by={}; ms.forEach(m=>{let v=venueKo(m.venue||m.venueLabel||m.__venue)||fallback; if(!avail.includes(v))v=fallback; (by[v]||(by[v]=[])).push(m); m.venue=m.venueLabel=m.__venue=m.mainBlock=v;});
    let seq=1;
    groups.forEach(g=>{const arr=ar(by[g.venue]).sort((a,b)=>Number(a.slot||0)-Number(b.slot||0)); const order=balancedPositionOrder(arr.length); order.forEach(idx=>{const m=arr[idx]; if(!m)return; m.v1036QueueSeq=seq; m.v1035QueueSeq=seq; m.v1022QueueSeq=seq; m.mainQueueSeq=seq; m.__mainQueueOrder=seq; seq++;});});
    try{const d=G.draws&&G.draws[k]; if(d){d.v1036QueueSequenceBuiltAt=now(); d.v1036QueueSequenceMode='position_extreme_balanced'; d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}}catch(e){}
    return seq-1;
  }
  function qNum(m){const n=Number(m&& (m.v1036QueueSeq??m.v1035QueueSeq??m.v1022QueueSeq??m.mainQueueSeq??m.__mainQueueOrder)); return Number.isFinite(n)&&n>0?n:999999;}
  function ready(k,m){const a=sideName(k,m,1),b=sideName(k,m,2); return a&&b&&!placeholder(a)&&!placeholder(b);}
  function clearFields(m){m.court='';m.currentCourt='';m.courts=[];m.manualCourtTarget='';m.__sharedCourtLabel='';m.manualSharedHold=false;m.__manualSharedHold=false;m.v1011SharedWait=false;delete m.waitingFirstAt;delete m.lastWaitingFirstAt;delete m.courtAssignedAt;delete m.manualCourtPinnedAt;delete m.courtQueueOrder;}
  function markCur(m,c,iso){clearFields(m);m.court=c;m.currentCourt=c;m.courts=[c];m.manualCourtTarget=c;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1036CourtState='current';m.updatedAt=now();}
  function markWait(m,c,iso){clearFields(m);m.courts=[c];m.manualCourtTarget=c;m.waitingFirstAt=iso;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1036CourtState='wait1';m.updatedAt=now();}
  function markShared(m,c,iso){clearFields(m);m.__sharedCourtLabel=c;m.v1011SharedCourt=c;m.manualSharedHold=true;m.__manualSharedHold=true;m.v1011SharedWait=true;m.waitingFirstAt=iso;m.courtAssignedAt=iso;m.manualCourtPinnedAt=iso;m.courtQueueOrder=iso;m.v1036CourtState='shared';m.updatedAt=now();}
  function assigned(m){return !!(S(m.court)||S(m.manualCourtTarget)||ar(m.courts).length||S(m.__sharedCourtLabel)||m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait);}
  function rotate(list,start){list=ar(list); if(!list.length)return list; start=((Number(start)||0)%list.length+list.length)%list.length; return list.slice(start).concat(list.slice(0,start));}
  function occupancy(k,exclude){const groups=venueGroups(k), by={}; groups.forEach(g=>by[g.venue]=ar(g.courts)); const all=Object.values(by).flat(); const o={}; all.forEach(c=>o[c]={cur:0,wait:0}); ar(G.matches&&G.matches[k]).filter(m=>m&&m.winner==null&&!exclude.has(S(m.id))).forEach(m=>{const c=S(m.court||m.currentCourt); if(c&&o[c])o[c].cur++; const w=S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0])); if(w&&o[w])o[w].wait++;}); return {groups,by,all,o};}
  async function persist(k){try{if(window.__FB_WRITE_CACHE){if(__FB_WRITE_CACHE.draws)delete __FB_WRITE_CACHE.draws[k];if(__FB_WRITE_CACHE.matches)delete __FB_WRITE_CACHE.matches[k];}}catch(e){} try{if(typeof window.stM==='function')await window.stM(k);}catch(e){console.warn('[v1036] stM failed',e);} try{if(typeof window.stD==='function')await window.stD(k);}catch(e){} }
  async function after(k){try{if(typeof window.renderBracket==='function')window.renderBracket();}catch(e){} setTimeout(()=>tick(),120);}
  async function assignBalanced(k){
    k=k||selectedKey(); if(!k){toast('대회와 부서를 먼저 선택하세요','error');return false;}
    rebuildQueue(k);
    const list=firstRound(k).filter(m=>!resultState(k,m).done&&ready(k,m)).sort((a,b)=>qNum(a)-qNum(b)||Number(a.slot||0)-Number(b.slot||0));
    if(!list.length){toast('배정 가능한 64강 카드가 없습니다. 양쪽 팀명이 확정된 카드부터 배정됩니다.','info'); renderAuthoritative(k); return false;}
    list.forEach(clearFields);
    const ex=new Set(list.map(m=>S(m.id))); const st=occupancy(k,ex); if(!st.all.length){toast('본선 사용 코트를 찾을 수 없습니다','error');return false;}
    const base=Date.now(); let cur=0,wait=0,shared=0;
    // 1단계: 균등 순서대로 각 코트 현재경기 1개씩
    list.forEach((m,i)=>{if(assigned(m))return; let v=venueKo(m.venue||m.__venue)||st.groups[0].venue; if(!st.by[v]||!st.by[v].length)v=st.groups[0].venue; const courts=rotate(st.by[v]&&st.by[v].length?st.by[v]:st.all,qNum(m)-1); const iso=new Date(base+i*1000).toISOString(); for(const c of courts){const o=st.o[c]||(st.o[c]={cur:0,wait:0}); if(o.cur===0){markCur(m,c,iso);o.cur++;cur++;break;}}});
    // 2단계: 남은 경기는 각 코트 대기1
    list.forEach((m,i)=>{if(assigned(m))return; let v=venueKo(m.venue||m.__venue)||st.groups[0].venue; if(!st.by[v]||!st.by[v].length)v=st.groups[0].venue; const courts=rotate(st.by[v]&&st.by[v].length?st.by[v]:st.all,qNum(m)-1); const iso=new Date(base+(1000*list.length)+i*1000).toISOString(); for(const c of courts){const o=st.o[c]||(st.o[c]={cur:0,wait:0}); if(o.wait===0){markWait(m,c,iso);o.wait++;wait++;break;}}});
    // 3단계: 나머지는 구장별 공용대기
    list.forEach((m,i)=>{if(assigned(m))return; let v=venueKo(m.venue||m.__venue)||st.groups[0].venue; if(!st.by[v]||!st.by[v].length)v=st.groups[0].venue; const courts=rotate(st.by[v]&&st.by[v].length?st.by[v]:st.all,qNum(m)-1); const iso=new Date(base+(2000*list.length)+i*1000).toISOString(); markShared(m,courts[0]||st.all[0],iso);shared++;});
    try{const d=G.draws&&G.draws[k]; if(d){d.v1036MainQueueEnabled=true; d.v1036AssignedAt=now(); d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}}catch(e){}
    await persist(k); await after(k); toast(`본선 64강 균등 코트배정 완료: 현재 ${cur} · 대기1 ${wait} · 공용 ${shared}`,'success'); return true;
  }
  async function refreshQueue(k,manual){
    k=k||selectedKey(); if(!k)return false;
    rebuildQueue(k); const d=G.draws&&G.draws[k]||{}; if(!manual&&!d.v1036MainQueueEnabled)return false;
    const add=firstRound(k).filter(m=>!resultState(k,m).done&&ready(k,m)&&!assigned(m)).sort((a,b)=>qNum(a)-qNum(b));
    if(!add.length){if(manual)toast('본선 큐 갱신: 새로 붙일 확정 카드가 없습니다.','info'); renderAuthoritative(k); return false;}
    const ex=new Set(add.map(m=>S(m.id))); const st=occupancy(k,ex); if(!st.all.length){if(manual)toast('본선 사용 코트를 찾을 수 없습니다','error'); return false;}
    const base=Date.now(); let wait=0,shared=0;
    add.forEach((m,i)=>{let v=venueKo(m.venue||m.__venue)||st.groups[0].venue; if(!st.by[v]||!st.by[v].length)v=st.groups[0].venue; const courts=rotate(st.by[v]&&st.by[v].length?st.by[v]:st.all,qNum(m)-1); const iso=new Date(base+i*1000).toISOString(); let done=false; for(const c of courts){const o=st.o[c]||(st.o[c]={cur:0,wait:0}); if(o.cur>0&&o.wait===0){markWait(m,c,iso);o.wait++;wait++;done=true;break;}} if(!done){markShared(m,courts[0]||st.all[0],iso);shared++;}});
    try{const dr=G.draws&&G.draws[k]; if(dr){dr.v1036MainQueueEnabled=true; dr.v1036RefreshedAt=now(); dr.v1022BalancedQueueEnabled=false; dr.v1019MainQueueEnabled=false; dr.v1011MainCourtAssignment='';}}catch(e){}
    await persist(k); await after(k); if(manual)toast(`본선 큐 갱신: 대기1 ${wait} · 공용 ${shared}`,'success'); return true;
  }
  function hideLegacy(){
    try{document.querySelectorAll('button,a').forEach(el=>{const t=S(el.textContent); if(/가지형\s*대진표\s*(펼치기|접기)|조별\s*본선\s*연결구조|본선\s*시드보호/.test(t)){el.classList.add(HIDE); el.style.setProperty('display','none','important');}});}catch(e){}
    try{document.querySelectorAll('.sec-title,.card-title,h2,h3,h4').forEach(el=>{const t=S(el.textContent); if(/^(🏅\s*)?최종\s*결과$/.test(t)||/^본선\s*경기\s*현황$/.test(t)||/^본선\s*토너먼트\s*대진표$/.test(t)){let box=el.closest('.card,section,article')||el; box.classList.add(HIDE); box.style.setProperty('display','none','important');}});}catch(e){}
    try{document.querySelectorAll('.card,section,article,div').forEach(el=>{if(el.closest('#v1003MainPanel'))return; const tx=S(el.innerText||el.textContent); if(tx.length<600&&/우승|준우승/.test(tx)&&/최종\s*결과/.test(tx)){el.classList.add(HIDE); el.style.setProperty('display','none','important');}});}catch(e){}
  }
  function bind(){
    const a=api(); if(a){a.version=VERSION; a.assignCourts=assignBalanced; a.refreshLiveMainQueue=(k,r)=>refreshQueue(k,/manual|button|force/i.test(String(r||''))); a.rebuildBalancedQueueSeq=rebuildQueue; a.renderAuthoritativeBracket=renderAuthoritative; window.MainDrawCleanV1036=a;}
    window.v1036AssignBalanced=()=>assignBalanced(selectedKey()); window.v1036RefreshQueue=(manual=true)=>refreshQueue(selectedKey(),manual); window.v773RunMainAssign=window.v1036AssignBalanced;
    ['v1003AssignBtn','v1005AssignBtn','v1005FloatAssign'].forEach(id=>{const b=ge(id); if(b&&!b.__v1036Assign){b.__v1036Assign=true; b.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return assignBalanced(selectedKey());};}});
    const q=ge('v1018QueueRefreshBtn'); if(q&&!q.__v1036Refresh){q.__v1036Refresh=true; q.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return refreshQueue(selectedKey(),true);};}
  }
  function neutralize(){try{Object.keys((G&&G.draws)||{}).forEach(k=>{const d=G.draws[k]; if(d&&ar(G.matches&&G.matches[k]).some(m=>m&&m.cleanMainDraw)){d.v1022BalancedQueueEnabled=false; d.v1019MainQueueEnabled=false; d.v1011MainCourtAssignment='';}});}catch(e){} }
  function tick(){neutralize(); bind(); hideLegacy(); renderAuthoritative(selectedKey());}
  const oldRender=window.renderBracket; if(typeof oldRender==='function'&&!oldRender.__v1036Auth){const w=function(){const r=oldRender.apply(this,arguments); setTimeout(tick,0); setTimeout(tick,160); setTimeout(tick,700); return r;}; w.__v1036Auth=true; w.__old=oldRender; window.renderBracket=w;}
  try{const style=document.createElement('style');style.id='v1036AuthoritativeStyle';style.textContent=`
    .${HIDE}{display:none!important;visibility:hidden!important;pointer-events:none!important}.v1036-empty{padding:18px;border:1px dashed #cbd5e1;border-radius:14px;text-align:center;color:#64748b;font-weight:900;background:#f8fafc}.v1036-head{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin:12px 0}.v1036-seg{display:inline-flex;margin:2px;padding:4px 9px;border:1.5px solid #bfdbfe;border-radius:999px;background:#eff6ff;color:#1d4ed8;font-size:.72rem;font-weight:1000}.v1036-wrap{overflow-x:auto;padding:8px 0 12px}.v1036-bracket{display:flex;gap:18px;align-items:stretch;min-width:max-content}.v1036-round{min-width:235px;display:flex;flex-direction:column}.v1036-round h4{background:#0f1e3a;color:#fff;text-align:center;font-weight:1000;border-radius:9px 9px 0 0;padding:8px 10px;margin:0}.v1036-round-body{display:flex;flex-direction:column;gap:10px;justify-content:space-around;flex:1;padding-top:10px}.v1036-bmatch{border:1.5px solid #cbd5e1;border-left:7px solid #2563eb;border-radius:13px;background:#fff;box-shadow:0 5px 14px rgba(15,30,58,.08);overflow:hidden}.v1036-bmatch.v1036-shared{border-left-color:#0f766e;background:#f0fdfa}.v1036-bmatch.v1036-wait1{border-left-color:#d97706;background:#fff7ed}.v1036-bmatch.v1036-current{border-left-color:#2563eb;background:#eff6ff}.v1036-bmatch.v1036-done{border-left-color:#16a34a;background:#ecfdf5}.v1036-top{display:flex;align-items:center;justify-content:space-between;gap:6px;padding:6px 8px;font-size:.74rem;font-weight:900;color:#334155}.v1036-venue{display:inline-flex;background:#2563eb;color:#fff;border-radius:999px;padding:2px 8px;font-size:.68rem;font-weight:1000}.v1036-team{display:flex;align-items:center;justify-content:space-between;gap:8px;min-height:34px;padding:7px 8px;border-top:1px solid #e2e8f0;font-size:.82rem;font-weight:900}.v1036-team.winner{background:#dcfce7;color:#166534}.v1036-team small{font-family:Oswald,sans-serif;color:#94a3b8;font-size:.9rem}.v1036-status{border-top:1px dashed #e2e8f0;padding:6px 8px;font-size:.72rem;font-weight:900;color:#475569;background:#f8fafc}.v1036-src{text-align:center;border-top:1px dashed #e2e8f0;padding:5px 8px;font-size:.66rem;color:#64748b;font-weight:800}.v1036-placeholder{border-left-color:#cbd5e1;background:#f8fafc}
  `;document.head.appendChild(style);}catch(e){}
  [0,200,800,1600,3200,6000].forEach(t=>setTimeout(tick,t)); setInterval(tick,2500); setInterval(()=>refreshQueue(selectedKey(),false),6000);
  try{console.log('[v1036] authoritative clean main bracket and balanced queue loaded');}catch(e){}
})();
