/* v1029 DEV stable reset main draw mode
...
(function(){
  'use strict';
  if(window.__V1029_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1029_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1019 uses its own guard and filename so cached older scripts cannot block this patch.
  const VERSION = 'v1029-dev-stable-reset-no-blank-fix';
...
/* v1029 DEV safe legacy quarantine: no broad hiding, no page blank */
(function(){
  'use strict';
  if(window.__V1029_SAFE_LEGACY_QUARANTINE__) return;
  window.__V1029_SAFE_LEGACY_QUARANTINE__=true;
  const VERSION='v1029-dev-stable-reset-no-blank-fix';
  function S(v){return String(v||'').replace(/\s+/g,' ').trim();}
  function isInsideSafe(el){
    try{return !!(el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat,#bracketExportBar,#myClubBracketBar,#page-home,#page-tournament,#page-register,#page-players,#page-ranking,.app-header,.nav-tabs'))}catch(e){return false;}
  }
  function hide(el){try{ if(!el) return; el.classList.add('v1029-legacy-hidden'); el.style.setProperty('display','none','important'); el.style.setProperty('visibility','hidden','important'); el.style.setProperty('pointer-events','none','important'); }catch(e){}}
  function headingLike(el){
    const t=S(el&&el.innerText||el&&el.textContent||'');
    if(!t || t.length>80) return false;
    return /^(🏅\s*)?최종\s*결과$|^본선\s*경기\s*현황$|^본선\s*진출팀|^본선\s*토너먼트\s*대진표/.test(t);
  }
  function hideLegacySections(){
    try{
      document.querySelectorAll('h1,h2,h3,.sec-title,.card-title,div').forEach(el=>{
        if(isInsideSafe(el)) return;
        if(!headingLike(el)) return;
        const txt=S(el.innerText||el.textContent||'');
        hide(el);
        const card=el.closest('.card,section,article');
        if(card && !isInsideSafe(card)) hide(card);
        let n=el.nextElementSibling;
        for(let i=0;n && i<3;i++,n=n.nextElementSibling){
          const nt=S(n.innerText||n.textContent||'');
          if(/우승|준우승|결승|본선|TBD|최종/.test(nt) && !/예선\s*경기표|코트\s*사용|새\s*본선/.test(nt)) hide(n);
        }
      });
      // Old tree containers known from legacy renderers; keep new v1003 panel.
      document.querySelectorAll('#bracketContent [id^="mainStage_"],#bracketContent .main-status-board,#bracketContent .t-bracket,#bracketContent .t-round').forEach(el=>{ if(!isInsideSafe(el)) hide(el.closest('.card,section,article')||el); });
      const panel=document.getElementById('v1003MainPanel');
      if(panel){ panel.style.setProperty('display','block','important'); panel.style.setProperty('visibility','visible','important'); }
      const b=document.querySelector('#v1003MainPanel .v1003-title small'); if(b) b.textContent=VERSION;
      try{ const api=window.MainDrawCleanV1022||window.MainDrawCleanV1021||window.MainDrawCleanV1020||window.MainDrawCleanV1019; if(api) api.version=VERSION; }catch(e){}
    }catch(e){ console.warn('[v1029] safe quarantine failed', e); }
  }
  function ensureNotBlank(){
    try{
      const active=document.querySelector('.page.active');
      const main=document.querySelector('main.main-content')||document.querySelector('main');
      if(main && (!active || !S(active.innerText))){
        const br=document.getElementById('page-bracket');
        if(br){ document.querySelectorAll('.page').forEach(p=>p.classList.remove('active')); br.classList.add('active'); }
      }
    }catch(e){}
  }
  function apply(){ensureNotBlank(); hideLegacySections();}
  [0,200,700,1500,3000,6000].forEach(t=>setTimeout(apply,t));
  ['click','input','change','keyup'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(apply,80),true));
  try{ new MutationObserver(()=>{ clearTimeout(window.__v1029HideTimer); window.__v1029HideTimer=setTimeout(apply,80); }).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']}); }catch(e){}
  setInterval(apply,1200);
  console.log('[v1029] safe reset/no-blank quarantine loaded');
})();

/* v1030 DEV active venue + wait1 promotion correction
   - Active court configuration is the source of truth for venue badges/segments.
   - Shared/common queue cards must not jump directly into current match during auto refresh.
   - When a current match finishes: wait1 -> current, shared #1 -> wait1.
*/
(function(){
  'use strict';
  if(window.__V1030_ACTIVE_VENUE_WAIT1_QUEUE_FIX__) return;
  window.__V1030_ACTIVE_VENUE_WAIT1_QUEUE_FIX__=true;
  const VERSION='v1030-dev-active-venue-wait1-queue-fix';
  const VENUE_ORDER=['국제','능동','원도심','삼계','금병','동부','장유중','기타'];
  const S=v=>String(v==null?'':v).trim();
  const ar=v=>Array.isArray(v)?v:[];
  const now=()=>new Date().toISOString();
  const $=id=>document.getElementById(id);
  const toast=(m,t='info')=>{try{window.toast?window.toast(m,t):console.log('[v1030]',m)}catch(e){}};
  const stateMemory=window.__V1030_MAIN_STATE_MEMORY__||(window.__V1030_MAIN_STATE_MEMORY__={});

  function api(){return window.MainDrawCleanV1022||window.MainDrawCleanV1021||window.MainDrawCleanV1020||window.MainDrawCleanV1019||window.MainDrawCleanV1018||window.MainDrawCleanV1017||null;}
  function selectedKey(){try{const a=api(); if(a&&typeof a.selectedKey==='function') return a.selectedKey();}catch(e){} try{const k=S(window.__CURRENT_RENDER_KEY||window.__v650CurrentGroupLabelKey||''); if(k&&G.draws&&G.draws[k]) return k;}catch(e){} try{return Object.keys((window.G&&G.draws)||{})[0]||'';}catch(e){return '';}}
  function splitKey(key){const i=S(key).lastIndexOf('_'); return {tid:S(key).slice(0,i),div:S(key).slice(i+1)};}
  function venueKo(v){const c=S(v); if(!c)return''; if(/wondosim|원도심|원도|인조/i.test(c))return'원도심'; if(/gukje|국제|장유국제/i.test(c))return'국제'; if(/neungdong|능동/i.test(c))return'능동'; if(/jangyu_jung|장유중|클레이/i.test(c))return'장유중'; if(/geumbyeong|금병/i.test(c))return'금병'; if(/samgye|삼계/i.test(c))return'삼계'; if(/dongbu|동부/i.test(c))return'동부'; return c;}
  function venueOfCourt(court){const c=S(court); if(!c)return''; if(/원도심|원도|인조/i.test(c))return'원도심'; if(/국제|장유국제|gukje/i.test(c))return'국제'; if(/능동|neungdong/i.test(c))return'능동'; if(/장유중|클레이/i.test(c))return'장유중'; if(/금병/i.test(c))return'금병'; if(/삼계/i.test(c))return'삼계'; if(/동부/i.test(c))return'동부'; return'';}
  function normCourts(list){const out=[]; ar(list).forEach(x=>{ if(Array.isArray(x)) x.forEach(y=>{const c=S(y); if(c&&!out.includes(c))out.push(c);}); else {const c=S(x); if(c&&!out.includes(c))out.push(c);} }); return out.filter(c=>!/공용|대기중|미배정|빈코트|undefined|null/i.test(c));}
  function sortCourts(list){return normCourts(list).sort((a,b)=>{const va=venueOfCourt(a)||'기타',vb=venueOfCourt(b)||'기타'; const ia=VENUE_ORDER.indexOf(va)<0?999:VENUE_ORDER.indexOf(va); const ib=VENUE_ORDER.indexOf(vb)<0?999:VENUE_ORDER.indexOf(vb); if(ia!==ib)return ia-ib; const na=Number((S(a).match(/(\d+)\s*$/)||[])[1]||9999), nb=Number((S(b).match(/(\d+)\s*$/)||[])[1]||9999); return na-nb||S(a).localeCompare(S(b),'ko');});}

  function activeCourts(key){
    const {tid,div}=splitKey(key);
    const candidates=[];
    function add(list,tag,hard){const arr=sortCourts(list); if(arr.length)candidates.push({list:arr,tag,hard:!!hard});}
    try{ if(typeof window.getCourtBoardConfiguredCourtsV505==='function'){ const cfg=window.getCourtBoardConfiguredCourtsV505(key)||{}; if(cfg.explicit&&Array.isArray(cfg.list)&&cfg.list.length) add(cfg.list,'boardExplicit',true); } }catch(e){}
    try{ if(typeof window.getSelectedCourtFilters==='function'){ const x=window.getSelectedCourtFilters(key); if(x&&x.length) add(x,'selectedFilters',true); } }catch(e){}
    try{ if(typeof window.getDivisionPhaseConfiguredCourts==='function') add(window.getDivisionPhaseConfiguredCourts(tid,div,'main'),'phaseMain',true); }catch(e){}
    try{ if(typeof window.getDivisionMainConfiguredCourts==='function') add(window.getDivisionMainConfiguredCourts(tid,div),'divisionMain',true); }catch(e){}
    try{ const d=(G.draws&&G.draws[key])||{}; add(d.mainAllowedCourts||d.mainCourts||d.allowedMainCourts,'drawMain',true); }catch(e){}
    // Only if no explicit/current configuration exists, fall back to legacy helpers. These may contain stale 원도심/능동 values.
    if(candidates.some(x=>x.hard)) return candidates.find(x=>x.hard).list;
    try{ if(window.OperationQueueV993&&typeof OperationQueueV993.getBoardCourtsForKey==='function') add(OperationQueueV993.getBoardCourtsForKey(key),'opqBoard',false); }catch(e){}
    try{ if(typeof window.getUsedCourtsForKey==='function') add(window.getUsedCourtsForKey(key),'used',false); }catch(e){}
    const merged=[]; candidates.forEach(c=>c.list.forEach(x=>{if(!merged.includes(x))merged.push(x);}));
    return sortCourts(merged);
  }
  function activeVenueGroups(key){const map={}; activeCourts(key).forEach(c=>{const v=venueOfCourt(c)||'기타'; (map[v]||(map[v]=[])).push(c);}); const arr=Object.keys(map).map(v=>({venue:v,courts:map[v],count:map[v].length})).sort((a,b)=>{const ia=VENUE_ORDER.indexOf(a.venue)<0?999:VENUE_ORDER.indexOf(a.venue); const ib=VENUE_ORDER.indexOf(b.venue)<0?999:VENUE_ORDER.indexOf(b.venue); return ia-ib;}); return arr.length?arr:[{venue:'국제',courts:[],count:1}];}
  function allCleanMains(key){return ar(window.G&&G.matches&&G.matches[key]).filter(m=>m&&String(m.phase||'')==='main'&&m.cleanMainDraw);}
  function qnum(m){const n=Number(m&& (m.v1022QueueSeq??m.mainQueueSeq??m.v1019QueueSeq??m.__mainQueueOrder??m.v1011QueueOrder)); return Number.isFinite(n)&&n>0?n:(Number.isFinite(Number(m&&m.slot))?10000+Number(m.slot):999999);}
  function sortQueue(list){return ar(list).slice().sort((a,b)=>qnum(a)-qnum(b)||Number(a.slot||0)-Number(b.slot||0)||S(a.id).localeCompare(S(b.id),'ko'));}
  function placeholder(v){const s=S(v); if(!s)return true; if(/^TBD$/i.test(s))return true; if(/슬롯\s*대기|이전\s*라운드|배정\s*대기/.test(s))return true; if(/^\d+\s*조\s*[12]\s*위$/.test(s))return true; return false;}
  function sideName(m,side){const raw=side===1?S(m.display1||m.nameOnly1||m.source1Name||m.source1Label):S(m.display2||m.nameOnly2||m.source2Name||m.source2Label); if(!raw)return''; return raw.replace(/\s*\([^)]*\)/g,'').replace(/\s+/g,' ').trim();}
  function ready(m){const a=sideName(m,1),b=sideName(m,2); return !!(a&&b&&!placeholder(a)&&!placeholder(b)&&!/부전승|BYE/i.test(a));}
  function matchId(m){return S(m&&m.id)||('slot_'+S(m&&m.slot));}
  function courtOf(m){return S(m.court||m.currentCourt||'');}
  function waitCourtOf(m){if(courtOf(m))return''; if(m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait)return''; return S(m.manualCourtTarget||(Array.isArray(m.courts)&&m.courts[0])||'');}
  function sharedCourtOf(m){return S(m.__sharedCourtLabel||m.v1011SharedCourt||'');}
  function clearFields(m){m.court='';m.currentCourt='';m.courts=[];m.manualCourtTarget='';m.__sharedCourtLabel='';m.v1011SharedCourt='';m.manualSharedHold=false;m.__manualSharedHold=false;m.v1011SharedWait=false;delete m.waitingFirstAt;delete m.lastWaitingFirstAt;delete m.courtAssignedAt;delete m.manualCourtPinnedAt;delete m.courtQueueOrder;}
  function qIso(m,off){return new Date(Date.UTC(2000,0,1,0,0,Math.max(1,qnum(m))+(off||0))).toISOString();}
  function markCurrent(m,c,reason){clearFields(m);m.court=c;m.currentCourt=c;m.courts=[c];m.manualCourtTarget=c;m.v1030QueueState='current';m.v1022CourtState='current';m.v1030CurrentReason=reason||'';m.courtAssignedAt=now();m.manualCourtPinnedAt=m.courtAssignedAt;m.courtQueueOrder=qIso(m,0);m.updatedAt=now();}
  function markWait1(m,c,reason){clearFields(m);m.court='';m.currentCourt='';m.courts=[c];m.manualCourtTarget=c;m.v1030QueueState='wait1';m.v1022CourtState='wait1';m.v1030WaitReason=reason||'';m.waitingFirstAt=now();m.courtAssignedAt=m.waitingFirstAt;m.manualCourtPinnedAt=m.waitingFirstAt;m.courtQueueOrder=qIso(m,1);m.updatedAt=now();}
  function markShared(m,c,reason){clearFields(m);m.__sharedCourtLabel=c;m.v1011SharedCourt=c;m.manualSharedHold=true;m.__manualSharedHold=true;m.v1011SharedWait=true;m.v1030QueueState='shared';m.v1022CourtState='shared';m.v1030SharedReason=reason||'';m.waitingFirstAt=now();m.courtAssignedAt=m.waitingFirstAt;m.manualCourtPinnedAt=m.waitingFirstAt;m.courtQueueOrder=qIso(m,2);m.updatedAt=now();}
  function assigned(m){return !!(courtOf(m)||waitCourtOf(m)||sharedCourtOf(m)||m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait);}

  function normalizeActiveVenues(key){
    let changed=false;
    const groups=activeVenueGroups(key); const activeVenues=groups.map(g=>g.venue); const fallback=activeVenues[0]||'국제';
    const d=(G.draws&&G.draws[key])||null;
    if(d){
      const mains=allCleanMains(key); const max=Math.max(0,...mains.map(m=>Number(m.slot||0))); const count=max+1;
      if(activeVenues.length===1){
        const seg=[{venue:fallback,start:0,end:Math.max(0,count-1),count}];
        if(JSON.stringify(d.mainDrawVenueSegments||[])!==JSON.stringify(seg)){d.mainDrawVenueSegments=seg; changed=true;}
      }else if(Array.isArray(d.mainDrawVenueSegments)){
        const fixed=d.mainDrawVenueSegments.map(s=>{const v=activeVenues.includes(venueKo(s.venue))?venueKo(s.venue):fallback; return {...s,venue:v};});
        if(JSON.stringify(fixed)!==JSON.stringify(d.mainDrawVenueSegments)){d.mainDrawVenueSegments=fixed; changed=true;}
      }
    }
    allCleanMains(key).forEach(m=>{
      const actualCourt=courtOf(m)||waitCourtOf(m)||sharedCourtOf(m);
      const byCourt=venueOfCourt(actualCourt);
      const wanted=byCourt || (activeVenues.includes(venueKo(m.venue||m.__venue))?venueKo(m.venue||m.__venue):fallback);
      if(m.venue!==wanted||m.__venue!==wanted||m.venueLabel!==wanted||m.mainBlock!==wanted){m.venue=wanted;m.__venue=wanted;m.venueLabel=wanted;m.mainBlock=wanted;changed=true;}
      // If assigned to a court that no longer belongs to the active configured court list, unassign it for clean requeue.
      const courts=activeCourts(key); const allSet=new Set(courts);
      if(actualCourt && allSet.size && !allSet.has(actualCourt)){ clearFields(m); changed=true; }
    });
    return changed;
  }

  function buildOcc(key,excludeIds){
    const groups=activeVenueGroups(key), byVenue={}; groups.forEach(g=>byVenue[g.venue]=ar(g.courts).slice());
    const all=Object.values(byVenue).flat(); const occ={}; all.forEach(c=>occ[c]={cur:0,wait:0});
    ar(G.matches&&G.matches[key]).forEach(m=>{
      if(!m||m.winner!=null||excludeIds.has(matchId(m))) return;
      const c=courtOf(m); if(c&&occ[c]) occ[c].cur++;
      const w=waitCourtOf(m); if(w&&occ[w]) occ[w].wait++;
    });
    return {groups,byVenue,all,occ};
  }
  function rotate(list,start){list=ar(list); if(!list.length)return list; start=((Number(start)||0)%list.length+list.length)%list.length; return list.slice(start).concat(list.slice(0,start));}
  function courtsFor(key,m,ctx){const v=venueKo(m.venue||m.__venue)||ctx.groups[0]?.venue||'국제'; return (ctx.byVenue[v]&&ctx.byVenue[v].length?ctx.byVenue[v]:ctx.all);}

  function snapshotStates(key){const mem=stateMemory[key]||(stateMemory[key]={}); allCleanMains(key).forEach(m=>{const id=matchId(m); let st='none'; if(courtOf(m))st='current'; else if(waitCourtOf(m))st='wait1'; else if(sharedCourtOf(m)||m.manualSharedHold||m.v1011SharedWait)st='shared'; mem[id]=st;});}

  function manualAssign(key){
    normalizeActiveVenues(key);
    const readyList=sortQueue(allCleanMains(key).filter(m=>m.winner==null&&ready(m)));
    readyList.forEach(clearFields);
    const ctx=buildOcc(key,new Set(readyList.map(matchId)));
    if(!ctx.all.length) return {noCourts:true,currentCnt:0,waitCnt:0,sharedCnt:0,changed:false};
    let currentCnt=0,waitCnt=0,sharedCnt=0,changed=!!readyList.length;
    readyList.forEach(m=>{
      const courts=rotate(courtsFor(key,m,ctx),qnum(m)-1); let done=false;
      for(const c of courts){const o=ctx.occ[c]||(ctx.occ[c]={cur:0,wait:0}); if(o.cur===0){markCurrent(m,c,'manual_initial');o.cur++;currentCnt++;done=true;break;}}
      if(!done){for(const c of courts){const o=ctx.occ[c]||(ctx.occ[c]={cur:0,wait:0}); if(o.wait===0){markWait1(m,c,'manual_initial');o.wait++;waitCnt++;done=true;break;}}}
      if(!done){markShared(m,courts[0]||ctx.all[0]||'','manual_overflow');sharedCnt++;}
    });
    snapshotStates(key);
    try{const d=G.draws[key]; if(d){d.v1030QueueEnabled=true;d.v1022BalancedQueueEnabled=true;d.v1030QueueMode='manual_current_wait_shared';d.v1030QueueUpdatedAt=now();}}catch(e){}
    return {changed,noCourts:false,currentCnt,waitCnt,sharedCnt};
  }

  function autoRefreshQueue(key){
    const mem=stateMemory[key]||(stateMemory[key]={});
    let changed=normalizeActiveVenues(key);
    const mains=sortQueue(allCleanMains(key).filter(m=>m.winner==null&&ready(m)));
    const ctx=buildOcc(key,new Set());
    if(!ctx.all.length) return {noCourts:true,changed:false,currentCnt:0,waitCnt:0,sharedCnt:0};

    // 1) If legacy code moved a shared/common card directly into current, demote it back to wait1/shared.
    mains.forEach(m=>{
      const id=matchId(m); const last=mem[id]; const c=courtOf(m);
      if(c&&last==='shared'&&!m.v1030PromotedFromWait1){
        const o=ctx.occ[c]||(ctx.occ[c]={cur:0,wait:0});
        // Remove this accidental current from occupancy, then try waiting1.
        o.cur=Math.max(0,o.cur-1); clearFields(m); changed=true;
        if(o.wait===0){markWait1(m,c,'demote_shared_jump');o.wait++;}
        else {markShared(m,c,'demote_shared_jump_overflow');}
      }
    });

    // 2) Rebuild occupancy after demotions.
    const ctx2=buildOcc(key,new Set());
    const byCourtWaits={};
    mains.forEach(m=>{const w=waitCourtOf(m); if(w)(byCourtWaits[w]||(byCourtWaits[w]=[])).push(m);});
    Object.keys(byCourtWaits).forEach(c=>byCourtWaits[c]=sortQueue(byCourtWaits[c]));

    // 3) If current is empty, promote existing wait1 to current. This is the only automatic path into 시합중.
    ctx2.all.forEach(c=>{
      const cur=ar(G.matches&&G.matches[key]).some(m=>m&&m.winner==null&&courtOf(m)===c);
      if(cur) return;
      const waits=byCourtWaits[c]||[];
      const first=waits[0];
      if(first){ markCurrent(first,c,'promote_wait1_after_current_done'); first.v1030PromotedFromWait1=true; changed=true; }
    });

    // 4) Rebuild occupancy again and fill each court's wait1 from shared/common queue, never directly to current.
    const ctx3=buildOcc(key,new Set());
    const shared=sortQueue(mains.filter(m=>sharedCourtOf(m)||m.manualSharedHold||m.__manualSharedHold||m.v1011SharedWait));
    ctx3.all.forEach(c=>{
      const o=ctx3.occ[c]||(ctx3.occ[c]={cur:0,wait:0});
      if(o.wait>0) return;
      const venue=venueOfCourt(c)||ctx3.groups[0]?.venue||'국제';
      let idx=shared.findIndex(m=>venueKo(m.venue||m.__venue)===venue);
      if(idx<0) idx=shared.findIndex(Boolean);
      if(idx>=0){const m=shared.splice(idx,1)[0]; markWait1(m,c,'shared_to_wait1_only'); o.wait++; changed=true;}
    });

    // 5) New ready-but-unassigned cards: put into open wait1 first, then shared. Do not jump to current during auto refresh.
    const ctx4=buildOcc(key,new Set());
    const newOnes=sortQueue(mains.filter(m=>!assigned(m)));
    newOnes.forEach(m=>{
      const courts=rotate(courtsFor(key,m,ctx4),qnum(m)-1); let done=false;
      for(const c of courts){const o=ctx4.occ[c]||(ctx4.occ[c]={cur:0,wait:0}); if(o.wait===0){markWait1(m,c,'new_ready_to_wait1');o.wait++;changed=true;done=true;break;}}
      if(!done){markShared(m,courts[0]||ctx4.all[0]||'','new_ready_to_shared');changed=true;}
    });

    snapshotStates(key);
    try{const d=G.draws[key]; if(d){d.v1030QueueEnabled=true;d.v1030QueueMode='wait1_promotion_only';d.v1030QueueUpdatedAt=now();}}catch(e){}
    const curCnt=mains.filter(courtOf).length, waitCnt=mains.filter(waitCourtOf).length, shCnt=mains.filter(m=>sharedCourtOf(m)||m.manualSharedHold||m.v1011SharedWait).length;
    return {changed,noCourts:false,currentCnt:curCnt,waitCnt,sharedCnt:shCnt};
  }

  async function persist(key){try{ if(window.__FB_WRITE_CACHE){ if(__FB_WRITE_CACHE.draws) delete __FB_WRITE_CACHE.draws[key]; if(__FB_WRITE_CACHE.matches) delete __FB_WRITE_CACHE.matches[key]; }}catch(e){} try{ if(typeof window.stM==='function') await window.stM(key); }catch(e){console.warn('[v1030] stM failed',e);} try{ if(typeof window.stD==='function') await window.stD(key); }catch(e){} try{ if(typeof window.waitForPendingWrites==='function'&&window.db) await window.waitForPendingWrites(window.db); }catch(e){} }
  async function render(key){try{ if(typeof window.renderBracket==='function') window.renderBracket(); }catch(e){} setTimeout(()=>{try{ const a=api(); if(a&&a.ensurePanel)a.ensurePanel(); patchPanel(); }catch(e){}},120);}
  async function assignCourts(key){key=key||selectedKey(); if(!key)return false; const res=manualAssign(key); if(res.noCourts){toast('본선 사용 코트를 찾을 수 없습니다.','error');return false;} await persist(key); await render(key); toast(`본선 코트배정 완료: 현재 ${res.currentCnt} · 대기1 ${res.waitCnt} · 공용대기 ${res.sharedCnt}`,'success'); return true;}
  async function refreshLive(key,reason){key=key||selectedKey(); if(!key)return false; const manual=/manual|button|force/i.test(String(reason||'')); const d=(G.draws&&G.draws[key])||{}; if(!manual&&!d.v1030QueueEnabled&&!d.v1022BalancedQueueEnabled&&!d.v1019MainQueueEnabled) return false; const res=autoRefreshQueue(key); if(res.noCourts){if(manual)toast('본선 사용 코트를 찾을 수 없습니다.','error'); return false;} if(res.changed){await persist(key); await render(key); if(manual)toast(`본선 큐 갱신: 현재 ${res.currentCnt} · 대기1 ${res.waitCnt} · 공용대기 ${res.sharedCnt}`,'success'); return true;} if(manual)toast('본선 큐 갱신: 변경할 내용이 없습니다.','info'); return false;}
  async function refreshAll(reason,quiet){const keys=Object.keys((window.G&&G.matches)||{}).filter(k=>allCleanMains(k).length); let n=0; for(const k of keys){try{if(await refreshLive(k,reason||'watchdog'))n++;}catch(e){console.warn('[v1030] refresh failed',k,e);}} if(n&&!quiet)toast(`본선 큐 보정 완료: ${n}개 부서`,'success'); return n>0;}

  function patchApi(){const a=api(); if(!a)return; a.version=VERSION; a.assignCourts=assignCourts; a.refreshLiveMainQueue=refreshLive; a.venueGroups=activeVenueGroups; a.mainCourts=activeCourts; window.MainDrawCleanV1030=a; window.v1030RefreshAllCleanMainQueues=function(){return refreshAll('manual_button',false);}; window.v1022RefreshAllCleanMainQueues=window.v1030RefreshAllCleanMainQueues; window.v1018RefreshAllCleanMainQueues=window.v1030RefreshAllCleanMainQueues; window.v773RunMainAssign=function(){return assignCourts(selectedKey());};}
  function patchPanel(){
    patchApi();
    try{const p=$('v1003MainPanel'); if(p){const t=p.querySelector('.v1003-title small'); if(t)t.textContent=VERSION; const h=p.querySelector('.v1003-title div'); if(h&&!/v1030/.test(h.textContent)){} }}catch(e){}
    ['v1003AssignBtn','v1005AssignBtn','v1005FloatAssign'].forEach(id=>{const b=$(id); if(b&&!b.__v1030Assign){b.__v1030Assign=true;b.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return assignCourts(selectedKey());};}});
    const q=$('v1018QueueRefreshBtn'); if(q&&!q.__v1030Refresh){q.__v1030Refresh=true;q.onclick=function(ev){ev&&ev.preventDefault&&ev.preventDefault();ev&&ev.stopPropagation&&ev.stopPropagation();return refreshAll('manual_button',false);};}
    try{const p=$('v1003MainPanel'); if(p){p.querySelectorAll('.v1003-seg,.v1004-bmatch-top span').forEach(el=>{el.textContent=venueKo(el.textContent);});}}catch(e){}
  }

  function tick(){try{patchPanel(); const k=selectedKey(); if(k) normalizeActiveVenues(k);}catch(e){}}
  document.addEventListener('click',()=>setTimeout(()=>refreshAll('click',true),500),true);
  [0,300,900,1800,3500,6500].forEach(ms=>setTimeout(()=>{tick();refreshAll('startup',true);},ms));
  setInterval(()=>{tick();refreshAll('watchdog',true);},1500);
  try{console.log('[v1030] active venue and wait1 promotion fix loaded');}catch(e){}
})();
