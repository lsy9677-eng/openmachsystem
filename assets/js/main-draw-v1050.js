/*
 * 230MATCH v1050 — clean main tournament engine
 *
...
(function(){
  'use strict';
  if (window.__V1050_MAIN_ENGINE__) return;
  window.__V1050_MAIN_ENGINE__ = true;
  window.__CLEAN_MAIN_ONLY_MODE__ = true;
  window.__MAIN_DRAW_AUTHORITY_VERSION__ = 'v1050-clean-main-engine';

  const VERSION = 'v1050-clean-main-engine';
  const PANEL_ID = 'v1050MainPanel';
  const BRACKET_ID = 'v1050MainBracket';
  const AUTO_KEY = 'V1050_MAIN_AUTO_ENABLED';
  ...
  const log = (...a) => { try{ console.info('[v1050]',...a); }catch(_e){} };
  ...
  function engineMatches(key){ return arr(G.matches[key]).filter(m=>m&&m.phase==='main'&&m.v1050Main===true); }
  ...
  function makeId(key,r,s){ return `v1050_${key.replace(/[^a-zA-Z0-9_-]/g,'_')}_r${r}_s${s}_${Date.now().toString(36)}`; }
  ...
  function makeFirst(key,size,slot,e1,e2,seq,venue){
    ...
    return {id:makeId(key,0,slot),phase:'main',round:0,slot,bracketN:size,v1050Main:true,engineVersion:VERSION,
  ...
  function makeLater(key,size,r,slot,left,right,seq,venue){
    return {id:makeId(key,r,slot),phase:'main',round:r,slot,bracketN:size,v1050Main:true,engineVersion:VERSION,
  ...
  function syncTree(key){
    syncFirstSlots(key);
    const size=Number((G.draws[key]||{}).v1050Size||64),max=Math.log2(size)-1;
    for(let r=1;r<=max;r++){
  ...
  async function generateDraw(key){
    ...
    G.draws[key]={...(G.draws[key]||{}),v1050Main:true,v1050Size:size,v1050Version:VERSION,v1050DrawAt:iso(),mainDrawSize:size,mainDrawVersion:VERSION};
  ...
  function updateAuto(){const b=$('v1050AutoBtn');if(!b)return;const on=autoOn();b.textContent=on?'▶ 자동배정 ON':'⏸ 자동배정 OFF';b.className='v1050-btn '+(on?'auto-on':'auto-off');}
  ...
  function resultButton(key,m,cls='v1050-result'){
    if(m.t1==null||m.t2==null||typeof window.openM3!=='function')return'';
    return `<button type="button" class="${cls}" data-v1050-result="${esc(m.id)}">${done(key,m)?'결과 수정':'결과 입력'}</button>`;
  }
  function cardHtml(key,m){
    ...
    return `<article class="v1050-match" style="--vc:${COLORS[v]||COLORS.기타};--vb:${BGS[v]||BGS.기타}"><header><b>${Number(m.slot)+1}경기</b><span>${esc(v)}</span></header>
      <div class="v1050-side ${win===1?'win':''}"><strong>${esc(sideName(key,m,1))}</strong><em>${score[0]||0}</em></div>
      <div class="v1050-side ${win===2?'win':''}"><strong>${esc(sideName(key,m,2))}</strong><em>${score[1]||0}</em></div>
      <footer>${esc(statusMeta(key,m))}</footer>${resultButton(key,m)}</article>`;
  }
  function bracketHtml(key){
    const ms=engineMatches(key);if(!ms.length)return'<div class="v1050-empty">새 본선 대진이 없습니다. 새 본선 추첨을 실행하세요.</div>';
    syncTree(key);const size=Number(G.draws[key].v1050Size||64),max=Math.log2(size)-1;
    let h=`<div class="v1050-bracket-head"><b>본선 대진표 · ${size}강부터 결승까지</b><span>결과는 다음 라운드에 즉시 반영</span></div><div class="v1050-scroll"><div class="v1050-tree">`;
    for(let r=0;r<=max;r++){
      h+=`<section class="v1050-round"><h4>${roundName(size,r)}</h4><div class="v1050-round-body">`;
      byRound(key,r).forEach(m=>h+=cardHtml(key,m));h+='</div></section>';
    }
    return h+'</div></div>';
  }
  function statusHtml(key){
    const ms=engineMatches(key);if(!ms.length)return'';const size=Number(G.draws[key].v1050Size||64),r=currentRound(key),list=byRound(key,r);
    return `<section class="v1050-current"><div class="v1050-current-title"><b>⚡ 본선 경기 현황</b><span>${roundName(size,r)}</span></div><div class="v1050-grid">${list.map(m=>`<article class="v1050-status"><header><b>${Number(m.slot)+1}경기</b><span>${stateLabel(m)}</span></header><div>${esc(sideName(key,m,1))}<i>vs</i>${esc(sideName(key,m,2))}</div><small>${esc(statusMeta(key,m))}</small>${resultButton(key,m,'v1050-status-result')}</article>`).join('')}</div></section>`;
  }

  function ensureCss(){if($('v1050Css'))return;const s=document.createElement('style');s.id='v1050Css';s.textContent=`
    ...
    .v1050-top{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:10px}.v1050-top b{font-size:1rem}.v1050-top small{color:#64748b;font-weight:800}
    .v1050-controls{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}.v1050-btn{border:0;border-radius:11px;min-height:40px;padding:8px 14px;font-weight:1000;cursor:pointer}.v1050-btn.blue{background:#2563eb;color:#fff}.v1050-btn.purple{background:#7c3aed;color:#fff}.v1050-btn.green{background:#0f766e;color:#fff}.v1050-btn.auto-on{background:#d4a017;color:#111827}.v1050-btn.auto-off{background:#e2e8f0;color:#334155}
    .v1050-note{font-size:.78rem;color:#475569;line-height:1.55;margin-bottom:10px}.v1050-empty{padding:20px;text-align:center;border:1px dashed #cbd5e1;border-radius:14px;color:#64748b;font-weight:900}
    .v1050-bracket-head,.v1050-current-title{display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin:12px 0 8px}.v1050-bracket-head span{font-size:.72rem;color:#64748b}.v1050-current-title span{background:#0f1e3a;color:#fff;border-radius:999px;padding:4px 10px;font-size:.74rem}
    .v1050-scroll{overflow-x:auto;padding:4px 0 10px}.v1050-tree{display:flex;gap:18px;min-width:max-content}.v1050-round{width:235px;display:flex;flex-direction:column}.v1050-round h4{margin:0;background:#0f1e3a;color:#fff;text-align:center;padding:8px;border-radius:8px 8px 0 0}.v1050-round-body{display:flex;flex-direction:column;justify-content:space-around;gap:10px;flex:1;padding-top:10px}
    .v1050-match{border:1.5px solid #cbd5e1;border-left:7px solid var(--vc);border-radius:12px;background:linear-gradient(90deg,var(--vb),#fff);overflow:hidden;box-shadow:0 3px 11px rgba(15,30,58,.08)}.v1050-match header,.v1050-status header{display:flex;justify-content:space-between;align-items:center;padding:6px 8px;font-size:.72rem}.v1050-match header span,.v1050-status header span{background:#e0ecff;color:#1d4ed8;border-radius:999px;padding:2px 7px;font-weight:1000}.v1050-side{display:flex;justify-content:space-between;gap:8px;border-top:1px solid #e2e8f0;padding:7px 8px;font-size:.8rem}.v1050-side.win{background:#dcfce7;color:#166534}.v1050-side em{font-style:normal;color:#64748b}.v1050-match footer{border-top:1px dashed #e2e8f0;padding:5px 8px;text-align:center;font-size:.68rem;color:#64748b;font-weight:900}.v1050-result,.v1050-status-result{width:calc(100% - 16px);margin:0 8px 8px;border:0;border-radius:8px;background:#d4a017;padding:7px;font-weight:1000;cursor:pointer}
    .v1050-current{margin-top:16px;border-top:2px solid #e2e8f0;padding-top:8px}.v1050-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.v1050-status{border:1.5px solid #bfdbfe;border-radius:12px;background:#f8fbff;padding:8px}.v1050-status>div{font-size:.82rem;font-weight:1000;line-height:1.4}.v1050-status i{font-style:normal;color:#64748b;margin:0 5px}.v1050-status small{display:block;color:#64748b;font-size:.7rem;margin:6px 0;font-weight:800}
    @media(max-width:900px){.v1050-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.v1050-grid{grid-template-columns:1fr}.v1050-controls .v1050-btn{flex:1 1 100%}}
  `;document.head.appendChild(s);}
  function ensurePanel(){
    ensureCss();const page=$('page-bracket');if(!page)return null;let p=$(PANEL_ID);
    if(!p){p=document.createElement('section');p.id=PANEL_ID;const bc=$('bracketContent');if(bc&&bc.parentNode===page)page.insertBefore(p,bc);else page.prepend(p);}
    if(!p.dataset.ready){p.innerHTML=`<div class="v1050-top"><b>🏆 새 본선 운영 · ${VERSION}</b><small>대진·결과·큐 단일 엔진</small></div><div class="v1050-controls"><button id="v1050DrawBtn" class="v1050-btn blue">🎲 새 본선 추첨</button><button id="v1050AssignBtn" class="v1050-btn purple">🎯 본선 코트배정</button><button id="v1050RefreshBtn" class="v1050-btn green">🔁 본선 큐 갱신</button><button id="v1050AutoBtn" class="v1050-btn auto-off">⏸ 자동배정 OFF</button></div><div class="v1050-note">64강 대진 전체를 먼저 만들고, 실제 점수와 승자를 다음 라운드에 즉시 반영합니다. 최초 배정은 각 코트 시합중 1경기 + 대기1 1경기, 나머지는 공용대기입니다.</div><div id="${BRACKET_ID}"></div>`;p.dataset.ready='1';}
    $('v1050DrawBtn').onclick=()=>generateDraw(selectedKey());$('v1050AssignBtn').onclick=async()=>{if(await assignInitial(selectedKey()))setAuto(true);};$('v1050RefreshBtn').onclick=()=>refreshQueue(selectedKey(),true);$('v1050AutoBtn').onclick=()=>setAuto(!autoOn());
    p.onclick=e=>{const b=e.target.closest('[data-v1050-result]');if(!b)return;const key=selectedKey();if(typeof window.openM3==='function')window.openM3(key,b.dataset.v1050Result);};
    updateAuto();return p;
  }
  function render(){const p=ensurePanel(),box=$(BRACKET_ID);if(!p||!box)return;const key=selectedKey();if(!hasData()||!key){box.innerHTML='<div class="v1050-empty">대회와 부서를 선택하세요.</div>';return;}syncTree(key);box.innerHTML=bracketHtml(key)+statusHtml(key);}
  function redrawHost(){try{if(typeof window.renderBracket==='function')window.renderBracket();}catch(_e){}setTimeout(render,50);}

  function wrapResultSave(){
    const old=window.saveM3;if(typeof old!=='function'||old.__v1050Wrapped)return;
    const wrapped=async function(){
      const keyBefore=window.CM_key||selectedKey();
      const result=await old.apply(this,arguments);
      setTimeout(async()=>{
        try{
          const key=keyBefore||selectedKey();
          if(!key||!G.matches[key])return;
          // Remove only legacy main matches created by old auto-advance after v1050 became authoritative.
          if(G.draws[key]&&G.draws[key].v1050Main) G.matches[key]=arr(G.matches[key]).filter(m=>m.phase!=='main'||m.v1050Main===true);
          syncTree(key);
          if(autoOn()) await refreshQueue(key,false); else await persist(key);
          redrawHost();
        }catch(e){console.error('[v1050] post-result sync failed',e);}
      },150);
      return result;
    };
    wrapped.__v1050Wrapped=true;wrapped.__old=old;window.saveM3=wrapped;try{saveM3=wrapped;}catch(_e){}
  }
  ...
  function boot(attempt=0){
    ...
    ensurePanel();wrapResultSave();installEvents();render();
    window.MainDrawV1050={version:VERSION,generateDraw,assignInitial,refreshQueue,render,syncTree,selectedKey,setAuto};
    window.startMainDraw=()=>generateDraw(selectedKey());window.buildMain=window.startMainDraw;window.v1050RefreshMainQueue=()=>refreshQueue(selectedKey(),true);
    log('clean main engine loaded');
  }
