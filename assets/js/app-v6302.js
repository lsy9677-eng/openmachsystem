...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow}from'./sync-engine-v6300.js?v=6300';
...
function renderHomeTournamentCards(){
  ...
  const records=sortUpcomingTournamentRecords(allRecords.filter(r=>{
    ...
    return !(snap?.completion?.completedAt||snap?.tournament?.completedAt);
  }));
  if(!records.length){
...
(function stage6301TournamentOrdering(){
  ...
  console.info('[230MATCH] 63.0.2 ready · nearest tournament first + completed tournament section');
})();
