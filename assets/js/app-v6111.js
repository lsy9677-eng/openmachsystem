...
function navigatePortalView(name,{pushHistory=false,replaceHistory=false,focus=true}={}){
  ...
  if(target==='home')setTimeout(showEligibleHomePopup,120);if(target==='my-match')setTimeout(v3252AutoMyMatch,60);if(target==='entry')setTimeout(renderApplicationPortal,30);if(target==='notifications')setTimeout(refreshNotificationManager,30);if(target==='messages')setTimeout(()=>{renderSmsAcceptance(lastSmsAcceptance);},30);if(target==='print')setTimeout(renderPrintPreview,30);if(target==='acceptance')setTimeout(renderAcceptance,30);if(target==='rehearsal')setTimeout(()=>renderRehearsal(),30);if(target==='performance')setTimeout(renderPerformanceCenter,30);if(target==='diagnostics')setTimeout(renderDiagnostics,30);if(target==='manual')setTimeout(renderOperationsManual,30);
...
/* 230MATCH 4.2.1 · navigation performance recovery */
(()=>{document.documentElement.dataset.build='6111';console.info('[230MATCH] 61.1.1 ready · fast route navigation + scoped observers');})();
