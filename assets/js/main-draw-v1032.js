/* v1032 DEV remove legacy bracket only mode
...
(function(){
  ...
  const VERSION = 'v1032-dev-remove-legacy-bracket-only';
...
(function(){
  ...
  try{console.log('[v1032] balanced court distribution override installed');}catch(e){}
})();


/* v1032: safe legacy bracket deletion patch */
(function(){
  'use strict';
  if(window.__V1032_SAFE_LEGACY_BRACKET_DELETE__) return;
  window.__V1032_SAFE_LEGACY_BRACKET_DELETE__=true;
  try{ console.log('[v1032] legacy bracket delete patch loaded'); }catch(e){}
})();


/* v1032: remove legacy lower main bracket/results sections only, keep new main panel and prelim/court boards */
(function(){
  'use strict';
  if(window.__V1032_LEGACY_MAIN_SECTIONS_REMOVED__) return;
  window.__V1032_LEGACY_MAIN_SECTIONS_REMOVED__=true;
  const VERSION='v1032-dev-remove-legacy-bracket-only';

  function txt(el){ try{return String((el&&el.innerText)||el?.textContent||'').replace(/\s+/g,' ').trim();}catch(e){return '';} }
  function isNewMain(el){ try{return !!(el&&el.closest&&el.closest('#v1003MainPanel,#v1005MainDrawQuickBar,#v1005MainDrawFloat,.v1004-bracket-wrap,.v1004-bracket'));}catch(e){return false;} }
  function hide(el){
    try{
      if(!el || isNewMain(el)) return;
      el.classList.add('v1032-legacy-main-removed');
  ...
  function looksLegacyText(t){
    return /본선\s*토너먼트\s*대진표|가지형\s*(본선\s*)?대진표|본선\s*경기\s*현황|최종\s*결과|본선\s*진출팀|본선\s*진출\s*\(\d+팀\)|우승|준우승/.test(t||'');
  }
  function keepStop(el){
    const t=txt(el);
    if(!t) return false;
    // Stop before sections we must keep.
    if(/예선\s*경기표|예선\s*내용|코트\s*사용\s*현황판|구장별\s*공용대기|공용대기|대회\s*$|부서\s*$/.test(t)) return true;
    if(el.id==='v1003MainPanel' || el.id==='bracketContent') return true;
    return false;
  }
  function hideSecAndFollowing(sec, max=30){
    if(!sec || isNewMain(sec)) return;
    hide(sec);
    let n=sec.nextElementSibling, c=0;
    while(n && c++<max){
      if(isNewMain(n) || keepStop(n)) break;
      const t=txt(n);
      // Hide the legacy block after the title until the next retained section.
      hide(n);
      n=n.nextElementSibling;
    }
  }
  function removeLegacyMainSections(){
    try{
      // Fixed legacy IDs generated by old main draw renderer.
      document.querySelectorAll('[id^="mainStage_"],[id^="mainPreviewTitle_"],[id^="mainPreviewCard_"],[id^="mainToggle_"],[id^="mainStageTitle_"],#mainAdvTeams,#mainStep1,#mainStep2,#mMain').forEach(hide);

      // Hide legacy buttons that re-open old bracket views.
      document.querySelectorAll('button,a').forEach(el=>{
        if(isNewMain(el)) return;
        const t=txt(el);
        const oc=String(el.getAttribute('onclick')||'');
        if(/가지형\s*대진표|본선\s*시드보호\s*재추첨|기존\s*본선|128고정|본선확정|본선잠금/.test(t+oc)) hide(el);
      });

      // Target legacy section titles only. Do not scan all divs broadly.
      document.querySelectorAll('.sec-title,.card-title,h2,h3,[id^="mainStageTitle_"],[id^="mainPreviewTitle_"]').forEach(el=>{
        if(isNewMain(el)) return;
        const t=txt(el);
        if(/본선\s*토너먼트\s*대진표|본선\s*경기\s*현황|최종\s*결과|본선\s*진출팀|본선\s*진출\s*\(\d+팀\)/.test(t)){
          const sec=el.classList&&el.classList.contains('card-title') ? (el.closest('.card')||el) : el;
          hideSecAndFollowing(sec, /본선\s*경기\s*현황/.test(t)?60:18);
        }
      });

      // Hide legacy notice cards tied only to old 가지형 area.
      document.querySelectorAll('.card,div').forEach(el=>{
        if(isNewMain(el)) return;
        const t=txt(el);
        if(t.length>20 && t.length<260 && /가지형\s*본선\s*대진표만\s*접혀|아래\s*본선\s*경기\s*현황|최종\s*결과가\s*아직|우승|준우승/.test(t)) hide(el);
      });

      // Keep new panel stamp explicit.
      const p=document.getElementById('v1003MainPanel');
      if(p){
        const small=p.querySelector('.v1003-title small');
        if(small) small.textContent=VERSION;
      }
    }catch(e){ console.warn('[v1032] legacy main remove failed',e); }
  }
  window.v1032RemoveLegacyMainSections=removeLegacyMainSections;
  [0,120,400,900,1800,3500].forEach(t=>setTimeout(removeLegacyMainSections,t));
  ['click','input','change','keyup','pointerup','touchend'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(removeLegacyMainSections,80),true));
  try{
    const mo=new MutationObserver(()=>{ clearTimeout(window.__v1032LegacyMainTimer); window.__v1032LegacyMainTimer=setTimeout(removeLegacyMainSections,80); });
    mo.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style']});
  }catch(e){}
  setInterval(removeLegacyMainSections,1200);
  try{console.log('[v1032] legacy lower bracket sections removed');}catch(e){}
})();
