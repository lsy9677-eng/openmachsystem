...
// 5.10.113: Firestore profile reads can temporarily fail during live operation.
...
export async function roleForUser(user,knownProfile=undefined){if(!user)return'viewer';const fixed=configuredRole(user);if(fixed)return fixed;const profile=knownProfile===undefined?await readProfile(user):knownProfile;if(!profile)return'viewer';const approved=profile.approved!==false&&!['pending','rejected','disabled'].includes(String(profile.status||'').toLowerCase());return approved?normalizeRole(profile.role||profile.requestedRole):'viewer'}
...
export async function startAuth(onChange){try{const a=await loadApi();await ensureAuth();await handleCustomTokenReturn();if(unsubscribe)unsubscribe();unsubscribe=a.onAuthStateChanged(auth,async user=>{if(!user){onChange?.(null,'viewer',null,null);return}try{const profile=await ensureProfile(user);const role=await roleForUser(user,profile);onChange?.(user,role,null,profile)}catch(error){const role=configuredRole(user)||'viewer';const fallbackProfile={id:user.uid,uid:user.uid,email:user.email||'',name:user.displayName||'',readUnavailable:true};console.warn('[230MATCH V3] auth profile unavailable; cloud profile writes skipped',error);onChange?.(user,role,error,fallbackProfile)}});return true}catch(error){onChange?.(null,'viewer',error,null);return false}}
