/*
 * 230MATCH v1052 — persistent boot clean main tournament engine
 *
...
(function(){
  ...
  window.__MAIN_DRAW_AUTHORITY_VERSION__ = 'v1052-persistent-boot-main-engine';

  const VERSION = 'v1052-persistent-boot-main-engine';
  const PANEL_ID = 'v1052MainPanel';
  const BRACKET_ID = 'v1052MainBracket';
  ...
  const log = (...a) => { try{ console.info('[v1052]',...a); }catch(_e){} };
  ...
  function afterResultSaved(keyHint){
    setTimeout(async()=>{
      ...
      }catch(e){console.error('[v1052] post-result sync failed',e);}
    },180);
  }
  ...
  function boot(){
    // The panel must exist even before Firebase/global G data finishes loading.
    if(!$('page-bracket') || !$('v1051MainPanelHost')){ setTimeout(boot,250); return; }
    ensurePanel(); installResultHooks(); installEvents();
    window.MainDrawV1052={version:VERSION,generateDraw,assignInitial,refreshQueue,render,syncTree,selectedKey,setAuto};
    window.MainDrawV1051=window.MainDrawV1052;
    window.startMainDraw=()=>generateDraw(selectedKey()); window.buildMain=window.startMainDraw;
    window.v1051RefreshMainQueue=()=>refreshQueue(selectedKey(),true);
    window.v1052RefreshMainQueue=window.v1051RefreshMainQueue;
    log('persistent-boot clean main engine loaded');
    let eventsInstalled=true;
    (function watchdog(){
      try{
        ensurePanel();
        installResultHooks();
        // Keep waiting for asynchronous Firebase data; never give up after 15 seconds.
        if(hasData()) render();
      }catch(e){ console.error('[v1052] watchdog',e); }
