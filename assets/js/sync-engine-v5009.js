...
let db=null,api=null,unsubscribe=null,pollTimer=null,visibilityHandler=null,saveTimer=null,reconnectTimer=null,pushInFlight=false,pendingState=null,connectedRoom='',applyingRemote=false,lastDigest='',lastManifestRevision=0,lastRemoteToken='';
const SAVE_DEBOUNCE=2200,READ_ONLY_POLL_MS=45000;
const CACHE_DB='230match-v6-cloud-cache';
const CACHE_STORE='bundles';
let cacheDbPromise=null;
function openCacheDb(){if(cacheDbPromise)return cacheDbPromise;cacheDbPromise=new Promise((resolve,reject)=>{const req=indexedDB.open(CACHE_DB,1);req.onupgradeneeded=()=>{const cdb=req.result;if(!cdb.objectStoreNames.contains(CACHE_STORE))cdb.createObjectStore(CACHE_STORE,{keyPath:'roomId'});};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});return cacheDbPromise;}
async function cacheBundle(bundle){if(!bundle?.state)return;try{const cdb=await openCacheDb();await new Promise((resolve,reject)=>{const tx=cdb.transaction(CACHE_STORE,'readwrite');tx.objectStore(CACHE_STORE).put({roomId:DEFAULT_ROOM,savedAt:new Date().toISOString(),state:clone(bundle.state),manifest:clone(bundle.manifest||{})});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});}catch(_error){}}
async function readCachedBundle(){try{const cdb=await openCacheDb();return await new Promise((resolve,reject)=>{const tx=cdb.transaction(CACHE_STORE,'readonly');const req=tx.objectStore(CACHE_STORE).get(DEFAULT_ROOM);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error);});}catch{return null;}}
function localStateIsEmpty(){const s=getStateFn?.();return !String(s?.tournament?.id||'').trim()&&!(s?.multiTournament?.tournaments?.length);}
async function applyCachedFallback(reason='네트워크 연결이 불안정합니다.'){const cached=await readCachedBundle();if(cached?.state&&localStateIsEmpty()){acceptRemote({state:cached.state,manifest:cached.manifest||{}},'cache');status('오프라인 캐시 사용','warning',`${reason} 마지막 정상 대회 목록을 표시합니다.`,{roomId:DEFAULT_ROOM,schemaVersion:6});return true;}status('클라우드 연결 대기','warning',`${reason} 연결이 복구되면 자동으로 다시 불러옵니다.`,{roomId:DEFAULT_ROOM,schemaVersion:6});return false;}
function scheduleReconnect(){if(reconnectTimer)return;reconnectTimer=setTimeout(()=>{reconnectTimer=null;if(navigator.onLine)connectCloudSync().catch(()=>scheduleReconnect());else scheduleReconnect();},12000);}
...
function normalizeRecords(input){
  ...
  const validWorkspace=w=>Boolean(w&&typeof w==='object'&&String(w.tournament?.id||'').trim()&&isRealTournamentName(w.tournament?.name));
...
function summary(row){const s=row.workspace||{};return{id:row.id,name:row.name||s.tournament?.name||'',division:row.division||s.tournament?.division||'',updatedAt:row.updatedAt||s.updatedAt||'',createdAt:row.createdAt||'',status:s.completion?.completedAt||s.tournament?.completedAt?'completed':'active'};}
...
async function writeBundle(state,{migration=false}={}){
  ...
  try{const prev=await api.getDoc(manifestRef());const keep=new Set(bundle.rows.map(r=>r.id));for(const item of (prev.data()?.tournaments||[])){const id=safeId(item?.id,'tournament');if(!keep.has(id))batch.delete(tournamentRef(id));}}catch(_e){}
...
async function fetchRemote({quiet=false}={}){try{await ensureDb();const bundle=await readBundle();if(bundle)acceptRemote(bundle,'firebase');else if(!quiet)status('클라우드 연결','success','새 저장구조에 아직 대회가 없습니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});}catch(e){await applyCachedFallback(e?.message||String(e));scheduleReconnect();}}
...
export async function connectCloudSync(){disconnectCloudSync(false);try{await ensureDb();if(canWriteFn()){unsubscribe=api.onSnapshot(manifestRef(),async snap=>{if(!snap.exists()){status('실시간 연결','success','새 저장구조에 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});return;}const m=snap.data();lastManifestRevision=Math.max(lastManifestRevision,Number(m.revision||0));const bundle=await readBundle();if(bundle)acceptRemote(bundle,'firebase');else status('자동 저장 정상','success','등록된 운영 대회가 없습니다. 새 대회를 만들어 시작하세요.',{roomId:DEFAULT_ROOM,schemaVersion:6});},async e=>{await applyCachedFallback(e?.message||String(e));scheduleReconnect();});status('실시간 연결','success','대회별 분리 저장 구조로 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});}else{await fetchRemote();startPolling();status('저부하 조회','success','대회별 문서를 45초 간격으로 조회합니다.',{roomId:DEFAULT_ROOM,schemaVersion:6});}return true;}catch(e){await applyCachedFallback(e?.message||String(e));scheduleReconnect();return false;}}
export function disconnectCloudSync(show=true){clearTimeout(saveTimer);saveTimer=null;pendingState=null;if(reconnectTimer){clearTimeout(reconnectTimer);reconnectTimer=null;}if(unsubscribe){unsubscribe();unsubscribe=null;}if(pollTimer){clearInterval(pollTimer);pollTimer=null;}if(visibilityHandler){document.removeEventListener('visibilitychange',visibilityHandler);visibilityHandler=null;}db=null;connectedRoom='';if(show)status('로컬 저장','info','클라우드 연결을 해제했습니다.');}
...
window.addEventListener('online',()=>{if(!unsubscribe&&!pollTimer)connectCloudSync().catch(()=>scheduleReconnect());});
