...
async function readInitialBundle(){
  ...
  // Backward compatibility: decode non-active tournaments once, but never again on every snapshot.
  const meta=[];for(const row of rows){if(row.id===activeRow.id){meta.push(metaFromRaw(row.id,row.raw,activeDecoded));continue;}const decoded=await decodeWorkspace(row.raw);if(decoded){decoded.tournament={...(decoded.tournament||{}),id:row.id};meta.push(metaFromRaw(row.id,row.raw,decoded));}else meta.push(metaFromRaw(row.id,row.raw));}
  const state=normalizeState(clone(activeDecoded));state.multiTournament={activeTournamentId:activeRow.id,tournaments:meta,noActiveTournament:false};localStorage.setItem('230match-v7-active-tournament',activeRow.id);lastKnownRoomRevision=Number(room.revision||0);lastWriterUid=String(room.lastWriterUid||'');return{state,room,count:meta.length};
}
async function readOneTournament(id,registry=[]){const snap=await api.getDoc(tournamentRef(id));if(!snap.exists())return null;const raw=snap.data(),decoded=await decodeWorkspace(raw);if(!decoded)return null;decoded.tournament={...(decoded.tournament||{}),id:snap.id};const records=(registry||[]).map(r=>r.id===snap.id?{...r,...metaFromRaw(snap.id,raw,decoded)}:r);if(!records.some(r=>r.id===snap.id))records.push(metaFromRaw(snap.id,raw,decoded));const state=normalizeState(clone(decoded));state.multiTournament={activeTournamentId:snap.id,tournaments:records,noActiveTournament:false};return{state,raw};}
...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();const remoteBundle=await readInitialBundle(),requested=String(remoteBundle?.state?.multiTournament?.activeTournamentId||localStorage.getItem('230match-v7-active-tournament')||''),cached=await readCachedState(requested),chosen=chooseInitial(cached,remoteBundle?.state||null);if(chosen){applyState(chosen,cached&&chosen===cached?'cache':'firebase');if(cached&&chosen===cached&&canWriteFn())schedule();}unsubscribeRoom=api.onSnapshot(roomRef(),snap=>{void handleRoomSnapshot(snap);},e=>status('실시간 연결 오류','warning',e?.message||String(e)));status('실시간 연결','success','저부하 단일 리스너로 연결되었습니다.');return true;}
