/* v1011 cache-proof clean main draw slot mode
...
(function(){
  'use strict';
  if(window.__V1010_MAIN_DRAW_CLEAN_INSTALLED) return;
  window.__V1010_MAIN_DRAW_CLEAN_INSTALLED = true;
  // v1011 uses its own guard so it can replace a cached v1008/v1009 script if a service worker returns an old asset.
  const VERSION = 'v1011-slot-first-queue-fix';
  ...
  function resolveCleanMatches(key, persistNames){
    ...
    mains.forEach(m=>{
      ...
      if(persistNames){m.display1=a?.nm||m.source1Label||'TBD'; m.display2=b?.nm||m.source2Label||'TBD';}
    });
    return mains;
  }
  ...
  async function assignCourts(key){
    ...
    const base=Date.now();
    mains.forEach((m,i)=>{
      const v=S(m.venue||m.venueLabel||m.__venue)||vg[0].venue;
  ...
  function installSharedQueueBridge(){
    ...
    }catch(e){ console.warn('[v1011] shared queue bridge failed', e); }
  }
  window.MainDrawCleanV1007={version:VERSION,ensurePanel,generateDraw,assignCourts,hideLegacyMainUi,selectedKey,prelimComplete,resolveCleanMatches,isPrivileged:cleanMainPrivileged}; window.MainDrawCleanV1004=window.MainDrawCleanV1007; window.MainDrawCleanV1003=window.MainDrawCleanV1007;
  install();
  function apply(){installSharedQueueBridge();ensurePanel();hideLegacyMainUi();forcePanelVisible();}
  ...
  try{console.log('[v1011] cache-proof slot main draw loaded');}catch(e){}
})();
...
(function(){
  ...
  var VERSION='v1011-button-slot-queue-fix';
  function $(id){return document.getElementById(id);}
  ...
  try{console.log('[v1011] main draw buttons cache-proof fixed');}catch(e){}
})();
