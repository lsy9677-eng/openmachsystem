...
import{startStateSync,getSyncSettings,saveSyncSettings,connectCloudSync,disconnectCloudSync,pushStateNow,pullStateNow,testCloudConnection,prepareCriticalCloudWrite,deleteTournamentNow,loadTournamentNow}from'./sync-engine-v6202.js?v=6202';
...
function switchDivisionWorkspace(id){
  ...
  if(!next)return false;if(id===currentId)return true;
  if(!applyDivisionSnapshot(next))return false;
  ...
  try{const tid=String(state.multiTournament?.activeTournamentId||state.tournament?.id||'');if(tid)localStorage.setItem(`230match-v7-active-division:${tid}`,id);}catch(_e){}
  safePersistState(`부서 전환 · ${next.name}`);syncInputs();syncPrelimInputs();renderVenueSettingsEditor();
  render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});
  renderOperatorControls();applyRoleUI();renderPortalViews();renderDivisionWorkspaceBar();autoSmsSnapshot=buildAutoSmsSnapshot();notice(`${state.tournament.name} · ${next.name}으로 전환했습니다.`,'success');return true;
}
...
let workspaceTournamentSwitching=false;
async function switchTournamentWorkspace(id,{view='home'}={}){
  ensureMultiTournamentRuntime();id=String(id||'').trim();if(!id)return false;
  if(id===String(state.multiTournament?.activeTournamentId||'')){if(view&&typeof navigatePortalView==='function')navigatePortalView(view,{pushHistory:true});return true;}
  if(workspaceTournamentSwitching)return false;workspaceTournamentSwitching=true;
  const previousName=String(state.tournament?.name||'현재 대회');
  try{
    try{syncCurrentTournamentRuntime();saveState(state);}catch(_e){}
    // Save the tournament being left first. Selection itself is local and never changes another device's selected tournament.
    if(currentRole==='admin'||currentRole==='operator'){try{await pushStateNow(state);}catch(error){console.warn('[230MATCH] switch pre-save skipped',error);}}
    const registry=(state.multiTournament?.tournaments||[]).map(r=>({id:r.id,name:r.name||r.snapshot?.tournament?.name||'',division:r.division||r.snapshot?.tournament?.division||'',createdAt:r.createdAt||'',updatedAt:r.updatedAt||''}));
    let loaded=null;try{loaded=await loadTournamentNow(id,registry);}catch(error){console.warn('[230MATCH] selected tournament cloud load failed',error);}
    let next=loaded?.state||null;
    if(!next){const fallback=state.multiTournament?.tournaments?.find(r=>String(r.id)===id)?.snapshot;if(fallback){next=normalizeV5RuntimeState(structuredClone(fallback));next.multiTournament={activeTournamentId:id,tournaments:registry,noActiveTournament:false};}}
    if(!next){notice('선택한 대회 데이터를 불러오지 못했습니다. 네트워크 연결을 확인해 주세요.','error');return false;}
    state=normalizeV5RuntimeState(next);ensurePortalState();ensurePrelimState(state);ensureTimeState(state);ensureDrawMeta(state);ensureMessagingState(state);ensureContacts(state);ensureAuditState(state);ensureEarlyMainSettings(state);ensureVenueSettings(state);ensureVenueQueues(state);ensureCourtStatuses(state);ensureCourtManualQueues(state);ensurePrelimCourtStatuses(state);ensureOperatorState();ensureMultiDivisionRuntime();
    try{localStorage.setItem('230match-v7-active-tournament',id);}catch(_e){}
    saveState(state);syncInputs();syncPrelimInputs();renderVenueSettingsEditor();
    render(state,{openResult,openPrelimResult,selectActiveSwap,selectReserveSwap,copyMessage,openSmsMessage,setMessageSent,removeMessage,openContactEdit,openMessageHistory,reorderQueue,openQueueMove,openManualAssign,returnWait1,openCourtTransfer,openUnifiedCourtTransfer,openCourtStatus,openManualQueueAssign,reorderManualQueue,returnManualQueue,reorderPrelimQueue,openPrelimMove,returnPrelimWait1,openPrelimCourtStatus,holdMainMatch,releaseHeldMatch});
    renderOperatorControls();applyRoleUI();renderPortalViews();renderDivisionWorkspaceBar();autoSmsSnapshot=buildAutoSmsSnapshot();
    if(view&&typeof navigatePortalView==='function')navigatePortalView(view,{pushHistory:true});notice(`${state.tournament?.name||previousName} 대회로 전환했습니다.`,'success');return true;
  }finally{workspaceTournamentSwitching=false;}
}
...
(()=>{
  ...
  @media (max-width:760px){
    ...
    #divisionWorkspaceBar .division-v6002-title strong{font-size:13px!important;line-height:1.2!important}
    #divisionWorkspaceBar .division-v6002-title>span{display:none!important}
    ...
    #divisionWorkspaceBar .division-v6002-card.active{padding:6px 7px!important}
    #divisionWorkspaceBar .division-v6002-card span{font-size:10px!important;line-height:1.2!important}
    #divisionWorkspaceBar .division-v6002-card strong{font-size:14px!important;line-height:1.25!important}
    #divisionWorkspaceBar .division-v6002-card small{
    ...
    #divisionWorkspaceBar .division-v6002-card em{display:none!important}
    ...
    #divisionWorkspaceBar .division-v6002-context{display:none!important}
  }
...
(()=>{document.documentElement.dataset.build='6202';console.info('[230MATCH] 62.0.2 ready · compact mobile workspace + local selection core');})();

console.info('[230MATCH] 62.0.0 core · single Firestore listener, lightweight save events, Storage-decoupled editor');

console.info('[230MATCH] 62.0.2 UI · compact mobile tournament/division workspace');
