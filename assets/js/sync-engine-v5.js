import{getAuthRuntime}from'./auth-engine.js?v=4000';
...
const MANIFEST_COLLECTION='matchRoomsV5';
const LEGACY_COLLECTION='v3TournamentRooms';
const DEFAULT_ROOM='230match-production';
const CLIENT_ID=sessionStorage.getItem('230match-v5-client')||crypto.randomUUID();
sessionStorage.setItem('230match-v5-client',CLIENT_ID);
...
function activeTournamentId(state){return safeId(state?.multiTournament?.activeTournamentId||state?.tournament?.id||'main-tournament','tournament');}
function normalizeRecords(state){
  const activeId=activeTournamentId(state);const rows=[];const found=new Set();
  const registry=Array.isArray(state?.multiTournament?.tournaments)?state.multiTournament.tournaments:[];
  for(const r of registry){const id=safeId(r?.id,'tournament');if(found.has(id))continue;let workspace=r?.snapshot?clone(r.snapshot):null;if(id===activeId)workspace=clone(state);if(!workspace)continue;delete workspace.multiTournament;workspace.tournament=workspace.tournament||{};workspace.tournament.id=id;rows.push({id,name:workspace.tournament.name||r?.name||'대회 준비 중',division:workspace.tournament.division||r?.division||'',updatedAt:r?.updatedAt||workspace.updatedAt||new Date().toISOString(),createdAt:r?.createdAt||workspace.tournament.createdAt||new Date().toISOString(),workspace});found.add(id);}
  if(!found.has(activeId)){const workspace=clone(state);delete workspace.multiTournament;workspace.tournament=workspace.tournament||{};workspace.tournament.id=activeId;rows.push({id:activeId,name:workspace.tournament.name||'대회 준비 중',division:workspace.tournament.division||'',updatedAt:workspace.updatedAt||new Date().toISOString(),createdAt:workspace.tournament.createdAt||new Date().toISOString(),workspace});}
  return{activeId,rows};
}
...
function buildBundle(state){const{activeId,rows}=normalizeRecords(state);return{activeId,rows,manifest:{schemaVersion:5,roomId:DEFAULT_ROOM,activeTournamentId:activeId,tournaments:rows.map(summary),clientId:CLIENT_ID,stateUpdatedAt:state?.updatedAt||new Date().toISOString()}};}
...
async function migrateLegacyIfNeeded(){
  const current=await api.getDoc(manifestRef());if(current.exists())return false;
  const legacy=await api.getDoc(api.doc(db,LEGACY_COLLECTION,DEFAULT_ROOM));if(!legacy.exists())return false;const data=legacy.data();if(!data?.state)return false;
  if(!canWriteFn())return data.state;
  await writeBundle(data.state,{migration:true,force:true});status('새 저장구조 이전 완료','success','기존 단일 문서 데이터를 대회별 문서 구조로 분리했습니다.',{roomId:DEFAULT_ROOM,migrated:true});return true;
}
async function writeBundle(state,{migration=false}={}){
  ...
  const revision=lastManifestRevision+1;batch.set(manifestRef(),{...bundle.manifest,revision,lastWriterUid:rt.user.uid,lastWriterEmail:rt.user.email||'',serverUpdatedAt:now,migration:Boolean(migration)},{merge:false});await batch.commit();lastManifestRevision=revision;lastDigest=digest(state);return bundle;
}
...
function acceptRemote(bundle,source='firebase'){
  if(!bundle?.state)return;const token=`${bundle.manifest?.revision||0}:${bundle.manifest?.stateUpdatedAt||bundle.state.updatedAt||''}`;if(token===lastRemoteToken)return;lastRemoteToken=token;lastManifestRevision=Number(bundle.manifest?.revision||0);const d=digest(bundle.state);if(d===lastDigest)return;applyingRemote=true;try{applyRemoteFn(bundle.state);lastDigest=d;status(source==='firebase'?'다른 기기 반영':'클라우드 불러오기','success',`대회 ${bundle.state.multiTournament?.tournaments?.length||1}개의 최신 상태를 반영했습니다.`,{roomId:DEFAULT_ROOM,revision:lastManifestRevision,schemaVersion:5});}finally{applyingRemote=false;}}
async function fetchRemote({quiet=false}={}){try{await ensureDb();const migrated=await migrateLegacyIfNeeded();if(migrated&&migrated!==true){acceptRemote({state:migrated,manifest:{revision:0}},'legacy');return;}const bundle=await readBundle();if(bundle)acceptRemote(bundle,'firebase');else if(!quiet)status('클라우드 연결','success','새 저장구조에 아직 대회가 없습니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});}catch(e){status('클라우드 조회 실패','warning',e?.message||String(e),{roomId:DEFAULT_ROOM});}}
async function flush(){if(pushInFlight||!pendingState||!canWriteFn())return;const state=pendingState;pendingState=null;pushInFlight=true;try{await ensureDb();status('대회별 저장 중','info','대회별 문서로 나누어 저장하고 있습니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});await writeBundle(state);status('클라우드 저장 완료','success',`대회 ${normalizeRecords(state).rows.length}개를 분리 저장했습니다.`,{roomId:DEFAULT_ROOM,revision:lastManifestRevision,schemaVersion:5});}catch(e){status('클라우드 저장 실패','error',e?.message||String(e),{roomId:DEFAULT_ROOM,schemaVersion:5});}finally{pushInFlight=false;if(pendingState)setTimeout(flush,500);}}
function schedule(state){const d=digest(state);if(d===lastDigest)return;pendingState=clone(state);clearTimeout(saveTimer);saveTimer=setTimeout(flush,SAVE_DEBOUNCE);status('저장 대기','info','변경을 모아 현재 대회 문서와 대회 목록을 저장합니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});}
...
export async function connectCloudSync(){disconnectCloudSync(false);await ensureDb();await migrateLegacyIfNeeded();if(canWriteFn()){unsubscribe=api.onSnapshot(manifestRef(),async snap=>{if(!snap.exists()){status('실시간 연결','success','새 저장구조에 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});return;}const m=snap.data();lastManifestRevision=Math.max(lastManifestRevision,Number(m.revision||0));const bundle=await readBundle();acceptRemote(bundle,'firebase');},e=>status('동기화 오류','error',e?.message||String(e)));status('실시간 연결','success','대회별 분리 저장 구조로 연결되었습니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});}else{await fetchRemote();startPolling();status('저부하 조회','success','대회별 문서를 45초 간격으로 조회합니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});}return true;}
...
export async function pushStateNow(state=getStateFn()){if(!canWriteFn())throw new Error('관리자 또는 진행자만 클라우드에 저장할 수 있습니다.');await ensureDb();await writeBundle(state);status('클라우드 저장 완료','success','전체 대회를 대회별 문서로 저장했습니다.',{roomId:DEFAULT_ROOM,schemaVersion:5});return true;}
export async function pullStateNow(){await ensureDb();const migrated=await migrateLegacyIfNeeded();if(migrated&&migrated!==true)return migrated;return(await readBundle())?.state||null;}
